...
**"스로틀된 period는 quota를 다 썼다"고 가정하지 말 것.** CFS 슬라이스 좌초(kubernetes#67577) 때문에 실사용이 quota에 못 미쳐도 스로틀이 걸린다. `perpod.py`가 이 전제로 limit을 역산하는 블록을 갖고 있는데, 그 출력은 **명목 limit이 아니라 실효 quota**로 읽어야 한다.
