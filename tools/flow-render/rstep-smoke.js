/* rstep.js 렌더 스모크 — 브라우저 없이 최소 DOM 스텁 위에서 전 구간을 훑는다.
   보는 것: NaN/undefined 속성, viewBox 이탈, 음수 width/r, 캡션 누락.
   단계형이라 한 시점만 보면 안 된다 — variant × phase × t 격자로 전부 돈다. */
'use strict';
...
require('fs');
const src = require('fs').readFileSync(process.argv[2] || 'static/flow/rstep.js', 'utf8');
eval(src);
...
for (let lap = 0; lap < 2; lap++) {
  for (let p = 0; p < PHASE_COUNT; p++) {
    for (let k = 0; k < 12; k++) {
      const ts = lap * PHASE_MS * PHASE_COUNT + p * PHASE_MS + (k / 12) * PHASE_MS;
      const pending = RAFS.splice(0, RAFS.length);
      for (const fn of (pending.length ? pending : frames)) fn(ts);
      checkOnce(`lap${lap} ph${p} t${(k / 12).toFixed(2)}`);
      steps++;
    }
  }
}
