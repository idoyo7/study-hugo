...
스펙의 `groups`·`nodes`·`edges` 라벨은 shortcode가 접힌 `<details>`("도식 텍스트")로 한 번 더 낸다 — 자동이라 따로 쓸 게 없다. JSON은 `<script>` 안에 있어 텍스트 추출기가 통째로 버리므로, 그것 없이는 도식이 담은 말이 검색 인덱스와 요약 쪽으로 하나도 안 간다.
...
`participants`의 `label`과 `steps`의 `label`·`lines`·`alt`도 flow와 같이 접힌 `<details>`로 한 번 더 나간다. `alt` 갈래는 중첩된 채로 펼쳐진다.
...
**`caption`을 안 줄 때는 `alt`를 반드시 준다.** `caption` 없이 두면 `<figcaption>`이 빈 채로 나가고, JS 없이 오는 쪽(텍스트 추출기·색인기)에는 그 자리에 아무 말도 남지 않는다. `alt`는 캡션을 얼리지 않는다 — 접힌 `<details>`("도식 텍스트")로 따로 나가고 단계별 캡션 애니메이션은 그대로 돈다. 문장은 엔진의 기본 캡션에서 가져오면 화면과 어긋나지 않는다.

````
{{< rstep variant="deploy" alt="정상 배포 — 인덱스를 하나씩 밟습니다. …" >}}
````
