...
| **`rstep.js`** | **없음** | hugo 전용 | **미이식.** 이 디렉토리가 그걸 대비한다 |
| **`rrev.js`** | **없음** | hugo 전용 | **미이식.** `rstep` 과 같은 이유로 이 디렉토리가 대비한다 |

study-hugo 엔진은 지금 **7개**(flow·seq·expand·cfstl·bscore·mnode·rstep·rrev — expand 는 공용 셸이라 위 표에서 별도 행) 다. 그중 **`rstep`·`rrev` 둘만 미이식**이고, 둘 다 이 디렉토리가 대비한다.

그래서 실제로 해야 하는 일이 셋으로 좁혀진다.

1. **`_flow/*.json` · `_seq/*.json` 은 복사만 하면 된다.** `Flow.tsx`·`Seq.tsx` 가 이미 있고 같은 스펙을 먹는다. 프레임워크 무의존 데이터다.
2. **`rstep`·`rrev` 는 새로 만들어야 한다** — `RolloutStep.tsx`·`RevisionHandoff.tsx` 를 여기 준비해 뒀다.
...
| `RolloutStep.tsx` | 그 모듈을 소비하는 React 컴포넌트 (760줄) | **nextra 빌드에서 컴파일·렌더된 적 없음** |
| `rrev-frames.mjs` | `rrev` 도식("리비전이 바뀌면 트래픽은 어떻게 따라가는가")의 **의미만**. 상수·variant 1종(`handoff`)·캡션·`computeFrame(variant, phase, t)` 순수 함수. 의존성 0, DOM 접근 0 | Hugo 엔진과 패리티 통과 |
| `RevisionHandoff.tsx` | 그 모듈을 소비하는 React 컴포넌트 | **nextra 빌드에서 컴파일·렌더된 적 없음** |

### 드리프트를 붙잡는 장치

`static/flow/rstep.js`·`static/flow/rrev.js` 는 정적 호스팅용이라 번들러 없이 도는 IIFE 다. 그래서 `rstep-frames.mjs`·`rrev-frames.mjs` 를 `import` 하지 못하고 **같은 계산을 자기 안에 들고 있다.** 두 곳에 같은 로직이 있으면 드리프트가 난다.

```
node tools/flow-render/port-parity.js
```

한 파일에서 두 엔진을 각각 검사한다(`runRstepParity()` · `runRrevParity()`). rstep 은 엔진을 최소 DOM 스텁 위에서 돌려 **실제로 SVG 에 칠한 값**을 되읽고, 모듈의 프레임과 `variant 4종 × 6단계 × 10시점 × 필드 12종`(240회)을 대조한다. 상수 한 자리만 어긋나도 잡힌다(확인함 — `RPS: 96` 을 `97` 로 바꾸면 20종 불일치로 FAIL). rrev 는 같은 방식으로 `variant 1종 × 6단계 × 10시점 × 필드 16종`(60회)을 대조한다(확인함 — `CANARY_STEP_PCT: 5` 를 `7` 로 바꾸면 30종 불일치로 FAIL).

`./tools/flow-render/run.sh` 에 들어가 있으니 도식을 고칠 때마다 같이 돈다.

**한쪽만 고치고 이 테스트를 느슨하게 만들지 마라.** 그러면 이식 시점에 조용히 다른 그림이 나온다.

### `RolloutStep.tsx`·`RevisionHandoff.tsx` 가 지킨 관례
...
- `<DiagramFigure className="vm-rstep"><style>{CSS}</style><svg ref={…}>` 구조. CSS 는 템플릿 문자열 상수(`RevisionHandoff.tsx` 는 `vm-rrev`)

`RevisionHandoff.tsx` 는 위 관례를 전부 그대로 따르되 한 가지가 다르다 — `rrev-frames.mjs` 는 트래픽 패킷의 목표(`target`)와 주기 진행률(`cyc`)만 내고, 그것을 픽셀 좌표(`cx`/`cy`)로 바꾸는 지오메트리는 컴포넌트 쪽 레이아웃 상수(`packetPos()`)에 있다. `RolloutStep.tsx` 가 스킵 호·역탐색 화살표의 좌표 계산을 컴포넌트 쪽에 둔 것과 같은 이유다 — 픽셀 배치는 프레임워크마다 다시 그릴 수 있는 렌더링 관심사이고, 의미(무엇이 어디로 가는가)만 모듈의 SSOT 다.
...
1. `rstep-frames.mjs`·`rrev-frames.mjs` 를 가져간다. 손댈 것 없다. `.ts` 로 바꿀지 아니면 `.d.ts` 를 붙일지 결정한다 — 지금 `RolloutStep.tsx`·`RevisionHandoff.tsx` 의 `Frame`·`Cfg` 타입은 손으로 옮긴 것이라 모듈과 강제 연결돼 있지 않다.
2. `RolloutStep.tsx`·`RevisionHandoff.tsx` 를 가져간다. **머리의 `/* eslint-disable */`·`// @ts-nocheck` 두 줄을 지우고** 그쪽 tsconfig 로 실제 타입 검사를 받는다 — 그게 이 파일이 처음으로 진짜 검증되는 시점이다. import 경로들(`./rstep-frames.mjs`/`./rrev-frames.mjs`, `DiagramFigure`)을 실제 위치로 맞춘다.
3. **variant 를 hugo 쪽과 나란히 띄워 눈으로 대조한다.** 아직 브라우저에서 렌더된 적이 없다. `rrev` 는 variant 가 `handoff` 하나뿐이라 대조할 그림도 하나뿐이다.
4. `port-parity.js` 를 nextra 테스트로 옮기고 **비교 방향을 뒤집는다** — 옮긴 뒤에는 Hugo 엔진이 아니라 이 모듈들이 SSOT 이므로, 되읽을 Hugo 엔진이 없어진다. 지금 한 파일에 있는 `runRstepParity()`/`runRrevParity()` 두 함수를 각각 옮기면 된다.
