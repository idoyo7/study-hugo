...
export function computeFrame(variant, phase, t) {
  ...
  if (phase === 0) {
    ...
    if (variant === 'promote') {
      ...
      /* verdict 는 가중치 바의 '요구'·'Ready' 문구와 같은 두 값(requiredPods(weight), cAvail)을
         그대로 쓴다 — REPLICAS·MIN_PODS 상수를 박아두면 가중치가 아직 안 바뀐 t 구간에서
         캡션만 앞서 나간다. */
      const wNeed = requiredPods(f.weight);
      const wShort = wNeed - f.cAvail;
      f.verdict = wShort > 0
