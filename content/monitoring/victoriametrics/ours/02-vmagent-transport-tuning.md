...
- **대안(미적용) `statefulMode=true`** — 큐를 PVC로 옮기면 유실이 없어진다. 다만 무상태 원칙의 예외이므로 그 예외를 인정할 때만 택한다.
...
- 상한에 도달하면 **가장 오래된 블록부터 ~500MB 청크 단위로 FIFO drop**한다. 최신 데이터를 살리고 오래된 것을 버리는 쪽이다.
- 값은 **500MiB 배수**를 권장한다. drop 청크 단위와 맞물리기 때문이다.
