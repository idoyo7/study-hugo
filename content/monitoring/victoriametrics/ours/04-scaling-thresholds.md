...
`maxDiskUsagePerURL`은 "링크 장애가 나도 몇 시간치 지표를 큐에 버틸 수 있게 할 것인가"로 잡습니다.

```
maxDiskUsagePerURL ≈ sum(rate(vmagent_remotewrite_bytes_sent_total[1h])) × 버틸 장애 시간
```

즉 **평시 전송 바이트레이트 × 목표 버팀 시간**입니다. 링크가 끊긴 동안 이 큐가 지표를 붙잡아 두고, 복구되면 다시 흘려보냅니다. 상한을 넘기면 오래된 블록부터 버립니다(→ drop 동작은 [02]({{< relref "02-vmagent-transport-tuning.md" >}})).
