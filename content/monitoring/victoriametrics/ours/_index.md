...
원리가 궁금하면 concepts로, "어떻게 설계해야 하나"가 궁금하면 practice로 올라갑니다. "우리는 지금 어떤 값으로 돌고 있나"가 궁금하면 이 서브섹션에 머뭅니다.
...
- **구조부터**: 01에서 우리 스택 전체 그림과 stage/prod 값 차이를 잡습니다.
- **왜 이렇게 튜닝했나**: 02에서 Phase 1 두 변경(zstd 고정·디스크 큐 상한)의 근거를 봅니다.
- **잘 도는지 확인**: 03의 자기감시 메트릭 4개로 적용 효과와 이상을 판정합니다.
- **얼마나 버티나**: 04에서 큐 상한 산정식과 리소스·HA 기준치를 정리합니다.
