...
> 관련 블록: [02 아키텍처]({{< relref "02-architecture.md" >}}) · [03 수집]({{< relref "03-ingestion.md" >}}) · [05 쿼리·운영 컴포넌트]({{< relref "05-query-and-ops-components.md" >}}) · [06 카디널리티]({{< relref "06-cardinality.md" >}})
...
캐시에 없으면(캐시미스) 아래로 폴백한다.

```
TSID 캐시 조회 → miss
   → IndexDB(디스크 인덱스, prefix별로 엔트리 정리) 조회
        → hit  → 그 TSID 반환
        → miss → 처음 보는 시계열 → New TSID 발급
```
...
원래 16바이트가 필요한 데이터포인트를 **0.92바이트로 저장** — 약 17배 압축이다. 이 값은 벤치마크가 아니라 12.5억 활성 시계열 규모의 **실운영 환경 실측치**이며(D2 기사 6475419), Gorilla 계열 압축이 이론이 아니라 실전에서 통한다는 가장 강력한 증거다. 이 압축이 절약하는 것은 디스크만이 아니다. 더 많은 데이터가 메모리에 올라가므로 연산도 그만큼 빨라진다. 이 규모의 전체 그림은 [07 초대규모 운영]({{< relref "07-operations-at-scale.md" >}})에서 다룬다.
