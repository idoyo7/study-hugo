...
- **주의**: STT 자동 전사라 고유명사 표기 오류가 있다. 각 블록은 정규 표기로 교정해 반영했다.
  - 예: "빅토리아 매트릭스" → VictoriaMetrics · "VM 에이전트" → vmagent · "아파트 2.0" → Apache 2.0 · "프롬키" → PromQL · "리모트라이트" → remote_write · "고릴라" → Gorilla · "미드" → read.

## 원본 대사집을 직접 볼 때

블록은 요약·재구성본이다. 발표자의 정확한 워딩, 타임스탬프, 맥락이 필요하면 작업 저장소 `evejuni/monitoring/d2-victoriametrics/`의 `01_대사집_*.md`, `02_대사집_*.md`를 보라(GitBook 미포함). 두 대사집은 5분 단위 타임스탬프(`## 00:00 ~`)로 구획돼 있어 영상과 대조하기 쉽다.
