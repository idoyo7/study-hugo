...
이 지식베이스는 네이버 D2/DEVIEW의 VictoriaMetrics 자료 **5건**(발표 영상 2편 + 텍스트 기사 3편)을 재구성한 것입니다. 주제별 재구성은 개념/실전/우리 운영 문서가 담고, 원문별로 하나씩 충실히 정리한 판은 [원문별 정리]({{< relref "../by-source/_index.md" >}})에 있습니다. 여기서는 각 문서가 어떤 원본에서 나왔는지, 원본을 직접 확인하려면 어디를 보면 되는지 정리합니다.
...
문서는 요약·재구성본입니다. 발표자의 정확한 워딩, 타임스탬프, 맥락이 필요하면 작업 저장소 `evejuni/monitoring/d2-victoriametrics/`의 `01_대사집_*.md`, `02_대사집_*.md`를 보십시오(GitBook 미포함). 두 대사집은 5분 단위 타임스탬프(`## 00:00 ~`)로 구획돼 있어 영상과 대조하기 쉽습니다.
