...
이 지식베이스의 바탕은 네이버 D2/DEVIEW가 공개한 VictoriaMetrics 자료 **5건**입니다 — 발표 영상 2편과 텍스트 기사 3편. 이 자료를 주제별로 다시 묶은 결과가 개념/실전/우리 운영 문서입니다. 자료 하나하나를 원문 단위로 충실히 정리한 판은 [원문별 정리]({{< relref "../by-source/_index.md" >}})에 따로 있습니다. 여기서는 각 문서가 어떤 원본에서 나왔는지, 원본을 직접 확인하려면 어디를 봐야 하는지 정리합니다.
...
- **주로 반영된 문서**: [실전 02]({{< relref "../practice/02-operations-at-scale.md" >}}), 그리고 [04]({{< relref "04-storage-and-compression.md" >}}) 일부(0.92B 압축 실증). 원문 단위 정리는 [원문별 02]({{< relref "../by-source/02-2026-04-large-scale-metric-store.md" >}}).
...
- 모델 파일: 작업 저장소 `evejuni/monitoring/d2-victoriametrics/models/ggml-large-v3-turbo.bin` (약 1.6GB, GitBook 미포함). 재전사할 일이 없으면 삭제해도 무방.
...
문서는 요약·재구성본입니다. 발표자의 정확한 워딩과 타임스탬프, 맥락까지 필요하면 작업 저장소 `evejuni/monitoring/d2-victoriametrics/`의 `01_대사집_*.md`, `02_대사집_*.md`를 보십시오(GitBook 미포함). 두 대사집은 5분 단위 타임스탬프(`## 00:00 ~`)로 구획돼 있어 영상과 대조하기 쉽습니다.
