...
{{< flow caption="vmselect의 Fanout 쿼리 흐름 — Grafana 요청을 모든 vmstorage로 뿌리고 결과를 Merge·Sort해 같은 Grafana로 되돌린다(응답 경로를 오른쪽으로 폈다)" >}}
{
  "nodes": [
    { "id": "g", "col": 0, "row": 1, "label": "Grafana", "sub": "PromQL/MetricsQL 발신", "kind": "sink" },
    ...
    { "id": "gr", "col": 4, "row": 1, "label": "Grafana", "sub": "JSON 응답 수신", "kind": "sink" }
  ],
  "edges": [
    { "from": "g", "to": "vs", "label": "쿼리", "rate": 700 },
    ...
    { "from": "mg", "to": "gr", "label": "응답", "rate": 700 }
  ]
}
{{< /flow >}}
