...
- **그 위의 스케일 문제.** 수천만 개를 넘는 대용량은 Prometheus 하나만으로는 풀리지 않습니다. 여기서 갈래가 나뉩니다. 널리 쓰이는 것이 **Thanos**와 **Cortex** — 스케일러블한 Prometheus 확장 솔루션입니다. 반면 네이버 검색 SRE는 이들과 **완전히 다른 계열인 VM을 택했습니다.**
...
- 자체 벤치마크 기준으로 **메모리 5배, 스토리지 7배 더 효율적**이라고 주장합니다. 이 효율의 비결은 앞서 본 Time Series/Sample 분리와 Gorilla 계열 압축입니다. 실제 운영에서는 데이터포인트당 1바이트 미만까지 줄어든다(상세는 [04 저장과 압축]({{< relref "04-storage-and-compression.md" >}})).
