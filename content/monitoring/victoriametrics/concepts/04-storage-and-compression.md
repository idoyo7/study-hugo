...
{{< flow caption="TSID 변환 — 캐시 hit이면 빠른 경로, 캐시·IndexDB 모두 miss면 New TSID 발급" >}}
{
  "nodes": [
    { "id": "RAW", "col": 0, "row": 0, "label": "로우", "sub": "metric name + labels", "kind": "src" },
    { "id": "CN", "col": 1, "row": 0, "label": "Canonical Name 정규화", "sub": "64bit TSID로 변환", "kind": "proc" },
    { "id": "Q1", "col": 2, "row": 0, "label": "TSID 캐시", "kind": "query" },
    { "id": "Q2", "col": 3, "row": 1, "label": "IndexDB 조회", "kind": "query" },
    { "id": "T", "col": 4, "row": 0, "label": "TSID", "sub": "빠른 경로", "kind": "sink" },
    { "id": "N", "col": 4, "row": 1, "label": "New TSID 발급", "kind": "proc" },
    { "id": "X", "col": 5, "row": 1, "label": "⚠ 남발 = 카디널리티 폭발", "kind": "sink" }
  ],
  "edges": [
    { "from": "RAW", "to": "CN", "rate": 700 },
    { "from": "CN", "to": "Q1", "rate": 700 },
    { "from": "Q1", "to": "T", "label": "hit", "rate": 600, "speed": "fast" },
    { "from": "Q1", "to": "Q2", "label": "miss", "rate": 700 },
    { "from": "Q2", "to": "T", "label": "hit", "rate": 700 },
    { "from": "Q2", "to": "N", "label": "miss", "rate": 800 },
    { "from": "N", "to": "X", "rate": 800 }
  ]
}
{{< /flow >}}
...
{{< flow caption="파티션 — 인메모리에서 Small로 플러시, Small에서 Big으로 머지" >}}
{
  "nodes": [
    { "id": "IM", "col": 0, "row": 0, "label": "인메모리 파츠", "kind": "store" },
    { "id": "SM", "col": 1, "row": 0, "label": "Small 파티션", "kind": "store" },
    { "id": "BG", "col": 2, "row": 0, "label": "Big 파티션", "kind": "store" }
  ],
  "edges": [
    { "from": "IM", "to": "SM", "label": "플러시", "rate": 700 },
    { "from": "SM", "to": "BG", "label": "머지 (Merge Multiplier ↑)", "rate": 700 },
    { "from": "IM", "to": "BG", "label": "아주 큰 블록", "rate": 800, "speed": "slow" }
  ]
}
{{< /flow >}}
...
{{< flow caption="IndexDB 3단계 로테이션 — Next → Current → Previous → 통째로 드롭" >}}
{
  "nodes": [
    { "id": "N", "col": 0, "row": 0, "label": "Next", "sub": "다음 로테이션 준비", "kind": "store" },
    { "id": "C", "col": 1, "row": 0, "label": "Current", "sub": "실시간 수신", "kind": "store" },
    { "id": "P", "col": 2, "row": 0, "label": "Previous", "sub": "보존기간 내·쿼리 가능", "kind": "store" },
    { "id": "D", "col": 3, "row": 0, "label": "통째로 드롭", "kind": "sink" }
  ],
  "edges": [
    { "from": "N", "to": "C", "label": "rotate", "rate": 800, "speed": "slow" },
    { "from": "C", "to": "P", "label": "rotate", "rate": 800, "speed": "slow" },
    { "from": "P", "to": "D", "label": "rotate", "rate": 800, "speed": "slow" }
  ]
}
{{< /flow >}}
...
{{< flow caption="Gauge는 Delta로, Counter는 Delta-of-Delta로 차분한 뒤 ZigZag/Varint로 인코딩" >}}
{
  "nodes": [
    { "id": "G1", "col": 0, "row": 0, "label": "원본", "sub": "110, 105, 98, 110", "kind": "src" },
    { "id": "G2", "col": 1, "row": 0, "label": "저장", "sub": "110, -5, -7, +12 (Gauge → Delta)", "kind": "proc" },
    { "id": "C1", "col": 0, "row": 1, "label": "원본", "sub": "120, 130, 150, 170", "kind": "src" },
    { "id": "C2", "col": 1, "row": 1, "label": "차분", "sub": "10, 20, 20", "kind": "proc" },
    { "id": "C3", "col": 2, "row": 1, "label": "차분의 차분", "sub": "≈ 0 (Counter → Delta-of-Delta)", "kind": "proc" },
    { "id": "ZZ", "col": 3, "row": 0, "label": "ZigZag (부호 제거)", "sub": "Varint (7bit + 연속플래그)", "kind": "sink" }
  ],
  "edges": [
    { "from": "G1", "to": "G2", "rate": 700 },
    { "from": "C1", "to": "C2", "rate": 700 },
    { "from": "C2", "to": "C3", "rate": 700 },
    { "from": "G2", "to": "ZZ", "rate": 700 },
    { "from": "C3", "to": "ZZ", "rate": 700 }
  ]
}
{{< /flow >}}
