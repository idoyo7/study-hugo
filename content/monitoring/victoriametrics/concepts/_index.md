...
- 관점은 하나로 이어진다 — 데이터가 **들어와서(03) → 저장되고(04) → 쿼리로 나가기(05)**까지의 내부 동작.
...
네이버 D2/DEVIEW의 VictoriaMetrics 자료 **4건**(발표 영상 2편 + 텍스트 기사 2편)을 하나로 병합해, 데이터가 **들어와서 → 저장되고 → 쿼리로 나가기**까지의 내부 동작을 주제별로 재구성한 지식베이스입니다. 목적은 **개념 이해**입니다. VM이 어떤 컴포넌트로 이루어지고 각 단계에서 무슨 일이 일어나는지, 왜 그런 설계가 대용량 시계열을 감당하는지를 순서대로 따라 읽도록 엮었습니다. 실제로 잘 쓰고 운영하는 방법은 [잘 쓰는 방법]({{< relref "../practice/_index.md" >}})에서 이어집니다.
...
- **06 소스맵은 참고용이다.** 각 문서가 어떤 원본 발표·기사에서 나왔는지, 직접 확인하려면 어디를 보면 되는지 정리한 지도다. 마지막에 필요할 때 펼쳐 보면 된다.
