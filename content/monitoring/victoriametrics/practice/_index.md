...
[기본 개념]({{< relref "../concepts/_index.md" >}})이 VM의 내부 동작을 다뤘다면, 이 문서는 그 동작을 아는 상태에서 "어떻게 써야 잘 쓰는가"를 다룹니다. 시계열 폭발을 예방하는 카디널리티 설계 원칙, 네이버 검색이 초대규모 운영과 무중단 전환을 겪으며 쌓은 운영 패턴, 저장된 시계열을 올바르고 가볍게 꺼내 쓰는 쿼리 패턴이 차례로 나옵니다. 모두 특정 환경에 매이지 않는 일반 원칙으로 정리했습니다. 규모가 커질수록 더 크게 작동합니다.
