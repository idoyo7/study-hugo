...
VM 운영에서 가장 자주 사고를 내는 개념이 **카디널리티**입니다. 이 문서는 "한 시계열이 무엇으로 정의되는가"에서 출발해, 카디널리티 폭발이 왜 곧 메모리·인덱스 폭발인지, 그리고 설계 단계에서 이를 어떻게 막는지를 봅니다.
...
예를 들어 아래 둘은 이름이 같아도 서로 다른 두 개의 시계열입니다.
...
카디널리티란 이렇게 만들어지는 **서로 다른 시계열의 총 개수**입니다. 레이블 값의 조합이 늘어날수록 곱셈으로 불어납니다.

## New TSID 폭발이 곧 카디널리티 폭발

[개념 04 저장]({{< relref "../../concepts/04-storage-and-compression.md" >}})에서 본 저장 경로를 떠올리겠습니다. vmstorage는 들어온 지표를 TSID로 변환하는데, 그 과정은 TSID 캐시 → IndexDB 순으로 조회하다가 **둘 다 없으면 처음 보는 시계열로 판단해 New TSID를 발급**합니다.
...
공통점은 **값의 가짓수가 시간이 지나며 계속 늘어난다**는 것입니다. 이런 레이블은 시계열 교체율(뒤의 churn)을 끌어올려 인덱스를 부풀립니다.

레이블이 왜 곱셈으로 불어나는지는 숫자로 보면 분명합니다. 아래는 감을 잡기 위한 **가상의 계산 예시**입니다 — 실측치가 아니라 레이블 조합이 어떻게 곱해지는지 보기 위한 것입니다. `http_requests_total` 지표 하나에 레이블을 한 겹씩 얹어 시계열 수를 세어 봅니다.
...
**churn rate**는 앞서 본 New TSID 발급을 그대로 관측한 값입니다. 자주 바뀌는 레이블이 들어오면 이 수치가 튑니다. **slow insert rate**는 TSID 캐시가 메모리 부족으로 미스를 자주 내며 IndexDB 폴백이 잦아질 때 오릅니다 — 즉 활성 시계열이 메모리 캐시에 다 담기지 못하고 있다는 뜻입니다. **지속적으로 10% 초과는 "메모리가 활성 시계열을 감당하지 못한다"는 명확한 경고**로 읽어야 합니다.
