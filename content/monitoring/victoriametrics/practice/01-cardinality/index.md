...
레이블이 왜 곱셈으로 불어나는지는 숫자로 보면 분명하다. 아래는 감을 잡기 위한 **가상의 계산 예시**다 — 실측치가 아니라 레이블 조합이 어떻게 곱해지는지 보기 위한 것이다. `http_requests_total` 지표 하나에 레이블을 한 겹씩 얹어 시계열 수를 세어 본다.
...
`service·pod·path`까지는 1,000개로 유한하다. 그런데 worst case인 `session_id`를 한 겹 더 얹는 순간 마지막 항이 사실상 무한대라 전체 곱이 통제 불능으로 튄다. **곱셈은 한 항만 무한히 늘어도 전체가 무한히 늘어난다** — 이것이 고카디널리티 레이블 하나가 클러스터를 무너뜨리는 원리다. (`pod`도 순간에는 유한하지만 재시작마다 새 이름이 쌓여, 시간이 지날수록 churn으로 이 수마저 계속 커진다.)
...
설계로 배제하는 것이 최선이지만, 이미 돌고 있는 클러스터의 **현재 카디널리티는 직접 확인할 수 있다.** vmstorage(단일 노드 포함)의 **`/api/v1/status/tsdb`** 엔드포인트는 전체 활성 시계열 수와 카디널리티가 높은 상위 메트릭·레이블을 요약해 준다. 시각적으로 파고들려면 **vmui의 Cardinality Explorer**(`/vmui/#/cardinality`)에서 어떤 메트릭·레이블 값이 시계열을 많이 먹는지 훑으면 된다. 구체적인 확인 쿼리와 우리 환경의 인벤토리 도구는 [우리의 운영 03 자기감시 메트릭]({{< relref "../../ours/03-self-monitoring-metrics.md" >}})에 정리해 두었다.
