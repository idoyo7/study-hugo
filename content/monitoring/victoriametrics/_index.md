...
VictoriaMetrics(이하 VM)를 세 갈래로 나눠 정리한 지식베이스입니다. **① 기본 개념**은 네이버 D2/DEVIEW 발표 2편과 텍스트 기사 2편을 정독해, 데이터가 **들어와서 → 저장되고 → 쿼리로 나가기**까지 VM 내부 동작을 파헤칩니다. **② 잘 쓰는 방법**은 그 내부 동작에서 끌어낸 설계 원칙과 초대규모 운영 패턴을 다룹니다. **③ 우리의 운영**은 실제 우리 스택의 구성·튜닝·기준치·노하우를 기록합니다. 개념에서 원리를 얻고, 원리를 실전 설계로 옮기고, 우리 환경에 적용하는 흐름입니다.
...
시계열이 무엇이고 VM 내부에서 데이터가 어떻게 흐르는지를, 네이버 D2/DEVIEW 발표를 정독해 컴포넌트별로 파헤칩니다. 처음이라면 여기서 시작합니다.
...
내부 동작에서 끌어낸 설계 원칙과, 수천만~수십억 시계열 규모의 실전 운영 패턴입니다. 운영자라면 여기가 실무 직결입니다.
...
실제 우리 스택의 구성과, 운영하며 정리한 튜닝·기준치·노하우입니다. 우리 환경을 파악하려면 여기를 봅니다.
...
위 세 갈래가 **주제별 재구성**이라면, 이건 **원문(발표·기사)별 정리**입니다. 네이버 D2의 VictoriaMetrics 자료를 게시 순서대로 각각 한 문서에 충실히 담아, 어느 글이 무엇을 말했는지 그대로 보존합니다. 각 문서 최상단에 원문 링크와 "참조한 내용정리" 표시를 둡니다.
