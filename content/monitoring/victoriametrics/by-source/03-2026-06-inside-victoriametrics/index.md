...
수많은 `vmstorage` 중 어디로 보낼지는 **랑데부 해싱**으로 정합니다. 단순 모듈로/해시를 쓰면 노드 추가·삭제 시 거의 모든 시계열이 다른 노드로 옮겨 가 리밸런싱 폭풍이 입니다. 랑데부 해싱은 지표가 들어오면 모든 노드에 대해 `hash(지표 이름 + 노드 이름)`으로 점수를 매기고 **가장 높은 노드에만** 보냅니다.
