...
555조 개의 데이터포인트가 약 510TB에 담긴다. 데이터포인트당 0.92바이트라는 압축이 이 규모를 지탱하며, 그 원리는 {{< relref "../concepts/04-storage-and-compression.md" >}}에서 다룬다. 하루 약 7.4억 개의 신규 시계열은 쿠버네티스 환경에서 컨테이너가 얼마나 자주 생성되고 사라지는지를 보여 준다.
...
{{< flow caption="vmagent 이중 쓰기와 쿼리 시간 범위 기반 Hot/Warm 분기" >}}
{ "nodes": [
  { "id": "a", "col": 0, "row": 1, "label": "vmagent", "kind": "proc" },
  { "id": "h", "col": 1, "row": 0, "label": "Hot 클러스터", "sub": "SSD·12개월", "kind": "store" },
  { "id": "w", "col": 1, "row": 2, "label": "Warm 클러스터", "sub": "HDD·36개월", "kind": "store" },
  { "id": "qr", "col": 0, "row": 3, "label": "쿼리 시간 범위", "kind": "query" }
], "edges": [
  { "from": "a", "to": "h", "label": "이중 쓰기", "rate": 600 },
  { "from": "a", "to": "w", "label": "이중 쓰기", "rate": 600 },
  { "from": "qr", "to": "h", "label": "최근 12개월", "dashed": true },
  { "from": "qr", "to": "w", "label": "12개월 이전", "dashed": true }
] }
{{< /flow >}}
...
VictoriaMetrics 공식 문서는 이런 상황을 피하기 위해 전체 노드 RAM의 50%를 여유 공간으로 남겨 둘 것을 권장한다.
...
기존 장비에서는 카디널리티 증가에 따라 RSS anonymous 메모리 사용률이 꾸준히 상승했고, 권장 수준의 여유를 안정적으로 유지하기 어려웠다. `vmstorage` 장비 메모리를 128GB에서 512GB로 늘리기로 결정했다. 단순한 사양 업그레이드보다 더 중요한 과제는 무중단 전환이었다. VictoriaMetrics 클러스터는 네이버 검색 전체의 핵심 모니터링 인프라이므로, 장비 교체 중 메트릭이 누락되거나 알림이 멈추면 곧바로 서비스 안정성에 영향을 준다. 180대 노드에 걸친 대규모 장비 교체를 서비스 영향 없이 완료하는 것이 최우선 요구사항이었고, 계층별 특성에 맞춰 서로 다른 전환 전략을 설계했다.
...
new-A가 primary가 되면 복제는 old-A, old-B로 향하고, new-B가 추가돼도 다시 old-A, old-B가 복제 부하를 받아 목록 앞쪽의 기존 장비에 새 시계열 등록 부하가 반복해서 누적된다. 이미 메모리 한계에 가까운 128GB 장비에서 이 추가 부하는 OOM으로 이어질 수 있으며, 실제로 과거 장비 증설 과정에서 비슷한 장애를 겪기도 했다.
...
{{< flow caption="Warm Tier 세트 단위 점진 전환 — 서비스 중단 없이 반복하는 4단계" >}}
{
  "nodes": [
    {"id": "s1", "col": 0, "row": 0, "label": "① vminsert 제외", "sub": "기존 장비 유입 중단", "kind": "proc"},
    {"id": "s2", "col": 1, "row": 0, "label": "② 증분 백업·복원", "sub": "vmbackup / vmrestore", "kind": "store"},
    {"id": "s3", "col": 2, "row": 0, "label": "③ vmselect 교체", "sub": "기존 제거·신규 투입", "kind": "query"},
    {"id": "s4", "col": 3, "row": 0, "label": "④ vminsert 투입", "sub": "실시간 수집 재개", "kind": "proc"}
  ],
  "edges": [
    {"from": "s1", "to": "s2", "rate": 700},
    {"from": "s2", "to": "s3", "rate": 700},
    {"from": "s3", "to": "s4", "rate": 700}
  ]
}
{{< /flow >}}
...
Hot Tier는 역순 추가 전략을 모두 마친 뒤 `vminsert`에서 기존 장비군을 제거하는 방식으로, Warm Tier는 세트 단위 점진 전환을 반복하는 방식으로 각각 교체를 마쳤다. 두 전략은 방식이 달랐지만, 수집 누락과 조회 실패 없이 장비를 바꾸는 목표는 같았다.
...
이 규모에서는 공식 문서나 커뮤니티에서 바로 참고할 사례를 찾기 어려웠다. 소스 코드를 직접 읽고, 운영 메트릭으로 가설을 세우고, 실제 환경에서 검증하는 과정을 반복해야 했다. 랑데부 해싱과 복제 계수의 내부 동작을 정확히 이해한 뒤에야, 단순한 장비 증설이 아니라 장애를 만들지 않는 전환 전략을 설계할 수 있었다. 이 클러스터가 이후 어떤 방향으로 더 최적화됐는지는 후속편인 {{< relref "04-2026-07-three-stage-optimization.md" >}}에서 이어진다.
