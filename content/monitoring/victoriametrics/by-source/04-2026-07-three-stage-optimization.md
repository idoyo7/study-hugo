...
```mermaid
flowchart LR
  C["수집 레이어<br/>비서비스 컨테이너 제외"] --> S["저장 레이어<br/>RetentionPeriod 12개월→6개월"]
  S --> Q["조회 레이어<br/>쿼리 36분할"]
```
...
```mermaid
flowchart TD
  A["① 쿼리 파싱<br/>함수·필터·윈도우 분리"] --> B["② vmstorage 팬아웃<br/>필터+시간범위를 전 노드에 전송"]
  B --> C["③ vmstorage 응답<br/>TSID 매칭 후 매칭 블록만 스트리밍"]
  C --> D["④ 병합과 반환<br/>시간순 정렬·병합 후 dedup"]
```
...
```mermaid
flowchart LR
  Q["단일 집계 쿼리<br/>전체 container 대상"] --> F["container 접두사 기준 36분할"]
  F --> A["a.* 쿼리"]
  F --> B["b.* 쿼리"]
  F --> Z["z.* 쿼리"]
  F --> D["0.* ~ 9.* 쿼리"]
