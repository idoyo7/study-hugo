...
장비 증설 대신 파이프라인을 구간별로 나누어 분석해 다음 세 레이어에서 최적화를 진행했다.
...
파이프라인 순서로 나타내면 다음과 같다.

{{< flow caption="장비 증설 없이 데이터 파이프라인 세 레이어를 각각 최적화했다." >}}
{
  "nodes": [
    {"id": "c", "col": 0, "row": 0, "label": "수집 레이어", "sub": "비서비스 컨테이너 제외", "kind": "proc"},
    {"id": "s", "col": 1, "row": 0, "label": "저장 레이어", "sub": "Retention 12→6개월", "kind": "store"},
    {"id": "q", "col": 2, "row": 0, "label": "조회 레이어", "sub": "쿼리 36분할", "kind": "query"}
  ],
  "edges": [
    {"from": "c", "to": "s", "rate": 640},
    {"from": "s", "to": "q", "rate": 640}
  ]
}
{{< /flow >}}
...
이 옵션들로 안전 장치를 설정했지만, 리소스 제한은 쿼리가 시스템을 과도하게 사용하는 일을 막을 뿐 쿼리 결과셋 자체를 줄이지는 못한다. 제한에 걸리면 쿼리는 오류로 실패한다. OOM을 근본적으로 줄이려면 한 번의 쿼리가 처리하는 시계열 수를 줄여야 했다. 먼저 메트릭이 `vmstorage`에 저장되는 방식과 `vmselect`가 데이터를 조회하는 방식을 살펴봤다.
...
{{< flow caption="vmselect의 쿼리 처리 4단계 — 파싱 → vmstorage 팬아웃 → 응답 → 병합" >}}
{
  "nodes": [
    {"id": "parse",  "col": 0, "row": 0, "label": "① 쿼리 파싱",       "sub": "함수·필터·윈도우 분리",              "kind": "query"},
    {"id": "fanout", "col": 1, "row": 0, "label": "② vmstorage 팬아웃", "sub": "필터+시간범위를 전 노드에 전송",       "kind": "query"},
    {"id": "resp",   "col": 2, "row": 0, "label": "③ vmstorage 응답",  "sub": "TSID 매칭 후 매칭 블록만 스트리밍",    "kind": "query"},
    {"id": "merge",  "col": 3, "row": 0, "label": "④ 병합과 반환",     "sub": "시간순 정렬·병합 후 dedup",           "kind": "query"}
  ],
  "edges": [
    {"from": "parse",  "to": "fanout", "rate": 700},
    {"from": "fanout", "to": "resp",   "rate": 700},
    {"from": "resp",   "to": "merge",  "rate": 700}
  ]
}
{{< /flow >}}
...
쿼리 하나의 결과셋 전체는 `vmselect` Pod 한 대의 메모리에 올라간다. 매칭되는 시계열이 적을 때는 문제가 되지 않지만, 카디널리티가 커질수록 병합 과정에서 메모리 사용량이 급증한다. 겪은 OOM도 이 구조에서 비롯되었다.
...
{{< flow caption="container 레이블 접두사 기준 36개 쿼리 분할(예시로 4개만 표시) — vmselect 부하 분산" >}}
{
  "nodes": [
    {"id": "q",  "col": 0, "row": 1, "label": "단일 집계 쿼리", "sub": "전체 container 대상", "kind": "query"},
    {"id": "ba", "col": 1, "row": 0, "label": "a.* 쿼리",       "kind": "proc"},
    {"id": "bb", "col": 1, "row": 1, "label": "b.* 쿼리",       "kind": "proc"},
    {"id": "bz", "col": 1, "row": 2, "label": "z.* 쿼리",       "kind": "proc"},
    {"id": "bn", "col": 1, "row": 3, "label": "0.* ~ 9.* 쿼리", "kind": "proc"},
    {"id": "v",  "col": 2, "row": 1, "label": "vmselect",       "sub": "부하 분산", "kind": "query"}
  ],
  "edges": [
    {"from": "q",  "to": "ba", "rate": 650},
    {"from": "q",  "to": "bb", "rate": 650},
    {"from": "q",  "to": "bz", "rate": 650},
    {"from": "q",  "to": "bn", "rate": 650},
    {"from": "ba", "to": "v",  "rate": 650},
    {"from": "bb", "to": "v",  "rate": 650},
    {"from": "bz", "to": "v",  "rate": 650},
    {"from": "bn", "to": "v",  "rate": 650}
  ]
}
{{< /flow >}}

- **쿼리 방식** — 변경 전: 단일 쿼리로 전량 집계. 변경 후: `container` 접두사별 36개 쿼리로 분할.
- **예시** — 변경 전: `max(http_requests_total) by (container)`. 변경 후: `max(http_requests_total{container=~"a.*"}) by (container)` ... `max(...{container=~"9.*"}) by (container)`.
...
{{< flow caption="IndexDB 3-슬롯 로테이션 — next → current → prev → 통째로 삭제" >}}
{
  "nodes": [
    {"id": "next",    "col": 0, "row": 0, "label": "next",       "sub": "예비 인덱스",       "kind": "store"},
    {"id": "current", "col": 1, "row": 0, "label": "current",    "sub": "쓰기·읽기 메인",     "kind": "store"},
    {"id": "prev",    "col": 2, "row": 0, "label": "prev",       "sub": "직전 주기 조회용",   "kind": "store"},
    {"id": "drop",    "col": 3, "row": 0, "label": "통째로 삭제", "kind": "store"}
  ],
  "edges": [
    {"from": "next",    "to": "current", "label": "rotate", "rate": 700, "dashed": true},
    {"from": "current", "to": "prev",    "label": "rotate", "rate": 700, "dashed": true},
    {"from": "prev",    "to": "drop",    "label": "rotate", "rate": 700, "dashed": true}
  ]
}
{{< /flow >}}
...
로테이션 시점은 프로세스 기동 시각을 기준으로 계산되지 않는다. UNIX 타임스탬프 0, 즉 1970-01-01을 기준으로 정확히 `RetentionPeriod` 주기마다 동작한다.
...
`RetentionPeriod` 값 하나를 조정해 디스크 고갈 위기를 해소했다. 배포 전에 내부 로테이션 구조와 실제 쿼리 패턴을 함께 검토했기 때문에 서비스 영향 없이 운영을 이어 갔다.

## 3. 수집 레이어 최적화: 수백만 컨테이너에서 주요 지표만 수집

앞선 두 최적화가 저장된 데이터를 더 효율적으로 다루는 작업이었다면, 세 번째 최적화는 저장되기 전의 데이터 유입량을 줄이는 작업이다.

저장 레이어의 디스크 여유를 확보하더라도 유입량 자체가 줄지 않으면 같은 문제는 반복된다. 카디널리티 폭증의 원인을 거슬러 올라가면 수집 단계에 도달한다. `vmstorage`에 저장되는 시계열의 수와 형태는 수집 대상에 의해 결정되기 때문이다(카디널리티 폭증의 일반 원리는 [실전 01 카디널리티]({{< relref "../practice/01-cardinality.md" >}}) 참고).
...
데이터 흐름의 가장 앞단인 수집 파이프라인이 먼저 영향을 받았다. `vmagent`를 비롯한 [수집 파이프라인]({{< relref "../concepts/03-ingestion.md" >}}) 컴포넌트의 부하는 약 70% 줄었다.

줄어든 수집량은 그대로 저장소 유입량 감소로 이어졌다. 컨테이너 수 감소폭(91.6%)보다 Active Time Series 감소폭(63.6%)이 작은 이유는 Active Time Series에 컨테이너 지표뿐 아니라 노드 등 다른 수집 대상에서 생성되는 시계열도 포함되기 때문이다.

유입량 자체를 줄인 만큼, 수집 파이프라인부터 저장소까지 데이터 경로 전반에서 향후 증설 비용을 선제적으로 줄일 수 있게 되었다.

### 결과: Data 영역 디스크 사용량이 감소 추세로 진입

이번 최적화의 효과는 디스크 사용량 추이에서도 확인된다. 신규 데이터 유입과 `RetentionPeriod` 만료에 따른 데이터 삭제가 동시에 발생하는 Data 영역의 사용량 변화를 보면 효과가 더 명확하게 드러난다.
...
- **쿼리 최적화**(조회) — 카디널리티가 높은 레이블 기준으로 쿼리를 분할했다. 응답 속도 82% 개선 및 `vmselect` 메모리 점유율 안정화.
- **RetentionPeriod 최적화**(저장) — IndexDB 로테이션 분석 및 데이터 기반 보관 정책을 재수립했다. 디스크 가용량 확보 및 장비 증설 비용 절감.
- **수집 범위 최적화**(수집) — 수집 대상 대부분을 차지하던 비서비스 컨테이너를 제외했다. 컨테이너 수집 대상 90% 이상 감소, 과도한 메트릭 유입 통제, 디스크 일별 증가율 43% 감소.

인프라 규모가 커질수록 물리 자원을 늘리는 것만으로는 문제를 해결하기 어렵다. 이 사례에서는 시스템의 한계를 정확히 이해하고 실제 데이터에서 해법을 찾는 접근이 장비 증설 없이 위기를 해소하는 결과로 이어졌다. 이 문서가 다룬 카디널리티·저장·수집의 세부 메커니즘은 [개념 03 수집]({{< relref "../concepts/03-ingestion.md" >}}), [개념 04 저장과 압축]({{< relref "../concepts/04-storage-and-compression.md" >}}), [개념 05 쿼리·운영 컴포넌트]({{< relref "../concepts/05-query-and-ops-components.md" >}}), [실전 01 카디널리티]({{< relref "../practice/01-cardinality.md" >}})에서 이어 다룬다.
