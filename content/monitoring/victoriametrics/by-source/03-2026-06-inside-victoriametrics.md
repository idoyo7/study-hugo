...
이 문서는 40분짜리 발표영상을 컴포넌트별로 훑는 지도 역할을 한다. 발표가 데이터의 흐름을 따라 6개 박스를 하나씩 까 보는 구성이므로, 여기서도 그 순서대로 각 박스가 무엇을 하는지 다루되, 내부 동작의 깊은 세부는 그 주제를 전담하는 concepts 문서로 넘긴다.
...
발표는 "TSDB란 무엇인가"부터 짚는다. TSDB(시계열 데이터베이스)는 한 줄로 줄이면 시간 순서대로 기록된 숫자 값들의 연속이다. 9시에 36.5도, 10시에 36.7도처럼 시간축을 따라 숫자가 하나씩 찍히는 데이터이며, CPU 사용률·요청 수·응답 시간 같은 모니터링 지표도 전부 "어떤 시점에 어떤 숫자가 찍혔는가"로 환원된다. 그 숫자가 찍히는 방식에 따라 지표는 **Counter / Histogram / Gauge / Summary** 4타입으로 나뉜다(Prometheus 진영의 표준 분류). 단조 증가하는 Counter는 압축이 극단적으로 잘 되며, 이 성질이 뒤의 압축 설명으로 이어진다. → [개념 01 시계열과 VM]({{< relref "../concepts/01-tsdb-and-victoriametrics.md" >}})

문제는 데이터 양이다. 1초에 한 번만 찍어도 하루 8만 6천 개고 지표가 수만 개면 하루치만 해도 어마어마하다. 그래서 TSDB는 제한된 자원으로 얼마나 잘 압축하느냐가 관건이 되고, 그걸 잘하는 솔루션 중 하나가 VictoriaMetrics다. VM은 **Apache 2.0 라이선스의 오픈소스 TSDB**로 Prometheus와 호환된다(`PromQL` 사용 가능, `remote_write` 프로토콜 그대로 수용). 자체 벤치마크 기준 **메모리 5배·스토리지 7배** 더 효율적이라고 주장한다.
...
발표자가 강조한 두 가지. 첫째, **리레이블링이 두 번 걸린다.** 전 트래픽 공통 룰은 글로벌 단계에서, 특정 리모트에만 걸 룰은 퍼-리모트 단계에서 처리해 티어별로 나눌 수 있다. 둘째, 인메모리 큐가 가득 차면 자동으로 Persistent Queue(디스크)로 떨어진다. `vminsert`와의 네트워크 장애나 지연으로 잠시 전송하지 못해도 `vmagent`는 지표를 잃지 않는다. 운영 관점에서 데이터 유실을 막는 안전장치다. 입력 프로토콜·pull/push 예시·7단계·큐 버퍼링의 상세는 [개념 03 수집]({{< relref "../concepts/03-ingestion.md" >}})에서 다룬다.
...
저장 효율은 압축 인코딩에서 나온다. **Gauge → Delta 인코딩**은 위아래로 변동하는 값의 첫 값만 남기고 차분만 저장한다(원본 `110, 105, 98, 110, 103` → `110, -5, -7, +12, -7`). **Counter → Delta-of-Delta 인코딩**은 단조 증가값의 1차 차분이 거의 일정하고 그 차분의 차분이 0에 수렴한다는 성질을 이용해, 뒤에 100개·1000개를 이어 붙여도 크기가 거의 늘지 않는다. 그래서 Counter가 압축에 극단적으로 유리하다. 어떤 블록이 Counter인지 Gauge인지는 값의 패턴으로 판별하는데, 단조 증가면 Counter, 오르내리면 Gauge다. 단 Counter도 리셋이 너무 잦으면 Gauge로 분류돼 Delta-of-Delta의 이점을 잃는다.
...
`-5`는 최종적으로 `9`로 저장된다. 실운영 실측으로 데이터포인트당 0.92바이트까지 줄어드는 수치는 [개념 04 저장·압축]({{< relref "../concepts/04-storage-and-compression.md" >}})에서 다룬다.
{{% /details %}}

파티션은 **인메모리 파트 → Small 파티션 → Big 파티션**으로 굳어간다. 인메모리에서 플러시되면 Small로 이동해 머지되고, Small들이 모여 Big으로 머지된다(인메모리에 아주 큰 블록이 뭉텅이로 들어오면 곧바로 Big으로 가기도 한다). 머지에는 **Merge Multiplier** 알고리즘을 쓰는데, `(합쳐질 파트들의 총 출력 크기) / (가장 큰 입력 파트 크기)` 값이 클수록 좋다. 비슷한 크기끼리 합칠수록 가성비가 좋다.
...
이 밖에 Deduplication과 Retention이 있다. **Deduplication**은 `dedup.minScrapeInterval`을 10초로 두면 그 구간 중복 값 중 하나만 남기고, Deduplication Watcher가 매시간 지터를 두고 확인한 뒤 필요 시 파티션 전체 머지를 트리거한다. **Retention**은 기본 1개월, 최소 하루에서 최대 100년까지 설정하며 Retention Watcher가 매분 실행된다. 단 파티션 내 보존 기간 내 샘플이 하나라도 있으면 파트 전체가 유지되고, Big 파티션은 경계에 걸치면 통째로 들고 있어야 한다.

이어서 IndexDB 3단계 로테이션을 다룬다. IndexDB에는 삭제되거나 더 이상 수집되지 않는 시계열 엔트리가 누적되는데, 인덱스 자료구조 특성상 단건 삭제 비용이 매우 크다. 그래서 IndexDB를 슬롯 단위로 교체한다. **버전 1.133.0부터 3단계 로테이션**이 정착됐다.
...
마지막 섹션은 운영에서 마주치는 best/worst case, 곧 카디널리티 이야기다. 시계열은 그 자체로 하나의 정의이므로 레이블이 단 하나만 달라져도 다른 시계열로 인식된다. 발표자가 든 예에서 `pod_name`과 `session_id`는 하이 카디널리티를 유발할 수 있는 대표 레이블이다. 변경이 잦은 값은 가능하면 레이블에서 제외한다.
...
카디널리티의 원인·측정 지표(churn rate, slow insert rate)·설계 원칙은 [실전 01 카디널리티]({{< relref "../practice/01-cardinality.md" >}})가 전담한다.
