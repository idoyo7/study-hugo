...
S3 native 저장과 remote_write native 수신으로 매력적으로 보이지만 **다운샘플링이 존재하지 않습니다.** 그래서 400d Mimir tier는 **400d 전부 full-resolution raw가 S3에** 앉는 구조가 됩니다. "5m 해상도 400d" 요구는 원리적으로 충족할 수 없습니다 — 5m 아카이브 요구에서 **항상 탈락합니다**. 대규모 멀티테넌시·Grafana 스택 표준화가 별도의 독립 목표일 때만 재검토 대상입니다.
...
다운샘플링이 없으니 5m 집계를 만들려면 **ruler recording rules로 전 메트릭을 5m마다 쿼리 재평가**해야 합니다. 인제스트-시점 스트림 처리(streamAggr)가 아니라 사실상 수동 사전집계입니다. **대상이 전 메트릭이면 쿼리 기반 평가 부하가 비현실적**입니다. streamAggr가 라우터 vmagent 메모리에서 스트림으로 접는 것과 근본적으로 성격이 다릅니다([07 핵심논점]({{< relref "07-streamaggr-vs-downsampling.md" >}}) 참조).
