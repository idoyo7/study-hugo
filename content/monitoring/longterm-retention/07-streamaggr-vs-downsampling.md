...
Thanos 공식 문서가 명시합니다: 다운샘플링은 **"공간을 절약해 주지 않는다"**. 5m·1h 블록이 raw와 비슷한 크기로 2세트 추가되어 공존 구간 ~3x가 됩니다. 샘플 수로는 5 aggregates/5m vs 10 raw samples/5m라 ~2x 감소지만 포인트당 5개 값 + 시리즈 수 불변(인덱스 불변)이 이를 상쇄합니다. 실제 절감은 오직 raw 삭제에서 나옵니다 — 그래서 Thanos안의 400d 5m+1h가 14.9~30.7 TiB로 부풀고 저장비가 VM 아카이브안의 두 배 이상이 됩니다.
