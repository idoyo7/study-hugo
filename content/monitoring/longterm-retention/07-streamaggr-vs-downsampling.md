...
축별 요약은 아래 표가, 상세 논거는 그 뒤 (a)~(d) 절이 받습니다.
...
Thanos 공식 문서가 명시합니다: 다운샘플링은 **"공간을 절약해 주지 않습니다"**. 5m·1h 블록이 raw와 비슷한 크기로 2세트 추가되어 공존 구간 ~3x가 됩니다. 샘플 수로는 5 aggregates/5m vs 10 raw samples/5m라 ~2x 감소지만 포인트당 5개 값 + 시리즈 수 불변(인덱스 불변)이 이를 상쇄합니다. 실제 절감은 오직 raw 삭제에서 나옵니다 — 그래서 Thanos안의 400d 5m+1h가 14.9~30.7 TiB로 부풀고 저장비가 VM 아카이브안의 두 배 이상이 됩니다.

streamAggr는 output을 1~2개로 제한하면 샘플 볼륨이 raw의 10~30%로 실제 감소합니다. 기본 치환 의미론(또는 `dropInput: true`)으로 raw가 아카이브에 아예 들어가지 않으니 400d에 0.9~2.7 TiB에 그칩니다.

**평가**: 목표 함수가 "저장 비용 최소화"라면 이 축이 결정적입니다. Thanos 다운샘플링은 장기 쿼리 **속도** 장치이지 비용 장치가 아닙니다(→ [08 하지 말 것 #3]({{< relref "08-recommendation-and-pitfalls.md" >}})). VM 측 압축·retention 모델은 [저장과 압축]({{< relref "../victoriametrics/concepts/04-storage-and-compression.md" >}}), 서울 리전 단가 비교는 [06]({{< relref "06-storage-pricing.md" >}}).
...
**평가**: "넓은 커버리지"는 keep-list 없이 2규칙으로 충족되지만 Thanos처럼 무설계 자동은 아닙니다.
...
VM아카이브 월 저장비는 sc1 기준이며 st1로 하면 $415~507로 늘어납니다. Mimir가 부적합한 이유는 다운샘플링 부재이고 raw로 우회하면 시나리오 ① 경제성으로 취급합니다.
