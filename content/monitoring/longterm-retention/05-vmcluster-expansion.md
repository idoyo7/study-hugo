...
90d 이후 데이터에는 세 가지가 전부 과잉이고, 낭비는 여기서 납니다.
...
아키텍처로 보면 **최단 경로**입니다. 기존 클러스터에 설정 한 줄을 얹으면 vmstorage가 자동으로 다운샘플합니다. 걸림돌은 라이선스 쪽입니다 — Enterprise 라이선스(정가 비공개)가 필요해 OSS-우선 제약상 기본 배제합니다.
