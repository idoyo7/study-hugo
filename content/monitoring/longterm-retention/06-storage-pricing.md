...
서울 리전(ap-northeast-2)의 스토리지 단가와 gp3/st1/sc1/S3의 성능·내구성 특성을 여기 모아 두었습니다. 아카이브 볼륨 타입을 고르는 판단 구조도 함께 담았습니다 — gp3로 시작 → 실측 → st1/sc1 최적화. 단가 상세는 이 문서에 있습니다.
...
표에 단서가 몇 개 붙습니다. 같은 리전 S3↔EC2 전송은 무료입니다. gp3의 용량 상한 1 GiB~64 TiB는 2025-09에 올라간 값이고 IOPS 상한 수치는 모두 1MiB I/O 기준입니다. S3 Standard 단가도 50TB를 넘는 구간부터 0.024/0.023으로 더 내려갑니다.
...
**sc1이 S3 Standard보다 쌉니다.** S3를 들이는 실익은 단가가 아니라 **11-nines 내구성 모델과 사후 재계산 여지**입니다. 그 대가로 운영 컴포넌트 수(Receive/StoreGW/Compactor/캐시)와 PromQL 제약을 받습니다. 이 트레이드오프의 비용 종합은 [07 비용 비교표]({{< relref "07-streamaggr-vs-downsampling.md" >}})에서 다룹니다.

### IA/Glacier IR은 primary 저장소가 될 수 없다

저장 단가만 떼어 보면 S3 Standard-IA($0.0138)와 Glacier Instant($0.005)는 매력적입니다. 여기에 **GB당 리트리벌 수수료**(IA $0.01/GB, GIR $0.03/GB)가 따라붙습니다. 자주 읽는 primary 아카이브에 못 쓰는 이유입니다:
...
- EBS 세 종의 내구성 등급은 같습니다 — 모두 단일 AZ 블록 스토리지입니다. 차이는 **매체 속도와 IOPS 상한**입니다.
- st1은 gp3 대비 **44% 저렴**하고 스루풋 여유도 커서 **중간 안전지대**로 쓸 수 있습니다.
- 이 챕터의 최대 제약은 sc1의 **250 IOPS 상한**입니다 — 아래 3절에서 다룹니다.

## 3. ★ 아카이브 볼륨 타입 선택 가이드 — "gp3 기본 아니야?"에 대한 답

**맞습니다 — 시작은 gp3가 기본값입니다.** 분석에서 sc1을 올린 것은 "최저가 옵션"이어서였습니다. sc1은 채택 전 검증이 필요한 공격적 최적화입니다.
...
- VM은 LSM 구조여서 IO가 대체로 순차적입니다(머지·압축). VM 문서도 HDD성 스토리지 동작을 인정합니다(→ [VM 스토리지·압축]({{< relref "../victoriametrics/concepts/04-storage-and-compression.md" >}})).
- 아카이브는 "거의 안 읽고, 조금씩 쓰는" 워크로드라 cold HDD 프로파일과 이론상 부합합니다.
...
2. **절대액 차이가 작습니다.** 시나리오 ②의 아카이브는 **0.9~2.7 TiB**뿐입니다:
...
| sc1 | $16~47 | $66~199 |
   ...
   월 **$66~199**을 아끼려고 검증 안 된 스토리지에 400일치 아카이브를 올려두는 것은 순서가 틀렸습니다. 맞는 순서는 **gp3로 검증 → IO 실측 → 여유가 크면 st1/sc1로 최적화**입니다.
3. 단, **시나리오 ①(raw 400d, 9~18 TiB)로 가는 경우엔 이야기가 다릅니다** — gp3 $820 vs sc1 $157/월로 차이가 커져 저가 볼륨 검증이 우선순위에 올라옵니다.

### 전환 방법 (나중에 최적화할 때)

- AWS 레벨에서는 EBS Elastic Volumes로 볼륨 타입을 라이브 변경할 수 있습니다(gp3↔st1↔sc1).
- k8s에서는 PVC의 StorageClass를 바꿀 수 없습니다. 정석은 **새 StorageClass·PVC로 vmsingle 재배포 + (필요 시) vmbackup/vmrestore로 데이터 이전**입니다. 아카이브는 유실 허용 등급이라 재축적으로 갈음해도 됩니다.
...
옵션별 월 저장비 종합(VM아카이브/Thanos/Mimir/확장/확장+Ent)은 이 문서의 몫이 아닙니다 — [07 비용 비교표]({{< relref "07-streamaggr-vs-downsampling.md" >}})가 주인입니다. 이 문서는 볼륨 단가와 단일 볼륨 환산까지만 확정합니다.
