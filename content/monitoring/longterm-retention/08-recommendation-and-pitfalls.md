...
{{< callout type="info" >}}
한눈에
- 권장은 VM OSS 아카이브안(라우터 RW#4 + streamAggr 5m → vmsingle-archive 400d)입니다. 4안 중 비용이 가장 낮고(월 $385~416), 가역적이고, service 무상태에 영향이 없고, 신규 기술 0이며, MetricsQL이 그대로 남습니다.
- 업계 선례가 "장기 = 집계만"을 뒷받침합니다. Criteo(계층화 절감), Uber M3(1h/5y rollup), MHI Vestas(VM 13개월 안정 운영) — 방향성 근거일 뿐 예산 근거는 아닙니다.
...
- 가역적입니다. RW#4는 언제든 Thanos Receive로 갈아끼워 Thanos안으로 넘어갈 수 있습니다. 드라이런에서 f가 예상을 크게 벗어나면 그 시점에 재평가합니다.
- service는 무상태 그대로입니다. 이미 설계한 라우터 vmagent 패턴에 RW#4로 자연 결합하고, service 클러스터는 손대지 않아 무상태 원칙을 유지합니다.
- 신규 기술 0입니다. 신규 stateful 컴포넌트가 1개(vmsingle)뿐이고 그마저 기존과 동일 기술스택이어서 새로 배울 것이 없습니다.
- MetricsQL은 그대로 갑니다. 아카이브도 VM이라 `keep_metric_names`로 기존 쿼리가 datasource만 바꾸면 동작하고, 미확인 의존도 리스크가 사라집니다.

잔여 리스크는 "확정 집계가 재조사에 부족할 가능성"입니다. hot 90d raw가 최근 장애의 golden window를 담당하고 >90d 재조사는 추세·수준 비교가 주라는 전제에서 이를 수용합니다. 이 전제 때문에 아카이브 검증 전 hot retention 축소는 금지됩니다(§3-10).

스토리지 클래스는 gp3(기본값)로 시작하는 것이 맞습니다. 본 분석의 sc1은 "최저가 옵션"일 뿐입니다. 아카이브 볼륨이 작아(0.9~2.7 TiB) gp3 프리미엄이 월 $66~199 수준입니다. 검증 기간엔 gp3로 안전하게 가고 이후 IO 실측을 보고 st1/sc1 최적화를 판단합니다. 단가·볼륨 선택 상세는 [06번]({{< relref "06-storage-pricing.md" >}}).
...
1. VM에 S3 primary/티어링 기대 — 메트릭 엔진의 쿼리 가능한 오브젝트 스토리지는 미출시·일정 미약속입니다("VictoriaLogs 이후 검토" 발언과 #38 self-assign은 신호일 뿐입니다). 로드맵 베팅으로 설계를 미루지 말 것.
