...
400d 보관 결정의 최종 종합입니다. VM 아카이브안을 왜 권장하는지의 근거를 한자리에 모으고, 업계 선례로 패턴을 뒷받침하며, 검증에서 기각된 함정 10개와 진행 전 드라이런 2주 실측 목록을 정리합니다.
...
- **최저 저장비** — 시나리오 ② 월 **$385~416**(sc1). 4안 중 최저다. 단순 확장안($1,642) 대비 **~70% 절감**이고 Thanos안($780~1,200 + 컴퓨트)의 절반 이하이며 컴퓨트 차이는 더 벌어진다.
- **가역적** — RW#4는 언제든 Thanos Receive로 갈아끼워 Thanos안으로 전환 가능하다. 드라이런에서 f가 예상을 크게 벗어나면 그 시점에 재평가한다.
- **service 무상태·무영향** — 이미 설계한 라우터 vmagent 패턴에 RW#4로 자연 결합한다. **service 클러스터는 손대지 않아 무상태 원칙을 유지**한다.
- **신규 기술 0** — 신규 stateful 컴포넌트는 **1개(vmsingle)**뿐이며 그마저 기존과 동일 기술스택이다. 신규 기술 학습이 없다.
- **MetricsQL 보존** — 아카이브도 VM이라 MetricsQL을 그대로 유지한다. `keep_metric_names`로 기존 쿼리가 datasource만 바꾸면 동작해 미확인 의존도 리스크가 소멸한다.
...
**스토리지 클래스**: 본 분석의 sc1은 "최저가 옵션"일 뿐, **시작은 gp3(기본값)가 맞습니다.** 아카이브 볼륨이 작아(0.9~2.7 TiB) gp3 프리미엄이 월 $66~199 수준이므로 검증 기간엔 gp3로 안전하게 가고, 이후 IO 실측을 보고 st1/sc1 최적화를 판단합니다. 단가·볼륨 선택 상세는 [06번]({{< relref "06-storage-pricing.md" >}})입니다.

## 2. 업계 선례 — "장기 = 집계만"은 표준 패턴

VM 아카이브안의 "hot raw 단기 + 장기는 집계 tier"라는 계층화는 신규 발명이 아니라 대규모 운영에서 반복 확인된 패턴입니다.
...
이 선례들은 방향성 근거일 뿐 예산 산정 근거가 아닙니다. 비용 수치는 [07번 종합표]({{< relref "07-streamaggr-vs-downsampling.md" >}})와 [06번 단가]({{< relref "06-storage-pricing.md" >}})의 서울 실측치만 씁니다.

## 3. 하지 말 것 (검증에서 기각·경고된 사항) 10선

이 목록은 리서치 과정에서 "그럴듯하지만 검증 결과 틀렸거나 함정인" 것들입니다. 각 항목은 400d 설계에서 반복적으로 나오는 오판입니다.
...
5. **Thanos/Store Gateway가 읽는 버킷을 S3-IA/Glacier IR에** — GB당 리트리벌 수수료(IA $0.01 / GIR $0.03)가 쿼리·동기화마다 발생한다. IA/GIR는 vmbackup 사본 전용이다.
6. **Thanos Receive를 service 클러스터에** — hashring 상태를 보유한 StatefulSet이라 설정 변경마다 ~5분 unready가 된다. 무상태 원칙과 양립 불가하며 반드시 chain에 둔다.
7. **vmagent→Thanos/Mimir 레그를 기본 queues로 송신** — out-of-order 409를 유발한다. **`-remoteWrite.queues=1` 필요**하고 버퍼 한도 설계(그 자체의 백프레셔·OOM 리스크 포함)가 따라야 한다. per-URL queues는 v1.135.0+.
...
아래는 소스가 명시적으로 "검증 필요/실측 필요"라 표시한 항목입니다. 확정치가 아니므로 예산·설계를 이 가정값으로 굳히지 말고 드라이런으로 확정합니다.
...
- **vmctl 시드 / vmbackup×S3-IA** · Standard 시작 후 관찰 권장 — 기존 80d raw를 vmctl로 라우터 경유 재주입해 아카이브 시드 가능 여부와, vmbackup 증분 오브젝트 churn·S3-IA 최소 30일 과금의 상호작용을 확인한다.
