...
# 08 · 권장안 A 종합 근거 · 업계 선례 · 하지 말 것 10선

400d 보관 결정의 최종 종합이다. A안을 왜 권장하는지의 근거를 한자리에 모으고, 업계 선례로 패턴을 뒷받침하며, 검증에서 기각된 함정 10개와 진행 전 드라이런 2주 실측 목록을 정리한다.

> 관련 블록: [02 A안 상세]({{< relref "02-option-a-vm-archive.md" >}}), [07 핵심논점·비용종합·판단트리]({{< relref "07-streamaggr-vs-downsampling.md" >}}), [06 스토리지 단가]({{< relref "06-storage-pricing.md" >}}), [01 문제·2축]({{< relref "01-problem-and-axes.md" >}})

## 1. 권장: A안 — 라우터 RW#4 + streamAggr 5m → vmsingle-archive 400d

핵심 논점(사전집계 vs 사후집계)과 비용 종합표는 [07번]({{< relref "07-streamaggr-vs-downsampling.md" >}})이 주인이다. 여기서는 A안을 고른 근거만 종합한다.

| 선정 근거 | 내용 |
|---|---|
| **최저 저장비** | 시나리오 ② 월 **$385~416** (sc1). 4안 중 최저 — D안($1,642) 대비 **~70% 절감**, B안($780~1,200 + 컴퓨트)의 절반 이하이고 컴퓨트 차이는 더 벌어진다 |
| **가역적** | RW#4는 언제든 Thanos Receive로 갈아끼워 B안으로 전환 가능. 드라이런에서 f가 예상을 크게 벗어나면 그 시점에 재평가 |
...
A안의 "hot raw 단기 + 장기는 집계 tier"라는 계층화는 신규 발명이 아니라 대규모 운영에서 반복 확인된 패턴이다.

| 사례 | 규모·구성 | 시사점 |
|---|---|---|
| **Criteo** | 약 **10억 활성 시리즈**, VM cluster 계층화(고카디널리티·고churn은 단기 7d / 집계는 90d / 장기는 별도 클러스터 1y+) | 계층화로 자릿수 비용 절감을 자체 보고 — A안의 hot/아카이브 분리와 동형 |
...
| 4 | **Mimir를 5m 장기 tier로 선택** | 다운샘플링이 OSS/GEM 어디에도 없고 3.0에서도 없다. Adaptive Metrics는 Grafana Cloud 전용 (→ [04번]({{< relref "04-option-c-mimir.md" >}})) |
...
**롤아웃 순서**(요약, 상세는 [02번]({{< relref "02-option-a-vm-archive.md" >}})): ① vmsingle 배포 → ② RW#4 드라이런 2주(f·시리즈 수·카운터 오분류·rate/histogram_quantile 정합 확인) → ③ 예외 규칙 보강 → ④ Grafana DS + 재조사 대시보드 1개 시범 이관 → ⑤ vmbackup CronJob → ⑥ (선택) vmctl 시드. **모니터링**: RW#4의 `vmagent_remotewrite_pending_data_bytes`, 라우터 vmagent 메모리, vmsingle 디스크 증가율.
...
- 교차: [02 A안]({{< relref "02-option-a-vm-archive.md" >}}), [06 단가]({{< relref "06-storage-pricing.md" >}}), [07 핵심논점]({{< relref "07-streamaggr-vs-downsampling.md" >}}), [VM 챕터 07 운영]({{< relref "../victoriametrics/07-operations-at-scale.md" >}})
