...
- RUM은 SDK 교체가 정답입니다. dd browser-sdk를 걷어내고 `@hyperdx/browser`로 교체합니다.
- ClickHouse는 self-host + Altinity operator(i8g 로컬 NVMe + ReplicatedMergeTree)로 갑니다. 메트릭은 VictoriaMetrics+Grafana로 분리 존치하고 HyperDX에 넣지 않습니다.
- 최대 리스크는 OSS 접근통제 공백입니다(SSO/RBAC/멀티테넌시/감사로그 전무). RUM 대체는 공개 프로덕션 전례도 없습니다. 자체 PoC 성공을 Wave 1 컷오버의 필수 게이트로 삼습니다.
- 실행은 2주 스프린트 6개로 쪼갭니다. 각 스프린트는 앞 게이트를 통과해야 다음으로 넘어갑니다.
{{< /callout >}}

RUM/Datadog 탈출 실행 계획입니다. rip-and-replace 대신 dual-write/dual-instrument → 병행 검증 → 단계적 컷오버로 끌고 갑니다. 앞선 페이지들의 판정([HyperDX 심층]({{< relref "01-hyperdx-deep-dive.md" >}}), [Datadog RUM 커버리지]({{< relref "02-datadog-rum-coverage.md" >}}), [프록시 매핑]({{< relref "03-dd-proxy-mapping.md" >}}), [대체 매트릭스]({{< relref "04-datadog-replacement-matrix.md" >}}))을 스프린트 단위 액션으로 접습니다. ClickHouse 배포·스토리지·operator 상세는 [ClickHouse 자체 운영]({{< relref "../clickhouse/_index.md" >}}) 챕터로 위임했고, 이 페이지는 RUM/Datadog 이관 실행만 다룹니다.

## Executive 판정 (6줄)

- 프록시는 브릿지일 뿐입니다. OTel Collector `datadogreceiver`가 받는 것은 Datadog Agent intake(로그·인프라 메트릭·APM 트레이스)뿐입니다. 브라우저 RUM·세션 리플레이 intake는 수신 대상 자체가 아닙니다 `✓`. 서버사이드 신호의 단기 무중단 다리로만 씁니다.
- RUM은 SDK 교체가 정답입니다. dd browser-sdk를 걷어내고 `@hyperdx/browser`로 교체합니다. dd browser-sdk의 `proxy` 옵션은 과도기 트래픽 통제용으로만 활용합니다. 변환에는 쓰지 않습니다.
- ClickHouse는 self-host + Altinity operator로 갑니다. i8g 로컬 NVMe + ReplicatedMergeTree가 기본형입니다(상세는 clickhouse 챕터). SharedMergeTree는 Cloud 전용이라 self-host에서 재현할 수 없습니다 `✓`.
- 메트릭 계층은 HyperDX로 몰지 않습니다. 메트릭·대시보드·모니터·SLO는 `VictoriaMetrics + Grafana + Sloth/Pyrra`로 분리 존치하고 Grafana가 ClickHouse도 조회해 단일 UI로 봉합합니다.
- 최대 리스크는 OSS 접근통제 공백입니다. OSS HyperDX는 SSO/RBAC/멀티테넌시/감사로그가 전무합니다. RBAC는 Managed(ClickHouse Cloud) 전용으로만 GA됐습니다 `✓`. "앱 레벨 RBAC/SSO"와 "self-hosted EKS/NVMe"는 함께 가질 수 없습니다.
- 전례가 없으므로 PoC를 진입 게이트로 삼습니다. RUM 대체는 공개 프로덕션 전례가 없는 개척 경로입니다 `?`. 자체 PoC 성공을 Wave 1 컷오버의 필수 통과 조건으로 명문화합니다.

## 리스크 Top 5

- R1 · OSS 접근통제 공백 — SSO/RBAC/멀티테넌시/감사로그 전무, RBAC는 Cloud 전용 GA `✓`. "앱 레벨 RBAC/SSO"와 "self-hosted"는 함께 성립하지 않습니다.
...
- R2 · RUM 대체 프로덕션 전례 부재 `?` — 기술 대등성은 높으나 검증된 전환 사례 없음. Datadog RUM→OTel/ClickHouse 인테이크 번역기의 공개 구현체·프로덕션 운영 사례도 2026-07 딥리서치 재검색(2회)에서 나오지 않음 — 전량 자체 구축 전제 `?`.
...
각 스프린트가 끝나면 dual-write/dual-instrument 병행 검증 결과를 보고 다음 스프린트로 넘어갈지 결정합니다. 게이트를 통과하지 못하면 다음 Wave로 넘어가지 않습니다.
...
- [ ] **게이트**: 지속 롤아웃(단일 종료 게이트 없음), 각 잔여 트랙은 개별 PoC를 진입 조건으로 둡니다
...
- (CH 배포 측 오픈 퀘스천 — 재수화 TB당 소요 시간, i7i 순차 대역 실측 등 — 은 [clickhouse 챕터]({{< relref "../clickhouse/_index.md" >}})에서 다룹니다.)
{{% /details %}}

## 우리 케이스에서는

전제부터 다릅니다. 이 로드맵은 조사 문서의 세 전제 — ① Datadog **RUM 대체**가 드라이버, ② ClickHouse를 관측성 외 **범용 분석**에도 쓸 예정, ③ 이미 **EKS·인프라 운영 인력 보유** — 위에 서 있습니다. [로깅 챕터]({{< relref "../logging/08-recommendation.md" >}})의 결정은 **로그 내재화** 관점에서 나왔고 전제가 더 좁습니다. 두 문서는 모순이 아니라 적용 범위가 다릅니다.

로깅 챕터의 결정을 이 로드맵이 뒤집지는 않습니다:

- 로그는 VictoriaLogs입니다(D 결정 유지). 위 Sprint 3의 "로그 이관"은 *Datadog에서 빠져나오는 신호를 어디로 보낼지*의 문제이지, 로깅 챕터가 고른 [VictoriaLogs]({{< relref "../logging/03-victorialogs.md" >}})를 ClickHouse로 갈아치우라는 뜻이 아닙니다. volatile한 istio access log 경로에는 여전히 단일 바이너리 VictoriaLogs가 더 가볍습니다.
- 통합 저장소(ClickHouse)는 earn-it-last입니다. 이 로드맵이 CH self-host를 밀 수 있는 유일한 이유가 전제 ②·③입니다 — CH가 **RUM/범용 분석 용도로 이미 정당화**되어 들어오는 경우에 한해, 관측성 데이터를 그 위에 더하는 한계비용이 낮습니다. CH를 순수 로그 저장만 하려고 새로 도입하는 상황이라면 로깅 챕터의 "self-hosted CH를 1차 채택안으로 밀지 않는다"가 그대로 유효합니다.
- 오너십이 최종 관문입니다. 우리는 PLG 방치 이력이 있는 소규모 플랫폼 팀입니다. 전제 ③(전담 오너)을 **명시적 오너 + 런북 + 정기 리뷰**로 명시해 두지 않으면, R1(접근통제 공백)·R4(NVMe lifecycle)가 그대로 폭탄이 됩니다. 그 경우 self-host CH 대신 Managed(ClickHouse Cloud / Altinity.Cloud) 견적과 반드시 비교합니다. RUM은 웹 코어만 HyperDX로 떼어내되 나머지는 Datadog 잔류가 현실적입니다.

착수 전 필수 확인(RUM 도메인 공통): Datadog RUM usage를 소스별(웹/모바일)로 분해해 모바일 비중부터 측정합니다 — 모바일이 과반이면 웹 전용 HyperDX는 청구서를 별로 못 줄이면서 관리 스택(CH+MongoDB)만 늘립니다. 도메인 큰 그림은 [RUM 내재화]({{< relref "_index.md" >}})에서 다룹니다.

근거 등급은 조사 문서의 판정을 이어받습니다. `≈`은 자릿수 추정, `?`은 공개 전례·검증 부재를 뜻합니다. 조사 기준 2026-07.
