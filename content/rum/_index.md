...
웹 RUM은 **HyperDX(ClickStack)** 로 탈출할 여지가 크다. `@hyperdx/browser`가 rrweb 세션 리플레이 + 에러 + Web Vitals + 네트워크 캡처 + **백엔드 트레이스 연동**(TraceId·rum.sessionId 상관)까지 지원해 Datadog 웹 RUM 대체로 현실성이 있다. HyperDX를 로그·트레이스 스토어로 함께 보는 관점은 [로깅 챕터의 솔루션별 특징]({{< relref "../logging/01-solutions.md" >}})(HyperDX/ClickStack 절) 참고.
