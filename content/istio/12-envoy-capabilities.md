...
이 챕터는 지금까지 Envoy를 Istio의 부품으로만 다뤘다. [01]({{< relref "01-mesh-basics.md" >}})에서는 파드에 붙는 사이드카로, [02]({{< relref "02-istiod-control-plane.md" >}})에서는 xDS를 받아가는 대상으로, [08]({{< relref "08-envoyfilter-extension.md" >}})에서는 EnvoyFilter가 패치하는 설정 덩어리로 등장했다. 이 문서는 방향을 뒤집어 **Istio를 걷어낸 Envoy 자체의 기능 카탈로그**를 그린다 — 조립되기 전의 부품이 원래 무엇을 할 수 있는가. 조립 이야기는 [13]({{< relref "13-istio-envoy-assembly.md" >}})으로 넘긴다.
...
Envoy의 자기 정의는 홈페이지 첫 문장에 있다 — "A high performance C++ distributed proxy designed for single services and applications, as well as a communication bus and 'universal data plane' designed for large microservice 'service mesh' architectures." 서비스 하나 앞에 세우는 프록시와 메시 전체의 데이터 플레인이 같은 프로그램이라는 뜻이다.

배치 형태도 문서가 못 박는다: "Envoy is a self contained process that is designed to run alongside every application server." 앱과 같은 주소 공간이 아니라 옆에서 따로 도는 프로세스다. 여기서 따라오는 결론이 [01]({{< relref "01-mesh-basics.md" >}})이 "공통 관심사의 중복"으로 서술한 문제의 해답이다 — "Envoy works with any application language. A single Envoy deployment can form a mesh between Java, C++, Go, PHP, Python, etc." 재시도 라이브러리를 언어마다 다시 짜는 대신, 프로세스 하나가 언어와 무관하게 같은 동작을 제공한다.
...
같은 소개 문서가 다른 자리에서는 Envoy를 "an L7 proxy and communication bus designed for large modern service oriented architectures"라고도 부른다. 둘 다 공식 표현이고 서로 어긋나지 않는다 — 앞은 구현 구조(L3/L4 코어 + L7 레이어), 뒤는 실제 쓰임새를 말한 것이다.

프로토콜과 운영 기능도 프록시 자체가 들고 있는 것들이다.
...
**HTTP/3 상태는 버전에 붙어 있다.** 위 표의 downstream/upstream 구분은 이번에 확인한 최신 문서 트리(1.40.0-dev) 기준이다. Envoy 마이너 릴리스마다 바뀔 수 있으므로, 인용할 때는 **실제 배포 중인 Envoy 버전의 문서**로 다시 확인해야 한다. hot restart도 마찬가지로 "기존 커넥션이 새 프로세스로 옮겨간다"는 뜻이 아니다 — drain 동안 **기존 커넥션을 마저 처리**하는 것이다.
{{< /callout >}}

## 2. 코어 모델 — listener에서 endpoint까지

Envoy 설정을 읽는 문법은 사실 하나뿐이다. 요청이 들어와서 나가기까지 거치는 단계가 그대로 설정의 뼈대다.

{{< flow caption="Envoy의 코어 모델 — 각 단계가 설정 파일의 최상위 항목이자 기능이 붙는 자리다." >}}
{
  "nodes": [
    { "id": "L", "col": 0, "row": 0, "label": "listener", "sub": "IP/포트 바인드 · 커넥션 수락", "kind": "src" },
    { "id": "N", "col": 1, "row": 0, "label": "네트워크 필터 체인", "sub": "전송 소켓 + 네트워크 필터", "kind": "proc" },
    { "id": "H", "col": 2, "row": 0, "label": "HCM", "sub": "HTTP connection manager", "kind": "proc" },
    { "id": "F", "col": 3, "row": 0, "label": "HTTP 필터 체인", "sub": "스트림마다 실행 · 끝이 router", "kind": "proc" },
    { "id": "C", "col": 4, "row": 0, "label": "cluster", "sub": "LB가 endpoint 선택 · 커넥션 풀", "kind": "sink" }
  ],
  "edges": [
    { "from": "L", "to": "N", "label": "리스너 필터", "rate": 650 },
    { "from": "N", "to": "H", "rate": 650 },
    { "from": "H", "to": "F", "rate": 650 },
    { "from": "F", "to": "C", "label": "라우트 선택", "rate": 650 }
  ]
}
{{< /flow >}}

listener는 "responsible for binding to an IP/port, accepting new TCP connections (or UDP datagrams) and orchestrating the downstream facing aspects of request processing"이다. 커넥션이 들어오면 리스너 필터가 먼저 돌고, 그다음 전송 소켓 설정과 네트워크 필터로 이루어진 필터 체인이 매칭된다.

그 네트워크 필터 체인의 **마지막이자 가장 중요한 필터가 HTTP connection manager(HCM)**다. HCM이 하는 일을 문서는 이렇게 적는다 — "translates raw bytes into HTTP level messages and events (e.g., headers received, body data received, trailers received, etc.)". 바이트가 HTTP가 되는 지점이 여기다. 그리고 HCM은 다음을 한곳에서 관리한다.
...
L7 기능 대부분이 "HCM 아래"에 모여 있다는 사실이, [11]({{< relref "11-request-path-anatomy.md" >}})에서 "HTTP로 파싱되는 홉에서만 L7 기능이 생긴다"고 한 규칙의 구현 쪽 이유다.

HTTP 필터는 스트림마다 실행되고, 그중 **router 필터가 목적지를 정한다**. 문서 표현 그대로 "When decodeHeaders() is invoked on the router filter, the route is selected and a cluster is picked." 여기서 고른 cluster의 로드밸런서가 "picks an endpoint when a new request arrives"하고, 커넥션 풀이 그 엔드포인트로의 커넥션을 얻거나 새로 맺는다 — **이때 서킷 브레이커 한도가 걸린다.**

즉 복원력 설정이 두 층에 나뉘어 사는 이유가 여기서 나온다. 재시도처럼 "어디로 보낼지"에 붙는 것은 route에, 커넥션 한도처럼 "상대를 어떻게 대할지"에 붙는 것은 cluster에 있다.
...
로드밸런싱 정책도 프록시가 이름을 가진 채로 들고 있다.
...
{{< callout type="important" >}}
**Istio CRD는 이 표의 스위치를 밖으로 꺼낸 창구다.** 대략의 위치만 말하면 route 층(재시도·타임아웃)은 `VirtualService`, cluster 층(서킷 브레이킹·아웃라이어 감지·로드밸런싱)은 `DestinationRule` 쪽에서 노출된다 — [01]({{< relref "01-mesh-basics.md" >}})의 CRD 표가 이미 그렇게 묶어 두었다. **필드 단위의 정확한 대응과, Istio가 어떤 기본값을 얹는지는 [13]({{< relref "13-istio-envoy-assembly.md" >}})에서 다룬다.** 재시도 예산처럼 표준 CRD 대응을 이 문서의 재료로 확인하지 못한 항목도 있다.
{{< /callout >}}

## 4. 관측성 — Envoy가 만들어내는 것

[06]({{< relref "06-observability-points.md" >}})은 메시가 준 지표를 **어떻게 쓰는지**를 다뤘다. 여기서는 그 지표가 애초에 어디서 만들어지는지, 프록시 쪽 생산 메커니즘만 본다.

### 통계

Envoy의 stats는 **꽂아 바꿀 수 있는 싱크 모델**이다 — "As of the v2 API, Envoy has the ability to support custom, pluggable sinks." 표준 싱크 구현이 함께 제공되고, 필요하면 커스텀 싱크를 붙인다. 메트릭 타입은 셋뿐이다.
...
액세스 로깅은 2절에서 본 대로 **HCM이 중앙에서 관리하는 책임 중 하나**다.

### 트레이싱

여기가 이 절에서 가장 중요한 지점이다. **Envoy는 트레이싱 헤더를 통과시키기만 하는 것이 아니라, 스팬을 직접 만들어 보낸다** — "Envoy automatically sends spans to tracing collectors." 사이드카로 동작할 때 인바운드에서는 SERVER 스팬, 아웃바운드에서는 CLIENT 스팬이 되고, 독립적인 홉으로 명시적 스팬을 만들게 하려면 `spawn_upstream_span`을 쓴다. `x-request-id` UUID도 Envoy가 발급한다.

지원하는 컨텍스트 포맷은 하나가 아니다.
...
B3와 W3C 사이의 추출 fallback은 설정으로 정한다. [06]({{< relref "06-observability-points.md" >}})이 "트레이싱만은 앱의 헤더 전파가 필요하다"고 한 경계가 여기서 선명해진다 — **스팬 생성·ID 발급·포맷 해석까지가 프록시의 몫**이고, 앱이 인바운드에서 받은 헤더를 아웃바운드 호출에 실어 나르는 in-process 전파만 남는다.

{{< callout type="info" >}}
**이 절에서 하나는 간접 확인이다.** 액세스 로그 전용 아키텍처 개요 페이지는 이번 확인 시점에 404였다. 액세스 로깅이 HCM이 중앙 관리하는 책임이라는 것은 HTTP connection management 문서로 확인했지만, 액세스 로그 자체의 포맷·싱크 설명은 이 문서의 재료에 들어 있지 않다.
{{< /callout >}}

## 5. 동적 설정 — xDS는 Envoy의 API다

[02]({{< relref "02-istiod-control-plane.md" >}})는 xDS를 "istiod가 프록시에 설정을 내려보내는 프로토콜 묶음"으로 소개했다. 종류별 표도 거기에 있다. 이 문서의 각도에서 보면 소유 관계가 반대다.

Envoy 자신의 xDS 문서는 이렇게 시작한다 — "Envoy discovers its various dynamic resources via the filesystem or by querying one or more management servers. Collectively, these discovery services and their corresponding APIs are referred to as xDS."
...
Istio 쪽 서술도 이 방향과 맞는다. istio.io 1.5 릴리스 노트는 컨트롤 플레인 비용을 이야기하며 이들을 "Envoy xDS APIs"라고 부른다 — Istio가 정의한 규격이 아니라 **Envoy의 API를 istiod가 서빙한다**는 표현이다.

그래서 [02]({{< relref "02-istiod-control-plane.md" >}})의 CPU 이야기와 [11]({{< relref "11-request-path-anatomy.md" >}})의 agent 중계는, 부품 쪽에서 보면 "관리 서버 구현 하나가 이 API를 어떻게 서빙하는가"의 문제가 된다. 그 구현 선택이 어디서 갈리는지는 [13]({{< relref "13-istio-envoy-assembly.md" >}})의 주제다.

## 6. 확장 지점 — 안에서 돌리거나, 밖에 물어보거나

1절에서 봤듯 필터 체인은 Envoy의 부가 기능이 아니라 코어 정의의 일부다 — "A pluggable filter chain mechanism allows filters to be written to perform different TCP/UDP proxy tasks." 네트워크 필터가 TCP/UDP 층을, HTTP 필터가 스트림 층을 담당하고, 확장은 전부 이 두 자리에 필터를 꽂는 형태로 이루어진다.
...
이 필터들을 Istio에서 어떤 창구로 붙이는지 — EnvoyFilter의 위험, `WasmPlugin`이라는 상위 추상화, local과 global 레이트 리밋의 성격 차이 — 는 [08]({{< relref "08-envoyfilter-extension.md" >}})에 있다.

## 7. Istio 밖의 Envoy

같은 부품을 쓰는 프로젝트 목록은 Envoy 공식 커뮤니티 페이지에 그대로 있다. 메시 쪽에는 Istio 외에 **Kuma**, **Consul Connect**("first-class support for using Envoy as a proxy"), **AWS App Mesh**가 있고, API 게이트웨이·인그레스 쪽에는 **Ambassador**("An open source Kubernetes-native API Gateway built on Envoy"), **Contour**, **Enroute**, **Gloo Edge**, **Higress**, 그리고 Envoy 프로젝트 자신의 컨트롤 플레인인 **Envoy Gateway**가 올라 있다.

같은 데이터 플레인을 두고 컨트롤 플레인만 다른 것들이 이만큼 있다는 사실이, 5절의 관계 역전을 실물로 보여준다. 다만 이 목록의 설명 문구는 각 프로젝트가 자기 소개로 적은 것이고 시간이 지나면 갱신·삭제될 수 있다.
