...
{{< callout type="info" >}}
- 메시의 5xx는 **어느 홉에서 났느냐**가 전부입니다. 나침반은 Envoy **response flag**(UH/UF/UC/UO/NR…)입니다.
...
그때 무슨 일이 있었나. EKS에 올려둔 서비스 하나가 간헐적으로 응답 이상을 냈습니다. 산발적인 5xx와 지연이었습니다. 애플리케이션 로그는 멀쩡해 보이는데 클라이언트는 이따금 실패했습니다. 요청 경로에 메시가 껴 있으니 "앱이 문제냐"로 끝낼 수 없고 앱 · 사이드카 · 컨트롤 플레인 · 네트워크를 층으로 나눠 봐야 했습니다. 이 문서는 그 추적을 순서 있는 체크리스트로 정리합니다 — 메시 장애의 나침반은 Envoy가 붙이는 response flag입니다.

관련 문서: [01 메시 기초]({{< relref "01-mesh-basics.md" >}}) · [02 컨트롤 플레인]({{< relref "02-istiod-control-plane.md" >}}) · [03 게이트웨이]({{< relref "03-gateway-node-isolation.md" >}})

## 먼저: 5xx는 누가 낸 것인가

메시를 지나는 요청 하나는 프록시를 여러 번 거칩니다. 똑같은 "503"이라도 **어느 홉에서 났느냐**에 따라 원인이 완전히 다릅니다.

{{< flow src="_flow/먼저-5xx-는-누가.json" />}}

앱(④)이 낸 503과 dest 사이드카(③)가 "붙을 상대가 없어서" 낸 503은 전혀 다른 문제입니다. 이걸 구분해 주는 게 Envoy의 response flag입니다. Envoy 액세스 로그의 `%RESPONSE_FLAGS%` 필드에 두세 글자로 찍힙니다.
...
**간헐적** 5xx라면 UH·UC·UF·UO가 단골입니다 — 전부 "가끔" 발생하는 성격입니다.
...
의심 구간의 Envoy 액세스 로그에서 `response_flags`를 확인합니다. 이 한 컬럼이 범인이 앱인지 프록시인지, 어느 방향인지를 바로 알려줍니다.
...
flag가 없고 앱이 직접 503을 냈다면 그건 앱 문제입니다. flag가 붙어 있으면 아래로 갑니다.

### 3) 프록시가 든 설정이 최신인가 — proxy-status / proxy-config

간헐적 UH·NR은 대개 **stale한 설정**에서 옵니다. 컨트롤 플레인이 밀려([02]({{< relref "02-istiod-control-plane.md" >}})의 수렴 지연) 프록시가 옛 엔드포인트·라우트를 들고 있으면 이미 사라진 파드로 요청을 보내다 실패합니다.
...
`STALE`이 보이거나 endpoints 목록이 실제 파드와 다르면 02의 컨트롤 플레인 문제로 넘어갑니다 — `pilot_proxy_convergence_time`을 확인합니다.

### 4) mTLS·정책 불일치 — UF의 단골

간헐적 UF에는 **mTLS 미스매치**가 잦습니다. `PeerAuthentication`이 STRICT인데 일부 호출자가 평문이거나 마이그레이션 중 한쪽만 mTLS면 그쪽 연결이 실패합니다. `AuthorizationPolicy`가 특정 조건에서만 막고 있을 수도 있습니다.
...
간헐적 5xx가 **롤링 배포 시점에 몰린다면** 십중팔구 사이드카-앱의 시작·종료 순서 문제입니다.
...
- 엔드포인트 갱신 랙 — 파드는 죽었는데 EDS 갱신이 늦어 UH가 뜹니다. → 3번(수렴)과 이어집니다. `DestinationRule`의 outlier detection으로 문제 엔드포인트를 자동 축출하면 간헐 실패를 데이터 플레인에서 흡수할 수 있습니다.

### 6) 커넥션풀·서킷브레이커 — UO

간헐적 UO는 `DestinationRule`의 커넥션풀·서킷브레이커 상한에 트래픽이 순간 몰려 부딪힌 결과입니다. 상한이 너무 빡빡한지, 트래픽 특성(버스트)에 맞는지 재검토합니다.

## 층으로 가르는 지도

메시 장애는 **네 개 층**으로 나눠 놓고 나침반(response flag)을 따라갑니다.
...
- 메시의 5xx는 **어느 홉에서 났느냐**가 전부입니다. 그걸 판별하는 나침반이 **Envoy response flag**(UH/UF/UC/UO/NR…)입니다.
- 추적은 **범위 축소 → flag 확인 → 설정 stale 여부 → mTLS → 라이프사이클 레이스 → 커넥션풀** 순서. 간헐적 5xx는 특히 **stale 엔드포인트**와 **배포 시점 라이프사이클 레이스**가 잦습니다.
- 예방은 라이프사이클 훅 표준화·탄력성 설정·컨트롤 플레인 수렴 관측·flag 대시보드. 층을 나눠 보는 습관이 메시 장애 대응을 좌우합니다.
