...
이 섹션의 모든 문서는 아래 채널코퍼레이션 기술 블로그(tech.channel.io) 원문 6건을 읽고 이 지식베이스 형식으로 재구성한 요약이다. **원문 자체가 아니며**, 정확한 워딩·전체 맥락·원문에 실린 그림과 config 덤프 전문은 각 링크에서 확인한다.
...
- 결정의 무게추는 **약 4,000개 파드**였다. 전부 사이드카를 붙이면 idle 상태에서만 약 240Gi 메모리가 순수하게 프록시로 나간다. 사이드카 비용의 본질은 단가가 아니라 **프록시 개수가 파드 개수와 1:1로 묶여 있다**는 점이다.
...
- 프로덕션에서 실제로 터진 것은 전부 **프록시가 파드 밖으로 나가면서 생긴 타이밍 문제**다 — 죽은 Pod의 HBONE 터널을 재사용해 터지는 503, istio-cni보다 먼저 뜬 파드(partially enrolled), 노드 단위가 되어 버린 업그레이드 카나리, 그리고 xDS 단절을 못 잡는 readinessProbe.
...
이 레포의 [상위 Istio 챕터]({{< relref "../_index.md" >}}) 01~09편은 전부 **Sidecar mode를 전제로 쓰인 운영기**다. 파드마다 Envoy가 붙고, istiod가 그 프록시 전부에 xDS를 밀어 넣고, 게이트웨이를 전용 노드로 빼고, 5xx가 나면 게이트웨이 → 사이드카 → 앱 순으로 hop을 좁혀 간다. 이 `ambient/` 섹션은 그 전제 자체를 바꾼 **외부 팀(채널코퍼레이션)의 프로덕션 기록**이다. 우리 환경의 사례가 아니라 남의 사례이므로 따로 두고, 대신 같은 문제를 다른 아키텍처로 푼 **대조군**으로 읽는다. 컨트롤 플레인 부하·데이터 플레인 비용·장애 추적이라는 세 축은 양쪽이 똑같이 마주하지만, 사이드카 모드가 "파드당 프록시 하나"의 세계에서 답을 찾는다면 Ambient는 "노드당 ztunnel 하나 + 필요한 곳에만 waypoint"의 세계에서 답을 찾는다. 어느 쪽이 옳은지가 아니라 **같은 청구서의 항목이 어떻게 재배치되는지**가 이 섹션을 읽는 이유다.
...
- **장애 대응 관점이면** 03-1 → 03-2 순서로 본다. 03-1은 **이미 메시에 들어온 커넥션의 수명** 문제이고, 03-2는 **애초에 메시에 못 들어온 파드**의 문제다. 증상은 둘 다 5xx지만 원인 계층이 다르다.
- **Ambient를 이미 운영 중이라면** 03-3(업그레이드)과 03-4(탐지)가 실무 직결이다. 특히 03-4의 두 사례는 원문 스스로 **Ambient 고유 문제가 아니라고 못 박은** 것이라, 사이드카 모드 운영자에게도 그대로 적용된다.
- 시간순 01 → 02 → 03-1 → 03-2 → 03-3 → 03-4가 원문 시리즈의 순서이자 채널팀이 실제로 겪은 순서다.
...
| xDS 커넥션 | [09 istiod 스케일링과 xDS 커넥션 재분배]({{< relref "../09-istiod-scaling-connections.md" >}}) — 장수 gRPC는 스케일아웃해도 재분배되지 않는다 | [03-4]({{< relref "03-4-507-istiod-disconnected.md" >}}) — 한 번 끊긴 stream은 스스로 낫지 않는다, 그래서 탐지가 필요하다 |
...
| 5xx 추적 순서 | [05 장애 이야기: 간헐적 응답 이상]({{< relref "../05-incident-intermittent-5xx.md" >}}) — 게이트웨이 → 사이드카 → 앱으로 hop을 좁힌다 | [03-1]({{< relref "03-1-503-half-open-connection.md" >}}) — `via_upstream`은 목격자가 아니다, waypoint가 최초 목격자다 |
