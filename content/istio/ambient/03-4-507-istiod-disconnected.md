...
503이나 504라면 Istio/Envoy를 운영하며 흔히 만난다. 그런데 507은 익숙하지 않았다. 일반적으로 `507 Insufficient Storage`는 서버가 요청 처리에 필요한 저장 공간을 확보하지 못했다는 뜻이라, 채널팀도 처음에는 애플리케이션 서버의 스토리지 리소스 부족을 의심했다.

waypoint access log를 확인하니 이 응답은 애플리케이션이 만든 것이 아니라 **Envoy가 직접 만든 local reply**였다. 핵심은 `response_code_details`였다.

```text
request_payload_exceeded_retry_buffer_limit
```

Envoy 문서는 이 값을 이렇게 설명한다.

> Envoy is doing streaming proxying but too much data arrived while waiting to attempt a retry.

Envoy가 요청 body를 streaming으로 upstream에 전달하는 동시에 retry 가능성을 위해 일부 데이터를 buffer에 저장하고 있었는데, retry를 위해 보관해야 하는 request payload가 buffer limit을 넘었다는 뜻이다.

### 1.2 507은 request size 제한과는 다르다

헷갈리기 쉬운 지점은 507이 `413 Content Too Large`와 다르다는 점이다.
...
Envoy 구현을 보면 `request_payload_exceeded_retry_buffer_limit` 상황에서 `Http::Code::InsufficientStorage`, 즉 507을 local reply로 보낸다. 개념적 흐름은 다음과 같다.

{{< seq caption="같은 요청이 두 갈래로 갈린다 — retry 시점에 buffer가 body를 들고 있으면 replay해서 재시도하고, 1MB를 넘겨 buffering을 포기한 뒤였다면 507 local reply가 나간다." >}}
{
  "participants": [
    {"id": "C", "label": "클라이언트"},
    {"id": "E", "label": "waypoint Envoy"},
    {"id": "U", "label": "upstream 앱"}
  ],
  "steps": [
    {"msg": ["C", "E"], "label": "1. large POST body를 chunk로 전송"},
    {"msg": ["E", "U"], "label": "2. 받은 chunk를 그대로 흘려보냄"},
    {"note": ["E", "U"], "lines": [
      "retry·shadow에 대비해 같은 body를 decoding buffer에도 적재",
      "경계 = per_connection_buffer_limit_bytes (기본 1MB)"
    ]},
    {"alt": "누적 ≤ 1MB — replay 가능", "steps": [
      {"msg": ["U", "E"], "label": "3. upstream reset / 5xx", "dashed": true},
      {"msg": ["E", "U"], "label": "4. buffer의 body를 replay해 retry"},
      {"msg": ["E", "C"], "label": "5. 정상 응답", "dashed": true}
    ], "elseLabel": "누적 > 1MB — retry state reset", "elseSteps": [
      {"note": ["E", "U"], "lines": ["buffering 포기 — 이 시점부터 body를 replay할 수 없다"]},
      {"msg": ["U", "E"], "label": "3'. upstream reset / 5xx", "dashed": true},
      {"msg": ["E", "C"], "label": "4'. 507 local reply", "dashed": true}
    ]}
  ]
}
{{< /seq >}}
...
운영 중 특정 gateway/waypoint Envoy가 istiod와의 xDS 연결을 정상적으로 유지하지 못하는 것으로 보이는 상황이 있었다. 당시 로그에는 다음과 같은 메시지가 반복적으로 찍혔다.
...
Istio gateway/waypoint Pod 안에는 Envoy와 pilot-agent가 함께 있다. Envoy 입장에서 xDS upstream은 `xds-grpc` cluster지만, 이 cluster의 실제 목적지는 원격 istiod가 아니라 **같은 Pod 안의 pilot-agent UDS**다.

{{< flow caption="장애 지점은 두 구간으로 갈린다 — ① Envoy↔pilot-agent는 Pod 내부 UDS라 거의 실패하지 않고, 이번에 깨진 곳은 Kubernetes DNS와 network를 거치는 ② 구간이다." >}}
{
  "nodes": [
    {"id": "envoy",  "col": 0, "row": 0, "label": "Envoy", "sub": "xds-grpc cluster", "kind": "src"},
    {"id": "agent",  "col": 1, "row": 0, "label": "pilot-agent", "sub": "같은 Pod · xDS proxy", "kind": "proc"},
    {"id": "istiod", "col": 2, "row": 0, "label": "istiod", "sub": "xDS 원본 · TCP/TLS", "kind": "store"},
    {"id": "dns",    "col": 2, "row": 1, "label": "cluster DNS", "sub": "istiod.istio-system.svc", "kind": "query"}
  ],
  "edges": [
    {"from": "envoy", "to": "agent",  "label": "① UDS", "rate": 380},
    {"from": "agent", "to": "istiod", "label": "② TCP/TLS", "rate": 520},
    {"from": "agent", "to": "dns",    "label": "이름 해석", "rate": 900}
  ]
}
{{< /flow >}}

이번에 관찰한 로그는 ② 구간에서 실패했고, 그 에러가 ① 구간의 gRPC stream close로 전파된 형태였다.

### 2.3 pilot-agent 에러는 Envoy에 전파된다

Istio agent의 xDS proxy 구현을 보면, pilot-agent는 istiod upstream 연결 실패를 Envoy downstream stream에 숨기지 않고 전파한다. 개념적으로는 다음과 같다.
...
istiod 쪽 stream에서 `Recv()` 에러가 발생하면 upstream error channel로 전달되고, handler가 error를 반환한다. 그러면 Envoy가 보고 있던 gRPC config stream도 닫힌다.
...
{{< seq caption="istiod 연결 실패는 pilot-agent를 거쳐 Envoy의 stream close로 전파되고, backoff 재연결 때 UDS가 잠깐 열리면서 connected_state가 0과 1 사이를 오간다." >}}
{
  "participants": [
    {"id": "E", "label": "Envoy"},
    {"id": "A", "label": "pilot-agent"},
    {"id": "D", "label": "istiod"}
  ],
  "steps": [
    {"msg": ["E", "A"], "label": "1. xDS stream 생성 (Pod 내부 UDS)"},
    {"msg": ["A", "D"], "label": "2. istiod.istio-system.svc 연결 시도", "dashed": true},
    {"note": ["A", "D"], "lines": ["DNS lookup i/o timeout", "istiod에 도달하지 못함"]},
    {"msg": ["A", "E"], "label": "3. Recv() 에러를 downstream에 전파", "dashed": true},
    {"note": ["E", "A"], "lines": ["connected_state = 0 · retry timer 시작"]},
    {"msg": ["E", "A"], "label": "4. backoff 후 재연결 — UDS는 잠깐 열린다"},
    {"note": ["E", "A"], "lines": ["이 순간 connected_state = 1 로 보인다"]},
    {"alt": "istiod 도달 가능", "steps": [
      {"msg": ["A", "D"], "label": "stream 재수립 → 정상"}
    ], "elseLabel": "여전히 실패", "elseSteps": [
      {"msg": ["A", "D"], "label": "다시 연결 시도", "dashed": true},
      {"note": ["A", "D"], "lines": ["다시 i/o timeout"]},
      {"note": ["E", "D"], "lines": [
        "2~4가 반복 — closed since 1614s ago",
        "connected_state가 0↔1로 진동한다"
      ]}
    ]}
  ]
}
...
Istio의 기본 readinessProbe는 "최초 xDS config를 받았는가"에 더 가깝다. pilot-agent status probe 구현에는 `receivedFirstUpdate`, `atleastOnceReady` 같은 상태가 있고, 한 번 ready가 된 뒤에는 이후 xDS 연결이 끊겨도 readiness가 곧바로 실패하지 않을 수 있다. 형태는 이렇다.
...
Envoy 내부 stat 이름은 다음과 같다.

```text
control_plane.connected_state
```

Prometheus로 export되면 보통 다음 이름으로 보인다.
...
올바른 설정은 Envoy 내부 stat 이름인 `control_plane.connected_state`를 기준으로 해야 한다.
...
또 하나 볼 수 있는 값은 `xds-grpc` cluster의 active connection 수다.
...
{{< seq caption="같은 probe 결과라도 successThreshold 값에 따라 결말이 갈린다 — 기본값 1이면 재연결 루프의 우연한 1회 성공만으로 endpoint에 다시 들어가 flapping이 난다." >}}
{
  "participants": [
    {"id": "K", "label": "kubelet"},
    {"id": "P", "label": "gateway Pod"},
    {"id": "S", "label": "Service endpoint"}
  ],
  "steps": [
    {"msg": ["K", "P"], "label": "/healthz/ready + control_plane.connected_state"},
    {"msg": ["P", "K"], "label": "connected_state = 0 → 실패", "dashed": true},
    {"note": ["K", "P"], "lines": ["3회 연속 실패 = failureThreshold 3 → NotReady"]},
    {"msg": ["K", "S"], "label": "endpoint에서 제외"},
    {"msg": ["K", "P"], "label": "재연결 루프 중 다시 probe"},
    {"msg": ["P", "K"], "label": "state 1 — 우연히 1회 성공", "dashed": true},
    {"alt": "successThreshold = 1 (기본값)", "steps": [
      {"msg": ["K", "S"], "label": "곧바로 Ready → endpoint 재진입"},
      {"note": ["K", "S"], "lines": ["다음 실패에 다시 빠진다 — flapping"]}
    ], "elseLabel": "successThreshold = 3", "elseSteps": [
      {"note": ["K", "P"], "lines": ["연속 3회 성공까지 NotReady 유지"]},
      {"msg": ["K", "S"], "label": "3회 연속 성공 후에만 재진입"}
    ]}
  ]
}
{{< /seq >}}

그래서 단순히 `failureThreshold`만 두는 것보다 **Ready로 복귀할 때도 연속 성공을 요구하는 편이 안전하다.**
...
그럼에도 기본 readiness가 "최초 config 수신 여부"에 가깝다는 점을 고려하면, gateway/waypoint 같은 중요 진입점에서는 현재 control plane 연결 상태를 별도로 보는 것이 운영상 의미가 있다고 판단했다.
{{< /callout >}}
