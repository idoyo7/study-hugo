...
[3-1편]({{< relref "03-1-503-half-open-connection.md" >}})이 다룬 503과 half-open connection이 이미 메시에 들어와 있는 커넥션의 수명 문제였다면, 이 편은 애초에 메시에 제대로 들어오지 못한 Pod의 문제입니다. 증상은 5xx로 비슷하지만 원인 계층이 다릅니다 — 앞은 커넥션 타이밍, 이쪽은 Pod 등록 타이밍입니다.

Sidecar mode에서는 이 문제 자체가 성립하기 어렵습니다. Envoy 사이드카가 Pod 안에 있으니 Pod이 준비되면 메시 기능도 같이 준비됩니다. Ambient mode는 데이터플레인이 Pod 밖, 노드 위의 istio-cni와 ztunnel에 있습니다. 그래서 Pod의 readiness와 노드 데이터플레인의 readiness가 분리되고, 그 간격이 장애 구간이 됩니다([1편]({{< relref "01-why-ambient-mode.md" >}})이 다룬 트레이드오프의 연장선입니다).

## 1. 증상 — Running이고 Ready인데 트래픽이 실패한다

채널팀이 프로덕션에서 관측한 것은 다음과 같습니다.
...
클라이언트 쪽 Envoy에서는 다음 에러가 관찰되었습니다.
...
쿠버네티스의 모든 신호가 초록불인데 트래픽만 안 되는 상태입니다. 이 조합은 [상위 챕터 05 간헐적 응답 이상]({{< relref "../../05-incident-intermittent-5xx.md" >}})에서 다룬 사이드카 시절의 5xx 추적 순서로는 잘 잡히지 않습니다. 그쪽은 "프록시가 붙어 있다"는 전제 위에서 라우팅·정책·업스트림을 의심하는 순서인데, 여기서는 프록시 경로 자체가 아직 만들어지지 않았습니다.

특정 노드에 몰려서 발생한다는 점이 단서였습니다. 문제의 축은 Pod이 아니라 노드였습니다.

## 2. Ambient에서 "메시에 참여한다"의 정확한 의미

### 레이블은 의도 표명일 뿐이다

네임스페이스에 `istio.io/dataplane-mode=ambient` 레이블이 붙어 있다는 것은 "이 Pod은 Ambient mesh 대상이어야 한다"는 **의도**를 나타낼 뿐입니다. 레이블이 붙었다고 트래픽이 ztunnel을 지나가지는 않습니다.

실제로 트래픽이 ztunnel을 통과하려면 두 단계가 완료되어야 합니다.

{{< flow src="_flow/레이블은-의도-표명일-뿐이다.json" />}}

이 두 단계 중 하나라도 빠지면, 쿠버네티스 관점에서는 정상 Pod처럼 보이지만 메시 데이터플레인 관점에서는 완전하지 않은 상태가 됩니다. 이것이 **partially enrolled**입니다.
...
Ambient에서는 Pod이 뜨는 시점에 이 노드 컴포넌트들이 준비되어 있어야 합니다. 그렇지 않으면 트래픽 손실이 납니다. Ambient의 Envoy 설정 구조 자체는 [2편]({{< relref "02-envoy-config-anatomy.md" >}})에서 다룹니다.
...
{{< callout type="error" >}}
시나리오 1이 더 위험합니다. 트래픽이 실패하는 게 아니라 **정책을 우회한 채 성공**하기 때문입니다. mTLS도, `AuthorizationPolicy`도, 텔레메트리도 적용되지 않은 평문 트래픽이 흐르는데 메트릭에는 아무 이상도 잡히지 않습니다. 시나리오 2는 최소한 5xx로 시끄럽게 실패합니다.
{{< /callout >}}

### annotation이 상태를 드러낸다

Istio는 Pod의 `ambient.istio.io/redirection` annotation으로 등록 상태를 관리합니다.
...
채널팀은 istio 소스에서 이 경로를 직접 확인했습니다. 진입점은 `cni/pkg/nodeagent/meshdataplane_linux.go`의 `AddPodToMesh`입니다.

{{< seq src="_seq/소스코드에서의-위치.json" />}}

재시도 로직이 있지만, 재시도가 성공하기 전까지의 구간은 트래픽 유실 구간입니다.
...
readiness probe는 애플리케이션 관점의 준비 상태를 봅니다. 컨테이너가 포트를 열었는지, 헬스 엔드포인트가 200을 주는지를 확인할 뿐, 노드의 메시 데이터플레인이 이 Pod을 받을 준비가 됐는지는 검사 대상이 아닙니다. 사이드카 시절에는 프록시가 Pod의 컨테이너였으므로 Ready 판정 안에 프록시 준비가 자연히 포함됐지만, Ambient는 그 포함 관계가 끊깁니다. probe를 아무리 촘촘하게 짜도 이 문제는 걸러지지 않습니다.

## 5. 해법 — Untaint Controller

Istio 팀의 공식 해결책은 untaint-controller입니다. 떠버린 Pod을 사후에 고치는 대신, node-local 데이터플레인이 준비되기 전에는 워크로드 Pod이 그 노드에 스케줄되지 못하게 막습니다.
...
**untaint-controller는 taint를 붙이지 않습니다. 떼기만 합니다.** Istio 쪽 설정만 켜고 노드 프로비저닝 쪽에 startup taint를 넣지 않으면 아무 일도 일어나지 않으며, 경합은 그대로 남습니다. 노드가 어디서 만들어지든 그 경로마다 taint를 붙여야 합니다.
{{< /callout >}}
...
채널팀이 확인한 함정이 이것입니다. **`pilot.taint.enabled=true` 하나만으로는 컨트롤러가 돌지 않습니다.** Istio 1.22 릴리스 노트에는 `cni.istio.io/not-ready` taint를 제거하는 node taint controller가 추가됐다고 짧게 언급되어 있을 뿐이어서, Helm 값 하나만 켜고 끝났다고 오해하기 쉽습니다.
{{< /callout >}}
...
따라서 1.30 이상이면 위 Helm values의 `env` 블록은 생략해도 됩니다. 그 이전 버전에서는 두 값을 모두 명시해야 합니다.

## 6. 적용 시 주의점 — 이걸로 전부 막히지는 않는다

untaint controller가 없애는 것은 경합의 큰 축 하나입니다. 원문이 정리한 한계는 다음과 같습니다.
...
원문이 링크한 [ambientmesh.io 운영 가이드](https://ambientmesh.io/docs/operations/untaint-controller/)는 컨트롤러 동작 확인을 위해 istiod 로그를 보거나 `istioctl admin log`로 untaint 관련 로그 레벨을 올리는 방법을 안내합니다. 이 문단은 원문 본문이 아니라 원문이 인용한 문서의 내용입니다.
...
[01 서비스 메시와 Istio 기초]({{< relref "../../01-mesh-basics.md" >}})가 정리한 메시 운영 비용은 Ambient에서도 남습니다. 사이드카에서는 Pod당 프록시의 CPU·메모리였고, Ambient에서는 **노드 라이프사이클과 메시 데이터플레인 준비 사이의 정합성**입니다.
