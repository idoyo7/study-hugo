...
> **그때 무슨 일이 있었나.** 외부 트래픽을 받는 Ingress Gateway가 일반 워크로드 파드들과 **같은 노드에서 자원을 다투고** 있었다. 트래픽이 몰리면 게이트웨이가 옆 파드에 CPU·네트워크를 뺏기거나, 반대로 게이트웨이가 노드를 잡아먹어 이웃이 흔들렸다 — 전형적인 noisy neighbor다. 해법은 **게이트웨이를 전용 노드로 분리**하는 것이었다. 이 문서는 데이터 플레인 트래픽의 두 방향, 게이트웨이의 정체, 그리고 왜·어떻게 노드로 격리하는지를 정리한다.
