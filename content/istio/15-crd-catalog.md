...
`kubectl get crd`가 실제로 보여주는 목록이다. 130일 전 설치 시점의 것이므로 아래 버전 컬럼은 그때 배포된 Istio가 서빙하는 API 버전을 그대로 반영한다.
...
그룹으로 세면 **networking 9 · security 3 · telemetry 1 · extensions 1**이다. 그룹 이름이 곧 관심사 분리로 읽히지만, **왜 이렇게 갈렸는지에 대한 공식 설계 근거는 istio.io에서 찾지 못했다.** v1 승격을 알린 2024년 블로그도 네 그룹으로 분류한 표만 제시할 뿐 분리의 이유는 서술하지 않는다. 그러니 그룹 이름은 설계 의도의 증거가 아니라 **찾을 위치의 힌트** 정도로만 쓰는 게 안전하다.
...
트래픽 쪽 리소스들은 **각자 독립된 설정 조각이 아니라 서로의 이름을 부르는 사슬**이다. 사슬을 따라가면 최종적으로 파드 IP 목록에 닿는다.

{{< flow caption="트래픽 축의 참조 사슬 — 이름(gateways·host·subset)과 라벨로 이어져 엔드포인트에 닿는다" >}}
{
  "nodes": [
    { "id": "G",  "col": 0, "row": 0, "label": "Gateway", "sub": "받을 포트·프로토콜·TLS", "kind": "src" },
    { "id": "VS", "col": 1, "row": 0, "label": "VirtualService", "sub": "매칭·가중치·재시도", "kind": "proc" },
    { "id": "DR", "col": 2, "row": 0, "label": "DestinationRule", "sub": "subset 정의 · trafficPolicy", "kind": "proc" },
    { "id": "EP", "col": 3, "row": 0, "label": "엔드포인트", "sub": "파드 IP · VM 주소", "kind": "sink" },
    { "id": "SE", "col": 0, "row": 1, "label": "ServiceEntry", "sub": "메시 밖 서비스를 등록", "kind": "store" },
    { "id": "RG", "col": 2, "row": 1, "label": "서비스 레지스트리", "kind": "store" },
    { "id": "WG", "col": 0, "row": 2, "label": "WorkloadGroup", "sub": "WorkloadEntry 템플릿", "kind": "store" },
    { "id": "WE", "col": 1, "row": 2, "label": "WorkloadEntry", "sub": "VM 등 비-k8s 워크로드", "kind": "store" }
  ],
  "edges": [
    { "from": "G",  "to": "VS", "label": "gateways", "rate": 700 },
    { "from": "VS", "to": "DR", "label": "host+subset", "rate": 700 },
    { "from": "DR", "to": "EP", "label": "라벨 필터", "rate": 700 },
    { "from": "SE", "to": "RG", "label": "항목 추가" },
    { "from": "WG", "to": "WE", "label": "템플릿" },
    { "from": "WE", "to": "RG", "label": "엔드포인트 등록" },
    { "from": "RG", "to": "EP", "label": "후보 집합 공급" }
  ]
}
{{< /flow >}}
...
가장 자주 걸리는 곳이 첫 번째 고리다. 레퍼런스 원문이 기본 동작을 못 박는다.

> When this field is omitted, the default gateway (`mesh`) will be used, which would apply the rule to all sidecars in the mesh.

즉 **`gateways`를 안 쓰면 그 VirtualService는 게이트웨이가 아니라 메시 내부 사이드카 전부에 적용된다.** `mesh`는 "메시 내 모든 사이드카"를 뜻하는 예약어다. 인그레스 라우팅을 짰는데 게이트웨이에서 안 먹는 사례의 상당수가 여기서 나오고, 반대로 내부 라우팅 의도로 쓴 규칙이 필드 하나 때문에 전 사이드카로 퍼지기도 한다. [13]({{< relref "13-istio-envoy-assembly.md" >}})의 진단 순서대로 `istioctl proxy-config route`에 규칙이 안 보이면 이 필드부터 본다.

### DestinationRule이 실제로 담는 것

DestinationRule을 "subset 정의용"으로만 아는 경우가 있는데, 레퍼런스가 `trafficPolicy` 아래 두는 것이 더 많다. `loadBalancer`·`connectionPool`·`outlierDetection`이 [13]({{< relref "13-istio-envoy-assembly.md" >}})이 이미 다룬 "라우팅 이후의 정책"이고, 여기 `tls`("TLS related settings for connections to the upstream service")가 하나 더 있다 — 13은 다루지 않은 필드다. `portLevelSettings`·`tunnel`·`proxyProtocol`·`retryBudget` 등도 여기 있다.

이 목록이 두 방향으로 이어진다. **위쪽**으로는 [12]({{< relref "12-envoy-capabilities.md" >}})가 소개한 Envoy 기능들을 켜는 스위치이고, **아래쪽**으로 `tls` 한 필드가 3절의 보안 축과 맞물린다. 같은 리소스가 트래픽 정책과 mTLS 클라이언트 동작을 동시에 들고 있다는 사실이, 보안 축의 짝 맞추기가 자꾸 어긋나는 구조적 이유다.

### 레지스트리를 넓히는 셋

나머지 세 리소스는 라우팅 규칙이 아니라 **라우팅 대상 자체를 늘린다.**
...
우리 클러스터는 EKS 위 파드만 다루므로 이 셋 중 실제로 쓰는 것은 ServiceEntry뿐이다. 그래도 WorkloadEntry/WorkloadGroup이 깔려 있다는 사실 자체는 의미가 있다 — **메시의 경계가 쿠버네티스 경계와 같아야 할 이유가 없다**는 것이 API 수준에서 이미 전제돼 있다.

## 3. 보안 축 — 짝 맞추기와 2단 관문

security 그룹 3개는 서로 다른 층을 담당하는데, 셋 다 **단독으로는 의도한 결과를 못 낸다.** 각각 짝이 있다.
...
특히 **mTLS가 클러스터 전역으로 켜진 상태에서 DestinationRule에 `trafficPolicy.tls.mode: ISTIO_MUTUAL`을 명시하지 않으면 기본값이 `DISABLE`로 떨어진다.** 클라이언트는 평문을 보내는데 서버는 암호화를 기대하니 503이 난다. 가이드의 처방은 한 문장이다 — "ensure the `trafficPolicy` TLS mode matches the global server configuration."

{{< callout type="important" >}}
**리뷰에서 잡을 규칙.** DestinationRule에 `trafficPolicy`를 새로 추가하는 PR은 **`tls` 필드를 건드리지 않았더라도** 전역 mTLS 설정과의 정합성을 확인 대상으로 본다. 기본값이 `DISABLE`이라 "TLS를 안 건드렸으니 안전하다"가 성립하지 않기 때문이다. [04]({{< relref "04-config-as-code.md" >}})의 GitOps 리뷰 체크리스트에 넣기 좋은 항목이고, 배포 직후 503이 즉시 뜨는 형태라 [05]({{< relref "05-incident-intermittent-5xx.md" >}})의 간헐적 5xx와는 증상 모양이 다르다.
{{< /callout >}}

### RequestAuthentication → AuthorizationPolicy — 토큰이 없는 요청은 거부되지 않는다

두 번째 함정은 더 조용하다. RequestAuthentication을 걸면 JWT 검증이 켜지니 인증이 끝났다고 생각하기 쉬운데, 레퍼런스가 명시적으로 반대를 말한다.
...
거부되는 것은 **잘못된** 자격증명을 담은 요청뿐이다 — "will reject a request if the request contains invalid authentication information, based on the configured authentication rules". 아예 토큰이 없으면 그냥 통과한다. 그래서 같은 페이지가 짝을 요구한다.

> To restrict access to authenticated requests only, this should be accompanied by an authorization rule.

실제 공식 예제도 RequestAuthentication에 반드시 `requestPrincipals: ["*"]`를 쓰는 AuthorizationPolicy를 붙여 보여준다 — [07]({{< relref "07-from-nginx-to-istio.md" >}})의 JWT yaml 예제가 그 실무형이다. 결론은 같다: **RequestAuthentication은 "토큰이 있다면 검증한다"이고, "토큰을 요구한다"는 AuthorizationPolicy의 몫이다.**

{{< flow caption="JWT 2단 관문 — 토큰이 없는 요청은 RequestAuthentication을 그냥 통과한다" >}}
{
  "nodes": [
    { "id": "R",  "col": 0, "row": 0, "label": "요청", "kind": "src" },
    { "id": "RA", "col": 1, "row": 0, "label": "JWT 검증", "sub": "RequestAuthentication · 토큰이 있을 때만", "kind": "proc" },
    { "id": "AP", "col": 2, "row": 0, "label": "인가 판정", "sub": "AuthorizationPolicy · CUSTOM → DENY → ALLOW", "kind": "proc" },
    { "id": "W",  "col": 3, "row": 0, "label": "워크로드", "kind": "sink" },
    { "id": "X1", "col": 2, "row": 1, "label": "거부", "sub": "유효하지 않은 토큰", "kind": "sink" },
    { "id": "X2", "col": 3, "row": 1, "label": "거부", "sub": "DENY 매치 · ALLOW 불일치", "kind": "sink" }
  ],
  "edges": [
    { "from": "R",  "to": "RA", "rate": 700 },
    { "from": "RA", "to": "AP", "label": "통과", "rate": 700 },
    { "from": "AP", "to": "W",  "label": "허용", "rate": 700 },
    { "from": "RA", "to": "X1", "label": "invalid credential" },
    { "from": "AP", "to": "X2", "label": "인가 판정에서 탈락" }
  ]
}
{{< /flow >}}
...
두 번째 관문의 내부 규칙은 계층으로 정의돼 있다.

> Istio checks for matching policies in layers, in this order: **CUSTOM, DENY, and then ALLOW.**

레퍼런스는 알고리즘을 더 정밀하게 적는다 — 매치되는 CUSTOM 정책이 있으면 평가해서 deny면 거부, 매치되는 DENY 정책이 있으면 거부, 그다음 "If there are no ALLOW policies for the workload, allow the request. If any of the ALLOW policies match the request, allow the request. Deny the request."
...
정책 본문 구조는 `from`·`to`·`when` 셋이고, 각각 "If not set, any source/operation/condition is allowed"로 **비지정 = 전부 허용**이다. 다만 TCP 트래픽에 걸린 정책에서는 `method` 같은 HTTP 속성이 처리될 수 없고, DENY 규칙에서는 누락된 속성을 매치로 간주한다는 L4/L7 경계 서술이 따로 있으니 L4 대상 정책은 별도로 확인해야 한다. (ambient의 ztunnel에서 이 처리가 어떻게 달라지는지는 확인하지 못했다 — [10]({{< relref "10-ambient-migration-questions.md" >}})의 재심사 항목으로 남긴다.)

## 4. 스코프·튜닝 축 — 프록시 자체를 조정하는 셋

`Sidecar`·`ProxyConfig`·`Telemetry`는 그룹이 셋 다 다르지만(networking·networking·telemetry) 성격은 같다. **요청이 어디로 가는지를 바꾸지 않고, 프록시가 무엇을 들고 어떻게 동작하는지를 바꾼다.** 2절의 사슬 어디에도 끼지 않는 대신, 사슬 전체가 얹혀 도는 바닥을 건드린다.
...
[02]({{< relref "02-istiod-control-plane.md" >}})가 istiod 부하를 줄이는 가장 큰 레버로 이미 다룬 그 리소스다 — egress 쪽 목적은 레퍼런스에 "to limit the set of configuration for outbound traffic… useful to prune out unneeded configuration, to improve scalability of the mesh"로 적혀 있다. 02가 다루지 않은 것은 이 "범위 제한"의 정체다.

{{< callout type="important" >}}
**egress host 목록은 보안 경계가 아니다.** 공식 문서가 이 오해를 직접 경고한다 — "A common misunderstanding is that restricting the configuration amounts to blocking the traffic… The sidecar is not able to enforce an outbound traffic restriction." 02에서 `egress.hosts`로 범위를 좁힌 것은 **istiod의 push 계산량과 프록시 메모리를 줄인 것**이지, 목록 밖으로 나가는 트래픽을 막은 것이 아니다. 아웃바운드를 실제로 통제하려면 인가 정책이나 네트워크 계층의 별도 수단이 필요하다.
{{< /callout >}}

### ProxyConfig — 어노테이션과 메시 전역 설정 사이

ProxyConfig가 다루는 것은 트래픽이 아니라 프록시 프로세스다. `concurrency`("The number of worker threads to run. If unset, this will be automatically determined based on CPU limits."), `image`(default/debug/distroless), `environmentVariables`("Names starting with `ISTIO_META_` will be included in the generated bootstrap configuration.") 같은 것들이다.

같은 값을 정할 수 있는 통로가 여럿이라 병합 규칙이 중요한데, 레퍼런스가 규칙 두 개를 준다.

- 워크로드에 매치되는 ProxyConfig CR이 있으면 그 워크로드의 `proxy.istio.io/config` 어노테이션과 병합되고, **겹치는 필드는 CR이 이긴다** — "with the CR taking precedence over the annotation for overlapping fields."
...
두 규칙을 겹치면 **워크로드별 CR > 어노테이션 > 메시 전역 CR/`meshConfig.defaultConfig`** 순으로 읽히지만, 이 순서를 한 줄로 나열한 원문 문장은 확인하지 못했다(두 병합 규칙을 조합한 추론이다). `concurrency`처럼 성능에 직결되는 값을 여러 통로로 설정 중이라면 순서를 믿지 말고 프록시에 실제 반영된 값을 확인하는 편이 낫다.

### Telemetry — 계층이 명시된 유일한 리소스

[06]({{< relref "06-observability-points.md" >}})의 카디널리티 정리에서 실제로 쓰는 창구다. 적용 계층이 원문에 번호까지 붙어 있다.
...
제약도 명시적이다 — "For any namespace, including the root configuration namespace, it is only valid to have a single workload selector-less Telemetry resource." 즉 **네임스페이스마다 셀렉터 없는 Telemetry는 하나뿐**이고, 워크로드 단위도 마찬가지다. 팀별로 Telemetry를 각자 얹는 구조는 성립하지 않으므로 네임스페이스 단위 소유권을 먼저 정해야 한다. 병합은 명시하지 않은 값이 상위에서 채워지는 방식이되, `custom_tags`처럼 지정하면 상위 값을 통째로 대체하는 필드도 있다.

## 5. 확장 축 — 두 계층의 나머지 절반

`EnvoyFilter`와 `WasmPlugin`은 [13]({{< relref "13-istio-envoy-assembly.md" >}})이 그린 두 계층 중 **사용자 확장** 쪽이다. 왜 최후의 수단으로 다뤄야 하는지와 선택 사다리(표준 CRD → 상위 API → EnvoyFilter)는 [08]({{< relref "08-envoyfilter-extension.md" >}})에 이미 있다. 여기서는 그 판단에 보탤 사실 둘만 본다.
...
다만 WasmPlugin을 "EnvoyFilter를 안 쓰고 필터를 넣는 상위 API"로 소개하는 경우가 많은데, 레퍼런스 페이지 자체는 그렇게 위치 짓지 않는다. 거기 적힌 것은 "Extend the functionality provided by the Istio proxy through WebAssembly filters"뿐이다. 그 위치 부여는 [08]({{< relref "08-envoyfilter-extension.md" >}})의 운영 판단이고, 그 순서를 명시한 공식 문장은 이 조사에서는 찾지 못했다.

## 6. 버전 컬럼이 말하는 것

1절 표의 버전 컬럼을 다시 보면 성숙도가 그대로 드러난다.
...
EnvoyFilter가 특히 도드라진다. 같은 `networking.istio.io` 그룹의 다른 리소스가 전부 `v1`로 올라간 동안 **혼자 `v1alpha3`에 남았다.** 이건 방치가 아니라 계약의 성격 자체가 다르다는 뜻으로 읽는 게 맞다. 레퍼런스가 그 이유를 직접 말한다.
...
**API 껍데기는 호환을 지키지만, 그 안에 넣는 Envoy 설정은 호환 대상이 아니다.** [08]({{< relref "08-envoyfilter-extension.md" >}})이 EnvoyFilter를 "탈출구"이자 "최후의 수단"으로 부른 근거가 여기서 버전 문자열로 확인된다. 그리고 [13]({{< relref "13-istio-envoy-assembly.md" >}})의 결론과 정확히 맞물린다 — 사이드카 이미지 업그레이드는 Envoy 업그레이드이고, EnvoyFilter가 결합된 대상은 그 pin된 Envoy 커밋의 스키마다. 안정 버전을 서빙하는 CRD 옆에 알파 버전 CRD가 하나 있다면, 업그레이드 리스크가 그쪽에 몰려 있다는 지도이기도 하다.
