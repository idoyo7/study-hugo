...
01~09는 전부 **Sidecar mode** 기준이다. 같은 문제를 **Ambient mode**로 푼 외부 팀(채널코퍼레이션)의 프로덕션 기록은 하위 섹션 [Ambient mode 도입기 (채널코퍼레이션)]({{< relref "ambient/_index.md" >}})에 따로 모아 두었다 — 대조군으로 읽는다.
...
- **Ambient mode가 궁금하면** 01~09로 사이드카 모드의 비용 구조를 먼저 잡고 하위 섹션 [Ambient mode 도입기]({{< relref "ambient/_index.md" >}})로 간다. 같은 청구서의 항목이 파드에서 노드로 옮겨 가면 무엇이 달라지는지가 그쪽 주제다.
...
- **프록시를 파드 밖으로 빼면 비용이 사라지는 게 아니라 항목이 바뀐다.** Ambient mode는 프록시 개수를 파드 수에서 노드 수로 옮기는 대신, 노드 라이프사이클과 메시 데이터플레인 준비 사이의 정합성을 새 운영 축으로 떠안는다. → [Ambient mode 도입기]({{< relref "ambient/_index.md" >}})
