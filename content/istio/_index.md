...
서비스가 수십 개로 늘면 mTLS·재시도·트래픽 분할·관측성을 **애플리케이션마다** 구현하는 부담이 감당이 안 됩니다. 그래서 이 공통 관심사를 인프라 레이어로 내리는 게 서비스 메시고, 그 대가로 운영자는 **데이터 플레인의 사이드카 오버헤드**와 **컨트롤 플레인의 부하**를 떠안습니다. 이 챕터의 네 사건은 전부 그 "대가"를 관리한 기록입니다. 메시를 처음 얹는 것보다, 얹고 나서 **규모가 커질 때 무엇이 터지는지**가 훨씬 중요합니다.
...
- **처음이라면** 01로 메시의 구조와 비용을 잡고, 02(컨트롤 플레인) → 03(데이터 플레인)으로 두 축을 나눠 이해합니다.
- **운영자라면** 02와 03이 실무 직결입니다. istiod가 왜 헐떡이는지(02)와 게이트웨이를 왜 격리하는지(03)는 규모가 커질 때 반드시 만납니다.
- **장애 대응 관점이면** 05를 먼저 훑어 "메시가 낀 경로에서 무엇부터 의심하나"의 체크리스트를 잡고, 필요한 개념은 02·03으로 되짚습니다.
- **메시로 무엇을 얻나가 궁금하면** 06(관측성)으로 공짜로 얻는 모니터링 포인트를, 07(nginx→Istio)로 기존 nginx 설정이 어디로 갔는지를, 08(EnvoyFilter)로 표준 CRD 밖의 조작을 봅니다.
- **istiod를 오토스케일링하려면** 02로 부하의 구조를 잡은 뒤 09로 넘어갑니다. 09는 "몇 대를 띄울까"가 아니라 **"커넥션이 어느 파드로 가는가"** 를 다루는 문서입니다.
