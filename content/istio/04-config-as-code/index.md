...
{{< callout type="info" >}}
**한눈에**
- Istio 설정은 **설치 층**(Helm 권장, IstioOperator는 지양)과 **트래픽 층**(전부 CRD)으로 나뉩니다.
- 메시 CRD는 **적용 즉시 트래픽에 직결**됩니다 — 손 apply의 드리프트·추적불가·재현불가가 곧 장애 위험입니다.
- GitOps(Argo CD/Flux)로 **Git을 단일 진실**로 삼아 드리프트를 자동 복원하고, `istioctl analyze`로 사전 검증합니다.
- 컨트롤 플레인 업그레이드는 **revision 기반 카나리**로 폭발 반경을 줄입니다.
{{< /callout >}}

> **그때 무슨 일이 있었나.** 메시 설정 — IstioOperator/Helm 값, Gateway, VirtualService, DestinationRule — 이 그때그때 `kubectl apply`로 손보아지면서, **클러스터의 실제 상태와 Git 저장소가 어긋나기** 시작했습니다. 누가 언제 무엇을 바꿨는지 추적이 안 되고, 재현·롤백이 불안했습니다. "Istio Manifest Sync" 과제는 이 **드리프트를 없애고 Git을 단일 진실로** 만드는 일이었습니다. 이 문서는 Istio 설정 방식, GitOps로 옮기는 이유, 그리고 메시 설정 특유의 위험을 다룹니다.
...
Istio 설정은 성격이 다른 두 층으로 나뉩니다. 이 구분이 GitOps 설계의 출발점입니다. **설치·플랫폼 설정**은 메시 자체를 어떻게 깔고 구성하는지를, **트래픽·정책 설정**은 런타임에 트래픽을 어떻게 다루는지를 다룹니다.
...
- **Helm chart** — `base`(CRD·클러스터 리소스) → `istiod`(컨트롤 플레인) → `gateway`(게이트웨이)를 각각 설치. **현재 권장되는 방식**이며 GitOps 도구와 궁합이 좋습니다.
- (참고) 신형 게이트웨이는 Kubernetes **Gateway API**로도 구성할 수 있습니다.

**트래픽 층**은 전부 Kubernetes CRD입니다. 즉 평범한 YAML이므로 **그대로 Git으로 관리하기에 최적**입니다.

## 왜 손 apply가 위험한가

메시 CRD는 보통의 매니페스트와 결정적으로 다른 점이 있습니다: **적용하는 순간 살아있는 트래픽에 직결됩니다.** VirtualService의 라우팅 한 줄, DestinationRule의 서브셋 정의가 틀리면 그 즉시 요청이 엉뚱한 곳으로 가거나 503이 난다([05]({{< relref "05-incident-intermittent-5xx.md" >}})의 단골 원인). 그런데 손 apply는:
...
메시 설정은 트래픽 직결이므로, GitOps 파이프라인에 **적용 전 검증**을 반드시 끼웁니다.
...
설치 층에서 가장 위험한 작업은 **Istio 버전 업그레이드**입니다. Istio는 이를 위해 **revision 기반 카나리 업그레이드**를 제공합니다.
