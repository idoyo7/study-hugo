...
조치가 없는 것을 왜 없는지까지 적어 다시 안 보게 만드는 목록이다.
...
- **native sidecar 검증**(Job/CronJob, init egress, 웹훅·컨트롤러, hold 무효화): 비용 — 검증 환경에 **k8s 1.33 노드 필요**. 워크로드별 확인이라 시간이 든다. 효과 — 안 하면 기동·종료·Job 완료가 예고 없이 바뀐다. 노드 업그레이드 시점에 터진다. 판정: **지금**.
...
- **in-place 순차 1.24→…→1.30**(홉 6): §2·§3·§4 사건을 하나씩. 홉마다 istiod 교체 + 전량 재시작. 판정: **부적합**(6번의 전량 재시작과 검증 창. §3의 orphan을 유일하게 실제로 만나는 경로).
- **canary 2마이너 1.24→1.26→1.28→1.30**(홉 3): 1.27의 native 플립이 **1.26→1.28 홉 안에서** 일어난다. 판정: **좋음**(공식 최단 경로. 단 native 전환이 홉 하나에 묶여 원인 분리가 어렵다).
- **canary 3홉 + 노드 버전 통제**(노드는 1.32 유지, 마지막에 1.33, 홉 3+1): native 플립이 **Istio 업그레이드가 끝난 뒤** 별도 창에서 일어난다. 판정: **최적(기존 클러스터)**(`auto`의 성질(§2.1)을 일정 도구로 쓰는 방법).
- **blue-green 신규 설치 1.30.3 직행**(홉 0): 아무것도 통과하지 않는다. 처음부터 1.30.3 + native + 신 차트 이름. 판정: **최적(우리 상황)**.
