...
- **전환 시점을 정하는 건 Istio 버전이 아니라 노드 kubelet 버전이다.** `DetectNativeSidecar`는 **모든 Node**를 훑어 kubelet 마이너가 하나라도 33 미만이면 끈다(`pkg/kube/inject/webhook.go:1235-1286`, `minVersion := 33`). 1.30 + k8s 1.32에서는 **안 켜진다.** 우리 목표(k8s 1.33)에서는 켜진다.
...
| 버전 | 릴리스일 | EOL | 지원 k8s | breaking·필수 조치 |
|---|---|---|---|---|
| **1.25.0** | 2025-03-04 | 2025-09-30 | 1.29~1.32 | **OpenCensus 트레이싱 제거.** Grafana ≥7.2 요구. `istioctl analyze --recursive`·`proxy-status --xds-via-agents` 플래그 제거 |
| **1.26.0** | 2025-05-08 | 2025-12-22 | 1.29~1.32(1.33 "동작 예상") | `ENABLE_AUTO_SNI` 플래그·코드패스 제거. **`MAX_CONNECTIONS_PER_SOCKET_EVENT_LOOP` 기본 0→1** — upgrade-notes엔 없고 change-notes에만 있다 |
| **1.27.0** | 2025-08-12 | 2026-04-07 | 1.29~1.33 | **native sidecar 기본화**(§2). Lightstep·OpenCensus **완전 제거**. Grafana 대시보드 UID 고정 → **업그레이드 후 재생성 필요**. 플러그인 CA `cacerts` 불완전 시 조용한 self-signed 폴백 → **명시적 기동 실패** |
| **1.28.0** | 2025-11-05 | **2026-07-01(EOL)** | 1.29~1.34 | `METRIC_ROTATION_INTERVAL`·`METRIC_GRACEFUL_DELETION_INTERVAL` **제거** → `sidecar.istio.io/statsEvictionInterval`. `PILOT_SPAWN_UPSTREAM_SPAN_FOR_GATEWAY` 기본 `true`. accept 제한이 **명시 포트 바인드 리스너까지** 확장. `BackendTLSPolicy` v1alpha3·`InferencePool` alpha/rc 제거 |
| **1.29.0** | 2026-02-16 | ~2026-08(예상) | 1.31~1.35 | **base/istiod 차트 통합**(§3). **디버그 엔드포인트 인가 기본 on**(15014). **CB remaining 메트릭 기본 off**. **stats 압축 기본 on** + `statsCompression` 어노테이션 제거. istiod `GOMEMLIMIT` 100%→90% |
| **1.30.0** | 2026-05-18 | ~2026-11(예상) | 1.32~1.36 | **Gateway API CRD v1.5.x 필수**(설치돼 있는 경우). **sidecar 서비스 네임스페이스 선택 로직 변경.** **XDS 디버그(15010) 인증 필수** — CVE-2026-31838. `retryBudget` 기본 `percent` 0.2%→20% 수정. CNI config 권한 0644→0600. `istioctl` 최소 k8s 1.32 |
...
k8s 하한이 순서 제약을 만든다. 1.28까지는 k8s 1.29를 받아주지만 1.29부터 하한이 1.31, 1.30부터 1.32다. 반대로 **1.24 계열은 k8s ≤1.31이라 1.33에서 아예 지원 대상 밖**이다. k8s를 먼저 올리면 현재 istio가 범위를 벗어나고, istio를 늦게 올리면 목표 버전이 EOL로 밀린다(§8).
...
| 1.24.0 | bool `false` + 파드 단위 어노테이션 `sidecar.istio.io/nativeSidecar` 신설 | change-notes 1.24:208-209. Feature Status **Alpha** |
| 1.26.0 | bool `false` | 1.26.0 태그 `pilot/pkg/features/experimental.go:179` |
| **1.27.0** | **string `"auto"`** | 1.27.0 태그 `pilot/pkg/features/pilot.go:307`. 마스터 `da90b3536f`([#56428](https://github.com/istio/istio/pull/56428))가 release-1.27로 백포트(`55ea856868`, [#56918](https://github.com/istio/istio/pull/56918))되어 **1.27.0 태그에 실제로 포함** |
...
| `auto`/`true`면 istiod가 **모든 Node를 watch**한다 | `webhook.go:222`가 `EnableNativeSidecars != Disabled`일 때만 Node kclient를 만든다(`StripNodeUnusedFields` 적용). 노드가 많으면 istiod 인포머 비용이 는다. RBAC은 이미 `nodes: get/list/watch`가 있어 추가 조치 없음 |

우리 목표는 k8s 1.33이므로 **결국 켜진다.** 검증 일정은 Istio 업그레이드 창이 아니라 **k8s 노드 업그레이드 창**에 맞춰야 하고, 검증 환경도 **k8s 1.33 노드가 실제로 있는 곳**이어야 한다. 1.32 노드에서 테스트하면 `auto`가 계속 disabled라 검증 자체가 성립하지 않는다.

### 2.2 무엇이 바뀌나 — 기동·종료 순서와 init 컨테이너의 소속

{{< seq caption="같은 워크로드가 classic과 native에서 다르게 기동·종료한다. 핵심은 istio-init의 위치다 — classic은 initContainers 마지막이라 사용자 init이 메시를 우회하고, native는 맨 앞이라 메시 안으로 들어온다" >}}
{
  "participants": [
    { "id": "K", "label": "kubelet" },
    { "id": "I", "label": "istio-init" },
    { "id": "P", "label": "istio-proxy" },
    { "id": "A", "label": "app" }
  ],
  "steps": [
    {
      "alt": "classic (ENABLE_NATIVE_SIDECARS=false)",
      "steps": [
        { "note": ["I", "A"], "lines": ["initContainers: istio-validation → 사용자 init → istio-init(마지막)", "\"iptables setup last so we do not blackhole init containers\""] },
        { "msg": ["K", "I"], "label": "1. iptables 설치" },
        { "msg": ["K", "P"], "label": "2. istio-proxy 기동" },
        { "msg": ["P", "K"], "label": "3. postStart: pilot-agent wait", "dashed": true },
        { "msg": ["K", "A"], "label": "4. app 기동" },
        { "note": ["P", "A"], "lines": ["종료: SIGTERM이 전 컨테이너에 동시에 간다", "Job은 proxy가 살아 있어 Complete로 안 넘어간다"] }
      ],
      "elseLabel": "native (기본 · 전 노드 kubelet ≥ 1.33)",
      "elseSteps": [
        { "note": ["I", "P"], "lines": ["initContainers: istio-init → istio-proxy → 사용자 init", "\"istio first, so init containers are part of the mesh\""] },
        { "msg": ["K", "I"], "label": "1. iptables 설치" },
        { "msg": ["K", "P"], "label": "2. restartPolicy: Always" },
        { "msg": ["P", "K"], "label": "3. startupProbe 통과", "dashed": true },
        { "msg": ["K", "A"], "label": "4. app 기동(순서 보장)" },
        { "note": ["K", "A"], "lines": ["종료: 일반 컨테이너 전부 종료 후 proxy에 SIGTERM(역순)", "preStop이 drain을 먼저 친다 · Job은 proxy를 무시하고 Complete"] }
      ]
    }
  ]
}
...
| init 컨테이너가 하는 일 | classic | native | 판정 |
|---|---|---|---|
| 메시 내부 서비스 호출 | 평문 직통. mTLS STRICT면 **상대가 거부** | 프록시 경유로 mTLS 성립 | **좋음** — 오히려 고쳐진다 |
| 메시 외부 호출(외부 API·DB) | 직통 | `ServiceEntry`·`Sidecar` egress 스코프·`outboundTrafficPolicy`의 지배를 받는다 | **반쪽** — `REGISTRY_ONLY`면 **init에서 처음 막힌다** |
| 인바운드 `AuthorizationPolicy`가 걸린 대상 호출 | 정책 우회(source가 메시 밖으로 보인다) | principal 기반 규칙이 실제로 판정된다 | **반쪽** — 우회로 통과했던 호출이 403이 된다 |
| 순수 로컬 작업(파일 준비·볼륨 권한) | 무영향 | 무영향(기동이 프록시 Ready 뒤로 밀린다) | **좋음** |
...
| 항목 | classic + hold=true | native |
|---|---|---|
| 기동 순서 보장 | Istio가 `postStart` 훅으로 구현 | **kubelet이 보장.** init 컨테이너가 Ready 되기 전엔 일반 컨테이너를 안 띄운다 |
| Ready 판정 | `postStart` 완료 = xDS 최초 수신 완료(`pilot-agent wait`) | `startupProbe`(기본 활성, `failureThreshold: 600` ≈ 10분) + readinessProbe |
| 종료 시 drain | 없음. SIGTERM 동시 수신 | `preStop`에서 `POST /drain` 선행 |
| 컨테이너 순서 부수효과 | `proxyLocation = MoveFirst` — `kubectl exec`·`logs` 기본이 프록시 | `istio-proxy`가 `spec.containers`에 없으므로 기본이 사용자 컨테이너로 돌아온다 |
| `holdApplicationUntilProxyStarts` 값 | 동작함 | **무시됨(에러·경고 없음)** |
...
- **다른 mutating webhook·컨트롤러.** upgrade-notes 1.27이 직접 경고한다 — *"This can cause compatibility issues with other mutating webhooks or controllers … that expect to modify the `istio-proxy` as a regular container."* `spec.containers[?(@.name=="istio-proxy")]`를 찾는 도구는 `initContainers`도 보게 고쳐야 한다. istio.io의 SPIRE 연동 가이드가 실제로 그렇게 고쳐졌고(`docs/ops/integrations/spire/index.md:217`), **게이트웨이는 예외**로 계속 일반 `containers`다(같은 문서 `:218`).
- **`istioctl kube-inject`의 출력이 웹훅과 달라진다.** 오프라인 경로는 kube client가 없으면 `nativeSidecar = (EnableNativeSidecars == Enabled)`로 계산한다(`pkg/kube/inject/inject.go:857-864`, `webhook.go:1259-1267`). 즉 **기본값 `auto`에서 `istioctl kube-inject`는 항상 classic을 뱉는다.** 렌더 결과를 golden 파일로 비교하는 CI가 있으면 클러스터 실제와 어긋난다.
- **어노테이션 판정이 오타에 안전하지 않다.** 템플릿 `:29`가 어노테이션 값과 문자열 `"false"`를 `ne`로 비교한다 — 소문자 `false` **정확히 그 문자열일 때만** 비활성이고 `"False"`·`"no"`·`"0"`·`"disabled"`는 전부 **활성**으로 읽힌다. 어노테이션 레퍼런스는 *"Takes precedence over the ENABLE_NATIVE_SIDECARS environment variable"*라고만 적어(`docs/reference/config/annotations/index.html:609`) 값 형식의 엄격함을 알려주지 않는다.

### 2.5 되돌리는 방법과 되돌리면 잃는 것

| 수단 | 부작용 | 판정 |
|---|---|---|
| `values.pilot.env.ENABLE_NATIVE_SIDECARS=false` | istiod의 Node watch도 사라진다. 1.27 이후 sidecar 개선을 native 전제로 받은 것은 없으므로 기능 손실 없음 | **좋음** — 전환 시점을 우리가 정하고 싶을 때 가장 단순 |
| `compatibilityVersion` 1.25/1.26 | 1.30.3 `manifests/helm-profiles/compatibility-version-1.26.yaml`은 `ENABLE_NATIVE_SIDECARS=false`와 함께 `DISABLE_SHADOW_HOST_SUFFIX`·`PILOT_SPAWN_UPSTREAM_SPAN_FOR_GATEWAY`·`DISABLE_TRACK_REMAINING_CB_METRICS`·`PILOT_SIDECAR_PICK_BEST_SERVICE_NAMESPACE`를 **한꺼번에** 옛 값으로 되돌린다(1.27 프로필은 native만 빠진 나머지, 1.28은 뒤 셋, 1.29는 마지막 하나) | **반쪽** — 원치 않는 항목까지 묶여 와 §4의 변경들이 조용히 되돌아간다 |
| `sidecar.istio.io/nativeSidecar: "false"`(파드 단위) | 값이 정확히 `"false"`여야 한다(§2.4). 워크로드마다 관리해 누락이 생긴다 | **반쪽** — 특정 워크로드만 예외 처리할 때 |
| 노드를 1.33 미만으로 유지 | k8s 업그레이드 자체가 목적이므로 성립 불가 | **부적합** |

우리 판정: **되돌리지 않는다.** 목표가 k8s 1.33이므로 native가 최종 상태고, 되돌림은 "검증이 끝나기 전에 노드가 1.33이 되는" 일정 사고를 막는 임시 수단으로만 쓴다. 쓰더라도 `compatibilityVersion`이 아니라 **`ENABLE_NATIVE_SIDECARS=false` 한 줄**로 — 묶여 오는 항목이 없어야 원인 추적이 된다.
...
| 플래그 | 이전 → 이후 | 버전 | sidecar 영향 | 되돌리기 |
|---|---|---|---|---|
| `PILOT_ENABLE_IP_AUTOALLOCATE` | `false` → `true` | 1.25.0 | `ServiceEntry.status.addresses`가 채워진다. 트래픽 경로 불변 | `false` |
| `ENABLE_AUTO_SNI` | `true` → **플래그 삭제**(항상 on) | 1.26.0 | 명시적 `false`로 끄고 있었다면 그 오버라이드가 무의미해진다 | 없음 |
| `MAX_CONNECTIONS_PER_SOCKET_EVENT_LOOP` | unset(무제한) → `1` | 1.26.0(gateway·virtual outbound) → 1.28.0(**명시 포트 바인드 리스너까지**) | 소켓 이벤트당 accept 1개. 초고빈도 신규 연결 워크로드에서 연결 수립 지연 가능 | `0` |
| `ENABLE_NATIVE_SIDECARS` | `false` → `"auto"` | 1.27.0 | §2 전체 | `false` |
| `PILOT_SPAWN_UPSTREAM_SPAN_FOR_GATEWAY` | `false` → `true` | 1.28.0 | 게이트웨이 요청마다 upstream 스팬이 하나 더 — **트레이싱 볼륨·비용 증가** | `false` |
| `METRIC_ROTATION_INTERVAL` · `METRIC_GRACEFUL_DELETION_INTERVAL` | 존재 → **제거** | 1.28.0 | 설정해 뒀으면 무시된다 → `sidecar.istio.io/statsEvictionInterval` | 없음 |
| `proxyConfig.statsCompression` | (없음) → `true` | 1.29.0 | Envoy stats 엔드포인트가 brotli/gzip/zstd 협상. `sidecar.istio.io/statsCompression` 어노테이션은 **제거** | `proxy.istio.io/config`로 파드별 `statsCompression: false` |
| `ENABLE_DEBUG_ENDPOINT_AUTH` | (없음) → `true` | 1.29.0(15014) → 1.30.0(**15010 확장**) | non-system 네임스페이스는 `config_dump`·`ndsz`·`edsz`만, 동일 네임스페이스 proxy만 | `false`(1.30은 **CVE-2026-31838 수정 무력화**, 비권장). 1.30부터 `DEBUG_ENDPOINT_AUTH_ALLOWED_NAMESPACES` 화이트리스트 |
| `DISABLE_TRACK_REMAINING_CB_METRICS` | (없음) → `true`(트래킹 **비활성**) | 1.29.0 | 서킷브레이커 remaining 메트릭이 사라진다 | `false` |
| `GOMEMLIMIT`(istiod, `automemlimit`) | limit의 100% → **90%** | 1.29.0 | OOM 위험은 줄고 GC 빈도는 는다. istiod 메모리를 limit에 맞춰 운용 중이면 헤드룸 재산정 → [09]({{< relref "09-istiod-scaling-connections.md" >}}) | `AUTOMEMLIMIT=1`(비율 리터럴이므로 `1`=100%) 또는 `GOMEMLIMIT` 직접 지정 |
| `PILOT_SIDECAR_PICK_BEST_SERVICE_NAMESPACE` | (없음, 알파벳순) → `true` | 1.30.0 | **라우팅 대상이 바뀔 수 있다**(§6.1) | `false` 또는 `compatibilityVersion: "1.28"` |
...
1.25.0에서 `istioctl analyze --recursive`(재귀가 항상 true로 고정)와 `istioctl proxy-status --xds-via-agents`(실험 플래그)가 제거됐다. **1.26~1.30 구간에는 서브커맨드·플래그 제거가 없다** — change-notes의 `**Removed**` 항목을 전 버전 grep한 결과 istioctl 관련은 위 둘뿐이다. 대신 1.30은 `istioctl` 자체의 최소 지원 k8s를 `1.32.x`로 올렸고(구 클러스터를 같은 바이너리로 다루던 스크립트가 있으면 분리해야 한다), `istioctl bug-report`에 `--skip-cluster-dump`·`--skip-analyze`·`--skip-proxy-debug`·`--skip-netstat`·`--skip-coredumps`·`--tail`을 추가했다 — 대규모 클러스터에서 bug-report가 타임아웃하던 문제의 손잡이다.
...
⑥·⑦은 CI에 넣을 수 없다 — **클러스터 상태에 의존**하기 때문이다. 업그레이드 런북의 사전 점검 단계로 넣고 결과를 캡처해 티켓에 붙인다.
...
| 신호 | 방향 | 근거 |
|---|---|---|
| sidecar mode를 deprecated로 지정한 공지 | **없음** | 1.25~1.30의 `_index.md`·upgrade-notes·change-notes 전수 확인 |
| 현재 문서의 sidecar 서술 | 유지 | `docs/overview/dataplane-modes/index.md:25` — *"well understood and thoroughly battle-tested, but comes with a resource cost and operational overhead."* 비교표에서 트래픽 관리·보안·관측성 모두 sidecar가 **"Full Istio feature set"** |
| native sidecar 기본화 · sidecar용 native `nftables`(둘 다 1.27) | **sidecar 경로에 대한 투자** | 파드 라이프사이클 문제를 k8s 표준으로 해결했고 [#56487](https://github.com/istio/istio/issues/56487)은 sidecar 모드용이다. 폐기 예정 경로에는 이런 작업을 하지 않는다 |
| ambient 마이그레이션 가이드 신설(1.30) | 중립 | *"gradual and reversible, sidecar and ambient workloads can coexist during the process."* 전환을 **쉽게** 만들었지만 **강제**하지 않는다 |
| ambient의 하드 블로커 | **우리 이동을 막는 쪽** | `docs/ambient/migrate/_index.md:75-106` — VM 워크로드·SPIRE·`PeerAuthentication mode: DISABLE`·**primary-remote 멀티클러스터**는 마이그레이션 불가. **`EnvoyFilter`는 waypoint에 미지원** |
| `TrafficExtension` 신설(1.30) | **확장 축의 방향 전환** | 릴리스 공지가 *"replacing `WasmPlugin` as the primary proxy extensibility mechanism"*로 소개한다. change-notes:92는 Lua 확장만 적고 `WasmPlugin` deprecate·`EnvoyFilter` 언급은 없다 → [08 EnvoyFilter]({{< relref "08-envoyfilter-extension.md" >}}) |
...
| # | 트리거 | 어디서 보나 |
|---|---|---|
| 1 | 공지·`docs/overview/dataplane-modes/`가 sidecar를 **deprecated로 표기** — 그때는 선택이 아니라 일정 문제다 | 매 마이너의 `_index.md`·upgrade-notes |
| 2 | **ambient 전용으로만 출시된 기능**이 우리 요건에 필요해짐 — 기능 격차가 비용 논의를 대체한다 | change-notes의 "ambient only" 표기 |
| 3 | **`EnvoyFilter`가 waypoint에서 지원**되거나, 우리 EnvoyFilter 2개를 표준 API·`TrafficExtension`으로 대체 가능해짐 — 최대 블로커가 사라진다 | `docs/ambient/migrate/_index.md` known limitations |
| 4 | sidecar 오버헤드가 **노드 예산의 유의미한 비율**이 됨 — ambient의 유일한 확실한 이득이 이 비용이다 | 파드 수 × proxy requests 대 노드 총량 → [14 왜 서비스 메시인가]({{< relref "14-why-service-mesh.md" >}}) |
| 5 | `WasmPlugin`이 **명시적으로 deprecate**되고 `TrafficExtension`이 유일 경로가 됨 — 확장 축을 갈아야 하니 그 창에 같이 심사한다 | 1.31+ change-notes의 `**Deprecated**` 항목 |

역으로 **트리거가 아닌 것**도 적어둔다. ambient의 성숙도 승격, 기본값 변화(DNS 캡처·iptables 재조정), 마이그레이션 가이드 신설은 전부 "ambient가 좋아졌다"는 신호일 뿐 **sidecar가 나빠졌다는 신호가 아니다.** 이 구간의 ambient 헤드라인 전부가 여기에 해당한다.

## 6. 버전별 나머지

조치가 없는 것을 왜 없는지까지 적어 다시 안 보게 만드는 표다.

| 버전 | 항목 | 우리에게 |
|---|---|---|
| 1.25~1.30 | ambient 계열 전부 — iptables 재조정·DNS 캡처 기본 on(1.25/1.29), ztunnel 차트 리소스명 변경(1.25) 후 되돌림(1.26), waypoint `TCPRoute`(1.26), multi-network multicluster beta·ztunnel dry-run 정책(1.29), CIDR `ServiceEntry`·XFCC 합성·HBONE 윈도우·CNI Agent `excludeNamespaces` 준수(1.30) | ambient 미사용 — **전부 해당 없음.** 성숙도 신호로만 §5에서 읽는다 |
| 1.25 | DNS 트래픽(UDP/TCP)이 `traffic.sidecar.istio.io/exclude*`를 준수 | **실제 변경은 1.23 계열에서 일어났고 노트에서 빠졌다**고 upgrade-notes가 자백한다. DNS 포트를 exclude에 넣은 적이 있으면 동작이 이미 달라져 있다 |
| 1.25 | Grafana ≥7.2 요구, `istio-cni-node`에 `DAC_OVERRIDE`·AppArmor unconfined | 번들 대시보드를 쓰면 Grafana 버전 확인. 뒤쪽은 istio-cni 사용 여부에 따라 갈린다(§9) |
| 1.27 | 다중 서버 인증서(RSA+ECDSA), 플러그인 CA CRL(`ca-crl.pem`) | 선택 도입. 플러그인 CA를 쓰면 CRL은 검토 가치 있음 |
| 1.27 | **mTLS가 `PILOT_ENABLE_TELEMETRY_LABEL`/`PILOT_ENDPOINT_TELEMETRY_LABEL=false`에서 의도치 않게 꺼지던 버그 수정** | **업그레이드와 무관하게 지금 확인할 것.** 두 플래그를 `false`로 오버라이드한 적이 있으면 그 구간 동안 mTLS가 조용히 비활성이었을 수 있다(change-notes 1.27:70) |
| 1.27 | 플러그인 CA `cacerts` 불완전 시 self-signed 조용한 폴백 → **명시적 실패** | 플러그인 CA를 쓰면 업그레이드 전 번들 완전성 확인. 불완전하면 1.27부터 istiod가 기동에 실패한다(change-notes 1.27:133) |
| 1.28 | `seccompProfile: RuntimeDefault` 지원(`global.proxy.seccompProfile`), consistent hash LB의 쿠키 속성(`SameSite`·`Secure`·`HttpOnly`) | 둘 다 **옵트인이고 기본값 변경이 아니다.** 앞은 보안 기준을 올릴 때 한 줄로 켜는 카드, 뒤는 쿠키 세션 어피니티를 쓰면 도입 가치([#56468](https://github.com/istio/istio/issues/56468)) |
| 1.28 | dual-stack beta, Gateway API v1.4, `BackendTLSPolicy` v1, `InferencePool` v1, 원격 istiod `Endpoints`→`EndpointSlice` | 전부 미사용·자동 — 해당 없음. `BackendTLSPolicy`는 `PILOT_ENABLE_ALPHA_GATEWAY_API=true`도 더는 필요 없다 |
| 1.29 | **NetworkPolicy 옵션**(`global.networkPolicy.enabled=true`), `PILOT_IGNORE_RESOURCES`, `ENABLE_WILDCARD_HOST_SERVICE_ENTRIES_FOR_TLS`(alpha) | 전부 옵트인·기본 off. 첫째는 컨트롤 플레인 격리 카드, 셋째는 SNI 스푸핑 위험이 공지에 명시돼 있어 켜지 않는다 |
| 1.30 | CNI config 권한 0644→0600(CIS 1.12), `PILOT_ENABLE_NODE_UNTAINT_CONTROLLERS`가 Helm `taint.enabled`로 자동 구성, 기본 레지스트리 `registry.istio.io`, 웹훅 HTTPS(15017) 타임아웃 추가 | 권한은 런타임이 root로 읽으므로 통상 무영향(필요하면 `values.cni.env.CNI_CONF_GROUP_READ=true`로 0640). untaint는 쓰면 수동 env를 지울 수 있다(선택). 레지스트리는 ECR 미러라 **소스 URL만 갱신** |
| 1.30 | agentgateway(`PILOT_ENABLE_AGENTGATEWAY`, 실험), `ListenerSets` 상태, `TLSRoute` termination | 기본 off·Gateway API 전용 — 해당 없음 |
| 1.30 | **CUSTOM 인가 프로바이더 워크로드당 다중 지원**, `istio.io/connect-strategy: RACE_FIRST_TCP_CONNECT`, `DNS_FORWARD_TIMEOUT`(기본 `5s` 유지) | 첫째는 ext_authz를 경로별로 다르게 걸 수 있게 됐다 → [07 nginx→Istio]({{< relref "07-from-nginx-to-istio.md" >}}). 나머지는 필요 시 카드 |
...
| 식별자 | 내용 | 우리에게 |
|---|---|---|
| **CVE-2026-31837**(critical) | JWKS fallback 메커니즘이 **RSA 개인키를 유출** → JWT 위조·인증 우회([GHSA-v75c-crr9-733c](https://github.com/istio/istio/security/advisories/GHSA-v75c-crr9-733c)) | `RequestAuthentication`(JWT)을 쓰면 **최우선**. 1.30.0 이상 필수 |
| **인가 우회**([#59992](https://github.com/istio/istio/issues/59992)) | `source.principals`(suffix 매칭)·`source.namespaces`의 정규식 메타문자가 이스케이프되지 않아 **의도치 않은 identity가 정책에 매칭**될 수 있었다 | `AuthorizationPolicy`를 쓰는 모든 클러스터. 수정 후 매칭이 **좁아질 수 있으니** 정책 재검증 |
| **CVE-2026-39350** / **CVE-2026-41413** | `serviceAccount` matcher regex 미인용 / JWKS URI CIDR 차단이 리다이렉트·issuer discovery로 우회됨 | SA 이름에 정규식 특수문자가 있으면 매칭이 바뀐다 / `BLOCKED_CIDRS_IN_JWKS_URIS`를 쓰면 확인 |
| 리프 인증서 `NotAfter`가 서명 인증서 만료를 넘던 문제([#59768](https://github.com/istio/istio/issues/59768)) · CA 번들 rotation이 인증서 순서에 따라 안 되던 문제([#59909](https://github.com/istio/istio/issues/59909)) | 만료 검증이 엄격해진다 / `CERTIFICATE` PEM 블록만 비교 대상 | 플러그인 CA의 intermediate 잔여 수명이 짧으면 발급 인증서 수명도 짧아진다 / `TRUSTED CERTIFICATE` 블록을 섞은 번들은 무시된다 |

**이 CVE들은 전부 1.30.0에 최초 포함**됐다. 1.29.x 백포트 여부는 확인하지 못했으므로 **1.29에 머무는 경로를 검토한다면 백포트를 별도 확인**해야 한다. 목표를 1.30.3으로 잡으면 그 확인이 불필요해진다 — 목표 버전 선택의 보안 측 근거다.

### 6.4 `retryBudget`이 100배 달라진다 (1.30)

`DestinationRule.trafficPolicy.retryBudget`의 기본 `percent`가 **0.2%로 잘못 계산되던 버그**가 20%로 고쳐졌고([#59504](https://github.com/istio/istio/issues/59504)), 같은 릴리스에서 top-level `retryBudget`이 **subset이 자체 `trafficPolicy`를 가지면 조용히 버려지고** subset 레벨 `retryBudget`도 무시되던 문제가 고쳐졌다([#59667](https://github.com/istio/istio/issues/59667)).
...
전제: sidecar 유지·ambient 금지, k8s 1.33, 목표 **1.30.3**, 라이브 버전 미확정(하한 가정 1.24.1), istiod는 KEDA로 오토스케일, classic Gateway, `local-reply` EnvoyFilter 2개.

| 버전 | 우리에게 무엇인가 | 필수 조치 | 판정 |
|---|---|---|---|
| 1.25 | 통과 지점. OpenCensus·istioctl 플래그 제거만 걸린다 | §4.4의 ②·⑤ | **통과** |
| 1.26 | accept 제한 기본 변경이 **모든 sidecar 아웃바운드**에 닿는다 | 초고빈도 신규 연결 워크로드가 있으면 전후 벤치마크 | **관찰** — 사전 조치 없음, 회귀 시 `0`으로 복원 |
| 1.27 | **이 구간의 본체.** native sidecar 기본화 + 플러그인 CA 하드 실패 + 텔레메트리 프로바이더 제거 | §2 전체 검증, `cacerts` 완전성, Lightstep/OpenCensus grep, Grafana 대시보드 재생성 | **최대 작업량** — 여기에 시간을 몰아야 한다 |
| 1.28 | 플래그 2개 제거 + 트레이싱 볼륨 증가 + accept 제한 확장 | `METRIC_*` grep, 트레이스 샘플링·저장 비용 재산정 | **통과** — 단 트레이싱 비용은 사전 계산 |
| 1.29 | 차트 이름(§3, 아마 무해) + **알람 3종 무효화** | CB remaining 패널·알람 grep, stats 스크레이퍼 확인, Kiali 네임스페이스 확인, istiod 메모리 헤드룸 재산정 | **관측성 작업** — 코드가 아니라 대시보드·룰이 대상 |
| 1.30(목표) | 라우팅 선택 로직 + Gateway API CRD + 15010 인증 + CVE 5건 + `retryBudget` | hostname 중복 카운트, CRD 버전, `--plaintext` 스크립트 정리, `retryBudget` 값 재결정 | **도착점** — 전부 사전 점검이고 사후 발견이 비싸다 |

### 7.2 도입 우선순위

| 항목 | 비용(설정·리스크) | 효과 | 판정 |
|---|---|---|---|
| **native sidecar 검증**(Job/CronJob, init egress, 웹훅·컨트롤러, hold 무효화) | 검증 환경에 **k8s 1.33 노드 필요**. 워크로드별 확인이라 시간이 든다 | 안 하면 기동·종료·Job 완료가 예고 없이 바뀐다. 노드 업그레이드 시점에 터진다 | **지금** |
| **플래그·메트릭 grep**(§4.4 ①~⑤) | 리포지토리 grep + 대시보드 치환 | 안 하면 **알람이 조용히 안 온다.** 가장 발견이 늦는 종류 | **지금** |
| **Gateway API CRD 확인 · hostname 중복 카운트 · `--plaintext` 도구 정리** | `kubectl` 두 번 + 스크립트 수정 | TLS passthrough가 에러 없이 죽는 것, 라우팅 대상이 바뀌는 것, 업그레이드 직후 운영 도구가 죽는 것을 사전에 잡는다 | **지금** |
| **CVE 대응 = 1.30.3 이상 고정** | 목표 버전 그대로 | JWT 위조(CVE-2026-31837)·인가 우회(#59992)가 1.30.0에서 수정. 1.29 백포트 미확인 | **지금** |
| **`retryBudget` 값 재결정** | `DestinationRule` 수정. 값 판단 필요 | 안 하면 재시도 예산이 100배로 뛴다 → 업스트림 부하 증가 | **지금**(쓰고 있다면) |
| **stats 스크레이퍼 압축 확인 · 디버그 엔드포인트 네임스페이스 정리** | 스크레이프 설정·Kiali 배치 또는 값 한 줄 | 실패하면 sidecar stats 전량 소실 / 관측 도구 조회 실패 | **다음 분기**(1.29 통과 전) |
| **`seccompProfile: RuntimeDefault`**(1.28) · **`global.networkPolicy.enabled`**(1.29) | values 한 줄 + 호환 확인 / NetworkPolicy 도입 검토 | syscall 표면 축소 / 컨트롤 플레인 네트워크 격리 | **다음 분기** — 업그레이드와 분리해서 켠다 |
| **CUSTOM 인가 다중화**(1.30) · **native `nftables`**(1.27) | `MeshConfig`+정책 / values 한 줄 + 노드 OS 의존 | 경로별 ext_authz / 노드가 nftables 우선으로 갈 때 대비 | **보류** — 트리거는 "요건 발생"·"노드 OS 변경" |
| **`TrafficExtension`** · **agentgateway**(둘 다 1.30) · **ambient 전환** | 신규 API의 관계가 아직 얇다 / 실험·Gateway API 전용 / ztunnel·waypoint 도입 + `EnvoyFilter` 2개 이관 | Lua·Wasm 확장의 1급 경로 / AI 에이전트 트래픽 / 파드당 proxy 오버헤드 제거 | **보류** — 앞 둘은 §5 트리거 5, ambient는 방침이 금지(재검토는 §5) |
...
| 경로 | 홉 | 이 구간에서 통과하는 사건 | 판정 |
|---|---|---|---|
| in-place 순차 1.24→…→1.30 | 6 | §2·§3·§4 사건을 하나씩. 홉마다 istiod 교체 + 전량 재시작 | **부적합** — 6번의 전량 재시작과 검증 창. §3의 orphan을 유일하게 실제로 만나는 경로 |
| canary 2마이너 1.24→1.26→1.28→1.30 | 3 | 1.27의 native 플립이 **1.26→1.28 홉 안에서** 일어난다 | **좋음** — 공식 최단 경로. 단 native 전환이 홉 하나에 묶여 원인 분리가 어렵다 |
| canary 3홉 + 노드 버전 통제(노드는 1.32 유지, 마지막에 1.33) | 3 + 1 | native 플립이 **Istio 업그레이드가 끝난 뒤** 별도 창에서 일어난다 | **최적(기존 클러스터)** — `auto`의 성질(§2.1)을 일정 도구로 쓰는 방법 |
| **blue-green 신규 설치 1.30.3 직행** | 0 | 아무것도 통과하지 않는다. 처음부터 1.30.3 + native + 신 차트 이름 | **최적(우리 상황)** |

**blue-green이 유리한 이유는 "빠르다"가 아니라 "통과하지 않는다"다.** 이 문서가 다룬 사건의 절반은 **버전을 통과할 때만** 문제가 된다 — §3의 orphan은 발생하지 않고, §4의 기본값 변경은 "변경"이 아니라 "초기값"이 되어 되돌릴 이전 상태가 없고, §6.1·§6.4는 "바뀜"이 아니라 "그렇게 시작"이 되고, 스큐 자체가 없다. 그래도 **남는 것이 둘 반 있다.** ① **§2 native sidecar** — k8s 1.33 노드에서 신규 설치하면 처음부터 native이므로 검증은 어차피 해야 한다. ② **§6.3의 도구 정리** — CVE는 수정 포함이지만 `--plaintext` 스크립트는 그대로 죽는다. 그리고 §6.1이 반쪽 — 규칙 변경 자체는 무관해지지만 **의도한 대상으로 가는지는 여전히 확인해야 한다.**

절차 상세(차트 리워크, 이미지 ECR 미러, app-of-apps 핀 갱신, 배포 순서, 트래픽 컷오버 게이트, 롤백)는 [eks-upgrade/istio]({{< relref "../eks-upgrade/components/02-istio.md" >}})가 소유한다. 다만 그 문서의 첫 체크리스트 항목은 그대로 유효하다 — **라이브 istiod/proxy 버전이 미확인이면 위 표의 홉 산정 전부가 무의미하다.** 그리고 §3의 결과로 "base/istiod 차트 통합" 항목은 문구를 고쳐야 한다.

## 9. 근거

`news/…`는 `/Users/mont/evejuni/istio-io/content/en/news/releases/`, `docs/…`는 `/Users/mont/evejuni/istio-io/content/en/docs/`, 그 밖의 상대 경로는 `/Users/mont/evejuni/istio` 레포 루트 기준이다.
...
- **1.30.0 CVE들의 1.29.x 백포트 여부.** 로컬 클론에 패치 릴리스 상세가 없다. 1.29 체류 경로를 검토하면 GitHub Security Advisory를 직접 조회해야 한다.
- **1.29 "Base Helm chart removals"의 실제 제거 대상.** upgrade-notes 매핑표와 로컬 클론의 차트 트리가 어긋난다(§3.2). change-notes 1.29:229가 어느 파일을 가리키는지 특정하지 못했다.
- **1.25~1.30 각 버전의 Envoy 버전과 그로 인한 동작 변화.** change-notes가 Envoy 버전을 명시하지 않고, 로컬에 Envoy 클론이 없어 `istio.deps`의 proxy SHA를 태그로 변환하지 못했다. istio.io의 `supported-releases` 표는 EOL 마이너를 싣지 않아 과거 값도 못 찾았다.
