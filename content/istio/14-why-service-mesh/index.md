...
[01]({{< relref "01-mesh-basics.md" >}})은 메시의 출발점이 "공통 관심사의 중복"이라고 짚었습니다. 여기서는 그 중복의 **단가**를 봅니다.

Envoy 공식 문서는 이 문제를 분산 아키텍처 일반의 문제로 적습니다 — 분산 아키텍처로 옮길 때 생기는 운영 문제의 대부분이 결국 **네트워킹과 관측성** 두 영역에 뿌리를 둔다는 것입니다. 같은 문서의 철학 문장이 해법의 방향까지 못 박습니다: *"The network should be transparent to applications. When network and application problems do occur it should be easy to determine the source of the problem."*

istio.io도 메시를 같은 방향으로 정의합니다 — *"A service mesh is an infrastructure layer that gives applications capabilities like zero-trust security, observability, and advanced traffic management, without code changes."* 세 축(트래픽 관리 · 관측성 · 보안)을 **코드 수정 없이 인프라 레이어에서** 준다는 것이 공식 자기 정의입니다.
...
istio.io 쪽은 같은 결론에 다른 각도로 닿습니다. 아키텍처 문서가 사이드카 프록시 모델의 효용을 **재설계나 코드 재작성 없이 기존 배포에 기능을 얹을 수 있다**는 취지로 적습니다. 다만 "라이브러리 대비"라는 명시적 비교 프레이밍은 Envoy 문서 쪽에서만 확인되고, istio.io 문서 트리에서는 찾지 못했습니다.
...
네 가지 배치 — ① 앱 내 라이브러리, ② API 게이트웨이만, ③ 사이드카 메시, ④ ambient(노드 프록시) — 를 항목별로 비교하면 이렇습니다.
...
②와 ③ 사이의 칸이 실무에서 가장 자주 오해되는 지점입니다. 게이트웨이는 **클러스터 경계 한 지점**을 덮습니다. 그 지점을 지나지 않는 트래픽에는 게이트웨이가 건 정책도, 게이트웨이가 만든 지표도 존재하지 않습니다.
...
공식 문서의 자기 정의(§1)는 **주장**이지 증거가 아니다. 우리가 실제로 얻은 것이 무엇인지는 우리 문서에 남아 있으므로, 이점 목록 대신 증거의 위치로 적습니다.
...
지연도 공식 문서가 인정합니다 — *"Since Istio adds a sidecar proxy or ztunnel proxy on the data path, latency is an important consideration."* 그리고 P90/P99 지연 벤치마크를 sidecar · ambient L4 · ambient L4+L7 세 배포 모드별로 제시합니다. 문서의 절대값을 우리 SLO에 그대로 옮기면 안 됩니다. [01]({{< relref "01-mesh-basics.md" >}})이 말한 "요청당 프록시 홉 2회"가 우리 꼬리 지연의 어디를 먹는지는 우리 트래픽으로 재야 합니다.
...
공식 문서도 기본값 그대로 규모를 키우면 안 된다는 것을 인정합니다 — 대규모 배포에서는 **configuration scoping이 강력히 권장된다**고 성능 문서가 적습니다. 다만 istio.io가 '운영 복잡성(operational complexity)'이라는 표현을 그대로 써서 메시의 비용으로 인정하는 대목은 성능 문서 밖에서 확인하지 못했습니다.
...
가장 자주 과대평가되는 칸입니다. Istio 공식 트래픽 관리 개념 문서가 경계를 직접 긋습니다. 실패 복구 기능은 애플리케이션에 *"completely transparent"* 하지만, 그 실패를 다루는 책임까지 가져가지는 않는다는 것입니다.
...
즉 메시는 **네트워크 수준 복원력**(재시도·타임아웃·서킷브레이킹)을 앱 밖으로 빼주지만, **비즈니스 수준 폴백**은 못 가져갑니다. 라이브러리를 걷어냈다고 앱이 네트워크 실패를 몰라도 되는 상태가 되는 것은 아닙니다. 라이브러리 대비 절감폭을 계산할 때 이 칸을 빼먹으면 이득이 과대 계상됩니다.
...
서비스 수가 적을 때는 §1의 리드타임 계수도 함께 작습니다. 단일 언어·단일 팀에서는 업그레이드 결합도 팀이 하나면 조율 가능하다는 점이 라이브러리의 약점을 더 상쇄합니다.
