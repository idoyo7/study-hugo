...
{{< callout type="info" >}}
**한눈에**
- 사이드카가 이미 모든 요청을 가로채므로 **앱 무수정으로** 메트릭·액세스 로그·트레이싱(골든 시그널)이 공짜로 나옵니다.
- 힘은 **표준 라벨 차원**에 있다 — `response_flags`로 실패 원인, `connection_security_policy`로 mTLS 커버리지까지 서비스 무관하게 슬라이스합니다.
- **트레이싱은 유일한 예외**: 스팬 생성은 메시가 하지만 trace 컨텍스트 헤더 전파는 앱 몫입니다.
- **카디널리티는 공짜가 아니다** — Telemetry API로 태그를 정리·집약해 관리합니다.
{{< /callout >}}

> **왜 이 이야기.** 메시를 얹기 전에는 지표가 서비스마다 제각각이었습니다. 요청 수를 내보내는 팀이 있고 안 내보내는 팀이 있었습니다. 내보내도 라벨 이름이 서비스마다 달랐습니다. Istio를 얹자 **모든 서비스가 동일한 스키마의 골든 시그널을 앱 코드 수정 없이** 뿜기 시작했습니다 — 사이드카가 이미 모든 요청을 보고 있으니 공짜입니다. 이 문서는 메시가 주는 관측성의 세 축(메트릭·액세스 로그·트레이싱)과, 그 "공짜"의 한계·비용을 정리합니다.
...
사이드카 Envoy는 파드가 주고받는 **모든 요청을 가로챕니다** — [01]({{< relref "01-mesh-basics.md" >}})에서 본 대로입니다. 요청을 볼 수 있는 지점이 이미 트래픽 경로에 박혀 있다는 뜻입니다. 그래서 애플리케이션이 계측 코드를 한 줄도 넣지 않아도 Envoy가 요청 단위로 메트릭·로그·트레이스 스팬을 만들어냅니다. **관측 대상이 앱이 아니라 프록시**이므로 언어·프레임워크가 뭐든 동일하게 관측됩니다.
...
Istio가 내보내는 핵심 프록시 메트릭은 다음과 같습니다. 이름이 서비스와 무관하게 **고정**이라는 점이 핵심입니다.
...
진짜 힘은 **표준 라벨 차원**에 있습니다. 모든 요청 메트릭이 같은 라벨을 달고 나오므로 서비스가 무엇이든 같은 질문을 던질 수 있습니다.
...
라벨이 이렇게 고정이면 **통합 대시보드 하나**로 전 서비스의 RED를 봅니다. 에러율은 `sum(rate(istio_requests_total{response_code=~"5.."}[5m])) by (destination_service)` 한 줄로 나옵니다. 서비스별로 대시보드를 새로 짤 필요가 없습니다. mTLS가 안 걸린 트래픽 색출도 `connection_security_policy="none"` 한 줄이면 됩니다 — nginx 시절엔 없던 관측 포인트입니다.
...
사이드카는 요청마다 트레이스 스팬을 만듭니다. **함정**은 그 다음입니다: Envoy는 자기가 본 홉의 스팬만 만들 뿐, 그것들을 하나의 트레이스로 이으려면 **애플리케이션이 trace 컨텍스트 헤더를 전파**해야 합니다.

- 앱이 인바운드 요청의 `x-request-id`·`traceparent`(W3C) 또는 `b3`(Zipkin) 헤더를 **아웃바운드 호출에 그대로 실어보내야** 스팬들이 한 트레이스로 연결됩니다.
- 이걸 안 하면 서비스마다 끊긴 스팬 조각만 남습니다. **"트레이싱은 공짜"라는 말의 유일한 예외** — 메시가 스팬 생성은 해주지만 전파는 앱 몫입니다.

트레이싱 백엔드(Jaeger/Tempo 등)와 샘플링 비율도 Telemetry API로 지정합니다.

## 4) 토폴로지 — 메트릭에서 유도되는 서비스 그래프

`source_workload`→`destination_service` 라벨이 곧 서비스 그래프의 간선이므로 이 메트릭만으로 **서비스 의존 지도**를 그릴 수 있습니다. Kiali 같은 도구가 Prometheus 메트릭 + Istio 설정을 읽어 실시간 트래픽 토폴로지, mTLS 상태, 에러 흐름을 시각화합니다. 별도 계측 없이 "지금 무엇이 무엇을 부르고 어디서 에러가 나는지"가 그림으로 나옵니다.

## 공짜의 비용 — 카디널리티

관측이 공짜라고 지표 저장까지 공짜는 아닙니다. 표준 라벨이 많다는 건 곧 **시계열 카디널리티가 크다**는 뜻입니다.
...
메시가 주는 기본값을 그대로 쓰지 않고 다듬는 표준 창구가 **Telemetry API**입니다. 하나의 리소스로 세 축을 모두 제어합니다.
...
- 메시 관측성이 "공짜"인 건 **관측 지점(사이드카)이 이미 트래픽 경로에 있기 때문**입니다. 앱 무수정으로 모든 서비스가 동일 스키마의 골든 시그널을 냅니다.
- 힘은 **표준 라벨 차원**에 있다 — `response_flags`로 실패 원인, `connection_security_policy`로 mTLS 커버리지까지 서비스 무관하게 슬라이스합니다.
- 단, **트레이싱은 앱의 헤더 전파가 필요**하고, **카디널리티는 공짜가 아닙니다**. 커스터마이징·태그 정리는 Telemetry API로 합니다.
