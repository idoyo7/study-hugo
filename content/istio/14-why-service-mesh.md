...
네 가지 배치 — ① 앱 내 라이브러리, ② API 게이트웨이만, ③ 사이드카 메시, ④ ambient(노드 프록시) — 를 항목별로 비교하면 이렇다.
...
- ④ ambient(노드 프록시): 프록시 수는 줄고, **장애 격리 단위가 노드·네임스페이스로 커진다**
...
위 목록의 세 번째 항목은 양면이다. 프록시가 단서를 준 것도 맞지만, **조사해야 할 층을 하나 늘린 것도 프록시**다. 그 계산이 다음 절이다.
...
서비스 수가 적을 때는 §1의 리드타임 계수도 함께 작다. 단일 언어·단일 팀에서는 업그레이드 결합도 팀이 하나면 조율 가능하다는 점이 라이브러리의 약점을 더 상쇄한다.
