...
{{< callout type="info" >}}
**한눈에** — 우리가 실제로 돌리는 것은 표준 조립에 컴포넌트 하나가 더 붙은 형상이다.
...
기준 문서가 **표준이 어떻게 생겼고 왜 그렇게 정했나**를 다룬다면, 이 장은 **우리 클러스터가 지금 어떤 상태인가**만 다룬다. 표준 4컴포넌트의 배치·포트·의존은 [스택 토폴로지]({{< relref "../../hyperdx/01-stack-topology.md" >}}), 컴포넌트별 가용성 종합·blast radius·무손실 2트랙은 [operator 토폴로지·다운타임]({{< relref "../../hyperdx/04-operator-topology-downtime.md" >}}), Keeper의 역할과 유실 지점은 [Keeper]({{< relref "../../hyperdx/05-keeper.md" >}}), 승격 없는 복제와 EBS reattach는 [복제·failover]({{< relref "../../hyperdx/06-replication-failover.md" >}})가 소유한다. 여기서 반복하지 않는다.

표준과 다른 점은 하나다. RUM 데이터(브라우저 SDK·Mobile RUM)를 받기 위해 별도로 만든 **자체 RUM 컨버터**다 `✓` — Datadog Agent가 RUM을 보내는 방식을 참조해 구현했고, OTel Collector를 거치지 않고 ClickHouse에 직접 적재한다. HyperDX 웹 데이터 경로도 일부 커스터마이즈했다 `✓`.
...
"4컴포넌트"는 논리 구분이고 실제 배치(실행) 단위는 그보다 많다. 그런데 세는 수가 두 개다 — **표준 조립대로 세면 5개, 우리가 실제로 돌리는 것은 6개**다. 두 숫자는 충돌이 아니라 축이 다르다.
...
차이는 자체 RUM 컨버터 하나다 `Σ`. 표준 ClickStack에는 없는 우리 추가 컴포넌트이므로, "표준이 5개"라는 서술과 "우리가 6개"라는 서술을 같은 자리에 두면 어느 쪽이 틀린 것처럼 보인다 — 조립 문서(기준 문서)는 5를, 현황 문서(이 장)는 6을 쓴다.

소유권 경계도 같은 그림에서 갈린다. 차트가 관리하는 영역(`clickhouse:false`로 CH를 뺀 뒤 남는 것)과 Altinity operator가 관리하는 영역이 다르다.

{{< flow src="_flow/3-데이터-흐름-rum-인제스트.json" />}}

배치 형태와 상태 보유를 한 줄로 정리하면 이렇다. 표준 리슨 포트·의존 방향의 정본은 [스택 토폴로지]({{< relref "../../hyperdx/01-stack-topology.md" >}}) §2 표이고, 여기서는 **그 기본값과 어긋나는 우리 값만** 짚는다(Altinity CHK의 클라이언트 포트 등).
...
- **OTel Collector** — 표준 OTLP 텔레메트리(로그·트레이스·메트릭)를 받아 ClickHouse로 export하는 인제스트 게이트웨이 `✓`. RUM 경로(컨버터)와 독립이며 서로 직접 호출하지 않는다. **큐: 현재 stage는 인메모리 큐만 쓴다** `✓` — `file_storage` 퍼시스턴트 큐는 prod 목표이며 `≈`, 미구성 상태에선 재시작 시 in-flight가 유실될 수 있다.
...
`clickhouse.enabled:false`(HyperDX Only)로 HyperDX 차트는 자체 ClickHouse를 띄우지 않고, Altinity operator가 관리하는 CHI/CHK 클러스터에 연결한다 `✓`. 이 분기를 왜 택했는지(공식 operator 2종 공존 회피·범용분석 CH와 일원화)는 [스택 토폴로지]({{< relref "../../hyperdx/01-stack-topology.md" >}}) §1이 기준 문서이고, 그 분기 위에서 사건이 났을 때 무엇을 어떤 순서로 하는지는 [운영 런북]({{< relref "02-runbook.md" >}})이 담당한다.
...
광범위 관측 정지는 두 지점뿐이다 — **ClickHouse 전체 다운**(저장 원천)과 **Keeper 정족수 상실**(쓰기 경로) `Σ`. 나머지 컴포넌트 다운은 수집 일부·조회·설정에 국한된다 — 특히 RUM 컨버터와 OTel Collector는 독립 경로라 한쪽이 죽어도 다른 경로 적재는 계속된다 `Σ`. 단 stage는 위 축소 구성(replica 1·인메모리 큐·단일 티어)이라 이 방어선이 아직 prod만큼 두껍지 않다 `Σ`. 컴포넌트별 blast radius의 근거와 무손실 2트랙의 종합은 [operator 토폴로지·다운타임]({{< relref "../../hyperdx/04-operator-topology-downtime.md" >}}) §1·§6이 소유한다.

## 4. 검토했으나 채택하지 않은 것 — S3Queue / s3Cluster

S3를 인제스트 경로에 끼우는 방식(S3에 객체를 떨어뜨리고 ClickHouse가 그걸 빨아들이는 형태)은 우리 경로에 없다. 이유는 하나다 — **우리 인제스트는 OTel Collector와 자체 RUM 컨버터가 ClickHouse에 직접 쓰므로 S3를 경유할 지점 자체가 없다** `✓`. 굳이 끼우면 경로가 하나 늘고, `S3Queue`는 23.11에 production ready로 발표됐음에도 **exactly-once를 보장하지 않는다고 공식 문서가 명시**하므로 `✓` 중복 제거 책임을 우리가 새로 져야 한다. 여기에 ClickHouse Cloud의 S3 ClickPipes 광고를 self-host의 `S3Queue`와 같은 것으로 읽으면 판단이 뒤집힌다 `Σ`. 두 엔진의 기능 서술과 S3를 메인 스토리지로 쓰는 갈래의 판정은 [Iceberg·레이크하우스]({{< relref "../../clickhouse/09-iceberg-lakehouse.md" >}})가 소유하고, 이 장은 "우리 경로에 왜 없나"만 기록한다.

## 우리 케이스에서는

지금 돌아가는 것은 **stage 축소판 하나**다 — hdx replicas 1, ClickHouse Phase 1 replica 1, MongoDB `members:1`, Collector 인메모리 큐, EBS gp3 단일 티어. 표에 적힌 prod 목표(RF2 2AZ·Keeper 3노드·`members:3`·`file_storage` 큐·hot gp3+cold S3)는 아직 설계이고, 그 격차가 곧 승급 작업 목록이다 — 무엇을 어떤 신호에서 올리는지는 [의사결정 가이드]({{< relref "03-decision-guide.md" >}})가 소유한다.

표준과 우리를 가르는 것은 자체 RUM 컨버터 한 컴포넌트이고, 그것이 실행 단위를 5에서 6으로 만든다. 이 컨버터가 OTel Collector와 독립 경로라는 성질은 이득이자 부채다 — 한쪽이 죽어도 다른 경로 적재가 계속되는 반면 `Σ`, 표준 문서의 Collector 중심 서술이 우리 RUM 경로에는 그대로 적용되지 않는다. stage 스토리지가 블록 온리 형상이라는 점도 같은 성격이다 — S3 티어링을 전제한 기준 문서 대신 [블록 온리 튜닝]({{< relref "../../hyperdx/08-block-only-tuning.md" >}})을 읽어야 지금 형상의 손익이 맞는다. 사건이 났을 때의 순서는 [운영 런북]({{< relref "02-runbook.md" >}})으로 넘긴다. 시점 기준 2026-08.
