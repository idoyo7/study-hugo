...
이 페이지는 [스택 토폴로지]({{< relref "../hyperdx/01-stack-topology.md" >}}) §7("컴포넌트별 가용성")에서 분기했다. §7은 배치·데이터 흐름 서술 중간에 묻혀 있던 가용성 종합이며, **§7 자체는 기준 문서로 유지**한다 — 이 페이지는 운영 트랙의 독립 페이지로 그 골격을 그 자체로 완결되게 구체화하고, 개별 메커니즘의 근거·매니페스트는 [Keeper]({{< relref "../hyperdx/05-keeper.md" >}})·[복제·failover]({{< relref "../hyperdx/06-replication-failover.md" >}})·[MongoDB 최소 배포]({{< relref "../rum/07-hyperdx-mongodb.md" >}})로 위임한다.
...
Keeper는 이 트랙 어디에도 이벤트 데이터를 들고 있지 않다 — **정족수를 잃으면 쓰기 자체가 막힐 뿐**, "Keeper가 데이터를 붙잡고 있다가 재개"하는 동작은 없다 `✓`. Kafka와의 구분·async_insert 세만틱·유실 지점 표는 [Keeper]({{< relref "../hyperdx/05-keeper.md" >}})가, 멀티마스터·승격 없는 failover·split-brain 방지는 [복제·failover]({{< relref "../hyperdx/06-replication-failover.md" >}})가 기준 문서다.
