...
- [ ] **배포 후 검증** — 이미지 태그 v3.4.5, 파드 Running(server/repo-server/application-controller/applicationset-controller/redis-ha), CRD Established, 팀별 RBAC 회귀(로그 접근 포함), Keycloak OIDC 로그인 + redirect URI(3.1 PKCE 요건).
...
4. staging 먼저 → 검증(아래 실행 체크리스트) → 통과 후 prod에 동일 절차를 적용한다.

argo-rollouts는 1.36으로 갈 경우 **서드파티 차단 6종** 중 하나다 — 1.35 기준 목표(2.41.1)는 그대로 유효하며, 1.36 재검토 시점은 [목표버전 판정]({{< relref "../01-target-version.md" >}})을 따른다.
...
- [ ] **배포 후 검증** — 컨트롤러 이미지가 v1.9.1인지, 기존 Rollout이 정상 reconcile되는지 확인한 뒤 실제 이미지 bump로 canary 롤아웃 1회를 돌려 istio 서브셋 weight 전환·analysis 통과·자동 프로모션을 검증한다.
