...
버전 선택의 근거는 k8s tested 매트릭스입니다. 현행 chart 7.5.2가 담은 앱 v2.12는 목표 k8s 1.35의 tested 목록 밖입니다. 앱 3.4가 정확히 v1.32~1.35를 커버합니다. 그래서 chart 10.1.4(앱 v3.4.5) bump가 사실상 필수입니다. 차트는 argo-helm의 `argo-cd`를 upstream에서 직접 핀합니다(ECR OCI 미러가 아닙니다) — 3단계 메이저(7.x→8.x→9.x→10.x)를 관통해 최신 stable로 직행합니다.

### 무엇이 깨지나

CRD apiVersion(`argoproj.io/v1alpha1`)은 그대로입니다. 다만 ApplicationSet CRD 스키마가 대형화되면서 앱 3.3+부터 **Server-Side Apply가 사실상 필수**가 됐습니다.
...
반대로 신규 설치라서 해당되지 않는 것도 있습니다. blue-green 신규 클러스터는 신규 설치이므로 앱 2.12→…→3.4 순차 helm 적용이 필요 없습니다 — 목표 chart로 직행하면 됩니다. in-place 경로에서만 필요한 redis-ha haproxy selector 마이그레이션(chart 9.1.0)도 신규 설치에는 해당 없습니다.
...
argo-rollouts는 k8s 지원 매트릭스가 없는 tolerant 컴포넌트라 하드 블로커는 아닙니다. 동인은 보안입니다. 목표인 v1.9.1(패치)은 CVE-2026-35469(spdystream SPDY 프레임 파서 미검증으로 인한 원격 DoS, CVSS4.0 8.7 HIGH) 수정판이고, 이것이 이번 업그레이드의 핵심 동인입니다.

배포 구조부터 짚어야 합니다. argo-rollouts는 finance 워크로드에서 **`cluster-bootstrap-v2` umbrella 차트의 서브차트**로 배포됩니다(독립 ArgoCD 앱이 아닙니다). 따라서 단독 bump가 불가능하고 umbrella `Chart.yaml`의 dependency 핀을 리워크해 재퍼블리시해야 합니다. finance에서 실제로 Rollout을 쓰는 서비스는 3개, 표준 Deployment로 남는 서비스가 2개입니다. 전략은 canary + analysis(istio 메트릭 기반)이 기본이고 blueGreen도 지원합니다. trafficRouting은 istio 전용이라 ALB 관련 변경은 무관합니다.
