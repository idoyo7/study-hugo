...
- **native sidecar 전환 시점은 istio 버전이 아니라 노드 kubelet 버전이 정한다** — 기본값이 `true` 가 아니라 `"auto"` 이고 전 노드 kubelet ≥1.33 일 때만 켜진다. 목표 1.35 노드에서는 **처음부터 ON** 이고 green(1.31)에서는 **끝까지 OFF** 입니다 `✓`
