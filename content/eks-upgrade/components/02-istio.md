...
- **라이브 버전은 여전히 미확인**이지만 경로 A 확정으로 성격이 바뀌었다 — 홉 산정용 blocking 은 아니고 **green↔blue values·EnvoyFilter 설정 parity 확인용**입니다. 그래도 컷오버 전에 `istioctl version` 으로 실측은 남깁니다 `?`
- 목표 **1.30.3**(2026-07-16 릴리스)은 istio.io 최신 stable 이고 지원 k8s 가 **1.32~1.36** 이라 목표 클러스터 **1.35** 를 포함합니다 `✓`
- **경로 B(기존 green in-place)로는 1.30.3 에 도달할 수 없다** — istio 1.30 의 k8s 하한이 **1.32** 인데 green 은 **1.31** 입니다. green 이 1.31 인 동안 올릴 수 있는 상한은 **istio 1.29**(지원 1.31~1.35)이고 그마저 EOL 이 ~2026-08 입니다. 1.30.3 은 **1.35 신규 클러스터 직행(경로 A)에서만** 성립합니다 `✓`
- **native sidecar 전환 시점은 istio 버전이 아니라 노드 kubelet 버전이 정한다** — 기본값은 `true` 가 아닌 `"auto"` 이고 전 노드 kubelet ≥1.33 일 때만 켜집니다. 목표 1.35 노드에서는 **처음부터 ON** 이고 green(1.31)에서는 **끝까지 OFF** 입니다 `✓`
- **1.29 차트 통합의 리소스 rename 은 우리에게 해당 없을 가능성이 크다** — 코드 대조 결과 istiod 차트는 1.22.0 부터 이미 신 이름(`istiod-clusterrole`)입니다. 대응은 rename 추적 대신 **구 이름 orphan 탐색**입니다 `✓`
- blue-green 신규 클러스터라면 **canary 홉 체인 없이 1.30.3 을 처음부터 직행 설치**할 수 있습니다 `✓`
{{< /callout >}}

## 1. 왜 1.30.3인가

목표 버전은 남은 선택지가 좁아서 사실상 정해집니다. istio 1.24 계열이 지원하는 k8s 는 1.28~1.31 까지라 목표 **1.35** 에서는 아예 지원 대상 밖입니다. 1.35 를 지원하는 비-EOL 라인은 **1.29**(k8s 1.31~1.35)와 **1.30**(k8s 1.32~1.36) 둘뿐인데 그중 1.29 는 EOL 전망이 ~2026-08 입니다. 남는 건 **1.30.3** 하나입니다. 2026-07-16 릴리스이자 istio.io 최신 stable 이고, 지원 k8s 범위 안에 목표 클러스터 1.35 가 들어옵니다.

여기에는 확인되지 않은 전제가 하나 깔려 있습니다. 라이브 버전이 실제로 몇인지를 이 문서 작성 시점까지 확인하지 못했습니다. 배포 차트의 README 이력 배지가 훨씬 오래된 버전까지 내려가므로 최소한 chart tip 인 **1.24.1** 을 하한으로 가정하고 썼습니다. 아래 판정은 그 가정 위에서 읽어야 합니다.
...
- **1.26→1.27**: native sidecar 의 기본값이 바뀝니다. 릴리스노트의 "default to `true`"는 부정확합니다. **코드가 등록하는 값은 `"auto"`** 입니다 — 인젝션 웹훅이 전 노드의 kubelet 마이너를 훑어 하나라도 33 미만이면 끕니다. 그래서 **전환을 트리거하는 건 istio 업그레이드가 아니라 노드 업그레이드**입니다. 목표 1.35 노드에서는 신규 설치 시점부터 native 입니다. `istio-proxy` 가 init 컨테이너(`restartPolicy: Always`)가 되는 것을 전제로 Job/CronJob 완료·기동/종료 순서를 검증해야 합니다. 릴리스노트가 빠뜨린 파급도 있습니다 — ① **사용자 init 컨테이너가 메시 안으로 들어와** egress 가 `ServiceEntry`·`Sidecar` 스코프·`AuthorizationPolicy` 의 지배를 받습니다, ② finance 가 쓰는 **`holdApplicationUntilProxyStarts` 가 에러도 경고도 없이 무효**가 됩니다. 후자에서 할 일은 **`global.proxy.lifecycle` 을 직접 지정하고 있지 않은지** 확인하는 것입니다. 값을 지우는 조치가 아닙니다. 지정하고 있으면 native 의 `preStop` drain 훅이 렌더되지 않아 종료 시 인플라이트 요청이 끊깁니다.
- **1.28→1.29**: base·istiod 차트가 통합되며 중복 설정이 istiod 차트로 이관됩니다. upgrade-notes 는 `ClusterRole istiod` → `istiod-clusterrole` 매핑표를 싣지만 **코드 대조 결과 istiod 차트는 1.22.0 부터 이미 신 이름**입니다. base 차트에는 해당 템플릿이 애초에 없습니다. 우리 하한 가정(chart tip 1.24.1)에서는 rename 을 통과하지 않습니다. 할 일은 **`kubectl get clusterrole istiod` 로 1.22 이전 설치의 orphan 이 남아 있는지 확인**하는 것입니다. 비어 있으면 이 항목은 해당 없음입니다. 같은 절의 **접미사 규칙**(클러스터 스코프 리소스에 `-{revision}-{namespace}`)이 오히려 실질적입니다 — revision 을 쓰는 구성에서는 canary 홉마다 이름이 달라집니다.
- **1.29→1.30(목표)**: 디버그 엔드포인트 인가가 **1.29 에서 포트 15014, 1.30 에서 plaintext XDS 15010 까지** 확장됩니다(CVE-2026-31838). Kiali 같은 관측 도구와 `istioctl --plaintext` 를 쓰는 내부 스크립트가 영향을 받습니다. Kiali 를 함께 올린다면 1.30 요건인 2.26+ 로 lockstep bump 가 필요합니다. 네임스페이스 선택 로직도 바뀌어 동일 hostname 이 여러 네임스페이스에 있으면 "Kubernetes Service 우선"이 됩니다. `DestinationRule.trafficPolicy.retryBudget` 의 기본 `percent` 도 **0.2%→20% 로 정정**됩니다 — 쓰고 있었다면 재시도 예산이 **100배**로 뜁니다.

**1.29 구간에서는 알람 셋이 경고 없이 죽습니다** — 서킷브레이커 remaining 메트릭 기본 비활성, Envoy stats 압축 기본 활성(스크레이퍼가 디코드를 못 하면 stats 전량 소실), 위 디버그 엔드포인트 인가. 셋 다 에러가 없고 쿼리가 `No data` 를 리턴할 뿐이라 신규 설치라도 **대시보드·알람 룰 감사는 그대로 해야 합니다**. 1.27 의 Grafana 대시보드 UID 고정(업그레이드 후 재생성 필요)과 Lightstep·OpenCensus 프로바이더 완전 제거도 같은 성격입니다.

finance 가 실제로 쓰는 EnvoyFilter 2개(`local-reply` SIDECAR_OUTBOUND·`ingressgateway-local-reply` GATEWAY)는 안정적인 Envoy v3 API 를 쓰므로 1.25~1.30 릴리스노트에 깨짐 항목은 없습니다. EnvoyFilter 자체가 버전을 보증하지 않는 API 라서, 매 목표 버전에서 istiod validation 과 실제 Envoy config_dump 렌더는 검증해야 합니다.
...
**이 경로는 마지막 홉에서 막힙니다.** green 은 k8s **1.31** 인데 istio 1.30 의 지원 k8s 하한이 **1.32** 입니다. 홉별로 대조하면 1.26(k8s 1.29~1.33)·1.28(1.30~1.34)까지는 green 에서 지원 범위 안이지만 **1.30 은 범위 밖**입니다. `istioctl` 자체도 1.30 부터 최소 지원 k8s 를 1.32 로 올렸으므로 green 을 같은 바이너리로 다룰 수 없습니다. 즉 **green 이 1.31 인 동안 도달 가능한 상한은 istio 1.29**(k8s 1.31~1.35)입니다. 그 버전은 EOL 전망이 ~2026-08 이라 도착하자마자 다시 올려야 합니다. 1.30.3 을 목표로 두는 한 **경로 B 는 목표를 만족시키지 못합니다.**

native sidecar 도 경로에 따라 달라집니다 — green 의 kubelet 은 1.31 이라 `"auto"` 판정이 계속 `false` 입니다. **경로 B 에서는 `1.26→1.28` 홉을 지나도 native 전환이 일어나지 않습니다.** 검증이 성립하는 곳은 istio 업그레이드 창이 아니라 **k8s 1.35 노드가 실제로 있는 곳**입니다. 경로 B 를 택하면 native 검증은 나중으로 미뤄질 뿐 사라지지 않습니다.

**경로는 A 로 확정됐습니다** — 상위 문서 간에 "신규 클러스터 직행"과 "기존 green in-place"가 상충하는 서술이 남아 있었습니다. 목표가 EKS 1.35 · istio 1.30 으로 확정된 조합에서 경로 B 는 애초에 1.30.3 에 도달할 수 없습니다. 경로 B 에 남는 유일한 용도는 "1.35 컷오버가 지연될 때 green 을 istio 1.29 까지만 올려 EOL 노출을 줄이는 임시 수단"입니다. 그건 이 이관의 경로와는 별개 판단입니다. 차트/이미지 리워크·ECR 미러·Kiali 절차는 어느 쪽이든 동일합니다.
...
- [ ] **`global.proxy.lifecycle` 오버라이드 확인(1.27+)** — 이 키를 직접 지정하고 있지 않은지 확인합니다. `holdApplicationUntilProxyStarts` 는 값을 지울 필요 없이 **무효화됐다는 사실을 인지**하는 것이 조치입니다.
...
- [ ] **디버그 엔드포인트 인증(1.29의 15014 / 1.30의 15010)** — Kiali 를 2.26+ 로 lockstep 하지 않으면 topology/config 조회가 실패할 수 있습니다. Kiali 가 istio-system 밖이면 이전하거나 `DEBUG_ENDPOINT_AUTH_ALLOWED_NAMESPACES` 에 추가합니다. `istioctl --plaintext` 를 쓰는 내부 스크립트는 표준 인증 경로로 옮깁니다 — `ENABLE_DEBUG_ENDPOINT_AUTH=false` 는 CVE-2026-31838 수정을 무력화하므로 쓰지 않습니다.
- [ ] **알람·대시보드 감사(1.27~1.29)** — 서킷브레이커 `remaining` 메트릭 기본 비활성, Envoy stats 압축 기본 활성(스크레이퍼 디코드 확인), Grafana 대시보드 UID 고정으로 인한 재생성, `lightstep|opencensus` 프로바이더 잔재 grep. 전부 에러 없이 깨집니다.
...
- **코드 대조로 확정·정정된 항목**(`ENABLE_NATIVE_SIDECARS`의 실제 기본값 `"auto"`와 kubelet ≥1.33 판정, `holdApplicationUntilProxyStarts` 무효화와 `lifecycle` 분기, init 컨테이너 순서 역전, istiod 차트가 1.22.0부터 신 이름이라는 대조 결과, `retryBudget` 100배, 1.30 CVE 목록): [istio 1.25→1.30 changelog]({{< relref "../../istio/17-changelog-1.25-1.30.md" >}}) — 릴리스노트와 코드가 어긋나는 부분은 그 문서가 코드를 근거로 판정합니다
