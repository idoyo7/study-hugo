...
{{< callout type="info" >}}
**한눈에**
- 컨트롤러를 `0.36.2`에서 최신 stable **`1.14.0`**(2026-07-11 릴리스)로 올린다. 목표 k8s 1.35를 지원하는 최소가 1.9이고 1.14는 1.30~1.36을 커버하므로 "가능한 최신 stable" 방침에 맞는다 — 1.36으로 재검토할 때도 하한이 1.13이라 1.14.0이 그대로 유효하다 `✓`
- 넘을 스키마 경계는 **`v1beta1` → `v1` 하나뿐**이다. finance의 0.36.2는 이미 v1beta1(`karpenter.sh/v1beta1` NodePool·`karpenter.k8s.aws/v1beta1` EC2NodeClass)이다. v1alpha5의 Provisioner·AWSNodeTemplate은 v0.33에서 졸업해 애초에 없다 `✓`
- v1에서 **`amiSelectorTerms`가 필수화**된다. 빠뜨리면 EC2NodeClass와 그것을 참조하는 모든 NodePool이 통째로 `NotReady`가 된다 `✓`
- v1은 **drift를 GA로 승격시키며 비활성화가 불가능**해진다. finance가 명시한 `featureGates.drift: false`는 무효화되고 drift가 강제로 켜진다 `✓`
- **blue-green 신규 클러스터라 conversion 웹훅을 왕복하지 않는다.** 기존 클러스터를 in-place로 올릴 때만 밟아야 하는 `0.36 → 0.36.9 → 1.0.x → 1.1+` 순차 경로를 건너뛰고 v1 CRD와 v1 매니페스트로 1.14.0을 처음부터 세운다 `✓`
{{< /callout >}}

## 1. 왜 1.14.0인가

목표 클러스터가 k8s **1.35**라는 것이 버전을 거의 정해준다. karpenter 호환 매트릭스에서 1.35를 받는 최소 버전이 **1.9**이고, 1.14는 **1.30~1.36**을 커버한다. "가능한 최신 stable"이라는 방침과 하한 조건이 같은 답을 가리키는 셈이다. 나중에 1.36으로 재검토하더라도 그때의 하한이 1.13이라 1.14.0은 그대로 쓸 수 있다.

차트도 같이 갈아탄다. org의 `karpenter-v2`(v1beta1 스키마)를 폐기하고 이미 v1 스키마인 신 차트 `karpenter`를 채택한다.

문제는 그 차트의 버전이다. tip은 appVersion **1.1.0**으로 이미 v1 스키마이긴 하지만 **1.1.0이 지원하는 k8s는 ≤1.31**이라 1.35에서는 쓸 수 없다. 스키마가 맞다고 그대로 올릴 수 있는 게 아니라서 appVersion을 1.14.0으로 리워크한 뒤 재퍼블리시하는 작업이 선행돼야 한다.

## 2. v1이 깨는 것

이 업그레이드의 핵심은 CRD 경계다. `NodePool`이 `karpenter.sh/v1beta1`에서 `v1`으로, `EC2NodeClass`가 `karpenter.k8s.aws/v1beta1`에서 `v1`으로 옮겨가고, 그 이동에 세 가지가 딸려 온다.

**`amiSelectorTerms` 필수화.** 지금까지는 `amiFamily: AL2023`만 적어두면 자동으로 해석됐지만 v1은 `alias: al2023@latest` 같은 명시적 `amiSelectorTerms`를 요구한다. 빠뜨리면 EC2NodeClass와 그것을 참조하는 NodePool 전부가 `NotReady`로 떨어진다. 다행히 신 차트가 이 기본값을 이미 들고 있어서 override로 비우지만 않으면 손댈 일이 없다.

**kubelet 설정의 이사.** `NodePool.spec.template.spec.kubelet`에 있던 것이 `EC2NodeClass.spec.kubelet`으로 옮겨간다. finance는 kubelet을 따로 설정하지 않으므로 이 항목의 실질 영향은 낮다.

**disruption 어휘 변경.** `consolidationPolicy: WhenUnderutilized`가 `WhenEmptyOrUnderutilized`로 이름을 바꾼다. expiration은 forceful로 바뀌어 대체 노드를 선provision하지 않고 곧바로 drain을 시작한다. disruption taint도 `karpenter.sh/disruption=disrupting`에서 `karpenter.sh/disrupted`로 바뀐다.

여기에 이름 없이 조용한 변화가 하나 더 있다. **v1은 drift를 GA로 승격시키면서 비활성화 수단을 없앤다.** finance가 명시해 둔 `featureGates.drift: false`는 무효화되고 drift가 상시 켜진다. 그 상태에서 AMI나 설정을 바꾸면 대량 노드 교체가 유발될 수 있다. disruption budget(`defaultBudgets`)을 사전에 검토해야 한다.

설정 키 쪽은 대체로 조용하다. `settings.interruptionQueue`는 flat 키 구조가 v1에서도 유지된다. 다만 finance 일부 values에 pre-0.32 시절의 죽은 키 `settings.aws.interruptionQueueName`·`settings.aws.defaultInstanceProfile`가 잔재로 남아 있다 — 실제 interruption queue는 overlay의 flat 키가 이미 공급하고 있으니 기능 손실 없이 지우기만 하면 된다.
...
**롤백.** blue-green 신규 설치는 targetRevision을 이전 값으로 되돌리면 원복할 수 있다. 순차 경로를 엄수해야 하는 쪽은 in-place를 택했을 때뿐이다. 그 경우 `0.36.2 → 0.36.9 → 1.0.x`(매니페스트 v1 이관) `→ 1.1+ → … → 1.14` 순서를 지켜야 한다. 마이너 스킵이 금지되는 경계는 v1beta1 서빙이 끝나는 **v1.0/v1.1**이다.
