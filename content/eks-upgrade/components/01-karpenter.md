...
배포 순서는 신규 blue 클러스터 기준으로 (1) v1 컨트롤러 IAM 정책 + IRSA 롤 + 노드 롤 선행 → (2) v1 CRD 설치 → (3) karpenter 1.14.0 컨트롤러 설치 → (4) 워크로드 스케줄로 노드 프로비저닝 확인 순이다. 검증은 `kubectl get nodepool,ec2nodeclass -o wide`로 전부 `Ready=True`·apiVersion `v1`인지, `kubectl get nodeclaim`으로 노드가 목표 k8s 버전으로 등록되는지, 컨트롤러 로그에 `UnauthorizedOperation`이 없는지, interruption 큐가 실제로 소비되는지를 확인한다.

## 컨트롤러가 Fargate에서 돌 때 — 재작성 요건

신규 blue 클러스터는 managed nodegroup을 두지 않고, karpenter 컨트롤러 자신도 CoreDNS와 함께 **Fargate profile**(`{ns: karpenter}`)로 뜬다. 이 배치 자체가 컨트롤러 values에 두 가지 재작성을 강제한다 — 상세 토폴로지·부트스트랩 순서는 [Fargate + karpenter 토폴로지]({{< relref "../03-fargate-karpenter-topology.md" >}}) §6.3에서 이어받고, 여기서는 karpenter 쪽에서 반드시 지켜야 할 두 요건만 못박는다.

- **`affinity.nodeAffinity`(arm64 + system-primary)와 `tolerations`(arch/nodegroup/spot 등) 전량 제거** — Fargate는 **amd64 전용**이라 arm64 required affinity가 남아 있으면 컨트롤러 파드가 영구 `Pending`에 걸린다. Fargate에는 taint 자체가 없어 toleration도 의미가 없으므로 함께 제거한다.
- **`controller.resources`를 `cpu: 1` / `memory ≥ 1Gi`(requests=limits)로 명시** — Fargate는 requests 값으로 micro-VM 크기를 정하므로, 기존 기본값(0.25 vCPU/256Mi) 수준으로 두면 CPU 기아로 **리더 election이 반복 유실**되는 사고가 사내에서 실제로 있었다. 과소 설정은 곧 컨트롤러 가용성 사고로 이어진다.
...
- [ ] in-place 경로를 택할 경우에만 0.36.2→0.36.9→1.0.x(매니페스트 v1 이관)→1.1+→…→1.14 순서를 엄수해야 하며, 마이너 스킵 금지 경계는 v1.0/v1.1(v1beta1 서빙 종료)이다.
