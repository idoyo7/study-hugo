...
[EKS 버전 업그레이드]({{< relref "../_index.md" >}}) 챕터의 상위 문서들이 "어떤 버전으로, 어떤 토폴로지로 blue를 세우는가"를 다뤘다면, 이 하위 섹션은 그 클러스터의 **컨트롤 플레인에 무엇을 설정할 수 있는가**를 전수로 다룬다. 계기는 **2026-08-12**다. 그날 EKS가 kube-scheduler·kube-apiserver·kube-controller-manager의 파라미터 4종을 처음으로 고객 설정 대상으로 열었고 그 결과 "관리형이라 손댈 수 없다"는 오래된 전제에 금이 갔다. 다만 금이 간 폭이 정확히 어디까지인지가 이 섹션의 주제다 — 열린 것은 파라미터 4개이고 그 옆자리는 그대로 닫혀 있다.
...
컨트롤 플레인 설정은 성격이 다른 세 층으로 갈린다. 층을 섞어 읽으면 "무엇을 언제 정할 수 있는가"가 흐려지므로 페이지도 층으로 나눴다.
...
용량 축을 레이어로 세지 않은 이유는 축이 직교하기 때문이다. 값을 정하는 문제가 아니라 크기를 사는 문제이고 레이어 2의 HPA `syncPeriod` 하나가 이 축을 **전제조건으로** 요구해서 의존이 한 방향으로 걸린다.
...
네 페이지가 같은 질문을 층마다 반복한다 — **언제 정할 수 있나 → 정한 뒤 되돌릴 수 있나 → 되돌릴 수 없으면 무엇을 미리 결정해야 하나**. 사실 기준 시점은 **2026-08-14**이고 근거는 AWS 1차 문서(User Guide·API Reference·Best Practices·요금 페이지)와 로컬 업스트림 클론이다. 업스트림 인용의 체크아웃 시점은 각 페이지가 개별로 밝힌다.
