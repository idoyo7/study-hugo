...
consolidation의 교체 판정은 한 줄이다 — 후보 노드의 현재 가격(`candidatePrice`)을 상한으로, 시뮬레이션이 뽑은 대체 NodeClaim의 인스턴스 타입 목록을 거른다.
...
`instance-family Exists minValues: 3` 같은 제약은 예상 못 한 방어막이 된다 — 필터 후 남은 패밀리가 그 밑으로 줄면 `SatisfiesMinValues`가 에러를 던져 no-op이 된다.
...
v1.14의 `consolidationPolicy: Balanced`도 해법이 아니다 — `ScoreMove`는 절감액·중단 비용 비율만 보고, 7세대가 충분히 싸면 승인된다.
...
`NodeClaimTemplate`에 `NodePoolWeight` 필드가 있긴 하지만(`nodeclaimtemplate.go:60,71`) 읽는 곳이 없다. weight로 저-weight 풀 폴백을 막는 분기는 코드 전체에 하나뿐이고, 그건 예약 용량 전용이다.
...
그 외 실패는 `errs[i] = err; return true`로 스킵되고 다음(=더 낮은 weight) 템플릿이 이겨, 교체 NodeClaim은 **그 풀의 라벨을 그대로 달고** 생성된다.
...
`SimulateScheduling`은 `provisioner.NewScheduler`를 그대로 호출해(`disruption/helpers.go:113-121`) NodePool을 weight 내림차순으로 정렬하고(`provisioner.go:287-289`), 성공한 템플릿 중 **인덱스가 가장 앞선 것**을 채택한다(`scheduler.go:757-761`). gen8 풀이 파드를 수용하는 한 대체안은 gen8 안에서만 나오고, 같은 세대끼리는 strict 부등호를 넘기 어려워 대개 no-op이다.
...
**경계를 정확히 그으면 이렇다.** 크로스 풀 다운그레이드는 gen8 풀 템플릿이 `CanAdd`에 실패할 때만 일어난다. 실패 사유는 넷이다 — 오퍼링 전멸(ICE 마킹), NodePool limits 소진, `minValues` 불충족, 요구사항 비호환. 어느 쪽이든 **그 순간은 정확히 "gen7을 띄우고 싶은 순간"**이라, 이 경로 자체는 버그가 아니라 폴백이 의도대로 작동한 것이다.

## 4. 한 번 내려가면 consolidation은 되돌리지 않는다

폴백으로 뜬 gen7 노드는 용량이 회복돼도 gen7로 남는다. 이유는 단순하다 — **"더 비싼 것으로 교체한다"는 분기가 코드에 없다.** 가격 필터는 상한만 받고, 필터 후 목록이 비면 `Command{}`를 반환한다.
...
#1829는 저-weight 폴백 노드를 되돌리는 `consolidationPolicy: Underweight`를 제안했다 — 스팟 부재로 온디맨드 폴백한 뒤 안 돌아오는 문제로, 우리 문제와 **동형**인데 답은 "계획 없음"이다. #6721("Ability to prefer generation over price")도 같은 이유로 닫혔다.
...
**트레이드오프는 churn이다.** 48h면 gen7 노드가 이틀마다 통째로 교체되고, 용량 부족이 길어지면 "만료 → 다시 gen7" 왕복마다 재스케줄 비용이 든다. 짧을수록 복귀는 빠르므로 폴백 빈도를 관측(§7)한 뒤 조정한다 — 12h 미만은 권하지 않는다.
