...
> **왜 이 문서인가.** 릴리스노트는 "무엇이 머지됐냐"까지만 말한다. 필요한 구분은 셋 — ① 켜면 이득인 것, ② 설정·권한·CRD를 건드릴 것, ③ **업그레이드 자체가 노드를 교체하는 것**. 이 문서는 그 셋으로 1.7~1.14를 갈라 판정한다.
>
...
k8s 호환 하한도 올라갔다 — 1.14는 k8s 1.30~1.36 커버, **1.36엔 최소 1.13**, 1.35는 1.9, 1.33은 1.5가 하한이다.
...
첫 줄이 고약하다 — 노드마다 소진 시점이 달라 **같은 Deployment 파드 성능이 갈린다.** 배제는 **"flex 선택을 가격 우선 스케줄러에 맡기지 않겠다"**는 선택이다.

*베이스라인·버스트 동작은 AWS EC2 문서의 일반 서술을 근거로 한다. 코드로 검증한 것은 라벨 판정과 스케줄링 배제 동작뿐이다.*

### 2.4 배제보다 나은 선택 — 전용 NodePool로 격리

flex를 전량 지우면 사용률 낮은 워크로드의 비용 이점도 같이 버린다. NodePool을 둘로 갈라 flex를 **taint로 격리**하면 둘 다 챙긴다.
...
flex를 쓰려는 워크로드만 `tolerations`에 `capability=flex:NoSchedule`을 적는다. 기본은 안전, opt-in으로 절감 — "버스트로 충분하다"는 판단이 매니페스트에 남는다. **전량 배제는 flex 후보가 아예 없을 때만** 합리적이다.
...
`spec.replicas`가 존재하면 그 NodePool은 **static**이 된다([core#2521](https://github.com/kubernetes-sigs/karpenter/pull/2521), `designs/static-capacity.md`) — 파드 수요와 무관하게 고정 노드 수를 유지한다.
...
마지막 줄이 도입 리스크 전부다 — `replicas`를 잘못 붙이면 되돌릴 방법이 NodePool 삭제, 즉 노드 전량 교체뿐이다.

### 4.2 어디에 쓰나

설계 동기는 JIT 프로비저닝 지연을 못 견디는 성능 민감 워크로드다(`static-capacity.md:4-9`); 기존 우회책은 pause pod였다.
...
**CRD 업그레이드 필수** — `replicas`·`limits.nodes`·`status.nodes`가 스키마에 추가돼, `karpenter-crd`를 별도 관리하면 컨트롤러와 함께 올려야 한다. feature gate는 `StaticCapacity=false`(alpha).
...
같은 성격의 경고가 하나 더 있다 — **1.11.0 릴리스노트 상단**에 CPU 사용량 회귀 경고가 박혀 있다([karpenter#2954](https://github.com/kubernetes-sigs/karpenter/issues/2954)). 1.11에 머물면 컨트롤러 CPU requests를 넉넉히, 1.12+로 지나가면 해당 없다.
...
[aws#9083](https://github.com/aws/karpenter-provider-aws/pull/9083)이 CA 번들을 drift 해시 계산에 포함시켰다. 업스트림 가이드 표현이 정확하다 — *"The updated hashing logic will mark existing nodes as drifted."*

메커니즘은 단순하다. Karpenter는 EC2NodeClass·NodePool의 정적 설정을 해시해 NodeClaim에 annotation으로 박아두고, 해시가 달라지면 drift로 본다. CA 번들이 해시 입력에 **처음 들어가는 순간** 기존 모든 NodeClaim의 저장된 해시가 새 계산식과 어긋나 **전 노드가 일괄 drifted**된다 — 번들 내용이 아니라 해시 정의가 바뀐 것이라, 교체가 실제로 필요한 노드는 하나도 없는데 전량이 대기줄에 선다.

증상은 뚜렷하다 — 설정을 하나도 안 건드렸는데 `kubectl get nodeclaims`에 `Drifted=True`가 한꺼번에 뜨고 disruption 이벤트가 몰린다. `karpenter_nodeclaims_disrupted_total{reason="drifted"}`가 업그레이드 직후 계단식으로 뛰면 이 케이스다.

v1에서 drift는 GA로 승격돼 **비활성화가 불가능**하다. 방어는 disruption budget 하나뿐이고 **업그레이드 전에** 넣어야 한다.
...
빈 노드 삭제는 옮길 파드가 없어 피크에도 안전하다 — 차단 예산엔 `reasons: ["Underutilized", "Drifted"]`로 `Empty`를 남겨두는 편이 거의 항상 맞다.
...
`nodes: "0"`을 무기한 두면 안 된다 — **정상 drift(AMI 갱신 등)도 막히고**, 예산을 푸는 순간 쌓인 drift가 한꺼번에 터진다. 업그레이드 직후 며칠만 좁게 잡고 단계적으로 푼다.
...
headroom을 만드는 관습적 방법은 낮은 PriorityClass의 pause pod이었다 — 실제 파드라 실제 자원을 먹는다. Capacity Buffers는 같은 목적을 **가상 파드**로 달성한다 — 시뮬레이션 안에만 존재하고 매 프로비저닝 사이클마다 재계산된다([core#2898](https://github.com/kubernetes-sigs/karpenter/pull/2898) 설계, [core#3129](https://github.com/kubernetes-sigs/karpenter/pull/3129) v1beta1 승격, AWS 지원 [aws#9276](https://github.com/aws/karpenter-provider-aws/pull/9276)).
...
위 판정은 blue-green 신규 설치 전제다 — **기존 1.x 클러스터를 그대로 1.14까지 통과시키면 회피가 하나도 안 먹는다.** 순서대로 넷을 맞는다.
...
넷 중 1.12만 되돌릴 수 없다 — v1 drift는 비활성화 불가라, **예산을 먼저 안 넣고 컨트롤러를 올리면 그 순간부터 전 노드가 교체 대기줄에 선다.** IAM 3종(§9)도 해당 버전을 지나기 전에 미리 붙여둔다 — 권한은 먼저 줘도 무해하다.
