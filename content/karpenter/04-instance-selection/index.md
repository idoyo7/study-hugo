...
Cluster Autoscaler의 동작 단위는 ASG다. 하나를 골라 desired capacity를 올리면 된다. 그 ASG의 인스턴스 타입은 이미 정해져 있다. Karpenter의 단위는 `NodeClaimTemplate`이다. 여기 담기는 것은 **타입 하나가 아니라 후보 목록 전체**다.
...
이 `InstanceTypeOptions`가 API 객체로 나가는 순간이 `ToNodeClaim()`이다. 그 지점에서 후보 이름들이 requirement **하나**로 압축된다.
...
같은 함수는 capacity type도 역산해 requirement로 추가하고 `nc.Spec.Requirements`에 반영한다(`:120-127`, `:169`). 그래서 실제 NodeClaim은 이런 모양으로 나간다.
...
후보 목록이 잘리는 지점은 두 곳이다. 코어에서 600개, provider-aws에서 60개다.
...
세대·성능·패밀리를 보는 항이 하나도 없다 — **오히려 그 점이 "절단이 8세대를 죽인다"는 추측을 반증한다.** 정렬 키는 타입 하나당 스칼라 하나이고 **사이즈에 대해 단조**다 — `c8i.large`의 최저 오퍼링 가격은 `c8i.24xlarge`보다 압도적으로 싸다. 그 격차가 세대차보다 훨씬 커서 정렬을 지배한다.

가격 오름차순 상위 60개만 남기는 연산의 결과는 그래서 이렇게 된다.
...
**단, 이 안전성은 "가격을 손대지 않았을 때"의 이야기다.** [05]({{< relref "05-generation-preference.md" >}})에서 다루는 NodeOverlay `priceAdjustment`로 7세대에 인위적인 가격 페널티를 걸면 정렬 키 자체가 왜곡된다. 페널티가 과하면 7세대가 60위 밖으로 밀려 **폴백 후보가 사라진다.** 절단이 세대를 자르지 못한다는 성질은 자연 가격에서만 보장된다.
{{< /callout >}}

### 2.2 코어 600 절단은 순서를 전달하지 않는다

코어가 정렬해 잘라낸 결과는 `In` requirement로 들어간다. 그런데 `Requirement`가 In 값을 담는 자리는 **집합**이다.
...
`NewRequirementWithFlexibility`의 In 분기가 값을 `sets.Set[string]`에 밀어 넣으므로(`:61-73`) **순서가 여기서 사라진다.** 게다가 패밀리를 좁힌 NodePool에서는 후보가 600에 못 미쳐 절단 자체가 no-op이다.
...
`prioritized`로 바뀌는 조건은 `b.overlay`가 참일 때만이다(`instance.go:552-556`). 그건 `price-overlay-applied` 어노테이션이 있을 때만이고(`instance.go:365-367`), 그 어노테이션은 코어가 NodeOverlay 인스턴스를 발견했을 때만 붙는다(`nodeclaimtemplate.go:129-133`). 손익은 [05]({{< relref "05-generation-preference.md" >}}) 참고.
...
부수적 함정: `checkODFallback`은 on-demand로 결정됐는데 spot도 허용된 상황에서 후보가 5개 미만이면 경고한다(`instance.go:426-443`). 그러나 **로그만 남기고 launch는 막지 않는다**(`instance.go:360-362`) — 세대·사이즈를 좁힌 NodePool에서 계속 떠도 장애는 아니다.
...
첫 줄부터 확인해 두자. 후보 필터엔 가격이 없다 — `filterInstanceTypesByRequirements`의 채택 조건은 한 줄뿐이고 정렬도 없다.
...
다만 이 함수를 "3조건 AND"로 요약하면 부정확하다 — `daemonOverheadGroups`의 포트 충돌 그룹은 통째로 건너뛰고(`:562-564`), `eligibleInstanceTypes`로 한 번 더 게이팅하며(`:576-578`), Strict 정책의 minValues 위반 시 `remaining = nil`로 만든다(`:602-613`). **"정렬을 하지 않는다"만 정확한 서술이다.**
...
**네 필드가 전부다** — `weight`·`preference`·`priority`는 없다. 이 타입은 `NodePool.spec.template.spec.requirements`와 `NodeClaim.spec.requirements` 양쪽에 쓰이므로 선호를 적을 자리가 없다.

`weight`가 있는 곳은 한 층 위 `NodePool.spec.weight`다. 스케줄러가 이걸 `NodeClaimTemplate.NodePoolWeight`로 복사해 간다(`nodeclaimtemplate.go:71`). **우선순위는 NodePool 간 축이지 내부 축이 아니다.**
...
`c8i.large`면 카테고리 `c`, 세대 `8`이다. provider가 init 시점에 이 라벨을 코어 well-known 라벨 집합에 삽입하므로(`pkg/apis/v1/labels.go:31,39,137`), NodePool뿐 아니라 NodeOverlay requirements에서도 그대로 쓸 수 있다.
...
Go 타입 주석엔 `+kubebuilder:validation:Enum:=Gte;Lte`만 있는데, 코드 제너레이터가 기존 `v1.NodeSelectorOperator` 값들과 union하기 때문이다(`nodeclaim.go:99-101` 주석) — **생성된 CRD가 최종 진실이고 8개 전부 유효하다.**

Gt/Lt는 내부적으로 포함 경계로 정규화된다.
...
위반했을 때의 처리는 정책에 달렸다. Strict(기본)면 후보를 통째로 버리고(`scheduling/nodeclaim.go:608` `remaining = nil`), BestEffort면 무시한다. provider-aws의 `Truncate`도 같은 조건(`HasMinValues()`이고 Strict일 때만, `types.go:440-446`)에서 위반 시 `InsufficientCapacityError`를 반환한다(`instance.go:332-335`).

세대 선호에 갖다 붙이면 이런 뜻이 된다.
...
⇒ **`minValues`는 spot 중단 위험을 낮추려고 후보 다양성을 강제하는 손잡이**다. 세대 선호에 쓸 수 없다. 쓰면 반대 방향으로 작동한다.
...
이 처리 전체를 `--preference-policy`(기본 `Respect`)로 끌 수 있다(`pkg/operator/options/options.go:131`) — `Ignore`면 `NewStrictPodRequirements`가 쓰여 preferred term이 전혀 반영되지 않는다(`scheduler.go:554-560`).
...
- **8세대와 7세대의 실제 가격 대소** — 이 문서의 모든 서술은 "알고리즘이 가격만 본다"는 사실에 근거한다. 8세대가 항상 더 비싸다고 단정하지 않는다. 리전·AZ·할인 상황에 따라 역전 구간이 있을 수 있으므로 대상 리전에서 확인할 것.
{{< /callout >}}
