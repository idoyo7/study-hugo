...
> **왜 한 챕터인가.** 두 축은 성질이 같다 — **판단 근거가 공식 문서에 없다.** 버전 축의 근거는 릴리스노트의 Behavior Changes 절과 PR diff에 있다. 알고리즘 축의 근거는 정렬·절단·부등식 몇 줄의 소스 코드에 있다. 어느 쪽도 "Karpenter는 적당한 노드를 알아서 띄운다"는 소개 문장에서는 나오지 않는다. 이 챕터는 그 두 층을 각각 함수·PR 단위로 내려가 뜯고 **그 위에서 "그래서 무엇을 선언하고 무엇을 켤 것인가"까지** 결정한다.
...
- **[13 consolidation은 무엇을 하는가]({{< relref "13-consolidation-models.md" >}})** · 04 — 세 Method(Emptiness·MultiNode·SingleNode)와 비용 모델. **대체 노드는 액션당 최대 1대**이고, **`Emptiness`는 `consolidationPolicy`를 읽지 않아 `Balanced`에서도 빈 노드는 스코어링을 우회한다.** "세 정책은 같은 모델의 서로 다른 k"라는 흔한 설명이 왜 코드상 틀렸는지도 다룬다.
...
- **[09 무엇을 봐야 하나]({{< relref "09-metrics-logs-events.md" >}})** · 08 — 코어가 내보내는 메트릭 60개 전량과 판정 로그·이벤트. **`nodeclaims_disrupted_total{reason}` 하나가 "노드가 왜 갈렸나"를 가르고**, 판정 로그는 `--log-level debug`에서만 나온다. 이벤트 dedupe 창이 Reason마다 달라 카운트를 빈도로 읽으면 안 된다.
- **[10 메트릭 수집 비용]({{< relref "10-metric-cost.md" >}})** · 09 — 60개를 다 저장할 이유는 없다. **파드 단위 6개가 시리즈의 대부분을 차지하고 배포마다 전량 churn**한다. Datadog은 OpenMetrics로 긁은 것을 전부 custom metric으로 센다. VM에서는 청구가 아니라 `indexdb` 팽창이 비용이다.

## 읽는 순서

- **업그레이드를 앞두고 있으면** 01 → 02가 최단 경로다. 각 문서 말미의 "버전별 운영 판단 표"가 조치 목록이다. 실제 적용 절차는 [eks-upgrade / karpenter]({{< relref "../eks-upgrade/components/01-karpenter.md" >}})가 소유한다.
...
- **노드가 안 줄어들거나, 반대로 너무 자주 교체되면** 08이다. 원인의 대부분은 예산이다. 예산은 유일하게 이벤트로 증거를 남긴다.
- **대시보드를 짜거나 장애 중 로그를 켜야 하면** 09가 목록이다. 08의 진단 절차에서 쓰는 신호가 전부 거기 정리돼 있다.
...
- **버전을 안 밝힌 Karpenter 문장은 믿지 마라.** 코어(kubernetes-sigs/karpenter)와 provider(aws/karpenter-provider-aws)가 따로 태깅된다. provider 릴리스가 코어를 핀한다. 같은 "v1.11.3"이라도 안에 든 코어는 다르다.

## 검증 기준

이 챕터의 모든 코드·릴리스노트 인용은 로컬 체크아웃을 직접 읽어 확인했습니다. 본문에 `파일:라인` 형태로 나오는 인용은 아래 체크아웃을 연 것입니다.

- **kubernetes-sigs/karpenter (코어)** — `v1.14.0-6-gac7a021e` · main(2026-07-30). 스케줄링·disruption·NodeOverlay 코드가 여기 있다. 코어 쪽 라인번호는 전부 이 체크아웃 기준이다.
