...
- **[13 consolidation은 무엇을 하는가]({{< relref "13-consolidation-models.md" >}})** · 04 — 세 Method(Emptiness·MultiNode·SingleNode)와 비용 모델. **대체 노드는 액션당 최대 1대**이고, **`Emptiness`는 `consolidationPolicy`를 읽지 않아 `Balanced`에서도 빈 노드는 스코어링을 우회한다.** "세 정책은 같은 모델의 서로 다른 k"라는 흔한 설명이 왜 코드상 틀렸는지도 여기서 다룬다.
...
- **[10 메트릭 수집 비용]({{< relref "10-metric-cost.md" >}})** · 09 — 60개를 다 저장할 이유는 없다. **파드 단위 6개가 시리즈의 대부분을 차지하고 배포마다 전량 churn**한다. Datadog은 OpenMetrics로 긁은 것을 전부 custom metric으로 세고, VM에서는 청구가 아니라 `indexdb` 팽창이 비용이다.
