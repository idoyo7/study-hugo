...
**검증 기준 버전**: 코어 `kubernetes-sigs/karpenter` **v1.14.0-6-gac7a021e**(로컬 체크아웃), provider-aws는 `main` / `v1.7.0` / `v1.11.3`. 별도 표기가 없는 provider-aws 인용은 `main` 기준이다. 코어 라인번호는 v1.14 기준이라 실제 배포판과 몇 줄 어긋날 수 있다.

## 1. 폴백은 이미 공짜다

사람이 개입할 지점은 이 경로 어디에도 없다. ICE 한 번이 스케줄링에 반영되기까지 거치는 홉은 네 개다.
...
이 사슬 중간에 캐시 만료를 기다리는 구간은 없다. 프로비저닝 루프가 **매 루프마다 `cloudProvider.GetInstanceTypes()`를 새로 호출하므로**(`pkg/controllers/provisioning/provisioner.go`) ICE 마킹과 다음 스케줄링 사이에 별도의 캐시 만료 대기가 끼지 않는다. 다음 루프가 곧바로 갱신된 가용성을 본다.

결과적으로 "7세대 폴백"은 **구성과 무관하게 성립한다.** 구성이 갈라놓는 것은 "되느냐"가 아니라 **"몇 초 걸리느냐"**(아래 §2~§4)와 **"폴백 전에 8세대를 시도하기는 하느냐"**([04]({{< relref "04-instance-selection.md" >}})·[05]({{< relref "05-generation-preference.md" >}})의 주제)다.
...
그러니 다시 시도를 만드는 주체는 파드다 — `Trigger()` 호출부 네 곳 중 **NodeClaim 삭제를 프로비저너에 알리는 경로는 없어** 파드 컨트롤러의 주기 재큐가 유일한 복구 장치로 남는다.
...
배치창(`BATCH_IDLE_DURATION` 기본 1초 / `BATCH_MAX_DURATION` 기본 10초, `pkg/operator/options/options.go`)이 여기에 더 붙어 **코어 측 대기만 11~20초**가 된다. `CreateFleet` 왕복과 컨트롤러 처리 시간까지 얹으면 실측 기대치는 **대략 11~30초**다.

{{< seq src="_seq/2-ice-가-나면-코어는.json" />}}

이 경로에서 갈라지는 갈림길이 둘 있다.

**(a) `CreateFleet`을 한 번도 부르지 않고 ICE가 나는 경로가 따로 있다.** provider-aws는 런치 직전 필터 체인을 돌린다. 어느 필터에서든 남는 인스턴스 타입이 0이 되면 API 호출 없이 즉시 ICE를 반환한다.
...
ICE 캐시가 이미 채워진 뒤의 재시도가 바로 이 경로다 — `CreateFleet` 실패 왕복은 **첫 실패에만** 있고 두 번째부터는 EC2를 때리지 않는다.

**(b) fleet 에러 전부가 "ICE로 집계되는 에러"일 때만 위 삭제 경로를 탄다.** `combineFleetErrors()`가 세는 것은 순수 ICE만이 아니다 — **spot service-linked role 생성 불가**(`IsServiceLinkedRoleCreationNotPermitted`, §3 `capacityTypeCache`를 채우는 그 에러)도 함께 센다(`pkg/providers/instance/instance.go:798-803`). 다른 에러가 하나라도 섞이면 `CreateError`로 분류돼 NodeClaim이 삭제되지 않고 `Launched=Unknown`으로 남아 컨트롤러 백오프를 탄다 — **폴백이 느려지고 `disrupted_total`에도 안 잡힌다.** §8에서 메트릭으로 먼저 확인해야 하는 이유다.
...
실무에서 자주 어긋나는 지점은 이 캐시의 입도다. 키 포맷이 `<capacityType>:<instanceType>:<zone>`, 즉 **리전이나 패밀리 단위가 아니라 (capacity-type, 인스턴스 타입, AZ) 조합 단위**다 — `c8i.2xlarge`가 `ap-northeast-2a`에서 ICE를 냈다고 `c8i.4xlarge`나 다른 AZ의 `c8i.2xlarge`가 막히지는 않는다.

그리고 가용성 판정은 이 캐시 하나로 끝나지 않는다 — `IsUnavailable()`은 세 축의 OR이다(**아래는 provider-aws `main` 기준**).
...
헷갈리는 쪽은 늘 세 번째 축이다. "8세대가 안 뜬다"의 원인이 실제로는 서브넷 IP 고갈일 수 있다. 그렇다면 7세대로 폴백해도 똑같이 막힌다 — 세대 문제가 아니라 VPC 문제다.

3분에서 파생되는 운영 함의는 둘이다.
...
AZ를 하나로 묶는 토폴로지 제약(zone 고정 PVC 등)이 걸려 있으면 첫 함의가 악화된다 — 다른 AZ의 8세대가 멀쩡해도 그 AZ에서만 3분마다 반복 실패한다.

## 4. 구성별로 폴백은 어디서 일어나는가

01~03에서 다룬 세 구성이 ICE 상황에서 어떻게 갈리는지 나란히 놓는다.
...
**B의 평가 방식을 오해하지 마라.** 채택 규칙은 **성공한 것 중 인덱스가 가장 앞선(=weight가 가장 높은) 결과**다(`scheduling/scheduler.go:759`) — "먼저 시도하고 실패하면 내려간다"는 순차 short-circuit이 아니다.

그렇다고 **"모든 템플릿이 매 루프 평가된다"는 뜻도 아니다.** 워커 수가 `ceil(CPURequests/1000)`(`provisioner.go:390`)이라 컨트롤러 CPU request가 1코어 미만이면 워커는 1개뿐이다. 인덱스 0(8세대)이 성공하는 순간 `parallelizeUntil`이 멈춰(`scheduler.go:780`) **7세대 템플릿은 평가조차 되지 않는다**(워커 종료 로직 `scheduler.go:939-961`). 자세한 것은 [05 §1.2]({{< relref "05-generation-preference.md" >}}).

여기서 말하는 8세대 풀의 "실패"가 ICE만을 뜻하지도 않는다 — §1의 ICE는 **오퍼링 전멸**만 만들고 **NodePool limits 소진**(`scheduler.go:709-718`)·`minValues` 불충족·파드 요구사항 비호환도 용량과 무관하게 같은 폴백을 일으킨다([06]({{< relref "06-consolidation-traps.md" >}}) §3).

B의 왕복 반복도 같은 자리에서 나온다 — ICE 캐시가 (타입, AZ, capacity-type) 단위인데 8세대 풀엔 보통 수십 개 조합이 있어 몇 개만 마킹돼도 나머지로 계속 스케줄에 성공한다. **범위를 좁게 잡을수록 한 번의 ICE로 풀 전체가 무력화돼 폴백이 빨라진다** — 넓게 잡는 게 항상 좋은 게 아니다.

**C의 "지연 0초"는 확정 사실이 아니다.** 오버레이를 적용하면 provider-aws가 할당 전략을 `prioritized`로 바꾼다. `Priority`를 정수로 규정하는 AWS API에 Karpenter는 소수 달러값을 그대로 넣는다. **정수 절단이 일어나면 시간당 $1 미만 인스턴스가 전부 priority 0이 되어 세대 선호가 조용히 무력화될 수 있다.** 코드로도 문서로도 확정할 수 없어 **확인 필요로 남긴다** — C를 도입한다면 실제로 어떤 패밀리가 뜨는지 실측이 필수다. 자세한 것은 [05]({{< relref "05-generation-preference.md" >}})에 있다.

## 5. spot을 섞는 순간 논의가 바뀐다

여기까지의 모든 서술에는 **on-demand 전제**가 깔려 있다. capacity-type이 섞이는 순간 인스턴스 선택의 주체가 바뀌기 때문이다.
...
**허용된 요구사항에 가용 오퍼링이 있으면 reserved → spot → on-demand 순으로 내려간다.** 예약 소진이나 spot ICE 전멸 시 `Available()`가 비어 자동으로 다음 단계로 내려가니 폴백엔 좋다. 그러나 세대 강제 관점에서는 치명적이다.

{{< flow src="_flow/5-spot-을-섞는-순간.json" />}}

spot에서는 어떤 방법으로도 세대 우선이 보장되지 않는다. 기본 전략인 `price-capacity-optimized`부터가 EC2더러 **용량 깊이를 우선 보고** 가격을 그다음으로 보라는 것이다. 오버레이(구성 C)를 걸어도 spot 전략은 `capacity-optimized-prioritized`로 바뀔 뿐인데 AWS 문서상 이 전략은 우선순위를 **best-effort**로만 존중한다.
...
이 챕터의 목표가 "평소엔 8세대"라면 답은 하나뿐이다. **on-demand 전용 NodePool과 spot NodePool을 분리하고, 세대 선호는 on-demand 쪽에만 건다.** 한 NodePool에 둘을 섞어 두면 `getCapacityType`이 spot을 먼저 고르는 순간 weight도 오버레이도 의미를 잃는다.
...
가격이 사실상 0이니 **`OrderByPrice`·`Truncate`(600 → 60)·EC2 Fleet `lowest-price` 세 관문을 전부 무조건 통과한다.** 8세대에 ODCR을 잡아두면 7세대보다 싸게 보이는 셈이라 알파 기능도 NodePool 분리도 필요 없다 — 01의 "싼 게 이긴다" 성질이 여기선 유리하게 작용하는 자리다.

게이트는 기본으로 켜져 있다. `FEATURE_GATES` 기본값이 `ReservedCapacity=true`인 반면 `NodeOverlay`는 명시적으로 `false`다(`pkg/operator/options/options.go`).

폴백도 자연스럽게 딸려온다. §5에서 본 대로 `getCapacityType`은 **가용한 reserved 오퍼링이 있을 때만** reserved를 고른다. 예약분이 차면 on-demand로 내려가고 거기서 다시 ICE가 나면 §2의 왕복을 타고 7세대까지 내려간다. **"예약 → 온디맨드 8세대 → 온디맨드 7세대" 3단 폴백이 GA 기능만으로 성립한다.**

다만 예약과 weight를 같이 쓸 때 코어가 특별 취급을 한다는 점은 알아둬야 한다.
...
예약 오퍼링 확보에 실패한 경우 코어는 **하위 weight NodePool의 성공 결과를 채택하지 않는다** — 8세대 풀이 예약 슬롯 경합에 지면 7세대로 내려가는 대신 그 루프를 통째로 포기하고 다음 루프를 기다린다. [05]({{< relref "05-generation-preference.md" >}}) weight 구성과 겹칠 때 실제 지연은 **확인 필요**(코드 경로만 확인, 재현 실험은 안 함).
...
§1~§6이 다룬 것은 전부 **런치가 거부되는** 실패였다. 런치가 성공한 뒤 노드가 클러스터에 조인하지 못하는 쪽은 성질이 완전히 다르다 — RFC가 드는 대표 원인은 클러스터 security group의 outbound 규칙 누락이다(`designs/noderegistrationhealthy-status-condition.md`).
...
그래서 8세대 풀에서 노드가 뜨는데 등록이 안 되면 §3·§4의 폴백 어느 것도 구제하지 못한다. 캐시는 비어 있고, 8세대 풀은 스케줄 시뮬레이션에 계속 **성공**하니 7세대 풀 결과가 채택될 이유가 없다 — 겉으로 남는 증상은 노드가 주기적으로 뜨고 사라지는 것뿐이다.
...
**`False`까지 걸리는 시간**: 등록 타임아웃 경로는 `registrationTimeout = 15분`(`liveness.go:52`)이고 런치 타임아웃은 `5분`(`:59`)이다. 실패가 2회 쌓여야 하므로 **30분 이상**이 된다. 즉시 알려 주는 신호가 아니다.

### 7.3 재시작은 상태를 보존한다

재시작이 상태를 초기화한다는 것은 흔한 오해다. 재시작으로 버퍼가 비면 **기존 컨디션 값으로 재수화**한다(`registrationhealth/controller.go:82-89`, `tracker.go:94-101`). `Unknown`으로 되돌리는 것은 generation 변경뿐이다(`controller.go:92-97`) — NodePool 또는 NodeClass generation이 컨디션의 `ObservedGeneration`과 달라질 때.
...
부작용도 하나 있다 — 파드가 스케줄된 NodePool이 `NodeRegistrationHealthy=true`일 때만 스케줄 시각을 기록하고(`state/cluster.go:513-516`), 아니면 지운다(`:517-521`). 등록이 한 번도 성공하지 않은 풀의 파드는 바인딩/ready 메트릭에서 빠진다 — **메트릭이 조용해지는 것 자체가 신호다.**
...
**NodeClaim에는 흔적이 남지 않는다.** §2처럼 코어가 ICE 난 NodeClaim을 즉시 삭제하므로 `kubectl get nodeclaim`으로는 아무것도 못 본다. 이벤트마저 오브젝트와 함께 정리될 수 있어 지속적으로 남는 유일한 신호는 메트릭이다.
...
마지막 줄이 이 문서와 03을 잇는 지점이다. **ICE 폴백은 "다음에 뜰 노드"만 바꾼다.** 3분 뒤 8세대가 후보로 돌아와도 그 사이 떠버린 7세대 노드는 그대로 남는다. consolidation은 더 싼 방향으로만 움직이므로 스스로 되돌아오지도 않는다. 폴백이 공짜인 것과 복귀가 공짜인 것은 전혀 다른 얘기다.
