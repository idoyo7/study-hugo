...
설계 문서를 따라 세 정책을 하나의 스코어식에 다른 `k`를 넣은 것으로 읽는 설명이 흔하다. **코드는 그렇게 되어 있지 않다.**
...
Balanced는 새로운 통합을 **만들지 않는다.** `WhenEmptyOrUnderutilized`가 만든 커맨드에 승인 조건을 얹을 뿐이라 **거부만 할 수 있다.**
...
**풀의 "파괴 1단위당 비용"이 기준선이고, 액션은 그 절반 이상의 효율을 내야 한다.** 분모의 `TotalDisruptionCost`는 후보가 아니라 **풀의 모든 노드** 합이다(주석: *"Second pass over ALL nodes"*). 풀 크기는 약분되어 영향이 없고, 한산하고 비싼 노드가 많은 풀일수록 기준선이 높아 통과가 어렵다.
