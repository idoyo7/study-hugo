...
consolidation의 교체 판정은 한 줄이다 — 후보 노드의 현재 가격(`candidatePrice`)을 상한 삼아 시뮬레이션이 뽑은 대체 NodeClaim의 인스턴스 타입 목록을 거른다.
...
부등호가 `<`라는 게 중요하다. **동가격 교체는 일어나지 않는다.** 필터 후 목록이 비면 `"Can't replace with a cheaper node"` 이벤트를 남기고 no-op으로 끝난다(consolidation.go:228-233).

실제로는 교체를 막는 게이트가 넷 더 있다 — 다만 **어느 것도 세대나 선호를 인코딩하지 않는다.**
...
표의 둘째 행은 뒤집어 쓸 수도 있다 — `instance-family Exists minValues: 3` 같은 제약은 예상 못 한 방어막이 된다. 필터 후 남은 패밀리가 그 밑으로 줄면 `SatisfiesMinValues`가 에러를 던져 no-op이 된다.

multi-node consolidation도 같은 `computeConsolidation`을 탄다(multinodeconsolidation.go:141). `filterOutSameInstanceType`은 삭제 대상과 **같은** 타입에만 상한을 조이므로 **8세대 여러 개를 7세대 한 개로 합치는 시나리오는 이 필터를 그대로 통과한다.**

v1.14의 `consolidationPolicy: Balanced`도 해법이 아니다 — `ScoreMove`는 절감액·중단 비용 비율만 보므로 7세대가 충분히 싸면 승인된다.
...
NodePool weight가 사는 곳은 프로비저닝 경로뿐이다. disruption 패키지에는 아예 참조가 없다.
...
`NodeClaimTemplate`에 `NodePoolWeight` 필드가 있긴 하지만(`nodeclaimtemplate.go:60,71`) 읽는 곳이 없다. weight로 저-weight 풀 폴백을 막는 분기는 코드 전체에 하나뿐이다. 그건 예약 용량 전용이다.
...
그 외 실패는 `errs[i] = err; return true`로 스킵되고 다음(=더 낮은 weight) 템플릿이 이긴다. 교체 NodeClaim은 **그 풀의 라벨을 그대로 달고** 생성된다.
...
코어가 크로스 풀 이동을 "막아야 할 사고"가 아니라 **정상 경로로 인지한다**는 증거도 있다 — `Balanced` 절감액 정산 로직의 주석이다.
...
**대체안을 만드는 시뮬레이션이 프로비저닝과 완전히 같은 스케줄러이기 때문이다.**

`SimulateScheduling`은 `provisioner.NewScheduler`를 그대로 호출한다(`disruption/helpers.go:113-121`). 이 스케줄러는 NodePool을 weight 내림차순으로 정렬하고(`provisioner.go:287-289`) 성공한 템플릿 중 **인덱스가 가장 앞선 것**을 채택한다(`scheduler.go:757-761`). gen8 풀이 파드를 수용하는 한 대체안은 gen8 안에서만 나온다. 같은 세대끼리는 strict 부등호를 넘기 어려워 대개 no-op이다.

{{< seq src="_seq/3-그런데-크로스-풀-다운그레이드는.json" />}}

**경계를 정확히 그으면 이렇다.** 크로스 풀 다운그레이드는 gen8 풀 템플릿이 `CanAdd`에 실패할 때만 일어난다. 실패 사유는 넷이다 — 오퍼링 전멸(ICE 마킹), NodePool limits 소진, `minValues` 불충족, 요구사항 비호환. 어느 쪽이든 **그 순간은 정확히 "gen7을 띄우고 싶은 순간"**이다. 이 경로 자체는 버그가 아니라 폴백이 의도대로 작동한 것이다.

## 4. 한 번 내려가면 consolidation은 되돌리지 않는다

폴백으로 뜬 gen7 노드는 용량이 회복돼도 gen7로 남는다. 이유는 단순하다 — **"더 비싼 것으로 교체한다"는 분기가 코드에 없다.** 가격 필터는 상한만 받는다. 필터 후 목록이 비면 `Command{}`를 반환한다.

업스트림에도 이 방향의 기능은 없다. 요청은 두 건 다 닫혔다.
...
#1829는 저-weight 폴백 노드를 되돌리는 `consolidationPolicy: Underweight`를 제안했다 — 스팟 부재로 온디맨드 폴백한 뒤 안 돌아오는 문제다. 우리 문제와 **동형**이지만 답은 "계획 없음"이다. #6721("Ability to prefer generation over price")도 같은 이유로 닫혔다.

### 4.1 복귀 경로는 가격 필터가 없는 두 트리거뿐

Karpenter에서 노드를 교체하는 트리거는 consolidation 말고도 있다. 그중 둘은 가격을 전혀 보지 않는다. 이 둘이 사실상 유일한 복귀 장치다.
...
넷 중 시간만으로 도는 만료를 먼저 본다. 만료는 disruption 컨트롤러가 아니라 별도 nodeclaim 컨트롤러가 처리한다. NodeClaim을 **그냥 삭제**한다.
...
삭제된 노드의 파드는 재-pending된다. 그 파드는 **새 프로비저닝 루프**에 들어간다. 그 루프는 `OrderByWeight`를 타므로 **weight 100인 gen8 풀이 다시 먼저 평가된다.** 용량이 회복돼 있으면 gen8이 이기고 아직 없으면 다시 gen7이 된다 — 어느 쪽이든 우리가 원하는 판정이다.

두 가지를 같이 알아야 한다. 만료는 **`disruption.budgets`를 소비하지 않는다** — 예산 소비 지점은 disruption 컨트롤러 안에만 있고(`singlenodeconsolidation.go:82`, `multinodeconsolidation.go:70`, `disruption/drift.go:80`) 만료는 그 경로 밖이다. 삭제 후 드레인은 termination 컨트롤러가 처리하며 PodDisruptionBudget(PDB)은 존중된다(`terminator/eviction.go:200-205`). `expireAfter`는 `Spec.Template.Spec` 안에 있어 **나중에 바꾸면 그 자체가 NodePoolDrifted를 유발한다** — 처음부터 정해 두는 게 낫다.
...
**트레이드오프는 churn이다.** 48h면 gen7 노드가 이틀마다 통째로 교체된다. 용량 부족이 길어지면 "만료 → 다시 gen7" 왕복마다 재스케줄 비용이 든다. 짧을수록 복귀는 빠르므로 폴백 빈도를 관측(§7)한 뒤 조정한다 — 12h 미만은 권하지 않는다.

### 4.2 내려가는 것 자체를 막고 싶다면 — 임시 방어선 둘

복귀가 없으니 애초에 내려가지 않게 하려는 발상은 자연스럽다. 두 수단 다 **"다운그레이드를 막는" 게 아니라 "consolidation을 끄는" 것**임을 알고 써야 한다.
...
크로스 풀 다운그레이드는 gen8이 스케줄에 실패할 때만 일어나므로(§3) 상시 방어선은 대개 과잉이다 — gen7 비중이 튄 뒤 임시로 거는 용도가 맞다.
...
소스상의 원인은 **단일 프로비저닝 루프 내부의 빈패킹**이다 — "이미 떠 있는 노드가 있어서"가 아니다.
...
`addToNewNodeClaim`의 weight 우선 채택(`scheduler.go:757-761`)은 **세 번째 단계에서만** 도달한다. 결과적으로 — 파드 100개가 한 배치로 pending → 첫 파드가 gen8 풀에서 오퍼링 없음으로 실패해 gen7 NodeClaim이 하나 생김 → **뒤따르는 99개 파드는 그 gen7 NodeClaim에 빈패킹으로 얹힌다.** gen8 용량이 그새 회복됐어도 마찬가지다 — in-flight NodeClaim이 앞 단계에서 이미 걸린다.

실무 함의는 하나다. **대규모 스케일아웃 한 번이 gen7 비중을 크게 밀어 올릴 수 있다. 그 노드들은 §4의 만료 전까지 그대로 남는다.** 배치 창은 idle 1초 / 최대 10초라(`pkg/operator/options/options.go:129-130`) 한 배치에 들어가는 파드 수가 생각보다 크다.
...
교체 속도를 제어하는 손잡이는 `disruption.budgets` 하나뿐이다. drift의 `ComputeCommands`는 한 번에 커맨드를 **하나만** 반환하지만(`drift.go:75-107`) 컨트롤러가 커맨드를 낸 직후 **즉시 재큐**한다.
...
이 문서의 실패 모드는 전부 "조용하다" — 알람 없이 노드 구성만 서서히 바뀐다. 최소한 다음은 봐야 한다.
...
**NodeOverlay를 쓸 경우의 자동 복귀.** 오버레이로 부풀린 gen7 가격은 consolidation의 후보 가격 산정에도 쓰이므로(`disruption/types.go:113-121` `resolveNodePrice` → `instanceType.OfferingPrice`) 용량 회복 시 gen7이 "비싼 노드"로 인식돼 gen8로 교체될 **가능성**이 있다 — 사실이면 `expireAfter` 없이도 복귀되는 경로가 생긴다. 배선은 확인됐다: provider-aws `cmd/controller/main.go:44-45,53-63`가 오버레이로 감싼 `cloudProvider`를 코어 컨트롤러에 그대로 넘긴다. **코드 경로는 확인했으나 실제 교체는 재현하지 못했다** — 오버레이가 알파 기능이라 테스트 환경이 없었다.
{{< /callout >}}
