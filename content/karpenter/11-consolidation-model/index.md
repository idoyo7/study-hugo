...
- 세 정책은 **`WhenEmpty` ⊆ `Balanced` ⊆ `WhenEmptyOrUnderutilized`의 포함 사슬**이다 — 단 **정책 게이트 기준이고 실행은 예산이 갈라놓는다**(§4). `Balanced`는 거부만 할 수 있고, 임계 `k=2`는 **상수라 바꿀 수 없다**(§5).
{{< /callout >}}
...
정책은 셋이지만 **빈 노드 처리는 셋이 완전히 같다**(§2.1). 실제로 갈리는 것은 비어있지 않은 후보 하나뿐이다.

| | `WhenEmpty` | `WhenEmptyOrUnderutilized` | `Balanced` |
|---|---|---|---|
| **빈 노드** | Emptiness가 삭제 | 같다 | 같다 — 스코어까지 우회 |
| **그 외 후보** | 후보 단계에서 탈락(`consolidation.go:130-134`) | 시뮬레이션 후 삭제 또는 교체 | 시뮬레이션 후 **스코어 심사** |
| 통과할 게이트 | 없다 — 경로 자체가 없다 | 파드 재배치 + (교체면) 가격 | 위 둘 + `score ≥ 0.5` |

교체 경로에 조건부 게이트가 하나 더 있다 — spot→spot 단일 노드 교체는 `SpotToSpotConsolidation` feature gate가 켜져 있을 때 **더 싼 인스턴스 타입 15개 이상**을 추가로 요구한다(`consolidation.go:47-48, 311-314`). 게이트가 기본 `off`라 표에서 뺐다([03 §6]({{< relref "03-keyword-reference.md" >}})).

그래서 셋의 승인 집합은 **일직선으로 포함된다.**

```
WhenEmpty  ⊆  Balanced  ⊆  WhenEmptyOrUnderutilized
```

왼쪽이 성립하는 근거가 §2.1이다 — `Balanced`도 Emptiness를 그대로 돌리고 그 삭제는 채점하지 않으므로, **`WhenEmpty`가 후보로 올리는 빈 노드는 `Balanced`에서도 전부 후보가 되고 스코어에 막히지 않는다.** 오른쪽은 §5.2다. 실제 경로는 §5.1~5.2의 도식 둘이 보여준다.

**단 이 사슬은 정책 게이트 수준이고 실행까지 보장하지 않는다.** 예산은 이유별로 배분되지만 **차감은 이유를 가리지 않는다** — 진행 중인 disruption을 전부 세어(`helpers.go:291-292`) `Empty` 몫에서도 빼기 때문에(`:300-301`), `Balanced`에서 돌아가는 통합이 빈 노드 삭제의 예산을 잡아먹는다. 기본 예산은 `10%`라 10노드 풀에서 1이고, 통합 한 건이면 그 틱의 빈 노드는 밀린다(`emptiness.go:76-79`). 밀리는 사이 드레인된 파드가 그 노드에 얹히면 `IsEmpty()`가 깨져 삭제가 아예 사라진다. `WhenEmpty`는 그 통합을 만들지 않아 경합 자체가 없다 — **집합은 게이트에서 포함되고 실행에서 갈린다**(§7).

### 4.2 그래도 "같은 스코어식의 서로 다른 k"는 아니다

설계 문서를 따라 세 정책을 하나의 스코어식에 다른 `k`를 넣은 것으로 읽는 설명이 흔하다. 사슬은 맞지만 **식은 그렇게 되어 있지 않다.**
...
**기본 설정에서 게이트는 둘뿐이다** — "모든 파드가 다른 데 들어가는가", 그리고 교체라면 "엄격히 더 싼가"(예외는 §4.1). 절감의 **규모**를 묻는 곳이 없다. "10원 아끼려고 파드 40개를 옮긴다"가 여기서 나온다.
