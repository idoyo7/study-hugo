...
```
① 이유별로 순회      Emptiness → StaticDrift → Drift → MultiNode → SingleNode
② 후보 1차 선별      전체 노드 → NewCandidate의 공통 관문 넷
③ 후보 2차 선별      그 이유의 ShouldDisrupt
④ 예산 컷           BuildDisruptionBudgetMapping
⑤ 시뮬레이션         후보를 지우면 새 노드가 몇 대 필요한가
⑥ Command 생성       0대 → 삭제 · 1대 → 가격 필터 → 교체 · 2대+ → 포기
⑦ 큐 → 실행         삭제는 바로, 교체는 대체 노드 기동·대기 후

첫 성공에서 루프가 끝나고, 전부 실패하면 10초 뒤 재시도
```
...
"이 노드는 drift로 처리되나 교체로 처리되나"는 성립하지 않는 질문입니다. 둘은 배타적 선택지가 아닙니다.
...
코드로 살펴보면 다음과 같은데요, base `1.0`은 cordon·drain·대체 노드 기동 지연 자체의 비용입니다. 
**그래서 비용은 어떤 경우에도 1.0 미만이 되지 않습니다** 
— §5.2의 근거입니다. 파드별 비용은 기본 `1.0`에 `pod-deletion-cost / 2^27`과 `priority / 2^25`를 더하고 `[-10, 10]`으로 클램프합니다(`utils/disruption/disruption.go:47-69`). 대상은 `IsReschedulable` 통과분뿐이라 **DaemonSet은 세지 않습니다.**
...
빈 노드 열이 셋 다 같은 이유는 **빈 노드를 담당하는 쪽이 `consolidationPolicy`를 아예 읽지 않기 때문**입니다(§2.1). 그래서 정책을 뭘로 두든 빈 노드는 똑같이 지워집니다. 실제로 갈리는 건 가운데 열 하나뿐입니다.
...
**`Balanced`가 지우는 걸 `WhenEmptyOrUnderutilized`도 지우는 이유** — `Balanced`는 새 통합을 스스로 만들지 않습니다. `WhenEmptyOrUnderutilized`가 만들어낸 것을 받아 **거부만** 하는 구조라(§6.2), 원본보다 많아질 수가 없습니다.
...
`k→0`으로 표현할 수도 없습니다. 임계 `1/k`가 무한대가 되어 `Score`가 `+Inf`여야 통과하는데 §4에서 봤듯 **비용은 절대 0이 되지 않습니다.** `k→0`은 "빈 노드를 지운다"가 아니라 **"아무것도 지우지 않는다"** 에 수렴합니다.
...
주석이 선택 근거를 밝힙니다 — **같은 패밀리 한 단계 다운사이징(4xlarge → 2xlarge)이 정확히 50% 절감**이라, 그 교체가 겨우 통과하는 지점입니다. **조절하려면 임계가 아니라 분모(`pod-deletion-cost`, §7.1)를 건드려야 합니다.**
...
- **`Balanced`에서만 보호로 작동합니다.** 다른 두 정책은 스코어 게이트가 없어 삭제 여부가 안 바뀌고, 바뀌는 건 정렬 **순서**뿐입니다(§2.2).
...
`Drifted`는 consolidation이 만들지 않습니다. drift는 별도 경로이고(§2.1) v1에서 끌 수 없어, 정책을 뭘로 두든 계속 판정됩니다.
...
| message | 뜻 | 단계 |
|---|---|---|
| `NodePool %q has consolidation policy WhenEmpty, but node is not empty` | 정책이 막았다 | §5.1 |
| `Can't replace with a cheaper node` | 가격 부등식에서 탈락 | §3.2 |
| `Can't remove without creating %d candidates` | 대체가 2대 이상 필요 | §3.1 |
| `Node %q has buffer pods` | Capacity Buffers가 잡고 있다 | §2.1 |
| `NodePool %q has consolidation disabled` | `consolidateAfter`가 `nil` | §2.1 |
