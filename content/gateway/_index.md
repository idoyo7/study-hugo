...
- **링이 필요한지는 규모가 아니라 "누가 연결을 거느냐"가 정합니다.** 클라이언트가 걸면 소유권은 로드밸런서가 이미 정해버린 상태라 파드끼리 나눌 것이 없습니다. 파드가 걸어야만 소유권이 파드들의 문제가 되고 그때 비로소 링이 값을 합니다.
- **재개 보장(Last-Event-ID)을 요구하는 순간 링의 필요성은 대부분 증발합니다.** 이벤트가 파드 밖에 남아야 하니 "이 단말의 상태를 누가 들고 있나"를 물을 이유가 사라집니다. Loki ingester가 링을 쓰는 이유는 **flush 전 chunk가 그 파드 메모리에만 있어서**지 팬아웃 때문이 아닙니다.
- **API Gateway WebSocket API의 `Connection duration for WebSocket API: 2 hours`는 상향 불가다**([공식 쿼터 표](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-execution-service-websocket-limits-table.html)). 상시 연결을 전제하는 POS 워크로드와 구조적으로 어긋나는 지점이 여기입니다.
...
- **[02 수립 방향이라는 분기점]({{< relref "02-connect-direction.md" >}})** · 연결 수립 방향과 데이터 흐름 방향은 별개입니다. 두 분기가 무엇을 강제하는지, 그리고 어느 쪽인지 판정하는 체크리스트.
...
- **[04 분기 A — POS가 건다]({{< relref "04-branch-a-client-dials/index.md" >}})** · Redis Streams 샤드 pull 설계 전문. Stream ID = `Last-Event-ID` 직결, 읽기 증폭 공식과 탈출 임계, 재개 창 계약, 레지스트리 방식으로 전환하는 조건.
...
> 자매 문서: [Valkey / 2,000노드에서 부러지는 것]({{< relref "../valkey/cluster-xl-scale/01-부러지는-것/index.md" >}}) — 재접속 폭풍이 실제로 무엇을 먼저 부러뜨리는지 실측한 기록이 거기 있습니다. · [Istio]({{< relref "../istio/_index.md" >}}) — xDS 커넥션 재분배는 같은 문제의 컨트롤플레인판입니다.
