...
- **`terminationGracePeriodSeconds` 기본 30초는 이 워크로드에 짧습니다.** 파드 하나가 2,500개 SSE 연결을 들고 있고, 그것을 흩어서 끊어야 하므로 종료 절차가 분 단위입니다.
...
> **왜 이 문서인가.** "StatefulSet이나 headless service로 정의해야 하나"라는 질문에 답하고, 그보다 훨씬 자주 사고를 내는 **종료 설계**를 다룹니다. 장수명 연결 게이트웨이에서 배포는 평시 최대 부하 이벤트입니다 — 아무것도 안 하면 배포할 때마다 5만 대가 동시에 재접속합니다.
...
분기 A의 기본 구성에서 headless service는 **필요 없습니다.** 이 사실이 놀랍다면, 그건 [03]({{< relref "03-platform-precedents.md" >}})의 결론이 그대로 반영된 것입니다 — 파드끼리 나눠 가질 상태가 없으면 파드끼리 통신할 이유도 없습니다.
...
더 나쁜 것은 **타이밍이 겹칠 때입니다.** 파드가 아직 LB 타깃 목록에서 빠지지 않은 상태에서 연결을 끊으면, 방금 끊긴 POS가 **같은 파드로 다시 꽂힙니다.** 그리고 그 파드는 곧 죽습니다.
...
`/readyz`는 `/tmp/shutdown`이 있으면 즉시 실패를 반환합니다. 나머지(4~5단계)는 애플리케이션이 `SIGTERM` 핸들러에서 수행합니다 — preStop이 끝나야 `SIGTERM`이 가므로 순서가 보장됩니다.
...
**WebSocket에는 이에 해당하는 표준 필드가 없습니다.** 애플리케이션 메시지로 같은 걸 만들 수는 있지만, 그건 클라이언트가 그 메시지를 이해하도록 우리가 만들어야 한다는 뜻입니다. SSE에서는 프로토콜이 이미 정의한 동작입니다.

> **[Valkey / 2,000노드에서 부러지는 것]({{< relref "../valkey/cluster-xl-scale/01-부러지는-것/index.md" >}})** 에서 본 것과 같은 종류의 폭풍입니다. 그쪽은 primary 수백 개를 동시에 죽였을 때 재접속 경로가 먼저 부러졌고, 여기서는 그것이 배포마다 정기적으로 옵니다.
...
**노드 드레인(karpenter consolidation, 노드 업그레이드)도 이 예산을 따른다는 점이 중요합니다.** PDB가 없으면 consolidation이 게이트웨이 파드 여러 개를 한꺼번에 걷어갑니다.
...
경로상 어느 컴포넌트든 응답을 버퍼링하면, **연결은 성립하는데 이벤트가 도착하지 않습니다.** 그리고 버퍼가 찰 때 뭉텅이로 도착합니다. 원인 파악이 오래 걸리는 종류의 사고입니다.
