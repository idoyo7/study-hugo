...
이 문서는 [04]({{< relref "04-branch-a-client-dials/index.md" >}})의 설계를 JVM/Kotlin으로 옮길 때 실제로 발목을 잡는 것들만 모았습니다. 프레임워크 튜토리얼이 다루지 않는 세 가지 — blocking 명령의 커넥션 독점, Cluster의 CROSSSLOT, 느린 소비자 — 가 본론입니다.
...
**연결마다 하나의 sink**를 두고, 파드가 Redis에서 읽은 이벤트를 로컬 맵에서 찾아 그 sink에 밀어넣습니다.
...
**상한을 넘으면 그 연결을 끊습니다.** POS는 재접속하고 `Last-Event-ID`로 밀린 것을 다시 받거나 재개 창을 벗어났으면 `event: resync`를 받습니다. **끊는 것이 곧 복구 경로**라는 게 이 설계의 좋은 점입니다.
