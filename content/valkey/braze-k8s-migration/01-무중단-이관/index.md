...
왜 이 문서인가. 이 발표는 이 도메인에서 Kubernetes 프로덕션 증거가 있는 유일한 소스다. 자매 챕터의 AWS 발표는 EC2 벤치마크였고, 이쪽은 2023년 2월부터 2024년 5월까지 실제로 이관해 그 뒤로 2년을 굴린 기록이다. 이 문서는 발표 내용에 NLB 요금 자체 검산 · Kubernetes 스케줄링 프리미티브 판정 · in-place resize GA 시점 대조 · 회사 수치 원문 확인을 덧붙였고, 발표·슬라이드·1차 문서가 어긋나는 지점을 그대로 적었다. 검증 기준: 발표 전사(714줄) · 슬라이드 PDF 34장 · Braze 투자자 공시 · Redis/Valkey Sentinel 공식 문서 · AWS ELB 요금표 · Kubernetes 공식 문서와 KEP-1287.
...
Braze의 한 샤드는 primary 하나와 replica들로 이뤄진 HA 단위이고, 이걸 Sentinel이 감시한다. 샤드끼리는 서로를 모른다 — 슬롯도, gossip도, cluster bus도 없다. 그러면 어느 키가 어느 샤드로 가는지는 누가 정하는가. 발표자가 정확히 답한다.
...
전이되는 교훈이 서로 겹치지 않는다. AWS 발표의 재접속 폭풍·투표 분열·failure report는 전부 cluster bus 코드 안의 문제이므로 Braze 토폴로지에는 존재하지도 않는다. 반대로 이 문서가 다루는 announce 주소 문제와 Sentinel split brain은 클러스터 모드에는 없다.
...
용도는 캐시 하나가 아니다 — rate limiting, distributed lock, message deduplication, 그리고 Sidekiq이다(02:37~02:51). Sidekiq은 Ruby 잡 큐이고 Redis가 그 심장이므로, 이건 캐시가 아니라 작업 파이프라인 자체가 여기 얹혀 있다는 뜻이다. 발표자 표현대로 "the entire backbone of the Braze platform"이다. 다운타임을 못 받아들이는 이유가 여기서 나온다.

레거시는 EC2 + Chef였다. 3AZ에 primary/replica 쌍을 흩고, cluster-scoped Sentinel 3대(sentinel-001~003)가 그 클러스터의 모든 샤드를 한꺼번에 감시하는 구조다(슬라이드 7쪽).
...
R&D를 두고 슬라이드가 틀렸다고 말하면 과하다 — 원문이 "over $130M"이라 $133.969M은 그 안에 들어온다. 어긋나는 건 액수가 아니라 기간 라벨이고, 회계연도와 역년이 한 판에 섞여 있다. 인용할 일이 있으면 MAU 69억은 2024-10-31 기준, R&D는 FY2025 기준 $133.969M이라고 적는 편이 원문에 맞다. 발표 자체의 논지에는 영향이 없지만, 이 문서가 확인한 것은 그대로 적어 둔다.

## 2. Kubernetes가 Sentinel을 깨뜨리는 지점

Braze는 이관을 시작하기 전부터 이미 greenfield Kubernetes 클러스터에 Helm으로 Redis를 띄워 본 상태였다. 그런데 그쪽 토폴로지가 레거시와 다르다.
...
**Sentinel은 replica를 IP로 추적한다.** 정확히는 primary의 `INFO` 출력에서 replica 목록을 읽어 자동으로 찾아내는데, [Redis 복제 문서](https://redis.io/docs/latest/operate/oss_and_stack/management/replication/)가 그 성질을 이렇게 적는다 — replica는 "the IP address they use to connect to the master"로 보이고, 포트는 "the listening port configured into redis.conf"로 보인다. 둘 다 **접속 시점의 실제 값**이지 논리 주소가 아니다.

Kubernetes에서 파드가 롤되면 파드 IP가 바뀐다. Sentinel 입장에서는 **옛 IP가 응답을 멈춘 것**이고, 새 IP는 별개의 replica로 새로 등록된다. 슬라이드 8쪽의 `sentinel replicas cache-0` 출력에 그 상태가 그대로 남아 있다 — 살아 있는 셋 옆에 `down` 하나가 붙어 있다.
...
**안 2와 안 3을 가르는 건 오직 "복제가 되돌아올 수 있는가" 하나다.**
...
**이 선택은 이관 기간에만 드는 비용을 정하는 게 아니라 영구 비용을 정한다.** 다른 AZ 파드를 승격시키면 그 샤드의 이후 모든 복제 트래픽에 cross-AZ 요금이 붙는다. 이관 스크립트의 한 줄짜리 판단이 그 샤드의 수명 전체에 요금을 새긴다.
...
이 규칙이 이 발표에서 가장 이식성이 높은 조언이다. re-platforming 중에 "이왕 하는 김에" 파라미터를 손대면, 뒤에 문제가 생겼을 때 원인이 플랫폼인지 설정인지 영영 못 가른다. §7에서 볼 90%/15%/5% 수치가 의미를 갖는 이유도 정확히 여기 있다 — like-for-like config였기 때문에 엔진 차이로 귀속시킬 수 있다.
...
Prometheus로 실제 read·write·replication 트래픽을 재서 NLB를 상시 유지할 때의 비용을 산정했고, 월 $100,000 이상이 나왔다(17:45~17:53). 그래서 NLB는 임시 장치가 될 수밖에 없었고, 모든 클라이언트가 Kubernetes로 넘어간 뒤에야 내부 ClusterIP 광고로 전환하고 NLB를 걷었다.

발표는 계산 과정을 공개하지 않는다. 그러면 이 숫자가 그럴듯한지 직접 검산해 보면 된다. 재료는 [ELB 요금표](https://aws.amazon.com/elasticloadbalancing/pricing/)에 다 있다.
...
NLCU는 세 차원 중 그 시간에 가장 큰 것으로 청구된다. Braze 구성에서 어느 차원이 지배적인지부터 확인하자.
...
여기서 replication fan-out을 넣어야 payload 크기가 나온다. Kubernetes 샤드는 primary 1 + replica 2이므로, 쓰기 하나가 NLB를 건너는 횟수가 1회(클라이언트→primary)가 아니라 최대 3회(+ replica 2개로 가는 복제)다.
...
$100,000는 부풀린 숫자가 아니다. rate limiting(`INCR`), distributed lock(`SET NX`), dedup 같은 워크로드는 값이 카운터나 플래그라서 커맨드·키 이름을 포함해도 수십~수백 바이트다. 위 표의 어느 행이든 그 범위 안에 정확히 들어온다. 오히려 보수적일 수 있다 — NLB 개수만큼 붙는 $0.0225/LB-hour(월 $16.4)는 위 계산에 안 들어갔고, 샤드마다 NLB를 뒀다면 581대 × $16.4 = 월 약 $9,500이 별도로 얹힌다.

{{< callout type="warning" >}}
위 검산이 세운 가정을 그대로 밝힌다. 발표는 payload 크기도, 읽기/쓰기 비율도, NLB를 몇 대 뒀는지도 말하지 않았다.
...
이 검산은 "$100,000가 정확하다"를 증명하지 않는다. "36M ops/sec 규모에서 그 자릿수가 나오는 게 자연스럽다"까지만 말한다.
{{< /callout >}}
...
발표자의 설명은 이렇다 — 각 진영 3대가 primary가 안 보인다고 판단하고 자기들끼리 failover를 결의한다(18:22~18:40). 결과는 한 샤드에 primary 두 개다. [Sentinel 문서](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/)가 그 결말을 그대로 적어 뒀다.
...
그래도 대응 자체는 정당하다. 두 진영이 파티션 동안 서로를 sentinel 집합의 구성원으로 계속 세느냐에 따라 과반 계산이 갈리고, 그 상태를 이관 창 안에서 확신할 방법이 없다. 아래 수는 그 불확실성을 지우는 쪽에 건 것이다.
{{< /callout >}}
...
quorum 5가 왜 특별한지는 산수로 확인된다. 7의 단순 과반은 4다. Sentinel 문서는 quorum을 과반보다 크게 잡았을 때의 효과를 명시한다.
...
대가도 정확히 알고 썼다. quorum 5는 감지를 둔하게 만든다 — 진짜 장애가 났는데 sentinel 3대만 그걸 봤다면 failover가 안 일어난다. 발표자가 그 절충을 밝힌다: "if it so happened that it was at a time when we weren't monitoring it"(19:05~19:11). 사람이 지켜보는 동안에는 수동 개입이 가능하니, 자동 failover의 민감도를 낮추고 split brain을 확실히 막는 쪽을 택했다. 이관 기간에만 유지하는 설정이기에 성립하는 선택이다.

## 6. Valkey 이관은 두 줄이었다

Kubernetes 이관은 **2023-02 ~ 2024-05**에 걸쳐 **274샤드 / 7클러스터 / 데이터 손실 0**으로 끝났다(무대에서는 "nearly 300"). 그 위에서 Valkey 전환이 시작된다.

배경부터 정확히 적어 둘 게 있다. [Redis 라이선스 변경](https://redis.io/blog/redis-adopts-dual-source-available-licensing/)은 2024-03-20 발표됐고 KubeCon Paris(2024-03-19~22) 회기 중이었다 — 발표자는 현장에 있었고 "another rug pull"이라는 표현을 쓴다. 8일 뒤 [Linux Foundation이 Valkey를 출범](https://www.linuxfoundation.org/press/linux-foundation-launches-open-source-valkey-community)시켰다. **포크 기준점이 Redis 7.2.4인데, 그게 정확히 Braze가 돌리던 태그다.**
...
**즉 라이선스가 강제한 이관이 아니다.** 이 구분이 중요한 건 강제 이관이었다면 "어쩔 수 없이 했다"로 끝나지만 자발적 이관이면 **얻는 게 있어야 정당화되기 때문**이다. 그 정당화가 §7의 숫자다.
...
**config는 한 줄도 안 고쳤다.** Valkey가 Redis 설정과 완전 하위호환이기 때문이고, Sentinel 쪽도 마찬가지다 — [Valkey는 Sentinel 문서를 직접 유지](https://valkey.io/topics/sentinel/)하며 `announce-ip`/`announce-port`를 포함해 Braze가 의존하던 지시어가 그대로 있다. 참고로 [8.1.1](https://github.com/valkey-io/valkey/releases/tag/8.1.1)은 2025-04-23 릴리스이고 urgency가 SECURITY다 — CVE-2025-21605(미인증 클라이언트 출력 버퍼 제한)를 포함한 버그 수정 11건짜리 패치 릴리스다.
...
**이 대비가 이 발표의 진짜 결론이다.** 같은 회사가 더 커진 규모(274 → 350샤드)를 15개월이 아니라 6주에 옮겼다. 발표자는 그 원인을 Valkey가 아니라 **플랫폼**에 돌린다 — "all thanks to the velocity that Kubernetes allowed us to move at". 첫 이관이 사 준 것은 성능이 아니라 **두 번째 이관의 비용**이었다.
...
**90%와 15%의 거리는 6배다.** 이건 반올림 차이가 아니라 "가장 좋았던 한 곳"과 "평균"의 차이다. 두 수치를 바꿔치기하면 용량 계획이 통째로 틀어진다. 자기 환경에 기대치를 잡을 때 쓸 숫자는 **15%와 5%**다.
...
숫자가 뒷받침한다. Braze의 [2024년 API 호출은 8.6조 건](https://www.braze.com/resources/articles/2024-how-braze-powered-exceptional-marketing-at-scale)이다. **p50이 5% 빨라진다는 건 "거의 모든 요청이 조금씩 빨라진다"는 뜻이다.** 조 단위에서는 그 합이 p95 꼬리 개선보다 총 대기시간을 더 많이 줄인다. 꼬리 개선은 소수 요청에만 적용되기 때문이다.
...
발표자가 밝힌 현재 미해결 문제 셋이다. 셋 다 Braze만의 문제가 아니라 **Kubernetes에서 stateful을 굴리면 공통으로 만나는 것**이라 그대로 옮겨 온다.
...
**판정: 스케줄링 제약으로 표현할 수 있는 종류가 아니다.** 둘 중에서는 topologySpreadConstraints가 낫지만 그건 개수를 세기는 한다는 이유뿐이다. primary 역할이 런타임에 이동하는 이상 이걸 유지하려면 **failover를 관측하고 배치를 사후 교정하는 컨트롤러**가 있어야 한다 — 즉 §8.3의 operator나 descheduler 계열이다. 발표자가 이 문제를 "다음 과제"로 남겨 둔 것 자체가 그 결론과 일치한다.
...
발표는 2026-03-26이고 1.36은 아직 안 나왔다. **"last Kubernetes version"은 문자 그대로 정확하다.** VPA 쪽 GA는 발표 약 6주 전이니 "over the last few months"도 맞다. 두 릴리스 트레인이 별개라는 걸 아는 상태에서 한 문장으로 압축한 셈인데, 무대에서 한 축약치고 정확도가 높다.

**GA에서만 열린 능력이 하나 있는데 Braze의 용례에 직결된다** — [메모리 limit **감소**](https://kubernetes.io/blog/2025/12/19/kubernetes-v1-35-in-place-pod-resize-ga)다. beta는 이걸 금지했다. 캐시를 워크로드에 맞춰 **키웠다가 다시 줄여야 하는데** 감소 방향이 없으면 반쪽이다. 늘리기만 되는 오토스케일링은 오토스케일링이 아니라 사다리다.
...
슬라이드에 크게 박힌 **90%는 최선값 하나**다 — 발표자 본인이 무대에서 전체 평균 p95 15% · p50 5%를 덧붙였으니 90%만 떼어 인용하면 발표자가 하지 않은 주장을 하는 것이다.
