...
- **[커넥션 게이트웨이]({{< relref "gateway/_index.md" >}})** — POS 단말 1만~5만 대에 중앙에서 푸시하는 게이트웨이를 API Gateway WebSocket에서 자체 SSE로 옮길 때, **파드끼리 상태를 나눠 갖는 링이 정말 필요한가**를 판정합니다. Loki ingester·vmagent·Alloy·Thanos compactor·Kafka consumer group이 각각 왜 다른 답을 냈는지 해부하고, 그 전제가 우리에게 있는지 대조합니다. 결론은 수립 방향 하나로 갈린다 — 클라이언트가 걸면 링은 낭비이고, 파드가 걸면 링 위에 lease까지 필요하입니다.
