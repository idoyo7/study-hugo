...
> MSA 환경에서 로그·메트릭·RUM을 여러 저장소에 흩뿌리고 있는 팀입니다. **"이거랑 저거만 있으면 다 될 것 같은"** 최소 조합을 찾아가며 검토한 솔루션들을 이 챕터에 정리했습니다. Elasticsearch(OpenSearch) 기반 EFK 외에 애플리케이션 로깅을 어떤 방식으로 내재화할 수 있는지, 솔루션마다 성격이 어떻게 다르고 우리 케이스에서는 무엇을 골랐는지를 다룹니다.

> 자매 챕터: [VictoriaMetrics 지식베이스]({{< relref "../monitoring/victoriametrics/_index.md" >}}) · [메트릭 400일 보관]({{< relref "../monitoring/longterm-retention/_index.md" >}}) — 메트릭 계층은 그쪽이 맡고 로그·RUM 계층을 이 챕터가 맡습니다. istio 액세스 로그·APM·RUM의 단독 심화는 별도 도메인([Istio]({{< relref "../istio/_index.md" >}}) · [APM]({{< relref "../apm/_index.md" >}}) · [RUM 내재화]({{< relref "../rum/_index.md" >}}))에서 이어질 예정입니다. ClickHouse를 채택하기로 했을 때의 운영 전략(배포·스토리지·operator)은 [ClickHouse 운영]({{< relref "../clickhouse/_index.md" >}}) 도메인에서 다룹니다 — D4의 채택 여부 판단과는 전제가 다릅니다.

## 먼저, 이건 "하나의 큰 결정"이 아니다

여러 이야기가 섞여 있어서 하나로 뭉뚱그리면 ROI 판단 자체가 불가능해집니다. 발라내 보면 **서로 독립적인 4개의 결정**이고, 답이 각각 다르므로 따로 판단해야 합니다.
...
이 프레이밍을 먼저 못 박는 이유가 있습니다. D1은 스프린트 단위로 끝나는 저위험 작업입니다. D2는 자릿수 절감이 걸린 큰 건이고, D4는 조건이 성숙해야 성립하는 장기 베팅입니다. 하나로 묶으면 "전면 이전 ROI 나올까?"라는 답 없는 질문이 됩니다.

## 왜 이 고민을 하는가 — 현재 구조의 겹침

규모 있는 MSA를 운영하면 옵저버빌리티 데이터는 자연스럽게 여러 저장소로 번집니다. 문제는 **같은 성격의 데이터를 여러 곳에 중복 수집·저장**하면서 비용과 운영 부담만 늘어난다는 점입니다.
...
하나만 짚어둡니다. **로깅 스택이 죽는 원인은 대개 기술이 아니라 오너십**입니다. promtail이 방치되고 DaemonSet이 노드 교체와 함께 사라지는 일은 도구를 바꾼다고 해결되지 않습니다. 그래서 "어떤 솔루션이냐"만큼 "우리가 스택 하나를 더 썩히지 않을 수 있느냐"가 선택의 실질적 기준이 됩니다.
