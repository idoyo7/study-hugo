...
- 리소스 효율. 벤더는 다른 솔루션(Elasticsearch·Loki) 대비 RAM 최대 30x, 디스크 최대 15x 적게 쓰고 인덱스 튜닝이 필요 없다고 주장하며 `Ⓥ`, 압축률은 40x~80x typical(100TB→2.5TB) `Ⓥ`, Loki 대비 풀텍스트 검색은 최대 1000x 빠르다고 합니다 `Ⓥ`. 독립성이 완전하진 않은(VictoriaLogs-friendly) 소규모 3rd-party 벤치도 500GB 기준 RAM ~1.3GiB vs Loki 6–7GiB, 디스크 318 vs 501 GiB, needle 검색 ~900ms vs ~12s, ingest 66 vs 20 MB/s로 같은 방향입니다 `Ⓑ`. 단, 대규모(multi-TB/day) 독립 벤치는 아직 공개된 게 없습니다.
...
- 목적에 맞춘 LogsQL + 고카디널리티. 풀텍스트 검색에 풍부한 pipe 모델(stats · sort · limit/offset · coalesce · filter · subquery)을 얹었습니다. user_id · trace_id · IP 같은 고카디널리티 필드는 사전 스키마 설계 없이 바로 쿼리합니다(같은 필드가 Loki 라벨에는 독). live tailing도 됩니다. 언어 자체도 활발히 자랍니다(v1.51에 coalesce pipe 등).
...
- 쿼리 가능한 오브젝트 스토리지 티어가 없습니다(headline gap). 현재 local-disk-only입니다. native S3/GCS backend는 로드맵(issue #48)에 있으나 WIP입니다. 확정 일정도 없고 native hot/warm/cold tiering도 downsampling도 없습니다 — partition detach/attach + snapshot을 이용한 DIY tiering만 가능합니다. 그래서 보존 기간 전체가 붙은 block storage(EBS/NVMe) 위에 놓입니다. OpenSearch UltraWarm 같은 값싼 티어는 방정식에서 빠집니다. footprint가 작고 압축률이 높아 절대 스토리지 지출은 오히려 낮을 수 있습니다 — 다만 이질적 마이크로서비스 로그의 실제 압축률은 40–80x best case보다 훨씬 낮습니다. 자체 로그로 PoC 검증이 필요합니다.
...
우리는 이미 self-hosted VictoriaMetrics 클러스터를 운영 중입니다. 운영 모델과 감각이 거의 그대로 이전됩니다 — 학습 · rot 비용이 0에 가까워 로그 내재화를 시작하기에 리스크가 가장 작습니다. 규모가 크지 않은 istio 액세스 로그(~100–300GB/day)부터 얹으면 오브젝트-스토리지 부재가 걸리지 않습니다. cold(30–90일) tail만 S3 Parquet(VL cold mount / [CH-on-S3]({{< relref "04-clickhouse.md" >}}) / Athena)로 분리 설계하면 됩니다. VictoriaMetrics의 내부 동작은 [VictoriaMetrics 지식베이스]({{< relref "../monitoring/victoriametrics/_index.md" >}}) 참고.
