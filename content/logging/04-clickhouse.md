...
- **압축·저장 밀도가 이 클래스에서 최상급입니다.** 컬럼 파일 위에 타입별 codec(timestamp의 DoubleDelta, 느린 float의 Gorilla, 단조 int의 Delta)을 깔고 그 위에 LZ4/ZSTD를 얹습니다. Elasticsearch처럼 `_source`(원본 JSON의 거의 완전한 사본)와 inverted index를 함께 들고 있지 않고 컬럼에서 행을 재구성하므로 디스크가 근본적으로 작습니다.
