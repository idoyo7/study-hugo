...
이 챕터는 **쿠버네티스 코어 기능**을 다룬다. 생태계 컴포넌트 중 성격이 같은 주제 — 공식 문서가 "가장 싼 인스턴스를 고른다"까지만 말하고 멈추는 자리에서 정렬·절단·부등식이 실제로 무엇을 하는지 소스로 내려가는 이야기 — 는 자매 챕터 [Karpenter]({{< relref "../karpenter/_index.md" >}})가 소유한다.
