...
| [Karpenter (섹션)]({{< relref "karpenter/_index.md" >}}) | EKS · karpenter v1 | 노드를 **누가 어떤 기준으로** 고르는가 — 인스턴스 선택 알고리즘, 세대 선호(8세대 우선/7세대 폴백) 구성, consolidation이 되돌리는 것, ICE 폴백 |

위 셋이 코어 기능의 이야기라면, **Karpenter 섹션은 생태계 컴포넌트**다. 성격은 같다 — 공식 문서가 "가장 싼 인스턴스를 고른다"까지만 말하고 멈추는 자리에서, 정렬·절단·부등식이 실제로 무엇을 하는지 소스로 내려간다.
