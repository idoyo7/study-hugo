...
공식 문서와 릴리스 블로그는 "무엇이 있다"까지만 말해줍니다. "우리 클러스터에서 지금 써도 되는가"는 말해주지 않습니다. 이 챕터는 운영에 실제로 영향을 주는 Kubernetes 주제를 골라 구현이 실제로 하는 일·리포팅된 버그·케이스별 득실까지 내려갑니다. 설명이 목적은 아닙니다. "쓸지 말지, 쓴다면 어디에"를 판단할 수 있는 수준을 노립니다.
...
어느 쪽이든 결국 세 가지를 봐야 합니다 — 무엇이 구현됐고 무엇이 빠졌나, 누가 어떤 버그를 밟았나, 내 워크로드 유형에서 득인가 실인가. 이 챕터의 문서들은 그 세 질문에 답하는 형식으로 씁니다.
...
이 챕터는 쿠버네티스 코어 기능과 SIG가 관리하는 코어 인접 컴포넌트를 다룹니다. 생태계 컴포넌트 중 성격이 같은 주제는 자매 챕터 [Karpenter]({{< relref "../karpenter/_index.md" >}})가 소유합니다. 공식 문서가 "가장 싼 인스턴스를 고른다"에서 멈추는 자리에서, 정렬·절단·부등식이 실제로 무엇을 하는지 소스로 내려가는 이야기입니다.
