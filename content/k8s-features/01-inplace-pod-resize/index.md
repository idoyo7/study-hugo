...
수 GB 버퍼 풀을 데운 DB, 수만 커넥션을 문 게이트웨이, 웜업에 수십 분 걸리는 캐시. 이런 워크로드는 지금까지 "재시작 비용 > 리소스 낭비 비용"이라 방치가 합리적였습니다. in-place resize는 그 트레이드오프 자체를 없앱니다 — **늘리는 방향은 사실상 리스크가 없습니다**(수락되면 무중단, 실패해도 원상태 유지).
