...
그 결과가 CNI·로그 에이전트·메시 데이터플레인처럼 **node-local 인프라**가 빠진 노드에 워크로드가 계속 스케줄되는 상태다. 파드는 `Running`이고 readiness도 통과한다. 빠지는 것은 네트워크·관측·정책뿐이고 조용히 빠진다.
...
rollingUpdate 기본값은 `maxUnavailable: 1`, `maxSurge: 0`, `minReadySeconds: 0` 이다. `maxSurge: 0` 에서는 "At most one pod of the DaemonSet will be running on each node during the whole update process." — 즉 옛 파드를 죽인 뒤에야 새 파드가 뜬다. 새 템플릿이 깨져 있으면 그 노드는 **DS가 아예 없는 상태**로 남는다. 롤아웃 정지가 곧 노드 갭이 되는 경로다.
...
무엇을 고를지 정하기 전에 각 조치가 실제로 무엇을 하는지 붙여 놓고 본다.
...
NoExecute의 의미론은 세 갈래다. 첫째, "Pods that do not tolerate the taint are evicted immediately" · 둘째, "Pods that tolerate the taint without specifying `tolerationSeconds` … remain bound forever" · 셋째, `tolerationSeconds` 를 지정한 파드는 그 시간이 지나면 node lifecycle controller가 축출한다. 여기에 기본값이 하나 얹힌다 — 쿠버네티스는 모든 파드에 `not-ready`·`unreachable` 에 대한 `tolerationSeconds: 300` 기본 톨러레이션을 자동 부여한다. 노드가 죽어도 5분은 버틴다는 뜻이다. 격리를 서두르고 싶다면 이 5분이 그대로 지연으로 잡힌다.
...
세 사례의 공통 구조는 **부착 주체와 제거 주체의 분리**다. 제거 주체(DS·컨트롤러)만 있고 부착 주체(프로비저너·부트스트랩)가 없으면 패턴 자체가 성립하지 않는다. 3절에서 본 대로 제거를 맡은 DS 자신은 그 taint를 톨러레이트해야 스케줄된다.

### 4.2 반응 — 탐지에서 축출까지

반응 전략은 노드별 갭을 찾아내는 것에서 시작한다.
...
즉 **컨디션 → taint 변환 주체를 직접 두어야** 체인이 이어진다. NPD 저장소에는 NPD 자체 설정(`taintEnabled`/`taintKey`/`taintValue`/`taintEffect`)으로 taint를 붙이는 로직을 구현하려는 이력(PR #565)이 있다. 그 밖에는 별도 컨트롤러나 아래의 반응형 도구가 그 자리를 맡는다.
...
Descheduler는 taint를 붙이지 않는다는 점이 중요하다. 체인은 `컨디션 → (누군가 taint 부착) → RemovePodsViolatingNodeTaints 축출` 이고 가운데 고리가 바로 앞에서 본 빈자리다. `includedTaints`/`excludedTaints` 로 대상 taint를 좁히고 `includePreferNoSchedule: false` 로 NoSchedule만 검사하도록 설정한다.
...
위 다섯의 트리거는 **전부 built-in 또는 에이전트 컨디션**이다. "이 노드에 CNI DS가 없다"는 커스텀 신호는 여기 자동으로 들어오지 않는다. 반응 전략을 쓰려면 앞에서 본 컨디션화와 taint 부착 고리를 반드시 직접 만들어야 한다.

## 5. 권장 조합

**EKS + Karpenter.** 선제를 기본으로 깐다. NodePool `spec.template.spec.startupTaints` 에 node-local 인프라마다 키를 하나씩 두고(CNI, 로그 에이전트, 메시 CNI), 제거는 각 DS 또는 전용 컨트롤러가 맡는다. effect는 Cilium 권장을 따라 NoExecute를 기본으로 검토한다 — 운영 중 taint가 재부착되는 시나리오에서 이미 스케줄된 파드까지 막아준다. 한 번 제거된 startup taint는 Karpenter가 복원하지 않는다는 의미론(issue #1772) 위에서 설계한다. 다만 등록 taint(`karpenter.sh/unregistered`) 제거 시점의 복원 레이스는 알려진 버그이므로 자체 클러스터에서 재현 여부를 확인한다. 반응 축은 `eks-node-monitoring-agent` + node auto repair로 코어·커널·스토리지·네트워킹 컨디션을 관리형에 맡긴다. Karpenter Node Repair는 alpha이고 조치가 강제 종료이므로 feature gate를 켜기 전에 20% 임계값과 강제 종료 의미론을 먼저 합의한다.

**자체 클러스터.** startup taint를 붙일 주체부터 정해야 한다. Karpenter가 없으면 노드 등록 시점에 taint를 부여하는 경로를 프로비저닝 도구(부트스트랩 스크립트·이미지·설치 자동화) 쪽에 마련해야 한다. 이 부착 주체가 없으면 선제 전략은 성립하지 않는다. 반응 축은 [04]({{< relref "04-node-problem-detector.md" >}})의 NPD `CustomPluginMonitor` 로 DS 갭을 컨디션화하고 조치는 medik8s NHC + SelfNodeRemediation으로 잇는다. Draino는 마지막 커밋이 2020-12-14라 신규 채택 대상이 아니다. Descheduler는 taint가 이미 붙은 뒤의 축출 단계에만 쓴다.
