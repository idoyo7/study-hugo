...
그 결과, CNI·로그 에이전트·메시 데이터플레인처럼 **node-local 인프라**가 빠진 노드에도 워크로드가 계속 스케줄됩니다. 파드는 `Running`이고 readiness도 통과합니다. 조용히 빠지는 것은 네트워크·관측·정책입니다.
...
- node-local 인프라 DS를 먼저 나열합니다. CNI, 로그·메트릭 에이전트, 메시 데이터플레인, CSI 노드 플러그인 중 **없으면 워크로드가 조용히 깨지는 것**만 골라냅니다. 이 목록이 startup taint 키의 목록이 됩니다.
