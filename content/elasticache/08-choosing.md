...
{{< callout type="info" >}}
**한눈에**
- **신규 채택은 Valkey 9.1.x 가 기본값이다.** BSD-3 이고, 거버넌스가 라이선스 재변경을 2/3 super-majority + 조직 1/3 상한으로 묶어 두었고, ElastiCache 에서 7.1 위로 갈 수 있는 유일한 엔진이다 `Σ` — 단 **검색·벡터·JSON·시계열을 배포 하나로 끝내야 하면 이 선택은 성립하지 않는다**(§3.5).
- **이미 Redis 를 쓰고 있으면 대부분 그대로 있는 것이 맞다.** 사내 캐시 용도는 RSALv2·SSPLv1·AGPLv3 어느 쪽에서도 금지되지 않고([03]({{< relref "03-license-and-fork.md" >}})), **7.4 이상에서 Valkey 로 넘어가는 파일·복제 경로는 전부 막혀 있다**([05]({{< relref "05-valkey-8-to-9/index.md" >}})) — 이주 비용이 라이선스 리스크보다 크다 `Σ`. 움직여야 하는 것은 재배포·외부 제공이 로드맵에 있는 조직뿐이다 `✓`.
- **값이 blob 이고 잃어도 되는 순수 캐시는 memcached 1.6.45 가 여전히 이긴다.** 프로세스 하나가 코어를 다 먹고, 느린 커맨드라는 개념이 없고, 프로토콜에 stampede 방어가 들어 있다([02]({{< relref "02-memcached/index.md" >}})) `✓`. 지는 지점은 성능이 아니라 **관리형 경로와 버스 팩터**다 `Σ`.
- **판정 기준은 성능이 아니다.** 세 진영의 성능 수치는 전부 발행 주체가 당사자이고 같은 하드웨어 1:1 측정이 없다([02]({{< relref "02-memcached/index.md" >}})·[04]({{< relref "04-redis-7-to-8.md" >}})·[05]({{< relref "05-valkey-8-to-9/index.md" >}})) `✓`. 실제로 결정하는 것은 **라이선스·번들 모듈·RDB 버전·관리형 지원 버전** 네 개다 `Σ`.
- **되돌릴 수 있는지가 대부분의 결정을 대신 내려 준다.** Redis 8.6 부터 RDB 가 매 릴리스 오르고, Valkey 9.0 이 RDB 를 80 으로 영구히 갈랐고, ElastiCache 의 CMD → CME 는 단방향이다 — **이 셋은 전부 편도 티켓이다** `✓`.
- **"9 를 기다린다" 는 전략이 없다.** Redis 에 9.x 는 존재하지 않고 계획 문서도 확인되지 않았다([04]({{< relref "04-redis-7-to-8.md" >}})) `✓`. 9.x 를 가진 쪽은 Valkey 다.
{{< /callout >}}

> **왜 이 문서인가.** 앞 7개 문서는 각각 "무엇이 사실인가" 를 소유한다. 이 문서는 새 사실을 도입하지 않고 **그 사실들을 결정으로 압축**한다. 그래서 모든 행에는 근거 문서로 가는 링크가 붙고, 근거가 없는 취향 서술은 넣지 않았다 — 근거가 부족한 자리는 §7 에 미확인으로 남겼다.

> **배지 규약.** `✓` = 앞 문서가 1차 근거(소스·릴리스노트·공식 문서 원문)로 확정한 사실에서 곧바로 나오는 판단. `≈` = 2차 근거이거나 조건이 붙어야 성립하는 판단. `Σ` = 여러 사실을 엮은 종합 판단 — 사실이 아니라 추론이므로 전제가 깨지면 결론도 깨진다. 기준일 **2026-08-06**, 기준 버전은 **Redis Open Source 8.10.0**(2026-07-29) · **Valkey 9.1.1**(2026-07-21) · **memcached 1.6.45**(2026-07-09) 다.
...
세 줄이 서로 다른 축을 본다는 점이 중요하다. 신규 채택은 **거버넌스**가, 기존 Redis 는 **RDB 버전**이, 순수 캐시는 **관리형 지원 버전**이 결정한다 `Σ`.

## 2. 3자 판단표

각 칸은 앞 문서가 확정한 사실이고, 판정 열이 이 문서의 기여다.
...
- **조건** — 값이 blob 이고 잃어도 되고, 다중 키 원자성이 필요 없고, TTL·CAS·incr 이면 표현이 끝난다.
- **선택** — 자체 운영이면 **memcached 1.6.45**, 관리형에 얹어야 하면 **ElastiCache Valkey**(캐시 용도로) `Σ`.
- **왜** — memcached 는 이 조건에서 Redis 계열이 갖지 못한 세 가지를 준다: 프로세스 하나로 코어를 먹는 확장, 느린 커맨드가 존재할 수 없는 지연 예측성, 프로토콜 내장 stampede 방어([02]({{< relref "02-memcached/index.md" >}})) `✓`. 그런데 관리형으로 가면 그 강점이 사라진다 — ElastiCache 의 확인 최신이 1.6.22 이므로 **1.6.34 의 slab mover 재작성(재배치 시 아이템 손실 제거)과 1.6.4x 보안 수정을 못 받는다** `✓`. 아이템 크기 분포가 시간에 따라 변하는 캐시에서 calcification 리스크를 지고 갈 이유가 없다 `Σ`.
- **되돌릴 수 있나** — **거의 공짜로 되돌릴 수 있다.** 영속성이 없어 이주 대신 재구축이 되고, 대가는 cold cache 창의 thundering herd 하나다 `✓`. 그래서 이 케이스는 **이 문서에서 가장 낮은 리스크로 결정할 수 있는 자리**다 — 오래 고민할 가치가 없다 `Σ`.
...
- **되돌릴 수 있나** — **Valkey 9.x 를 고르는 순간 파일 레벨 복귀는 끝난다.** RDB 80 은 Valkey 8.x 도 거부하고, 12~79 예약 때문에 Redis 쪽으로 돌아가는 경로도 없다 `✓`. 그래서 롤백 계획을 **"RDB 되돌리기" 가 아니라 "전환 직전 스냅샷 보관 + 논리적 재적재"** 로 세워야 한다 `Σ`. 이 제약을 감당할 수 없다면 8.1.x 에 머무는 선택도 있다(RDB 11 이라 Redis 와 파일 호환) — 대가는 HFE·cluster multi-DB·DB 단위 ACL 을 포기하는 것이다 `✓`.

### 3.3 이미 Redis 7.2 이하를 쓰고 있다

**먼저 라이선스 경계에 걸리는지부터 판정한다.** 걸리지 않으면 이 절의 나머지는 읽을 필요가 없다 `Σ`.
...
- **왜** — Redis OSS 는 7.1 에서 멈췄고 그 위는 전부 Valkey 다. **ElastiCache 에 Redis 8 은 없다**([07]({{< relref "07-aws-endpoints/index.md" >}})) `✓`. 그리고 엔진 전환의 리스크가 모드 전환과 정반대로 낮다 — in-place 이고 *"including the endpoint DNS name, will remain unchanged"* 이며 바뀌는 것은 노드 IP 뿐이다 `✓`. 다운타임은 Redis OSS **5.0.6 이상**이면 failover 몇 초, 그 미만이면 DNS 전파 동안 30~60초다 `✓`.
...
- **관리형을 벗어날 이유가 있는지** — `config`·`debug`·`bgsave`·`replicaof` 가 막혀 있고 MemoryDB 는 `acl setuser`·`cluster setslot`·`module` 까지 막는다 `✓`. 남는 유일한 설정 경로가 파라미터 그룹이므로, **런타임 `CONFIG SET`·모듈·직접 슬롯 통제 셋 중 하나가 요구사항이면** self-host 가 후보가 된다 — 그 외에는 관리형을 벗어나는 것이 손해다 `Σ`.
- **되돌릴 수 있나** — 엔진 전환은 **Valkey 7.2 → Redis OSS 7.1 한 경로만** 있다(user/user group 이 `engine type REDIS` 여야 한다) `✓`. 8.0 이상으로 올린 뒤에는 없다 `✓`. 모드 전환은 `compatible` 단계에서만 되돌릴 수 있고 `enabled` 를 넘으면 없다 `✓`.
...
**8.10 을 장기 지원으로 가정하면 안 된다** — 지원 표에 등재조차 되지 않았고 Standard/Extended 구분이 미정이다 `✓`. 그래서 이 진영의 실제 선택은 **"5년(8.2) vs 최신 보안 표면(8.10)"** 이고, 둘이 충돌할 때 판정은 §3.4 의 마지막 줄에 있다 `Σ`.
...
**"9.1 은 2031년까지 안전하다" 를 계획의 전제로 삼지 않는다** — security 5년은 각 major 의 **"최신 minor"** 에만 붙고, 9.2.0 GA 목표가 2026-11-15 다. 지위를 잃으면 규칙상 2029-05-19 로 당겨지지만 **"되돌린다" 고 명시한 문장은 확인되지 않았다** `≈`. 지원 종료일을 계약이나 감사 근거로 써야 하면 이 불확실성을 그대로 문서에 남기는 편이 안전하다 `Σ`.
...
성숙도 판정은 한 줄이다 — **프로젝트는 살아 있지만 기능 개발은 멈춰 있다.** 최근 1년의 사용자 노출 신기능은 1.6.40 의 `mg` 조건부 CAS 페치 하나이고, 1.6.45 는 33 커밋 전부가 한 사람이다 `✓`. 이것을 "안정" 으로 읽을지 "정체" 로 읽을지가 이 진영 선택의 실제 쟁점이다 `Σ`.
...
**표를 관통하는 규칙 하나.** 되돌릴 수 없는 전환은 전부 **버전 숫자 하나**(RDB 12 / RDB 80 / `ClusterMode: enabled`)로 표시되고, 그 숫자를 넘기 전에는 거의 모든 것이 되돌릴 수 있다 `Σ`. 그래서 마이그레이션 계획의 첫 항목은 방법 선택이 아니라 **"이번 창에서 넘는 편도 경계가 몇 개인가" 를 세는 것**이다 — 두 개 이상이면 창을 쪼갠다 `Σ`.

## 6. 고르지 않아도 되는 것

이 절은 앞 다섯 절을 부분적으로 무력화한다. **위 판단표가 실제로는 거의 중요하지 않은 케이스가 있고, 그 경우 이주 자체가 순손실이다** `Σ`.
...
여기서 나오는 마지막 판정이 이 문서의 결론이다. **엔진 선택은 라이선스·번들 모듈·관리형 지원 버전이 실제로 우리를 제약할 때만 결정으로서 존재하고, 그 셋 중 아무것도 걸리지 않으면 지금 쓰는 것을 계속 쓰는 것이 정답이다** `Σ`. 앞 7개 문서가 확정한 사실 중 이주를 강제하는 것은 딱 세 개다 — **8.0 의 EOL 2026-12-01**, **외부 제공·재배포가 로드맵에 있는 경우의 라이선스 경계**, 그리고 **관리형에서 7.1 위로 가려는 경우** `✓`.

## 7. 근거

이 문서는 새 1차 근거를 만들지 않는다. 모든 사실의 소유 문서는 다음과 같다.
...
**이 문서가 근거 없이 판정하지 않은 것** — 앞 문서들이 미확인으로 남긴 항목은 이 문서에서도 결론으로 쓰지 않았다.

- **성능 배수 비교** — memcached 와 Redis 계열을 같은 하드웨어에서 1:1 로 측정한 1차 출처가 없고([02]({{< relref "02-memcached/index.md" >}})), Redis·Valkey 의 수치는 전부 발행 주체가 당사자다(`Ⓥ`). 그래서 §2 의 성능 행은 판정을 **"비교 불가"** 로 적었고, 어느 엔진이 몇 배 빠르다는 서술은 넣지 않았다 `?`
- **Redis 8.2 유지보수 라인에 8.10 의 ACL 우회 수정(#15478)이 백포트됐는지** — 2026-07-23 일괄 패치 웨이브는 8.10.0(2026-07-29)보다 앞서므로 그 웨이브에 포함될 수 없다. 앞 문서에 이후 패치의 근거가 없어 §3.4·§4.1 에서 `?` 로 남겼다
- **Vector Set 의 GA 시점** — 릴리스노트에 beta 해제 선언이 없고 `redis.io` 문서끼리 불일치한다([04]({{< relref "04-redis-7-to-8.md" >}})). §3.5 는 "GA 선언 문장이 없다" 까지만 쓴다 `?`
- **Valkey 9.1 의 security 종료일** — 9.2 GA 후 "최신 minor 지위를 잃으면 5년을 되돌린다" 고 명시한 문장이 확인되지 않았다([05]({{< relref "05-valkey-8-to-9/index.md" >}})). §4.2 는 2031-05-19 를 전제로 쓰지 말라는 형태로만 판정했다 `≈`
- **Valkey 9.x → Redis 방향의 차단** — 앞 문서의 마이그레이션 표에 이 행이 없다. `VALKEY080` magic 과 foreign 예약 규칙에서 나오는 추론이라 §5 에서 `Σ` 로 표시했다
- **그룹사·계열사 간 유상 제공이 라이선스상 "third parties" 인지** — 원문으로 판정되지 않는다([03]({{< relref "03-license-and-fork.md" >}})). §3.3 표에서 `?` 로 남겼고, **이 케이스는 원문만으로 판정하지 말 것** 이라는 [03]({{< relref "03-license-and-fork.md" >}})의 경고를 그대로 승계한다
- **ElastiCache 의 `compatible` 상태 API 응답 · `enabled` 후 구 엔드포인트 삭제 여부 · config endpoint 이름의 동일성** — AWS 1차 문서에 없다([07]({{< relref "07-aws-endpoints/index.md" >}})). §5 의 CMD→CME 행은 "재조회 단계를 반드시 넣는다" 로만 처리했다 `?`
- **ElastiCache for Memcached / MemoryDB 의 날짜 박힌 EOL 캘린더, MemoryDB 의 Valkey 8/9 지원 여부** — 확인되지 않았다([02]({{< relref "02-memcached/index.md" >}})·[07]({{< relref "07-aws-endpoints/index.md" >}})). §2 의 관리형 지원 행은 "웹 확인 최신" 이라는 한정을 붙였다 `?`
- **AWS 가격 인하율의 현재성** — 2024-10-08 발표문 수치이고 2026-08 현재 동일한지 확인되지 않았다([03]({{< relref "03-license-and-fork.md" >}})·[07]({{< relref "07-aws-endpoints/index.md" >}})). 그래서 이 문서는 **가격을 판정 축에 넣지 않았다** `?`
- **클라이언트 라이브러리의 cluster 클래스명·최소 버전, valkey-glide 의 언어별 기능 격차** — 검증되지 않았다([06]({{< relref "06-cluster-mode/index.md" >}})). §2 의 클라이언트 생태계 행은 "이 축은 판정에 거의 기여하지 않는다" 로 처리했다 `?`
