...
세 줄이 서로 다른 축을 본다는 점이 중요합니다. 신규 채택은 **거버넌스**가, 기존 Redis 는 **RDB 버전**이, 순수 캐시는 **관리형 지원 버전**이 결정합니다 `Σ`.
...
**8.10 을 장기 지원으로 가정하면 안 됩니다** — 지원 표에 등재조차 되지 않았고 Standard/Extended 구분이 미정입니다 `✓`. 그래서 이 진영의 실제 선택은 **"5년(8.2) vs 최신 보안 표면(8.10)"** 이고, 둘이 충돌할 때 판정은 §3.4 의 마지막 줄에 있습니다 `Σ`.
...
**표를 관통하는 규칙 하나.** 되돌릴 수 없는 전환은 전부 **버전 숫자 하나**(RDB 12 / RDB 80 / `ClusterMode: enabled`)로 표시되고, 그 숫자를 넘기 전에는 거의 모든 것이 되돌릴 수 있습니다 `Σ`. 그래서 마이그레이션 계획의 첫 항목은 방법 선택이 아니라 **"이번 창에서 넘는 편도 경계가 몇 개인가" 를 세는 것**입니다 — 두 개 이상이면 창을 쪼갑니다 `Σ`.

## 6. 고르지 않아도 되는 것

이 절은 앞 다섯 절을 부분적으로 무력화합니다. **위 판단표가 실제로는 거의 중요하지 않은 케이스가 있고, 그 경우 이주 자체가 순손실입니다** `Σ`.
...
여기서 나오는 마지막 판정이 이 문서의 결론입니다. **엔진 선택은 라이선스·번들 모듈·관리형 지원 버전이 실제로 우리를 제약할 때만 결정으로서 존재하고, 그 셋 중 아무것도 걸리지 않으면 지금 쓰는 것을 계속 쓰는 것이 정답입니다** `Σ`. 앞 7개 문서가 확정한 사실 중 이주를 강제하는 것은 딱 세 개입니다 — **8.0 의 EOL 2026-12-01**, **외부 제공·재배포가 로드맵에 있는 경우의 라이선스 경계**, 그리고 **관리형에서 7.1 위로 가려는 경우** `✓`.
