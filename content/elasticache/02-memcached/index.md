...
Redis 쪽 경계는 `redis.conf` 원문이 직접 그어 줍니다 — "Redis is **mostly single threaded**, however there are certain threaded operations such as UNLINK, slow I/O accesses and other things that are performed on side threads"(`redis 8.10.0:redis.conf:1381-1383`), 그리고 io-threads 는 "threads for **reads and protocol parsing**" 까지입니다. 같은 파일이 권하는 스케일 방식도 "spawn multiple instances in order to scale more" 습니다 `✓` Redis 6.0 threaded I/O 의 내부 경계(fan-out/fan-in 배리어)는 [01 · 2009 첫 커밋부터 6.2 까지]({{< relref "../01-origins-and-design/index.md" >}})가, Valkey 8.0 이 그것을 어떻게 바꿨는지는 [05 · Valkey 8.0 → 9.1]({{< relref "../05-valkey-8-to-9/index.md" >}})가 소유합니다.
