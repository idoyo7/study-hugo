...
2024 사건을 "여러 번 바꾼 것 중 하나"로 읽으면 성격을 놓친다. **2018·2019·2022 의 변경은 전부 모듈에 대한 것이고, core 는 그때마다 BSD-3 로 남았다.** 이 구분이 이 절의 전부다.
...
그래서 **2018·2019 에 Redis core 를 걷어낸 배포판은 없다.** 2024 사건이 처음으로 배포판·클라우드·자체 호스팅을 동시에 때렸고, 충격의 크기는 조항의 강도보다 **이 세 문장의 파기**에서 나왔다 `Σ`. 이 점이 §5 의 AGPL 복귀가 왜 신뢰를 회복하지 못했는지를 미리 설명한다 — 옵션 추가는 "약속 하나 더"로 읽혔다.

한편 압박의 방향은 처음부터 명확했다. Commons Clause 도, RSAL 도, SSPL 도 겨냥한 것은 개별 사용자가 아니라 **소스를 가져가 관리형 서비스로 파는 클라우드 사업자**였다. 이 인센티브 구조는 §2 의 조항 원문에 그대로 드러나고, §3 에서 그 클라우드 사업자들이 포크의 후원자가 되는 이유이기도 하다.
...
이 커밋이 `COPYING`(BSD-3, 10줄)을 삭제하고 `LICENSE.txt`(733줄)와 `REDISCONTRIBUTIONS.txt`(30줄)를 만들고 `src/` 전체의 파일 헤더를 교체했다. `redis 7.4.0:LICENSE.txt` 의 첫 5줄이 선언이다 — "Starting on March 20th, 2024, Redis follows a dual-licensing model with all Redis project code contributions under version 7.4 and subsequent releases governed by the **Redis Software Grant and Contributor License Agreement**. After this date, contributions are subject to **the user's choice of** the Redis Source Available License v2 (RSALv2) or the Server Side Public License v1 (SSPLv1)". `✓`

공식 발표문의 논지는 두 겹이었다. 하나는 "여러 배포판을 동시에 유지하는 것이 Redis 를 미래로 끌고 가는 능력과 상충한다"는 것 — 발표문이 커뮤니티 주도 거버넌스 모델의 한계를 스스로 인정한 문장이다("Despite efforts to support a community-led governance model … delivering multiple software distributions simultaneously … is at odds with our ability to drive Redis successfully into the future"). 다른 하나는 **자기 판정**이었다. 발표문 FAQ 가 "Redis is no longer open source under the OSI definition" 이라고 직접 썼다 `✓`

{{< callout type="warning" >}}
**OSI 측 성명은 인용하지 않는다.** SSPL 이 OSI 승인 라이선스가 아니라는 사실의 근거는 Redis 자신의 FAQ 와 2025 년 AGPL 발표문("the Open Source Initiative clarified it lacks the requisites to be an OSI-approved license")이다. opensource.org 도메인의 2024년 Redis 관련 게시물은 **직접 확인하지 못했다** `?` — 그래서 이 문서는 "Redis 스스로 OSI 정의상 오픈소스가 아니라고 인정했다"는 형태로만 쓴다.
{{< /callout >}}

### 2.2 RSALv2 와 SSPLv1 이 각각 무엇을 금지하나 — 원문

"SSPL 로 바꿨다"는 서술은 정확하지 않다. 듀얼이고, **사용자가 고르며**, 두 라이선스의 작동 방식이 정반대다.
...
두 라이선스 어느 쪽도 **자체 호스팅과 사내 사용을 막지 않는다.** FAQ #20 이 직접 쓴다 — "Hosting the products for the internal use of your organization is permitted. An organization includes its affiliates and subsidiaries." `✓` 걸리는 것은 SaaS 사업자, 그리고 **배포판**이다 — DFSG·Fedora 정책이 "field of use" 차별을 통과시키지 않기 때문이다(§4).

### 2.3 비소급 — 포크가 합법일 수 있었던 이유

`0b3439692` 가 `COPYING` 을 지우면서 만든 `REDISCONTRIBUTIONS.txt` 는 "BSD 잔여물 고지" 파일이다. 8.10.0 판의 원문은 이렇다 — "Despite the shift to the dual-licensing model with version 7.4 (RSALv2 or SSPLv1) and the shift to a tri-license option with version 8.0 (RSALv2/SSPLv1/AGPLv3), **portions of Redis Open Source remain available subject to the BSD-3-Clause License**." 그 아래 BSD-3 전문이 붙는다 `✓`

이 파일이 존재하는 이유는 **과거 기여자들의 BSD 기여분에 라이선스 변경을 소급 적용할 수 없기 때문**이다. FAQ 도 "The license change is not retroactive"라고 명시한다. 즉 Valkey·Redict 가 relicense 이전 트리를 BSD-3 로 계속 개발한 것은 라이선스 위반이 아니고, 그 법적 근거를 원 저작권자 쪽 문서가 스스로 제공한다 `Σ`. 반대로 **신규 기여는 CLA(Redis Software Grant and Contributor License Agreement) 수락이 전제**다 — `redis 8.10.0:CONTRIBUTING.md` 가 CLA 전문을 그 자리에 싣는다 `✓`. §5 와 §6 이 이 한 줄로 갈린다.

### 2.4 왜 7.4 부터인가 — 그리고 7.2 BSD 라인은 아직 살아 있다

라이선스 커밋은 `unstable` 에 들어갔고, 그 `unstable` 이 릴리스로 나온 첫 버전이 **7.4.0(2024-07-29)** 이다. 7.2 브랜치는 BSD 로 남았다. FAQ #12 는 "Redis will continue to backport critical security patches … to existing versions under the 3-clause license **until Redis Community Edition 9.0 is released**"라고 했다 `✓`

그런데 **Redis 9.0 이 나오지 않았다.** 8.0(2025-05-02) 이후 8.2·8.4·8.6·8.8·8.10 짝수 마이너 케이던스로 갔고 9.x 는 태그·브랜치·마일스톤·공표 계획이 모두 없다(→ [Redis 7.0 → 8.10]({{< relref "04-redis-7-to-8.md" >}})). 결과적으로 그 약속이 만료 조건에 도달하지 못한 채 계속 이행되고 있다 — 2026-07-23 에 **6.2.23 과 7.2.15** 가 나왔고, **두 태그의 루트에는 여전히 `COPYING`(BSD-3)만 있다** `✓`
...
배포판의 반응은 §4 에서 다룬다. 여기서 한 줄만 미리 적으면, Fedora 는 F41 에서 redis 를 retire 하고 `valkey-compat` 이 `Obsoletes: redis` 로 자동 대체하게 했으며 그 사유를 명문화했다 — "Redis's shift to the Server Side Public License (SSPL) that Fedora does not allow poses an issue." `✓`
...
즉 Valkey 는 relicense **1시간 54분 전**의 트리를 들고 나갔다. 홍보 문구의 "continue development on Redis 7.2.4"는 라이선스적 사실이고, 코드적으로는 7.2.4 이후 2개월 반치 unstable 커밋을 포함한다 — Redis 7.4 의 기능 일부가 Valkey 에도 처음부터 들어 있었던 이유다 `Σ`

첫 Valkey 커밋은 `38632278f` "A single commit to get stuff building", **2024-03-22 02:00:46Z**, 저자 **Madelyn Olson** — 라이선스 커밋 기준 **+27시간 22분**이다 `✓`. Linux Foundation 발표는 그로부터 6일 뒤인 **2024-03-28** 이었고, 같은 날 Olson 이 TSC Chair 로 취임했다(`valkey 9.1.0:MAINTAINERS.md` — "Term: March 28, 2024 – Present") `✓`

LF 를 고른 이유는 발표 구조에 드러난다. 새 재단을 세우지 않고 기존 중립 재단 아래로 들어가면 상표·자산 귀속과 참여 규칙을 처음부터 단일 회사 밖에 둘 수 있다 `Σ`. 초기 참여로 발표된 곳은 **AWS · Google Cloud · Oracle · Ericsson · Snap Inc.** 다섯이다 `✓` — 다만 valkey.io/topics/history 는 초기 연합을 "Alibaba, Amazon, Ericsson, Google, Huawei, Tencent"로 다르게 서술한다. **어느 쪽이 "창설 참여"의 정본 정의인지는 확정하지 못했다** `?`

### 3.2 거버넌스 — 단일 벤더 모델과 갈리는 지점

`valkey 9.1.0:GOVERNANCE.md` 는 재라이선스를 막는 장치를 조문으로 둔다.
...
**이것이 이 문서의 논지가 서는 자리다.** Valkey 를 재라이선스하려면 거버넌스 문서 수정 = 2/3 super-majority 를 통과해야 하고, 그 TSC 는 한 조직이 1/3 을 넘을 수 없다. Redis 를 재라이선스하는 데 필요한 것은 회사 결정 하나였고, 실제로 그렇게 됐다. core team 은 2020-06-30 에 도입됐고, 그 core team 이 존재하는 상태에서도 라이선스 변경은 **회사 직원의 커밋 하나로** unstable 에 들어갔다 `✓`. AGPL 추가(§5)는 이 비대칭을 건드리지 않았다.
...
정확한 서술은 이것이다 — **기존 core team 의 일부가 자기 회사의 자원을 들고 중립 재단 아래로 옮겨갔다.** 그 자원에는 상업적 이해가 걸려 있었다. "벤더 하나가 만든 포크 대 벤더 하나가 소유한 원본"이라는 프레임은 양쪽 다 벤더가 걸려 있으므로 판별력이 없고, 실제 대비축은 **CLA + 단일 소유 vs DCO + LF + 1/3 상한 TSC** 다 `Σ`

라이선스 자체는 BSD-3 를 유지했다. `valkey 9.1.0:COPYING` 은 두 개의 BSD-3 를 병기한다 — License 1 "Copyright (c) 2024-present, Valkey contributors", License 2 "Copyright (c) 2006-2020, Redis Ltd." 두 번째가 상속받은 BSD 코드의 저작권 고지다 `✓`
...
두 가지가 읽힌다. 첫째, **포크가 여러 개 생겨 생태계가 갈렸다는 진단은 사실과 어긋난다** — 살아남은 것은 하나다. 둘째, 승부를 가른 것은 라이선스 선택이 아니라 **유지 인력**이다. Redict 는 세 포크 중 가장 강한 copyleft(LGPL-3.0-only)를 골랐지만 10개월간 커밋이 없고 보안 패치를 Valkey 에서 받아 쓴다 `Σ`

라이선스 리스크 회피만이 목적이라면 후보는 **Valkey(BSD-3)와 Garnet(MIT)** 뿐이다. Dragonfly 를 대안으로 세우는 것은 판정 기준을 스스로 무너뜨리는 선택이다.
...
**여기서 비대칭이 하나 드러난다.** AGPL 추가는 법적 자격 문제를 실제로 해결했다(Debian 이 증거). 그러나 **패키징 관성과 메인테이너 의지는 되돌려주지 않았다**(Fedora 가 증거) — 원 메인테이너 Remi Collet 본인이 "Redis may be proposed for unretirement … by me if I find enough motivation and energy, or by someone else" 라고 썼다 `✓`. 배포판 기본 패키지가 무엇인지가 대부분 팀의 실질 선택을 결정하므로, 이 비대칭이 채택을 계속 한쪽으로 민다 `Σ`

GitHub stars(2026-08-05: redis/redis 75,890 / valkey-io/valkey 26,747)는 15년 누적 대 2년 누적이므로 **채택 지표로 쓰면 안 된다** `✓`
...
AGPLv3 와 SSPLv1 은 겉보기 구조가 같다 — 둘 다 "네트워크로 제공하면 소스를 공개하라"는 조항을 13조에 둔다. 갈리는 지점은 **공개 범위**다.
...
즉 AGPL 은 "당신이 고친 그것"까지, SSPL 은 "당신 인프라 전체"까지를 요구한다. 후자가 OSI 정의를 통과하지 못하는 이유이고, 그래서 AGPL 옵션 추가만으로 배포판 판정이 뒤집힌다(§4.2 Debian).
...
Redis 8 GA 블로그는 실무 이유를 하나 더 준다 — "We heard from **some customers** that it is easier for them to operate under an OSI-approved license." `✓` antirez 본인의 논지는 조금 다르다. 그는 SSPL 이 닫힌 라이선스라고 보지 않지만 "the SSPL, in practical terms, **failed to be accepted by the community**. The OSI wouldn't accept it, nor would the software community regard the SSPL as an open license" 라고 썼다 `✓`

같은 릴리스에서 이름도 되돌렸다 — "Redis Community Edition" → **"Redis Open Source"** `✓`

### 5.2 그런데도 트라이로 남긴 이유, 그리고 CLA

`redis 8.10.0:LICENSE.txt` 첫 문단이 현재 상태다 — "moving to a **tri-licensing** model … contributions are subject to **your choice of**: (a) RSALv2; or (b) SSPLv1; or (c) AGPLv3." `✓`

{{< callout type="warning" >}}
**"Redis 8 은 AGPL 이 됐다"는 서술은 틀리다.** AGPLv3 는 **추가**됐을 뿐 RSALv2/SSPLv1 은 그대로다. 트라이 라이선스는 **사용자 선택**이므로, 회사는 어느 쪽 고객도 잃지 않는다. 조직 중에는 AGPL 의 §13 때문에 AGPL 을 사내 정책상 못 쓰는 곳이 있다 — 2018-08 글이 이미 "the use of AGPL was against their company's policy"라고 언급했던 그 조직들이다. 이런 조직은 RSALv2 를 고르고, 배포판·OSI 요구가 있는 쪽은 AGPLv3 를 고른다 `Σ`
{{< /callout >}}

그리고 **CLA 는 그대로다.** FAQ #24 가 "acceptance of the contributor license agreement (CLA) by the contributor is necessary" 라고 명시하고, `redis 8.10.0:CONTRIBUTING.md` 는 여전히 CLA 전문을 싣는다 `✓`. 이것의 함의는 단순하다 — **8.10 의 코드도 회사가 원하면 옵션을 다시 뺄 수 있는 구조에 있다.** 2025-05 에 바뀐 것은 라이선스 옵션의 개수이고, 바뀌지 않은 것은 그 개수를 정하는 주체다.

부수 사실 하나. **Redis 8.0.0 이 실제로 실은 라이선스 본문은 AGPLv3 가 아니라 GPLv3 였다.** `8.0.0:LICENSE.txt` 의 13조가 "Use with the GNU Affero General Public License"(= GPLv3 의 13조)이고, AGPLv3 의 13조인 "Remote Network Interaction" 은 `8.2.0:LICENSE.txt` 에서야 나타난다. 수정 커밋은 `266835659`(2025-05-06, #14010) `✓`

## 6. 그래서 포크는 봉합됐나

안 됐다. 근거 넷이다.

**첫째, 대응 자체가 없었다.** valkey.io 블로그 전량(2024-04-12~2026-07-17)을 확인했고 2025-05 의 AGPL 발표에 대응하는 글은 **존재하지 않는다** `✓`. The Register 인터뷰(2025-05-15)는 "We spoke to Olson **before** Redis announced that it would be switching to the AGPL" 이라고 명시한다 — 즉 공식 반박 성명이 없었고, 무대응이 답이었다 `Σ`. 대신 2025-09-16 블로그가 자기 정의를 반복한다. "Valkey was founded just over a year ago to keep high-performance key/value storage in the open source community: **free from vendor lock-in and restrictive licenses**." `✓`

**둘째, 개발 활동이 돌아오지 않았다.** 로컬 클론 직접 계측(`--all` 기준)이다.
...
{{< callout type="important" >}}
**이 표를 배수로 읽지 마라.** `redis/redis` 는 core 만이고, Redis 8 이후 Query Engine·JSON·TimeSeries·Bloom 은 별 repo 다 — Redis 진영이 **과소평가**돼 있다. Valkey 도 valkey-search·valkey-json·valkey-bloom·valkey-glide 를 뺐으니 같은 방향으로 과소평가다. **방향성만** 유효하다. 교차검증인 Percona 분석(2025-12-05, fork 직전 4개월 Redis 기여자 24명 중 9명 이탈 / Valkey 18→49명)은 방향이 일치하지만 **Percona 는 Valkey 참여사**이므로 편향 가능성을 감안한다 `≈`
{{< /callout >}}

**셋째, 커맨드가 실제로 갈라졌다.** `src/commands/*.json` 파일 목록을 직접 diff 했다 — valkey 9.1.0 = 426개, redis 8.10.0 = 460개(core 만), **Valkey 전용 18 / Redis 전용 52** `✓`
...
결정적 사례는 **atomic slot migration** 이다. 양쪽이 같은 기능을 독립적으로, 반대 방향으로 구현했다 — Valkey 9.0.0(2025-10-21)은 `CLUSTER MIGRATESLOTS`(source 에서 push), Redis 8.4.0(2025-11-18)은 `CLUSTER MIGRATION IMPORT`(destination 에서 pull)다. **자동화 스크립트가 서로 호환되지 않는다.** 동작 비교는 [cluster mode]({{< relref "06-cluster-mode/index.md" >}})가 소유한다 `✓`

여전히 같은 것은 **RESP 프로토콜**이다. Harbor 는 교체 후 "same RESP protocol as Redis; **no application-level RESP changes were required**"라고 보고했고, Valkey 8.0 릴리스노트는 "fully compatible with Redis OSS 7.2.4"를 명시하며 `extended-redis-compatibility` 설정도 9.1.1 까지 살아 있다(`valkey 9.1.1:src/config.c:3320` — `MODIFIABLE_CONFIG`) `✓`. 즉 **드롭인 교체는 7.2 기능 집합 안에서만 참**이다 — 7.4 이후 어느 한쪽의 신규 커맨드에 애플리케이션이 의존하는 순간 락인이 시작된다. 버전별 신기능은 [Redis 7.0 → 8.10]({{< relref "04-redis-7-to-8.md" >}})과 [Valkey 8.0 → 9.1]({{< relref "05-valkey-8-to-9/index.md" >}})이 소유한다.

**넷째, 보안 패치 흐름이 역전된 구간이 있다.** Debian bug **#1136392**(2026-05-13, 신고자는 Debian redis 메인테이너 Aron Xu)가 사례다. Debian 의 redis 패키지가 CVE-2026-21863 패치를 **Valkey 커밋에서 그대로 복사해** 붙였는데, Valkey 는 그 검사를 전용 함수 `clusterIsValidPacket()`(0=invalid, 1=valid)에 두었고 Redis 는 `clusterProcessPacket()` 인라인(0=link freed, 1=link alive)에 두었다. **반환값 의미가 반대라서** 원 CVE 는 막았지만 새 remote DoS 를 만들었고 `5:8.0.6-2` 에서 수정됐다 `✓`. Redict 가 Valkey 에서 CVE 를 백포트하는 것(§4.1)과 합치면 이 계보의 보안 유지 중심이 어디인지가 보이고, 동시에 **두 코드베이스가 패치를 기계적으로 이식할 수 없을 만큼 갈라졌다**는 것도 보인다 `Σ`

다만 **법적 적대 상태는 아니다.** 2024-08-14 에 Valkey 에 들어간 커밋 `4d284daef` "Copyright update to reflect IP transfer from salvatore to Redis (#740)" 의 저자는 **Pieter Cailliau** — 5개월 전 relicense 커밋을 넣은 그 사람이다 `✓`. 소송이 제기됐다는 근거는 찾지 못했다(적극적으로 부재를 확인한 것은 아니다 `?`). Valkey 가 `redisServer` → `valkeyServer` 등 전면 개칭을 초기에 서둘러 한 것은 트레이드마크 때문이다. 2024-03-20 FAQ #22 가 "You can no longer use 'Redis' or 'for Redis' in your product name" 라고 명시했다 `✓`

## 7. 운영자에게 무엇이 남았나

{{< callout type="warning" >}}
**이 절은 법률 자문이 아니다.** 아래는 라이선스 원문과 발행 주체의 FAQ 를 읽은 결과이고, 실제 판단은 사용 형태·계약 관계·법인 구조에 따라 달라진다. 재배포나 외부 제공이 걸리는 순간 법무 확인을 거쳐야 한다.
{{< /callout >}}
...
**사내에서 캐시로 쓰는 것은 위 어느 라이선스에서도 걸리지 않는다.** RSALv2·SSPLv1 발표 FAQ #20 이 직접 쓴다 — "Hosting the products for the internal use of your organization is permitted. **An organization includes its affiliates and subsidiaries.**" 자체 호스팅, k8s 위 self-host, 관리형 서비스 이용, 사내 애플리케이션의 세션·캐시·큐 용도는 모두 허용 범위다 `✓`
...
버전 선택으로 이 문제를 피하려는 경우 두 함정이 있다. 하나는 **7.2.x 가 형식상 살아 있어도 2023-08-15 이후 기능이 동결된 상태**라는 것이고(패치의 실제 상류가 어디인지는 §6 넷째 항목), 다른 하나는 **RDB 포맷이 마이그레이션을 단방향으로 막는다**는 것이다 — Redis 7.4+ (RDB 12)는 Valkey 8.x 로 넘어가지 못한다. 차단선 전체는 [무엇을 고를 것인가]({{< relref "08-choosing.md" >}})와 [Valkey 8.0 → 9.1]({{< relref "05-valkey-8-to-9/index.md" >}})이 소유한다.

마지막으로 이 문서의 논지를 운영 언어로 옮기면 이렇게 된다. **라이선스는 스냅샷이고 거버넌스는 그 스냅샷이 얼마나 오래 유효한지에 대한 확률이다.** 2026-08 시점에 Redis 8.x 는 AGPLv3 옵션으로 OSI 승인 라이선스이고 사내 캐시 용도로는 아무 문제가 없다. 다만 그 옵션을 유지하거나 회수하는 결정은 단일 회사에 있다. 그 회사가 CLA 로 권리를 모아 두었기 때문이다. Valkey 쪽은 같은 결정을 2/3 super-majority 와 1/3 조직 상한을 통과해야 내릴 수 있다. 재배포·외부 제공이 로드맵에 있는 조직이라면 판단 근거를 현재 라이선스 문자열이 아니라 이 구조 차이에 두는 편이 안전하다 `Σ`
