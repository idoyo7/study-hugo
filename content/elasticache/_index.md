...
- **Redis 에 9 는 없다.** 태그·숫자 브랜치·마일스톤이 모두 없고 8.10 GA 이후에도 `unstable` 은 `8.9.241` 이다. 9.x 를 가진 쪽은 **Valkey**(9.0.0 = 2025-10-21)이고 홀짝 케이던스가 정반대다 `✓` → [04]({{< relref "04-redis-7-to-8.md" >}})
- **2026-08-06 기준 최신** — Redis **8.10.0**(2026-07-29) · Valkey **9.1.1**(2026-07-21) · memcached **1.6.45**(2026-07-09) `✓`
- **포크는 봉합되지 않았다.** 2025-05 에 Redis 가 AGPLv3 를 추가해 OSI 승인 라이선스로 돌아왔는데도 Valkey 는 돌아가지 않았다 — 바뀐 것은 라이선스 옵션의 개수이고 바뀌지 않은 것은 그 개수를 정하는 주체(CLA)다 `✓` → [03]({{< relref "03-license-and-fork.md" >}})
- **마이그레이션은 RDB 숫자가 정한다.** Redis 7.4+ (RDB 12)는 Valkey 8.x 가 거부하고, Valkey 9.x(RDB 80 / `VALKEY080`)는 8.x 로 되돌아가지 못하고, Redis 8.6·8.8·8.10 은 릴리스마다 올라가 롤백 창이 닫혔다 `✓`
- **양 진영 모두 `io-threads` 기본값은 1(비활성)** 이고 Redis 는 8.10.0 까지 `IMMUTABLE_CONFIG` 다. 벤더의 "2배"·"1.19M rps" 는 전부 8 스레드 조건이다 `Ⓥ`
...
> 릴리스노트는 "무엇이 추가됐다"까지만 말하고, **"왜 이렇게 생겼고 · 지금 무엇을 골라야 하나"** 는 말해주지 않는다. 인메모리 데이터스토어에서는 그 간격이 특히 크다 — 2009년에 "값은 blob 이 아니라 자료구조다"라고 정한 결정이 단일 스레드를 불렀고, 단일 스레드가 원자성을 공짜로 줬고, 그 대가를 17년째 갚고 있다. 그 사이 라이선스가 한 번 바뀌면서 엔진이 둘로 갈라졌고, 두 진영은 같은 기능을 다른 이름·반대 방향으로 구현하기 시작했다. 이 챕터는 세 축으로 정리한다 — **왜 이렇게 생겼나(설계 연쇄) · 왜 갈라졌나(거버넌스) · 지금 무엇을 고르나(판단)**.

이 챕터는 인메모리 데이터스토어를 한자리에 모은 **버킷**이다. 디렉토리 이름이 `elasticache` 인 것은 우리가 실제로 굴리는 형태에서 따온 것이고, 8개 문서 중 AWS 를 다루는 것은 [07]({{< relref "07-aws-endpoints/index.md" >}}) 하나뿐이다. 나머지는 엔진 자체의 역사·설계·포크·cluster 다.

## 왜 이걸 정리하는가

질문은 여섯 개였다 — Redis 가 첫 커밋부터 버전업하며 어떻게 발전해왔나, memcached 와는 무엇이 다르고 memcached 자체는 어떻게 변했나, Valkey 는 왜 찢어졌나, 7·8·9 에 무엇이 추가되나, cluster mode 는 어떤 구조인가, AWS 에서 엔드포인트가 어떻게 바뀌나.

그런데 **네 번째 질문은 전제부터 틀려 있다. Redis 에 9 는 없다.** `redis/redis` 에 `9*` 태그가 하나도 없고, 숫자 브랜치는 `8.10` 이 끝이다. 마일스톤 목록에도 `9.0` 이 없고, 8.10 GA(2026-07-29) 이후에도 `unstable` 의 `src/version.h` 는 여전히 `8.9.241` 이다 `✓`. 9.x 를 가진 쪽은 **Valkey** 다(9.0.0 = 2025-10-21). 게다가 두 진영의 버전 규칙이 정반대다. Redis 의 홀수 마이너는 스킵이 아니다. **프리릴리스 전용 번호**다(8.10-RC1 = `8.9.240`). Valkey 는 홀수 마이너를 **정식 GA** 로 쓴다(8.1 · 9.1) `✓`.

이 한 가지 사실이 나머지 질문의 성격을 바꾼다. 버전을 물었을 때 실제로 답해야 하는 것은 "9 에 무엇이 들어오나"가 아니라 **"지금 8.x 안에서 무엇을 고르나, 아니면 진영을 옮기나"** 다. 그리고 진영을 옮기는 판단은 기능 목록으로 서지 않는다. RDB 버전 숫자가 마이그레이션을 단방향으로 막는다(Redis 7.4+ 의 RDB 12 는 Valkey 8.x 가 거부한다). 같은 이름의 기능이 반대 방향으로 구현돼 자동화 스크립트가 호환되지 않고, 관리형에서 만나는 버전은 업스트림과 번호부터 다르다(ElastiCache 의 Redis OSS 는 **7.1 에서 멈췄다** — ElastiCache 에 Redis 8 은 없다) `✓`.

그래서 이 챕터는 각 문서에서 세 가지를 같은 형식으로 묻는다 — **무엇이 언제 들어왔나, 그 대가로 무엇이 막혔나, 우리 워크로드에서 득인가 실인가.** 원 질문과 문서의 대응은 이렇다.
...
**근거 배지.** 이 챕터의 모든 문서가 문장 끝에 근거의 종류를 표시한다 — `✓` 1차 출처(태그별 소스·릴리스노트·라이선스 원문)로 직접 확인 · `≈` 계산·추정 · `Ⓥ` 벤더·프로젝트 자체 주장(검증 안 됨, **측정 조건을 반드시 병기**) · `Ⓑ` 퍼블릭·3자 벤치마크 · `?` 미확인 · `Σ` 여러 사실을 묶은 종합 판단. 소스 인용은 `redis 8.10.0:src/rdb.h:21` 처럼 **레포:태그:경로:줄** 형식이고, 릴리스일은 GitHub `published_at` 기준이다 — `git log -1 <tag>` 의 author date 를 릴리스일로 쓰면 Redis 7.4 이후 구간에서 최대 이틀 어긋난다(태그 자체는 6.2.0 이후 전부 lightweight 이고, 벌어지는 것은 author date 와 committer date 다) `✓`.

## 참고 — 세 프로젝트 타임라인

결정적 사건만 남겼다. 릴리스일은 GitHub `published_at` 기준이고 시각이 걸린 사건은 UTC 다 `✓`.
...
타임라인의 마지막 행을 판단에 쓸 수 있는 형태로 접으면 이렇다. 근거는 각 문서가 소유하고, 여기서는 어느 문서로 가야 하는지만 표시한다.
...
`✓` (전 항목 태그별 소스·릴리스노트·라이선스 원문·AWS 문서 확인. 관리형 지원 버전은 발표문 기준이고 실계정으로 교차검증하지 않았다 `?`)

세 열을 나란히 놓으면 보이는 것이 셋이다. **memcached 는 2003년에 멈춘 소프트웨어가 아니다** — 최신 릴리스가 2026-07 이고, segmented LRU·extstore·meta·proxy·slab mover 재작성이 전부 Redis 등장 이후다. **Redis 의 라이선스 변경은 2018년부터 다섯 번 있었지만 core 가 넘어간 것은 2024-03-20 하루뿐이다** — 2018-08 · 2019-02 · 2022-11 은 전부 모듈에만 적용됐고, 2025-05 는 core 에 AGPLv3 를 **더한** 것이다. 그리고 **포크 이후 두 열의 항목이 서로를 참조하지 않는 방향으로 벌어진다** — 8.1 의 hashtable 교체와 8.0 의 Stack 번들은 되돌릴 수 있는 종류의 차이가 아니다 `Σ`.
...
- **9 를 기다리는 것은 근거 없는 유예다.** Redis 에 `9*` 태그·숫자 브랜치·마일스톤이 없고 `unstable` 은 8.10 GA 이후에도 `8.9.241` 이다. 9.x 를 가진 쪽은 Valkey 이고, 두 진영의 홀짝 케이던스는 정반대다 — 같은 숫자가 전혀 다른 의미를 갖는다. → [04]({{< relref "04-redis-7-to-8.md" >}}), [05]({{< relref "05-valkey-8-to-9/index.md" >}})
- **RDB 버전 숫자 하나가 롤백과 마이그레이션 계획 전체를 결정한다.** Redis 는 7.4~8.4 가 모두 12 였다가 8.6=13 · 8.8=14 · 8.10=15 로 매 릴리스 올라가 롤백 창이 닫혔고, Redis 7.4+ (RDB 12)는 Valkey 8.x 가 foreign 으로 예약해 거부하고, Valkey 9.x(RDB 80 / `VALKEY080`)는 8.x 로 되돌릴 수 없다. "가볍게 올려보고 안 되면 내리자"가 성립하는 구간이 어디까지인지를 먼저 확인한다. → [04]({{< relref "04-redis-7-to-8.md" >}}), [05]({{< relref "05-valkey-8-to-9/index.md" >}})
- **켜지 않은 기능은 빨라지지 않는다.** 양 진영 모두 `io-threads` 기본값이 1(비활성)이고 Redis 는 8.10.0 까지 `IMMUTABLE_CONFIG` 라 재시작 없이는 켜지지도 않는다. Valkey 의 dual channel replication 도 기본 `no` 이고, prefetch 배치는 `io-threads` 가 꺼져 있으면 무의미하다. 벤더의 "2배"·"1.19M rps" 는 전부 8 스레드 조건의 수치이므로 **업그레이드만으로는 얻지 못한다** — Redis 8.0 의 "2x ops/sec" 은 `io-threads=8` + multi-core Intel 에서 최대 +112% 이고, Valkey 8.0 의 1.19M rps 는 c7g.16xlarge(64 vCPU) + `io-threads 8` + 3M keys × 512B + 650 clients + sequential SET 이다(파이프라인 깊이는 어느 1차 출처에도 없다) `Ⓥ`. → [01]({{< relref "01-origins-and-design/index.md" >}}), [05]({{< relref "05-valkey-8-to-9/index.md" >}})
- **라이선스는 스냅샷이고 거버넌스는 그 스냅샷이 얼마나 오래 유효한지에 대한 확률이다.** 2025-05 에 Redis 가 AGPLv3 를 추가해 OSI 승인 라이선스로 돌아왔는데도 Valkey 는 돌아가지 않았다 — AGPL 은 **추가**였을 뿐 RSALv2/SSPLv1 철회가 아니고 CLA 도 그대로다. 판단 근거를 현재 라이선스 문자열이 아니라 "그 문자열을 누가 바꿀 수 있나"에 두어야 하는 이유다. → [03]({{< relref "03-license-and-fork.md" >}})
- **cluster mode 를 켠다는 것은 샤딩을 켜는 것이 아니라 애플리케이션 계약을 바꾸는 것이다.** cross-slot 다중 키 연산·트랜잭션·Lua 키 선언·pub/sub 전파 비용·`KEYS`/`SCAN`/`DBSIZE` 의 의미·커넥션 수가 모두 달라지는데 **standalone 테스트는 그대로 통과한다.** 그리고 AWS 에서는 그 구조가 엔드포인트로 노출되므로, 모드 전환은 문자열·클라이언트 라이브러리·다중 키 연산 3종 세트를 같이 바꾸는 작업이고 **되돌릴 수 없다.** → [06]({{< relref "06-cluster-mode/index.md" >}}), [07]({{< relref "07-aws-endpoints/index.md" >}})
- **"같은 기능"이 이름과 방향이 달라 자동화가 깨진다.** atomic slot migration 은 특정 진영 전용이 아니다. 양쪽에 다 있고 방향만 반대다 — Valkey 9.0.0 `CLUSTER MIGRATESLOTS`(source 에서 push) vs Redis 8.4.0 `CLUSTER MIGRATION IMPORT`(destination 에서 pull). fast full sync 도 capa 문자열이 달라 섞으면 조용히 레거시 경로로 폴백하고, `CLUSTER SLOTS` 의 deprecation 상태는 진영별로 정반대다. **리샤딩 런북과 모니터링 쿼리는 엔진을 갈아탈 때 재작성 대상이다.** → [06]({{< relref "06-cluster-mode/index.md" >}}), [05]({{< relref "05-valkey-8-to-9/index.md" >}})
- **memcached 는 다른 축을 끝까지 민 결과다. 열등한 과거가 아니다.** 값을 해석하지 않기로 한 대가로 스레드를 열어 한 프로세스가 N 코어를 먹는다. 느린 커맨드가 존재할 수 없어 지연이 예측 가능하고, 값만 NVMe 로 내려 용량을 산다. 대신 자료구조·다중 키 원자성·복제·영속성·ACL 이 전부 없다. **캐시 이외의 요구가 하나도 없을 때 이기고, 그 조건이 하나라도 깨지면 진다.** → [02]({{< relref "02-memcached/index.md" >}}), [08]({{< relref "08-choosing.md" >}})
