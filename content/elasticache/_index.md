...
- **RDB 버전 숫자 하나가 롤백과 마이그레이션 계획 전체를 결정한다.** Redis 는 7.4~8.4 가 모두 12 였다가 8.6=13 · 8.8=14 · 8.10=15 로 매 릴리스 올라가 롤백 창이 닫혔다. Redis 7.4+ (RDB 12)는 Valkey 8.x 가 foreign 으로 예약해 거부하고, Valkey 9.x(RDB 80 / `VALKEY080`)는 8.x 로 되돌릴 수 없다. "가볍게 올려보고 안 되면 내리자"가 성립하는 구간이 어디까지인지를 먼저 확인한다. → [04]({{< relref "04-redis-7-to-8.md" >}}), [05]({{< relref "05-valkey-8-to-9/index.md" >}})
...
- **라이선스는 스냅샷이고 거버넌스는 그 스냅샷이 얼마나 오래 유효할지의 확률이다.** 2025-05 에 Redis 가 AGPLv3 를 추가해 OSI 승인 라이선스로 돌아왔는데도 Valkey 는 돌아가지 않았다 — AGPL 은 **추가**였을 뿐 RSALv2/SSPLv1 철회가 아니고 CLA 도 그대로다. 판단 근거를 현재 라이선스 문자열이 아니라 "그 문자열을 누가 바꿀 수 있나"에 두어야 하는 이유다. → [03]({{< relref "03-license-and-fork.md" >}})
