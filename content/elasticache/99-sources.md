...
redis.io 의 기술 문서·블로그(라이선스·거버넌스 관련은 아래 `라이선스·포크` 절로 분리).
...
valkey.io 의 기술 문서·블로그(라이선스·포크 서사 관련은 아래 절로 분리).
...
redis/redis · valkey-io · memcached 조직의 이슈·PR·커밋 중 URL 로 직접 인용된 것.
