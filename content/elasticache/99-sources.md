...
redis.io 의 기술 문서·블로그(라이선스·거버넌스 주제는 아래 `라이선스·포크` 절로 분리).
...
valkey.io 의 기술 문서·블로그(라이선스·포크 서사는 아래 절로 분리).
...
memcached.org·docs.memcached.org 와 메인테이너 dormando 의 공식 채널.
...
- GitHub wiki `ReleaseNotes`(1.4 중반 이후 도입 버전의 정본 — `ChangeLog.txt` 는 2009-04-10 에서 끊깁니다) — [github.com/memcached/memcached/wiki/ReleaseNotes](https://github.com/memcached/memcached/wiki/ReleaseNotes)
...
AWS What's New 공지와 ElastiCache/MemoryDB 개발자 가이드·API·CLI 레퍼런스.
...
redis/redis · valkey-io · memcached 조직의 이슈·PR·커밋 중 URL 로 직접 인용한 것입니다.
