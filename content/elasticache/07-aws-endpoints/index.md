...
MemoryDB 는 엔드포인트가 `cluster endpoint` 하나이고 *"You need to connect to the cluster endpoint to discover node endpoints using cluster nodes or cluster slots command"* 입니다 — 즉 구조적으로 CME 와 같습니다 `✓` 노드별 엔드포인트는 *"not necessary for normal usage"* 로 명시됩니다 `✓` (문서가 "MemoryDB 는 항상 cluster mode 입니다"라고 직접 쓴 문장은 찾지 못했습니다. `ClusterMode`/`cluster-enabled` 에 해당하는 API 파라미터가 없다는 **간접 근거**뿐입니다 `?`)

판단에 직접 영향을 주는 변화는 2026-06 에 왔습니다. ElastiCache 문서의 서비스 비교 항목이 *"With durability enabled, ElastiCache for Valkey can also serve as a durable datastore"* 로 바뀌고, MemoryDB 권고가 **"You should consider using MemoryDB if your workload requires multi-Region active-active replication with conflict-free data types (CRDTs). For single-Region durable workloads, consider using ElastiCache with durability enabled."** 로 좁아졌습니다 `✓` 즉 **"내구성이 필요하면 MemoryDB" 라는 2021~2025 년의 판단 근거는 더 이상 AWS 자신의 권고가 아닙니다.** 엔진 버전 격차도 실무 요소입니다 — MemoryDB 문서의 엔진 버전 목록은 7.3(2024-12-01) / Valkey 7.2.6 까지인데 ElastiCache 는 Valkey 9.1 까지 갑니다 `✓` (2025~2026 년에 MemoryDB 용 Valkey 8/9 발표가 있었는지는 확인하지 못했습니다 `?`)
...
CME 는 *"a single configuration endpoint. By connecting to the configuration endpoint, your application is able to discover the primary and read endpoints for each shard in the cluster"* 입니다 `✓` 그리고 클라이언트 요구사항이 문서에 조건으로 박혀 있습니다 — *"You must use a client that supports either Valkey Cluster or Redis OSS Cluster."* `✓`
...
여기에 구조적 제약이 얹힙니다. CMD 는 샤드 1개 + replica 0\~5 이고, CME 는 샤드 1\~500(엔진 5.0.6 이상, 그 이하는 250) + 샤드당 replica 0\~5입니다 `✓` Multi-AZ 는 CMD 에서 "Optional" 이지만 CME 에서는 **"Required"** 이고 기본 켜짐입니다 `✓` 데이터베이스도 갈립니다 — CME 는 DB 0 만 쓸 수 있었고, **ElastiCache Valkey 9.0(2026-05-05)부터 cluster mode 에서 `SELECT 0`–`15` 가 열렸습니다** `✓`
...
엔드포인트를 손대야 하는 스케일링은 **offline 경로 하나뿐**입니다.
...
**지원 버전이 선택을 사실상 결정합니다.** ElastiCache 의 Valkey 는 9.1 / 9.0 / 8.2 / 8.1 / 8.0 / 7.2.6 이고, Redis OSS 는 *"all Redis OSS versions 7.1 and before"* 입니다 — 즉 **ElastiCache 에 Redis 8.x 는 없습니다.** 7.1 위로 가려면 Valkey 로만 갑니다 `✓` 그리고 AWS 의 버전 번호는 업스트림과 1:1 이 아닙니다 — *"ElastiCache v8.2 is compatible with Valkey v8.1"*, *"ElastiCache v7.1 is compatible with Redis OSS v7.0"* `✓` 업스트림 릴리스 이력과 기능 대응은 [Valkey 8.0 → 9.1]({{< relref "../05-valkey-8-to-9/index.md" >}})이 소유합니다.
...
**포트는 바뀌지 않습니다.** 문서의 모든 TLS 예시가 `-p 6379` 를 씁니다. Serverless 의 6380 은 TLS 때문이 아니라 read-optimized 엔드포인트입니다 `✓`
...
MemoryDB 는 더 넓습니다 — 위 목록에 **`acl deluser`/`acl load`/`acl save`/`acl setuser`** · **`cluster addslot`/`cluster delslot`/`cluster setslot`** · **`module`** · `psync` 가 추가됩니다 `✓` ACL 을 커맨드로 못 만지게 하고 AWS API 로만 관리하도록 강제한다는 뜻입니다.
...
- **확인하지 못한 것**: (a) 실계정 `aws elasticache describe-*` 로 DNS 패턴을 교차검증하지 않았다 — 표의 모든 문자열은 문서 예시입니다. (b) CME configuration endpoint 의 prefix 형(`clustercfg.<name>.…`) 예시는 ElastiCache 문서에 없다(MemoryDB 문서에만 있습니다). (c) TLS 켠 CMD primary 의 접두사가 `master.` 인지 `primary.` 인지 문서 간 불일치가 있고, TLS 켠 reader 의 접두사는 어느 문서에도 없습니다. (d) `compatible` 상태의 API 응답 예시, `enabled` 후 구 엔드포인트의 삭제 여부·시점, config endpoint 이름의 동일성은 전부 문서에 없습니다. (e) MemoryDB 의 Valkey 8/9 지원 여부와 Serverless 의 9.1 추가 여부는 확인하지 못했습니다. (f) Global Datastore(크로스리전 복제)의 엔드포인트 구조는 이번 범위에서 다루지 못했습니다.
