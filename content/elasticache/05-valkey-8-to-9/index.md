...
마이그레이션 호환성은 한 줄로 끝난다 — **Redis 7.2.x 이하에서만 RDB·복제로 넘어올 수 있고, Redis 7.4 이상과 Valkey 9.x → 8.x 는 소스 레벨에서 막혀 있다** `✓`. 상세는 §9.

버전 번호 읽는 법도 진영별로 반대다. **Valkey 는 홀수 마이너(8.1, 9.1)를 정식 GA 로 쓴다** — Redis 의 홀수 마이너는 프리릴리스 전용 번호다([04 · Redis 7.0 → 8.10]({{< relref "../04-redis-7-to-8.md" >}})) `✓`. "Redis 9" 를 찾는 사람은 사실 Valkey 9 를 보고 있다.

## 2. 포크 직후의 선택 — 7.2.4 에서 무엇을 먼저 손댔나

Valkey 리포의 `7.2.4` 태그는 Redis 7.2.4 히스토리를 그대로 승계한 지점이다. 라이선스 변경 커밋·거버넌스·Linux Foundation 발표의 시간선은 [03 · 왜 찢어졌나]({{< relref "../03-license-and-fork.md" >}}) 가 소유한다. 여기서 볼 것은 **그 다음 5개월 동안 무엇을 먼저 건드렸는가**다.
...
리브랜딩 커밋(`pidfile` `redis.pid` → `valkey.pid`, `syslog-ident`, 에러 메시지의 "Redis" 제거)은 같은 기간에 함께 들어갔지만 **신설 파일 4개가 전부 성능·전송 계층**이다 `✓`. 이 선택이 이후 2년의 분기를 결정했다.

호환성 쪽으로는 반대 방향의 스위치가 하나 있다. `extended-redis-compatibility` — Valkey 가 자신을 "Redis" 로 보고하게 만드는 임시 노브다. 8.1 의 `valkey.conf` 는 "9.0 에서 무효화, 10.0 에서 제거" 를 예고했지만 **그 예고는 실행되지 않았다** — 9.0 의 conf 에서 문구가 "will be removed in a future version" 으로 완화되고 9.1.1 의 `config.c:3320` 에도 `MODIFIABLE_CONFIG` 로 살아 있다 `✓`. 이 스위치에 의존하는 툴은 여전히 시한폭탄이다.

## 3. 8.0 (2024-09-15) — 스레딩·복제·메모리가 한 릴리스에서 갈라진다

### 3.1 비동기 I/O 스레딩 — 6.0 방식의 개선이 아니라 교체

Redis 6.0 threaded I/O 가 어디까지만 했고 왜 "안 켜는 게 낫다" 가 상식이 됐는지는 [01 · 2009 첫 커밋부터 6.2 까지]({{< relref "../01-origins-and-design/index.md" >}}) 가 소유한다. 8.0 이 바꾼 것만 본다.
...
세 번째로 넘긴 작업이 이 릴리스의 성격을 말한다. PR #763 이 추가한 것은 **poll offload**(`aeEventLoop` 에 `custompoll` 콜백 신설 — PR 본문은 poll-wait 가 메인 스레드 시간의 최대 30% 를 먹는다고 측정했다), **command lookup offload**(워커가 파싱하면서 커맨드 dict 조회까지 하고 `c->io_parsed_cmd` 에 넣는다 — 메인 런타임의 약 5%), **free offload**(argv 를 할당한 스레드에게 되돌려 free 시킨다 — 근거는 jemalloc 의 thread-local `tcache`)다 `✓`. 이름은 "I/O 스레드" 인데 하는 일은 이미 I/O 가 아니다.

PR #861 은 그 결과로 드러난 새 병목을 잡는다. async I/O 도입 후 `lookupKey` 가 메인 스레드 시간의 약 50%(SET 기준)를 차지하게 됐고, 해법은 배치 prefetch 다 — 실행 준비된 커맨드를 최대 16개 모아 argv → dict entry → value 순으로 prefetch 한 뒤 실행한다(`prefetch-batch-max-size`, 기본 16, 0 이면 비활성, 최대 128) `✓`. **이 최적화는 배치를 I/O 스레드가 만들어주므로 `io-threads` 를 켜지 않으면 사실상 작동하지 않는다.**
...
공식 수치는 **360K → 1.19M rps, 평균 레이턴시 1.792ms → 0.542ms(-69.8%)** 다 `Ⓥ`. 측정 조건은 **AWS EC2 c7g.16xlarge(64 vCPU) · `io-threads 8` · 3M keys · value 512바이트 · 650 clients · sequential SET** 이고, 블로그가 스스로 "these numbers include the Prefetch change" 라고 밝힌다 — #758 + #763 + #861 **합산치**이며 I/O 스레딩 단독 효과가 아니다. 파이프라인 깊이는 어떤 1차 출처에도 없다 `?`. 4 vCPU 인스턴스에서 재현되는 숫자가 아니고, `valkey.conf` 가 직접 경고하듯 벤치마크 클라이언트도 `--threads` 로 맞춰야 한다.

같은 문제에 Redis 8.0 은 **다른 답**을 냈다. 두 구현의 튜닝 가이드는 서로 통하지 않는다 `✓`.
...
관측성은 Redis 쪽이 낫다. Valkey 에서는 스레드별 부하 편중을 서버에서 볼 수 없다 — 9.1 이 active IO threads / main thread 사용률 누적 metric(#2463, #2931)을 붙이면서 겨우 완화된다 `✓`.

### 3.2 dual channel replication — 무엇이 아팠고, 그 아픔을 어디로 옮겼나

full sync 중 primary 는 RDB 를 보내는 동안 들어오는 쓰기를 **replica 별 client output buffer(COB)** 에 쌓는다. RDB 전송이 길어지면 두 가지가 동시에 나빠진다 — primary 메모리가 그만큼 부담을 받고, COB 가 `client-output-buffer-limit replica` 하드 리밋을 넘으면 primary 가 **replica 연결을 끊어서 복제 자체가 실패**한다. 큰 데이터셋에서 full sync 가 반복 실패하는 고전적 실패 모드가 이것이다 `✓`.

{{< seq src="_seq/3-2-dual-channel-replication.json" />}}

부수 효과가 성능의 실체다. 기존에는 TLS 제약 때문에 bgsave 자식이 RDB 바이트를 **파이프로 메인 프로세스에 넘기고 메인이 소켓에 다시 써야** 했다. 전용 커넥션이 생기면서 자식이 소켓에 직접 쓰게 되고, primary 메인 프로세스의 CPU 가 그만큼 풀린다 `✓`.

PR #60 자체 측정은 이렇다 `Ⓥ` — primary/replica 같은 머신, RDB 3.7GB, `valkey-benchmark -r 100000 -n 6000000 lpush my_list __rand_int__`. 클라이언트 50개 이하 경량 커맨드에서 sync 중 write 레이턴시 **5~7.5% 개선**, primary 가 `sdiff`/`sunion` 같은 무거운 읽기를 처리하는 상황에서는 **sync 시간 약 50% 단축**과 그에 따른 복제 diff 저장 메모리 **일부 케이스 60%+ 감소**. 즉 이 기능은 "복제가 빨라진다" 가 아니라 **"바쁜 primary 에서 full sync 가 덜 망가진다"** 쪽이다.
...
세 번째가 핵심 함정이다. Redis 는 같은 자리에 전용 리밋 `replica-full-sync-buffer-limit` 을 뒀다 — 이 항목만은 Redis 쪽 설계가 낫다 `Σ`. 그리고 두 진영의 와이어가 다르다.
...
Redis PR #13732 "Rdb channel replication" 은 본문 첫 줄에 `valkey-io/valkey#60` 을 명시 참조한다 — 설계를 가져다 재구현했지만 **capa 문자열이 서로에게 unrecognized 라 섞으면 조용히 레거시 단일 채널로 폴백**한다 `✓`. 모니터링 대시보드·알림 쿼리도 필드명이 달라 진영을 옮기면 재작성해야 한다.

### 3.3 메모리 효율 — 절감의 3분의 2는 Valkey 고유 기여가 아니다

8.0 의 절감은 두 층이다. 공식 블로그 측정 조건은 **1 primary + 2 replica 단일 샤드, `valkey-benchmark` 로 만든 6,318,941 키, value 16바이트**다 `Ⓥ`.
...
첫 행에 함정이 있다. `src/kvstore.c` 는 커밋 `8cd62f82`(2024-02-05, Redis #12822 "Refactor the per-slot dict-array db.c into a new kvstore data structure")에서 생겼고 이건 **포크 이전 Redis unstable 커밋**이다 — `git merge-base --is-ancestor` 로 valkey 8.0.0 과 redis 7.4.0 **양쪽의 조상**임이 확인된다 `✓`. Valkey 8.0 릴리스노트도 이 항목을 `Redis#12822` 로 표기한다. "Valkey 가 per-slot dict 로 클러스터 메모리를 줄였다" 는 서술은 절반만 맞다.

**엔진 측에서 per-slot 으로 쪼갠 이유**는 세 가지다 — 슬롯 소속 추적 연결리스트를 자료구조 자체로 대체해 엔트리당 16바이트를 없애고, 슬롯 단위 순회를 O(슬롯 크기)로 만들고, 해시 태그가 단일 슬롯을 함의하는 `KEYS`/`SCAN` 을 그 슬롯만 훑게 한다. 대가는 노드당 약 1MB 의 Binary Indexed Tree(슬롯별 키 개수 누적합)다 `✓`. **cluster 운영에서 무엇이 달라지는지는** [06 · cluster mode]({{< relref "../06-cluster-mode/index.md" >}}) 가 소유한다.

그리고 16바이트 절감은 **cluster mode 에서만** 발생한다 — standalone 에서 8.0 으로 올리면서 -20% 를 기대하면 틀린다 `Σ`.

Redis 도 같은 목표로 수렴했다. PR #13806 은 본문에 "This PR adopts Valkey's packing layout and logic for key, value, and TTL" 를 명시하고 `kvobj` 를 도입했다(8.2, 1M keys 77.34M→59.87M · 10M keys 883.98M→624.07M, 약 -29% `Ⓥ` 로컬 랩톱). **다만 hashtable 은 갈아끼우지 않았다** — 같은 PR 본문이 open addressing POC 를 해보고 `dict` + `no_value=1` 이 더 나은 균형이었다고 적는다 `✓`. 이 갈림이 8.1 에서 결정적으로 벌어진다.

## 4. 8.1 (2025-03-31) — dict 를 버린 릴리스

### 4.1 새 hashtable

PR #1186 은 `src/hashtable.c` 를 새로 써서 `src/dict.c`(chained hash)를 대체하고, `kvstore` 의 백엔드를 통째로 옮겼다. 버킷이 **정확히 64바이트 = 캐시라인 1개**다 — `presence` 비트 7개 + 해시 상위 1바이트 7개 + 엔트리 포인터 7개 `✓`(`valkey 8.1.0:src/hashtable.h:91` 의 `#define HASHTABLE_BUCKET_SIZE 64`, `src/hashtable.c:275-283` 의 `bucket` 구조체 + `static_assert`).
...
절감은 PR 본문 "roughly 20 bytes per key for short string keys", 공식 블로그는 **TTL 없는 key-value 당 약 20바이트, TTL 있으면 최대 30바이트**다 `Ⓥ`. 적용 범위는 keyspace + expires(#1186) → hash(#1502) → set(#1176) → sorted set(#1427) → command lookup 이고, 소스로는 `valkey 8.1.0:src/kvstore.h` 가 `#include "hashtable.h"` 로 바뀌고 모든 API 가 `kvstoreDict*` → `kvstoreHashtable*` 로 개명된 것으로 확인된다 `✓`.

공식 8.1 GA 블로그의 성능 주장은 항목이 많다 `Ⓥ` — 파이프라인 처리량 8.0 대비 약 +10%, iterator prefetch 로 키 순회 3.5배, `ZRANK` +45%, `PFMERGE`/`PFCOUNT` 12배, `BITCOUNT` 최대 +514%, TLS full sync +18%, fork CoW 오버헤드 -47%, TLS 연결 수락률 +300%, `SET` +10% / `GET` +22%. **각 항목의 인스턴스·데이터셋 조건이 개별 공개되지 않았다** `?`. 그리고 `PFMERGE`/`PFCOUNT` 는 AVX, `BITCOUNT` 는 AVX2 의존이므로 **Graviton 같은 ARM 에서는 이 수치가 나오지 않을 가능성이 크다** `≈` — Graviton 인스턴스로 표준화한 환경이라면 이 두 항목은 계획에서 빼는 게 맞다.

운영 관점의 결론은 단순하다. 8.0 → 8.1 은 **설정 변경 없이 재시작만으로 키당 20~30바이트를 회수**하는 업그레이드다. 대신 `MEMORY USAGE`·`INFO memory` 의 오버헤드 항목 절대값이 바뀌므로 메모리 알림 임계값을 재보정해야 한다 `Σ`.

hash 타입도 별도로 최적화됐다 — PR #1579 는 hashtable-encoded hash 에서 field 와 value 를 2단 레이아웃으로 저장한다. 합쳐서 128바이트 이하면 **한 번의 할당에 임베딩**하고, 더 크면 value 만 따로 할당한다. 레이아웃 구분은 sds 헤더의 미사용 비트에 인코딩한다 `✓`.
...
성능 주장은 8.0 RC1 블로그의 처리량 **최대 +275%** 하나인데 **테스트 조건이 명시되지 않았다**(하드웨어인지 RXE 에뮬레이션인지, 페이로드도 없다) `Ⓥ`/`?`. conf 가 스스로 "언제든 제거 가능" 이라 적은 기능이므로 프로덕션 전제로 삼을 수 없다.

의미는 성능보다 구조에 있다. **Redis 8.10.0 트리에는 `rdma.c` 가 없다** `✓`. PR #1209 는 `bind` 설정 추상화와 `closeListener` 커넥션 타입 메서드를 새로 도입하며 본문에 "Even for QUIC in the future" 를 적는다 — 두 진영의 커넥션 추상화 계층이 다르게 진화했다는 뜻이다.

### 4.3 8.1 의 그 외 — 리브랜딩이 아니라 재구조화

`src/` 파일 목록 diff 가 성격을 드러낸다 `✓`.
...
`COMMANDLOG`(#1294)는 slowlog 의 일반화다 — slow execution + **large request** + **large reply** 3종. `slowlog-max-len`/`slowlog-log-slower-than` 은 `commandlog-slow-execution-max-len`/`commandlog-execution-slower-than` 의 별칭으로 남고, 신설 임계값 `commandlog-request-larger-than`·`commandlog-reply-larger-than`(둘 다 기본 1MB)과 `commandlog-large-request-max-len`·`commandlog-large-reply-max-len`(둘 다 기본 128)이 붙는다 `✓`. Redis 에는 대응물이 없다.

마이그레이션 도구를 위한 장치도 8.1 이다. `import-mode yes` + `CLIENT IMPORT-SOURCE ON`(#1185)은 redis-shake 같은 동기 툴을 쓸 때 destination 에서 expire/evict 가 데이터를 깨뜨리는 문제를 막는다 — import-source 로 표시한 클라이언트의 커맨드만 예외 처리하고 그 외의 만료·축출을 정지시킨다 `✓`. §9 에서 다시 쓴다.

기본값·동작 변경 중 운영에 걸리는 것들. 실제로 가장 흔한 경로가 7.2 에서 8.1 로 한 번에 올리는 것이므로 8.0 항목을 함께 놓고 **각 행에 도입 릴리스를 붙인다** `✓`.
...
세 개 RC(2025-08-14 / 09-23 / 10-08)를 거쳤다. 릴리스노트 기준 신기능 전수는 이렇다 `✓`.
...
블로그 수치는 BITCOUNT/HLL SIMD 최대 200%, zero-copy 최대 20%, 파이프라이닝 최적화 최대 40%, 그리고 "2,000노드 클러스터에서 초당 10억 요청 이상" 이다 `Ⓥ`. **넷 다 측정 조건이 공개되지 않았다** `?` — 앞의 세 항목은 인스턴스·페이로드·클라이언트 수가 어디에도 없고, 특히 SIMD 항목은 구현이 x86 SIMD(#1741)와 ARM NEON(#1859·#1867)으로 나뉘어 있으므로 CPU 아키텍처를 고정하지 않으면 재현 대상조차 특정되지 않는다. 마지막 것은 하드웨어·방법론이 아예 없으므로 단일 인스턴스 수치와 섞으면 안 된다. 넷 다 용량 계획의 입력값으로 쓸 수 없다 `Σ`.

### 5.1 HFE 는 Redis API 호환이지만 동작이 다르다

API 를 일부러 그대로 복사해 클라이언트 호환성을 유지했지만, PR #2089 이 명시한 설계 결정 때문에 동작이 갈린다 `✓`.
...
메모리 쪽 갭도 열려 있다 — 작은 hash 에서 listpack 인코딩을 유지할 수 없어 hashtable 로 강제 전환된다(이슈 #2618, 9.2 계획에 "closes the 9.0 HFE gap" 으로 등재) `✓`. **그리고 9.0.2(urgency HIGH)의 버그 수정 17건 중 9건이 HFE 관련이다** — 9.0.0/9.0.1 에서 필드 TTL 을 프로덕션에 쓰는 것은 위험하다. 최소 9.0.2, 실무적으로는 9.0.5/9.1.1 이다 `Σ`.

Redis 는 같은 기능을 **7.4.0(2024-07-29)** 에 냈다 — Valkey 가 약 15개월 늦었고 동작은 다르다([04 · Redis 7.0 → 8.10]({{< relref "../04-redis-7-to-8.md" >}})) `✓`.

### 5.2 breaking change — 실제로는 매우 보수적이다

9.0.0 릴리스노트에 **"Breaking changes" 섹션이 없다** `✓`. behavior change 로 분류된 것은 3건이다 — auth 체크를 command exist/arity 검사보다 **앞으로** 이동(#1475, 미인증 클라이언트가 받는 에러 종류가 바뀐다), MULTI/EXEC 내부 에러 메시지에 command fullname 포함(#2286), `SCRIPT EXISTS`/`SHOW`/`FLUSH` 에 `STALE` 플래그 추가(#2419). **커맨드 제거 0건, 설정 제거 0건**이고 오히려 #2546 이 **25개 커맨드의 deprecation 을 되돌렸다**.

실질적 breaking 은 **RDB_VERSION 11 → 80** 하나다. `src/rdb.h` 의 주석이 의도를 그대로 적는다 — "RDB 12-79 are reserved for Redis non-compatible RDB formats. We start using high rdb version numbers since Valkey 9.0. This is in order to avoid collisions with non-OSS Redis RDB versions." `rdbUseValkeyMagic(rdbver)` 는 `rdbver > 79` 일 때 true 이므로 **9.0 부터 파일 magic 이 `VALKEY`** 다 `✓`.

다운그레이드 관점의 결론: 8.1.x 로더는 `rdb-version-check strict`(기본)에서 "Can't handle RDB format version 80" 으로 거부하고, `relaxed` 로 두면 버전 체크는 통과하지만 9.0 이 쓴 **mandatory opcode `RDB_OPCODE_SLOT_IMPORT`(243)** 나 HFE 인코딩을 만나면 로드가 중단된다. **9.0 → 8.x 다운그레이드와 9.0 primary → 8.x replica 의 full sync 는 신뢰할 수 없다** `✓`. 업그레이드는 replica 부터 잡고, 롤백 계획은 "RDB 되돌리기" 가 아니라 **"8.x 스냅샷 보관"** 으로 세워야 한다 `Σ`.

### 5.3 빌드 요구사항

"9.0 에서 CMake 로 전환했다" 는 틀렸다 — **CMake 는 8.1.0 부터** 있고 9.x 는 Makefile 과 CMake 를 **둘 다** 유지한다. `cmake_minimum_required(VERSION 3.10)` 은 8.1.0 / 9.0.0 / 9.1.1 이 동일하다 `✓`. 9.0 의 실제 의존성 변화는 **`hiredis` → `libvalkey`** 다(`deps/` 목록 diff). 9.1 의 변화는 §6 에 있다.
...
블로그 종합 수치는 **단일 서버 초당 210만 요청**이고 조건은 **512B payload · 9 IO threads · pipeline depth 10** 이다 `Ⓥ`.

**릴리스노트를 그대로 읽으면 틀리는 항목이 하나 있다.** 9.1.0-rc1 은 "Prevent invalid TLS certificates from being loaded"(#2999)를 신기능으로 실었지만, rc2 에서 **"as it is a breaking change, deferred to next major version" 으로 revert 됐다**(#3572). GA 릴리스노트에 rc1 목록이 그대로 남아 있어 오독하기 쉽다 — **9.1.0/9.1.1 에는 이 동작이 없다** `✓`.

9.1 의 그 밖의 breaking 성 변경은 넷이다 — `events-per-io-thread` 소멸(설정 파일 경로에는 shim 이 있어 조용히 무시된다. `CONFIG SET` 경로는 미검증 `?`), 모듈 API `VM_ACLCheckCommandPermissions()` 지원 중단, **querybuf CRLF 엄격 검사**(#2872 — 그동안 통과했던 malformed RESP 가 protocol error 가 된다. 9.0.5 에도 백포트됐다), `COMMAND INFO`(RESP3)에서 subcommand 없는 커맨드의 `subcommands` 필드가 Set → **Array**(#3939) `✓`.

빌드는 방향이 갈렸다. **서버는 순수 C 가 됐고, 유닛 테스트는 C++ 가 필요해졌다** `✓` — `deps/fast_float`(C++ 헤더) + `fast_float_c_interface` 가 `deps/fast_float/ffc.h`(C99 포트, #3329)로 대체되고 `src/Makefile` 의 `USE_FAST_FLOAT` 조건 블록 자체가 사라졌다. 반대로 `deps/gtest-parallel` 이 추가되고 CMake 옵션이 `BUILD_UNIT_TESTS` → **`BUILD_UNIT_GTESTS`** 로 바뀌었다. 선택적 빌드 옵션은 `BUILD_TLS=yes|module`, `BUILD_RDMA=yes|module`, `BUILD_LUA=static|module|no`, `USE_SYSTEMD=yes`, `USE_LIBBACKTRACE=yes`(#3034) 이고 **Sentinel 모드는 TLS module 빌드를 지원하지 않는다**.
...
취약 범위는 `<= 9.1.0, <= 9.0.4, <= 8.1.8, <= 8.0.9, <= 7.2.13` 이고 패치 버전 문자열은 두 advisory 가 동일하다. **`RESTORE` 를 애플리케이션에 허용했거나 TLS 를 쓰는 배포는 즉시 올려야 한다** `Σ`.

CVE 출처가 두 갈래라는 점도 알아야 한다. Valkey 자체 GHSA 는 총 10건뿐이고, CVE-2026-23479 / 25243 / 23631 과 CVE-2025-46817 / 46818 / 46819 는 **Redis 가 GHSA 를 발행하고 Valkey 는 릴리스노트로만 고지**했다 — **"valkey GHSA 목록에 없다" 가 "Valkey 는 영향 없다" 를 뜻하지 않는다.** 정본은 릴리스노트의 Security fixes 섹션이다 `✓`.
...
지원 정책은 문서화돼 있고 **"LTS" 라는 라벨은 쓰지 않는다** — **maintenance support 3년** + **각 major 의 "최신 minor" 에만 extended security support 5년** 이다 `✓`.
...
표를 읽는 법: security 종료가 maintenance 종료와 같은 줄(9.0, 8.0)은 **그 major 의 최신 minor 가 아니기 때문**이다. 7.2 는 7.x 의 유일·최신 minor 라 5년을 받는다. 7.2 의 기산일이 포크 기점 7.2.4(2024-01-09)가 아니라 **Valkey 가 처음 발행한 7.2 릴리스(7.2.5)의 2024-04-16** 인 점도 주의한다 `✓`.

여기서 유도되는 위험이 하나 있다. **9.2.0 GA(목표 2026-11-15, 이슈 #4218)가 나오면 9.1 이 "최신 minor" 지위를 잃는다.** 규칙을 그대로 적용하면 9.1 의 security 종료가 2031-05-19 → 2029-05-19 로 당겨진다 — 다만 공식 문서에 "최신 minor 지위를 잃으면 5년을 되돌린다" 고 명시한 문장은 확인되지 않았다 `≈`/`?`. **"9.1 은 2031년까지 안전하다" 를 계획의 전제로 삼지 않는 편이 맞다** `Σ`.

케이던스는 "안정 major 를 연 1회, minor 는 필요에 따라 최소 연 1회" 이고, 버저닝이 보호하는 API 계약을 7개로 명시한다 — 커맨드, Lua 에서 실행 가능한 함수·API, **RDB 버전**, **replication 프로토콜**, **cluster node 프로토콜**, Module API, AOF 디스크 포맷 `✓`. 9.0 이 RDB 를 80 으로 올린 것은 이 계약을 major 에서 깬 정당한 행사였다는 뜻이다.

## 8. Redis 8.x 대비 기능 매트릭스 — Valkey 관점

기준은 **Valkey 9.1.1(2026-07-21)** vs **Redis Open Source 8.10.0(2026-07-29)**. Redis 쪽 릴리스별 서사는 [04 · Redis 7.0 → 8.10]({{< relref "../04-redis-7-to-8.md" >}}) 가 소유하므로, 여기서는 **Valkey 를 고를 때 얻는 것과 포기하는 것**만 본다.
...
번들 모듈이 없다는 사실에는 **cluster 운영 쪽 부작용**이 하나 붙는다. Valkey 서버는 로드된 모듈 중 하나라도 `VALKEYMODULE_OPTIONS_HANDLE_ATOMIC_SLOT_MIGRATION` 을 선언하지 않으면 `CLUSTER MIGRATESLOTS` 를 거부하는데, **공식 4개 모듈 전부가 이 플래그를 선언하지 않는다** — `valkey-bundle` 이미지처럼 모듈을 얹은 배포에서는 atomic slot migration 을 못 쓴다 `✓`. 판정 기준과 대안은 [06 · cluster mode]({{< relref "../06-cluster-mode/index.md" >}}) 로 넘긴다.

**같은 기능, 다른 이름·시점** — 혼동이 실제로 사고를 만드는 지점이다 `✓`.
...
선택 기준은 성능이나 커맨드 개수가 아니다 — **(1) 라이선스, (2) 번들 모듈 필요 여부, (3) cluster 운영 도구** 셋이 결정한다 `Σ`. 검색·벡터·JSON·시계열을 배포 하나로 끝내야 하면 Redis 8.x 고, BSD-3 유지·multi-DB cluster·DB 단위 ACL·MPTCP/RDMA 가 필요하면 Valkey 9.x 다. 다만 Valkey 의 모듈은 **코어와 별도 케이던스**라는 점을 감안해야 한다 — `valkey-json` 은 2025-09-08 의 1.0.2 이후 11개월째 새 릴리스가 없다. 리포는 2026-08-01 까지 push 되고 archived 도 아니므로 `✓` 릴리스 케이던스가 느린 것으로 읽는 게 맞다 `Σ`.

## 9. Redis → Valkey 마이그레이션 사실관계

전부 RDB 버전 숫자 하나로 결정된다. 아래 표가 이 절의 전부다 `✓`.
...
거부 로직은 소스에 그대로 있다 — `valkey 8.1.0:src/rdb.h:53-59` 의 `RDB_FOREIGN_VERSION_MIN 12` / `MAX 79`, 그리고 `src/rdb.c:3038-3053` 의 로더가 `rdbver >= RDB_FOREIGN_VERSION_MIN && !is_valkey_magic` 이면 `Can't handle RDB format version %d` 를 남기고 실패한다 `✓`. **`DUMP`/`RESTORE`·`MIGRATE` 도 같은 규칙에 걸린다** — `verifyDumpPayload`(`valkey 8.1.0:src/cluster.c:155-179`)가 같은 검사를 하고, **DUMP 페이로드에는 magic string 이 없고 RDB 버전 숫자만 있으므로 foreign 범위는 무조건 거부**된다.

`rdb-version-check`(8.1 신설, 기본 **`strict`**)를 `relaxed` 로 바꿔도 **foreign 범위는 받지 않는다.** relaxed 는 Valkey 9.x RDB 를 8.1 이 읽어보게 하려고 만든 장치이고, 미래 RDB 를 알 수 없는 opcode 를 만날 때까지 best-effort 파싱하므로 **부분 로드 후 실패**가 가능하다 — 프로덕션은 strict 를 유지하는 게 맞다 `Σ`.

**복제로 붙이기.** Redis **7.2 이하 primary ← Valkey 8.x/9.x replica** 는 된다. 공식 문서의 절차는 Redis 의 host/port 확인 → Valkey 에서 `REPLICAOF <host> <port>` → `INFO REPLICATION` 으로 동기 확인 → 애플리케이션 전환 → Redis 종료 → Valkey 에서 `REPLICAOF NO ONE` 승격이고, 사실상 무중단 컷오버다 `✓`. **Redis 7.4+ primary 에는 붙지 않는다** — full sync 가 RDB 를 태워 보내므로 위 표의 두 번째 행에 그대로 걸린다. 다만 이 실패가 핸드셰이크 어느 단계에서 어떤 메시지로 나타나는지는 실행으로 확인되지 않았다 `?`(코드 경로 추론).

공식 문서의 호환 범위 표현은 **"Valkey reads and writes RDB and AOF files compatible with Redis OSS 7.2. RDB files produced by Redis CE 7.4 and later are not compatible."** 다 `✓`. AOF 도 같은 경계로 보는 게 안전하다 — AOF 는 base RDB 를 품으므로 RDB 버전 검사에 걸릴 것으로 추정되지만 MP-AOF manifest 레벨의 실패 양상은 소스로 추적되지 않았다 `≈`. 파일 복사 방식에는 문서가 명시한 함정도 있다 — **"If you enabled AOF in your Valkey configuration, disable it on the first start. Otherwise, the copied RDB file will not be imported into Valkey."**

**그래서 소스 Redis 의 마이너 버전이 경로를 완전히 갈라놓는다.** 7.4 이상이면 RDB·복제·`DUMP`/`RESTORE` 세 경로가 모두 막히므로 남는 것은 (a) 애플리케이션 이중 쓰기, (b) 논리적 재적재(키를 읽어 타입별 커맨드로 재생성)뿐이고, 실질적으로 **(b) + `import-mode yes` + `CLIENT IMPORT-SOURCE ON`** 조합이 유일한 안전 경로다 `Σ`. import-mode 는 동기화 중 destination 의 expire/evict 가 데이터를 깨뜨리는 것을 막고, import-source 로 표시한 클라이언트에서 오는 커맨드만 예외 처리한다.

AWS 환경의 엔진 전환(ElastiCache 의 Redis → Valkey, 엔드포인트가 바뀌는가)은 [07 · AWS 엔드포인트]({{< relref "../07-aws-endpoints/index.md" >}}) 가 소유한다.
