...
프로덕션에서 서비스 클러스터는 상태를 갖지 않습니다. 이 시리즈는 그 패턴을 집 두 곳에 걸친 2-클러스터 홈랩에서 구현한 기록입니다. 사설망도 VPN도 없이 공인망 하나로만 이어진, 이 패턴에 가장 불리한 조건에서 무엇이 성립하고 무엇이 대가로 남는지를 평면별로 나눠 다룹니다.
