...
클러스터 안쪽은 일부러 한 덩어리로만 그렸습니다. code-server를 클러스터 안에 넣지 않은 대신 클러스터는 도메인·TLS·인증이라는 관문 일만 합니다. 이 글에서 중요한 건 클러스터 내부 구성이 아니라 "관문을 지나면 호스트의 터미널이 있고, 그 터미널의 파일은 NAS에 있다"는 사실입니다.
