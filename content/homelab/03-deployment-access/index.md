...
edge가 상태를 갖지 않는다는 말은 뒤집으면 edge의 전부가 git에 있다는 뜻입니다. 그래야 통째로 날아가도 다시 세울 수 있으니까요. 그러니 이 평면에서는 코드가 클러스터에 닿는 경로에 사람이 끼어드는 지점이 없는지, 그리고 그 배포 시스템에 사람이 어떻게 들어오는지를 확인합니다.
...
두 클러스터는 같은 커밋을 각자 당겨갑니다. hub를 거쳐 edge로 흘러가는 게 아닙니다. hub가 죽어 있어도 edge는 GitHub만 보이면 배포됩니다. 중앙에 상태를 모으는 구조에서 중앙이 배포 경로까지 쥐면 단일 장애점이 하나 더 늘어나는데, 그건 피했습니다.
...
대가도 있습니다. hub Keycloak이 죽으면 edge ArgoCD에 로그인할 수 없습니다. 그때도 edge의 배포 자체는 멈추지 않습니다. apps-root가 GitHub에서 직접 당겨오니까요. 사람이 콘솔에 못 들어갈 뿐 클러스터는 계속 수렴합니다. 중앙에 몰아도 되는 것과 안 되는 것의 경계가 여기 있습니다.
