...
이 페이지는 [운영 로드맵]({{< relref "../00-operating-hyperdx.md" >}}) 5부(규모 산정)를 실체화한 것이다. 산식 유도·민감도 밴드·대조 시나리오 전문은 [용량 산정]({{< relref "../07-capacity-planning.md" >}})이 정본이며, TTL 정책 자체의 정본은 [S3 콜드 티어링]({{< relref "../03-s3-cold-tiering.md" >}})·[데이터 티어링]({{< relref "02-tiering.md" >}})이다. 이 페이지는 "얼마나 쌓이고 얼마나 드는가"라는 캐파 판단에 필요한 표·산식·결론만 압축한다.
...
TTL 정책([S3 콜드 티어링]({{< relref "../03-s3-cold-tiering.md" >}}) 정본)의 핵심: `hyperdx_sessions`는 hot(gp3)만, **S3로 내리지 않고** 30일 후 **DELETE**. `otel_logs`/`otel_traces`는 hot 14일 → S3 → 지평별 DELETE. `otel_metrics_*`는 hot 30일 → S3 → 지평별 DELETE.
