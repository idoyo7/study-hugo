...
- **첫 갈림길은 "0.7TB가 raw ingest냐 on-disk(압축 후)냐"** — 이 해석에 배포 규모·비용이 2~3배 갈립니다. 본 페이지는 **on-disk 해석 B를 1차 모델**로, raw 해석 A를 대조로 싣고, 배포 후 `system.parts` **1회 실측**으로 확정하게 합니다. `≈`
- **on-disk 구성은 세션 리플레이가 ~78%를 지배**(리플레이는 압축이 잘 안 됨). 그런데 리플레이는 **hot 30일만 두고 S3로 안 내리고 DELETE**({{< relref "03-s3-cold-tiering.md" >}} 기준 문서) → **리플레이는 누적되지 않습니다.** 이게 캐파의 단일 최대 지렛대입니다.
- **그래서 "0.7TB×12=8.4TB" 순진한 누적은 틀리다.** 1년 실제 누적(단일사본)은 **~2.35TB** — 차이 ~6TB가 전부 "안 쌓이는 리플레이"다. `≈`
- **hot·컴퓨트는 지평 무관 고정**(hot gp3 ~2TB, 2× r7g.2xlarge). 3→12개월 증분은 **거의 전부 싼 S3 cold**(서울 ×RF2 기준 $19→$86/mo). `≈`
- **1 shard × RF2 로 1년+ 충분**, gp3로 충분(io2 불필요), Keeper 3노드·MongoDB 3멤버는 데이터량과 무관하게 소형 고정. prod 월 **~$1.1~1.2K**(서울 `ap-northeast-2`, on-demand), RF3 시 ~$1.7K. `≈`
{{< /callout >}}
...
사용자 입력 "prod 세션 샘플링 100%, 월 0.7TB"는 **어느 지점의 바이트인지 불명**합니다. 캐파의 첫 갈림길이므로 두 해석을 모두 못박습니다.
...
여기서 핵심은 **on-disk 상에서는 리플레이 비중이 raw보다 더 커집니다**는 점입니다 — 리플레이는 압축이 안 되니까:
...
- **cold S3만 지평 따라 증가** → 1년까지 늘려도 추가 비용은 대부분 **서울 $0.025/GB-월**의 S3 Standard다(단가 정본은 {{< relref "02-hot-storage-ebs.md" >}} §1.3). 백업은 리플레이 제외(가치 급감) 후 로그/트레이스/메트릭만 Glacier IR.
...
> **인스턴스 EBS 대역이 실질 병목**: gp3 볼륨 스펙(최대 2,000 MiB/s)보다 **인스턴스의 EBS 파이프 상한이 먼저 천장**을 친다 — r7g.2xlarge의 EBS 대역이 볼륨 스펙보다 낮으므로 노드 사이즈업이 볼륨 프로비저닝보다 먼저 효과를 낸다({{< relref "../../clickhouse/02-storage-local-nvme.md" >}}) `✓`. 이 스케일(피크 8MB/s)에선 둘 다 여유라 무관하지만, 성장 시 이 순서를 기억합니다.
...
*1yr Savings Plan은 컴퓨트에만 ~40% 적용, 스토리지·S3·전송은 정가 `≈`. 백업·전송 열은 리전 계수를 얹은 값이고, PUT은 건수를 원 산정과 동일하게 두고 단가만 서울로 바꿔 us-east-1 대비 −10%다 `≈`.
...
> **읽는 법**: hot gp3·컴퓨트·Keeper·Mongo는 **지평 무관 고정 ~$1.0K**. 3→12개월 확장 비용은 **거의 전부 S3 cold**(×RF2 기준 $19→$86 증가). **긴 보관이 싼 이유**는 EBS가 아니라 S3에 쌓이고, 심지어 볼륨 지배자인 리플레이는 30일에 잘리기 때문이다. Datadog RUM으로 같은 워크로드(≈월 22M 세션)를 태우면 [RUM 문서]({{< relref "../../rum/_index.md" >}}) 기준 블렌디드 ~$0.42/1k → **연 수만~십수만 $**대인데, self-host는 월 ~$1.1~1.2K(연 ~$14K) — 수 배~10배 절감(단 people TCO 별도, {{< relref "../../clickhouse/01-managed-vs-selfhosted.md" >}}).
>
...
해석 A면 **매우 작습니다** — 2× 소형 노드 + Keeper 3 + Mongo 3로 충분. hot이 수백 GB라 gp3 볼륨 하나로 끝났습니다. **0.7TB가 wire 볼륨이면 배포는 "staging 확대판" 수준.** 어느 해석인지 확인(§1 콜아웃)이 사이징의 전부습니다. `≈`
{{% /details %}}
