...
{{< callout type="info" >}}
0.7TB/월(prod, 세션 샘플링 100%) 캐파의 결론부터 적습니다.

- 첫 갈림길은 "0.7TB가 raw ingest냐 on-disk(압축 후)냐"입니다 — 이 해석 하나에 배포 규모와 비용이 2~3배 갈립니다. 본 페이지는 on-disk 해석 B를 1차 모델로 세우고 raw 해석 A를 대조로 실은 뒤, 배포 후 `system.parts` 1회 실측으로 확정하게 합니다. `≈`
- on-disk 구성은 세션 리플레이가 ~78%를 지배합니다(리플레이는 압축이 잘 안 됨). 그런데 리플레이는 hot 30일만 두고 S3로 안 내린 채 DELETE합니다({{< relref "03-s3-cold-tiering.md" >}} 기준 문서) → 누적되지 않습니다. 캐파의 단일 최대 지렛대입니다.
- 그래서 "0.7TB×12=8.4TB" 순진한 누적은 틀립니다. 1년 실제 누적(단일사본)은 ~2.35TB이고, 차이 ~6TB가 전부 "안 쌓이는 리플레이"입니다. `≈`
- hot·컴퓨트는 지평과 무관하게 고정입니다(hot gp3 ~2TB, 2× r7g.2xlarge). 3→12개월 증분은 거의 전부 싼 S3 cold입니다(서울 ×RF2 기준 $19→$86/mo). `≈`
- 1 shard × RF2 로 1년+ 충분하고 볼륨은 gp3면 됩니다(io2 불필요). Keeper 3노드·MongoDB 3멤버는 데이터량과 무관하게 소형 고정입니다. prod 월 ~$1.1~1.2K(서울 `ap-northeast-2`, on-demand), RF3 시 ~$1.7K. `≈`
{{< /callout >}}
...
압축비 산식·지평별 물리 배치·월 절대 금액의 단일 정본은 이 장입니다. 시그널별·블렌디드 압축비의 유도(§2), 3/6/12개월 hot gp3 / cold S3 / 백업 물리량(§4.2·§4.3), 서울 기준 월 비용의 절대 금액과 배수(§4.6)가 여기 한 곳에만 있습니다. 다른 장과 운영 트랙은 이 숫자를 재기재하지 않고 이 장을 가리킵니다 — [블록 온리]({{< relref "08-block-only-tuning.md" >}})의 델타 산정도 §4.6을 기준선으로 삼는 파생이고, 어긋나면 이 장을 따릅니다. 반대로 GB 단가 rate 자체의 정본은 [hot 스토리지·EBS]({{< relref "02-hot-storage-ebs.md" >}}) §1.3이고, 이 장은 그 rate를 인용해 곱합니다. TTL 정책의 정본은 [S3 콜드 티어링]({{< relref "03-s3-cold-tiering.md" >}})이고 이 장은 DELETE 지평만 변주합니다(§6).
{{< /callout >}}

## 1. 입력 해석 — "월 0.7TB"는 raw인가 on-disk인가

사용자 입력 "prod 세션 샘플링 100%, 월 0.7TB"는 어느 지점의 바이트인지 불명하고, 이것이 캐파의 첫 갈림길이므로 두 해석을 모두 못박습니다.
...
캐파 산정의 대상은 결국 디스크에 실제로 쌓이는 양입니다. 그래서 본 페이지는 해석 B(on-disk 단일사본 0.7TB/월)를 1차 모델로 삼고 해석 A는 §4.7 대조로 싣습니다 `≈`.

{{< callout type="warning" >}}
배포 후 1회 실측하면 모호성이 사라집니다. 두 해석의 배포 규모·비용이 2~3배 차이 나므로 staging 또는 prod 초기 데이터가 며칠 쌓인 뒤 다음 쿼리로 어느 해석인지 확정합니다.
...
`ratio`가 시그널별 실제 압축비이고 `on_disk`의 월 증가분이 해석 B의 실측값입니다. 이 한 번의 실측으로 §2의 `≈`이 `✓`으로 바뀝니다.
{{< /callout >}}

### 1.1 세션 수 역산 — 두 해석의 현실성 교차검증 `≈`

RUM 볼륨은 세션 리플레이가 지배합니다(§2). 리플레이 on-disk를 전체의 ~78%(§2.2)로 두면:
...
두 해석은 서로 다른 크기의 자산을 기술할 뿐 모순이 아닙니다. `system.parts`(위 콜아웃) 또는 collector 메트릭으로 1회 실측하면 갈립니다.
...
리플레이가 압축이 안 되는 탓에 on-disk 상에서는 리플레이 비중이 raw보다 더 커집니다:
...
TTL 정책의 기준 문서는 {{< relref "03-s3-cold-tiering.md" >}}이고, 그 핵심은:

- `hyperdx_sessions`(리플레이): hot(gp3)만 두고, S3로 내리지 않고 30일 후 DELETE합니다. 오래된 리플레이는 거의 안 보고 볼륨을 지배하므로 S3 이전조차 낭비입니다.
- `otel_logs`/`otel_traces`: hot 14일 → `TO VOLUME 'cold'`(S3) → 지평별 DELETE.
- `otel_metrics_*`: hot 30일 → S3 → 지평별 DELETE.

on-disk의 78.5%를 차지하는 리플레이가 30일 상한으로 잘리고 S3로도 안 가면 시간이 지나도 누적되지 않습니다(steady-state ~0.55TB 단일). 누적을 만드는 건 나머지 ~22%(로그+트레이스+메트릭 ≈ 0.15TB/월)뿐입니다.

{{< callout type="important" >}}
"0.7TB × 12개월 = 8.4TB" 순진한 누적은 틀립니다. 그 계산은 리플레이가 계속 쌓인다고 보지만 리플레이는 30일 DELETE라 steady-state에 머뭅니다. 실제 1년 누적(단일사본)은 리플레이 고정분 ~0.55TB + 누적분(로그/트레이스/메트릭) ~1.8TB ≈ ~2.35TB이고, 차이 ~6TB가 전부 "안 쌓이는 리플레이"입니다. 리플레이 TTL을 로그/트레이스와 분리해 짧게 잡는 것이 캐파·비용의 단일 최대 절감 노브인 이유가 이 숫자입니다. `≈`
{{< /callout >}}
...
- hot 창(EBS gp3): 리플레이·메트릭 30일, 로그·트레이스 14일({{< relref "03-s3-cold-tiering.md" >}} 기준 문서와 정합). hot 창 밖은 로그/트레이스/메트릭만 S3 cold로 보내고 리플레이는 DELETE합니다.
...
지평이 늘어도 hot 은 ~0.63TB로 고정이고 커지는 건 오직 싼 cold S3입니다.
...
- cold S3만 지평 따라 증가합니다 → 1년까지 늘려도 추가 비용은 대부분 서울 $0.025/GB-월의 S3 Standard입니다(단가 정본은 {{< relref "02-hot-storage-ebs.md" >}} §1.3). 백업은 리플레이 제외(가치 급감) 후 로그/트레이스/메트릭만 Glacier IR로 보냅니다.
...
- 쿼리: RUM 대시보드·세션 검색 위주 light~moderate, page cache가 hot을 흡수합니다.
- 1 shard × 2 replica(RF2) 로 1년+ 충분하고 샤딩은 필요 없습니다(조기 수평 확장은 안티패턴). RF3는 임의 2대 동시 유실 방어가 필요할 때만 씁니다({{< relref "../../clickhouse/04-deployment-playbook.md" >}}).
...
실질 병목은 인스턴스 EBS 대역입니다. gp3 볼륨 스펙(최대 2,000 MiB/s)보다 인스턴스의 EBS 파이프 상한이 먼저 천장을 칩니다 — r7g.2xlarge의 EBS 대역이 볼륨 스펙보다 낮으므로 노드 사이즈업이 볼륨 프로비저닝보다 먼저 효과를 냅니다({{< relref "../../clickhouse/02-storage-local-nvme.md" >}}) `✓`. 이 스케일(피크 8MB/s)에선 둘 다 여유라 무관하지만 성장 시 이 순서를 기억합니다.

lean 옵션도 있습니다. 쿼리가 가벼우면 2× r7g.xlarge(4vCPU/32GB) 로 낮춰도 됩니다. 헤드룸을 감안하면 2xlarge를 권합니다.

### 4.5 Keeper·MongoDB — 데이터량과 무관하게 소형 고정

- Keeper: 정족수 3(1 장애 허용), 4GB RAM·gp3 20GB면 충분합니다. Keeper 부하는 데이터량이 아니라 INSERT 빈도·파트 생성 수에 비례하므로 지평이 늘어도 커지지 않습니다({{< relref "05-keeper.md" >}}) `✓`.
...
우리 배포 리전이 서울이므로 절대 금액의 기준선도 서울입니다. GB 단가 rate 자체의 정본은 {{< relref "02-hot-storage-ebs.md" >}} §1.3이고 이 절은 그 rate를 물리량(§4.3)에 곱합니다.
...
리전 환산 규칙 하나로 이 절의 모든 금액이 재현됩니다. 서울 단가를 아는 항목(gp3 GB · S3 GB · S3 요청)은 서울 단가로 직접 곱하고, 모르는 항목(인스턴스 · Glacier IR · cross-AZ 전송)은 us-east-1 값 × 1.1~1.15로 어림합니다. 그 계수는 스토리지 차원에서 실단가로 검증됐습니다 — gp3 $0.08→$0.0912는 +14.0%(밴드 안), S3 $0.023→$0.025는 +8.7%(밴드 바로 아래)입니다 `Σ`. 배수는 us-east-1 3.48x → 서울 3.65x로 서울에서 오히려 조금 벌어지므로, 티어링 결론은 리전을 바꿔도 뒤집히지 않습니다 `Σ`.

요청 축은 GB 축과 별개입니다 — 서울 PUT $4.50/M은 GET $0.35/M의 ~13배이고 `✓`, 여기에 part 파일 수가 곱해집니다. 다만 우리 설계는 머지를 hot에서 끝내므로 S3 쓰기가 이동 1회뿐입니다 — 그 구조와 `prefer_not_to_merge`를 안 켜는 비용 논거는 [S3 콜드 티어링]({{< relref "03-s3-cold-tiering.md" >}}) §5.6이 정본이고 이 절은 위 표의 단가와 아래 PUT 금액만 계상합니다.
...
RF3(12개월, 서울)은 컴퓨트 3대 $938×1.1~1.15 ≈ $1,032~1,079 + hot gp3 3TB(3,000GB×$0.0912) ≈ $274 + cold S3 ×RF3=5.16TB(5,160GB×$0.025) ≈ $129 + Keeper/Mongo $118×1.1~1.15 ≈ $130~136 + (백업 $7 + 전송 $70)×1.1~1.15 ≈ $85~89 ⇒ ~$1,650~1,710/mo(on-demand), 1yr SP ~$1,185~1,220/mo `≈`.

표를 읽는 법은 이렇습니다. hot gp3·컴퓨트·Keeper·Mongo는 지평과 무관하게 고정 ~$1.0K이고, 3→12개월 확장 비용은 거의 전부 S3 cold입니다(×RF2 기준 $19→$86 증가). 긴 보관이 싼 이유는 EBS가 아니라 S3에 쌓이고, 심지어 볼륨 지배자인 리플레이는 30일에 잘리기 때문입니다. Datadog RUM으로 같은 워크로드(≈월 22M 세션)를 태우면 [RUM 문서]({{< relref "../../rum/_index.md" >}}) 기준 블렌디드 ~$0.42/1k → 연 수만~십수만 $대인데, self-host는 월 ~$1.1~1.2K(연 ~$14K)로 수 배~10배 절감입니다(단 people TCO 별도, {{< relref "../../clickhouse/01-managed-vs-selfhosted.md" >}}).

us-east-1과 대조하면, 같은 물리량을 us-east-1 단가로 계산한 원 산정은 고정 소계 $904 / 총계 3개월 $971 · 6개월 $1,001 · 12개월 $1,052(1yr SP $720 / $750 / $800), RF3 12개월 ~$1,500(SP ~$1,150)이었습니다 `≈`. 서울과의 차이는 스토리지 실단가(+14.0% / +8.7%)와 나머지 항목의 리전 계수(×1.1~1.15)뿐이고, 비율이 유사해 지평·티어링 결론은 그대로입니다. 견적을 서울로 쓰는 이유는 그대로 제출할 숫자가 필요해서입니다.
...
해석 A면 규모가 매우 작습니다 — 2× 소형 노드 + Keeper 3 + Mongo 3로 충분하고 hot이 수백 GB라 gp3 볼륨 하나로 끝났습니다. 0.7TB가 wire 볼륨이면 배포는 "staging 확대판" 수준입니다. 어느 해석인지 확인(§1 콜아웃)이 사이징의 전부입니다. `≈`
{{% /details %}}
...
- io2 전환 트리거: (a) 단일 볼륨 >2,000 MiB/s 지속, (b) >80,000 IOPS/vol, (c) 규제상 볼륨 자체 99.999% 요구. → 0.7TB/월 RUM은 셋 다 도달하지 않으므로 io2 채택 근거가 없습니다 `≈`.
...
staging의 진짜 역할은 실측 캘리브레이션입니다. §2 압축비·구성비·세션당 KB는 전부 `≈`입니다. staging에서 §1 콜아웃 쿼리로 실제 압축비와 세션당 바이트를 측정해 prod 모델의 `≈`을 `✓`으로 바꿉니다. 이게 staging을 두는 캐파상 이유입니다.
...
- hot gp3 사용률 경보: 70% 경고 / 80% 조치 / 85% 하드실링. hot 볼륨은 항상 30~40% 여유를 둡니다(§4.1 헤드룸).
...
- shard 추가는 이 워크로드에서 1년+ 필요 없습니다. 트리거는 (a) hot 단일사본/노드가 노드 실용 상한(예 4~8TB)에 접근, (b) 머지/쿼리 CPU 지속 포화, (c) 재수화 위험 창을 줄이려 노드당 데이터를 낮추고 싶을 때입니다. 신규 shard 스키마·리밸런싱은 수동입니다({{< relref "../../clickhouse/05-altinity-operations.md" >}}).
- io2 전환은 §5 — >2,000 MiB/s·>80,000 IOPS/vol·볼륨 99.999% 요구 시입니다. RUM 0.7TB/월엔 도달하지 않습니다.
- RF2→RF3는 임의 2대 동시 유실 무손실 또는 재수화 창 동안 2차 장애 방어가 필요할 때입니다. 비용은 컴퓨트+cold S3가 ×1.5입니다. 확률·비용 결정은 {{< relref "../../clickhouse/04-deployment-playbook.md" >}}.
