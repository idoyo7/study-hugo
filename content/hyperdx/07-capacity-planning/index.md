...
**이 장이 압축비 산식·지평별 물리 배치·월 절대 금액의 단일 정본이다.** 세 가지가 여기 한 곳에만 있다 — 시그널별·블렌디드 압축비의 유도(§2), 3/6/12개월 hot gp3 / cold S3 / 백업 물리량(§4.2·§4.3), 그리고 **서울 기준 월 비용의 절대 금액과 배수**(§4.6). 다른 장과 운영 트랙은 이 숫자를 재기재하지 않고 이 장을 가리킨다 — [블록 온리]({{< relref "08-block-only-tuning.md" >}})의 델타 산정도 §4.6을 기준선으로 삼는 파생이고, 어긋나면 이 장을 따른다. 반대로 **GB 단가 rate 자체의 정본은 [hot 스토리지·EBS]({{< relref "02-hot-storage-ebs.md" >}}) §1.3**이고 이 장은 그 rate를 인용해 곱한다. TTL 정책의 정본은 [S3 콜드 티어링]({{< relref "03-s3-cold-tiering.md" >}})이고 이 장은 DELETE 지평만 변주한다(§6).
{{< /callout >}}
...
¹ `system.parts.bytes_on_disk` 기준(단일 replica). 표의 금액은 둘 다 **서울(`ap-northeast-2`) 기준**이고 산정 근거는 §4.6·§4.7이다 `≈`.
...
**우리 배포 리전은 서울이므로 절대 금액의 기준선도 서울이다.** GB 단가 rate 자체의 정본은 {{< relref "02-hot-storage-ebs.md" >}} §1.3이고, 이 절은 그 rate를 물리량(§4.3)에 곱한다.
...
**리전 환산 규칙** — 이 절의 모든 금액이 이 규칙 하나로 재현된다: **서울 단가를 아는 항목(gp3 GB · S3 GB · S3 요청)은 서울 단가로 직접 곱하고, 모르는 항목(인스턴스 · Glacier IR · cross-AZ 전송)은 us-east-1 값 × 1.1~1.15로 어림한다.** 그 계수는 스토리지 차원에서 실단가로 검증됐다 — gp3 $0.08→$0.0912는 **+14.0%**(밴드 안), S3 $0.023→$0.025는 **+8.7%**(밴드 바로 아래)다 `Σ`. 배수는 us-east-1 3.48x → 서울 3.65x로 **서울에서 오히려 조금 벌어지므로**, 티어링 결론은 리전을 바꿔도 뒤집히지 않는다 `Σ`.

**요청 축은 GB 축과 별개다** — 서울 PUT $4.50/M은 GET $0.35/M의 **~13배**이고 `✓`, 여기에 part 파일 수가 곱해진다. 다만 우리 설계는 머지를 hot에서 끝내므로 S3 쓰기가 이동 1회뿐이다 — 그 구조와 `prefer_not_to_merge`를 안 켜는 비용 논거는 [S3 콜드 티어링]({{< relref "03-s3-cold-tiering.md" >}}) §5.6이 정본이고, 이 절은 위 표의 단가와 아래 PUT 금액만 계상한다.
...
위 표는 us-east-1 단가로 쓴 원 산정이다. §4.6의 리전 환산 규칙을 그대로 적용하면(gp3 330GB×$0.0912 ≈ $30, S3는 서울 단가, 나머지 ×1.1~1.15) **서울 총계는 3개월 ~$470~490 / 12개월 ~$550~570** ≈ **~$0.5~0.6K/mo** 다 `≈`.

해석 A면 **매우 작다** — 2× 소형 노드 + Keeper 3 + Mongo 3로 충분. hot이 수백 GB라 gp3 볼륨 하나로 끝난다. **0.7TB가 wire 볼륨이면 배포는 "staging 확대판" 수준.** 어느 해석인지 확인(§1 콜아웃)이 사이징의 전부다. `≈`
{{% /details %}}
