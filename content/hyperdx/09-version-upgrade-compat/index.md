...
우리는 ClickStack 표준 차트를 그대로 쓰지 않고 `clickhouse.enabled: false`(HyperDX Only)로 CH/Keeper를 [Altinity CHI/CHK로 분리]({{< relref "01-stack-topology.md" >}}) 운영합니다. 그래서 "차트가 배포하는 기본 이미지"와 "우리가 실제로 핀하는 이미지"는 별개이며, 아래 매트릭스가 그 경계를 명확히 합니다.

### 1.1 마스터 매트릭스 (2026-07 확인)

4열로는 근거 주석까지 담기엔 폭 제약을 넘어서므로 항목별 불릿으로 풉니다. 각 항목은 **A ↔ B — 요구/권장 (근거)** 순서입니다.
...
⑨는 배포 전 반드시 실측해야 하는 지점입니다. operator **0.27.0+**는 `async_replication`과 `use_xid_64`를 **기본 활성화**하는데, 이 기능들은 **Keeper 25.3+**를 요구합니다 `✓⁽release notes⁾`. 우리가 CH/Keeper를 **24.8 LTS로 핀**하면(§1.2-2) operator가 이 기본값을 켠 상태에서 Keeper 24.8과 충돌할 수 있습니다.
...
- **Packed storage (`min_level_for_full_part_storage`, 25.10)** — 기본값이 **0 = Full storage**라 켜지 않으면 동작이 바뀌지 않습니다. 문제는 가용성 진술이 갈린다는 것이다 — 공식 표는 Availability를 "Open source and Cloud"로 적는데 별도 조회에서는 "Cloud 전용"이라는 **상충 진술**이 나온다 → OSS 실제 동작 미확정 `?`. 문서가 드는 효과(insert당 PUT 31.3 → 2.22)가 사실이면 S3 요청 비용 구조가 바뀌므로, 켜는 결정이 아니라 **실측 대상**으로만 남깁니다.
...
- **핵심 규칙**: **명시적으로 바꾸지 않은 설정만** 영향받는다 — 사용자가 이미 override한 설정은 존중됩니다. 즉 "바이너리는 신버전으로 올렸지만 동작 기본값은 옛 버전에 고정"하는 노브입니다.
...
CH 다운그레이드가 포맷 때문에 막힐 수 있으므로(§3), EBS-first에서는 **업그레이드 직전 데이터 볼륨의 EBS 스냅샷**을 롤백 지점으로 삼는 게 가장 확실합니다. 이것이 이 스택이 EBS-first이기에 추가로 얻는 롤백 축입니다.
...
> **EBS는 AZ-bound**라 복원 볼륨은 반드시 원 볼륨과 같은 AZ에 만든다 — AZ 종속·reattach 전제는 [operator 토폴로지·다운타임]({{< relref "04-operator-topology-downtime.md" >}})이 기준 문서입니다.

### 4.2 allowVolumeExpansion ↔ 업그레이드 순서 상호작용 `≈`

- gp3 온라인 확장(`allowVolumeExpansion: true`)과 CH 버전 업그레이드는 **별도 reconcile로 분리**한다 — [이미지+설정 동시변경 crash(#1926)]({{< relref "../../clickhouse/05-altinity-operations.md" >}})와 같은 결의 위험입니다. 볼륨 확장 중 롤링을 겹치면 STS 업데이트 경합이 생길 수 있습니다.
...
즉 HyperDX Only 구조가 ClickStack v1→v2의 가장 파괴적인 부분(내장 CH 삭제·재설치)을 **회피**시켜 줍니다 — 우리의 ClickStack 차트 업그레이드는 사실상 HyperDX + OTel + 외부 CH 참조 설정 표면으로 국한됩니다.
