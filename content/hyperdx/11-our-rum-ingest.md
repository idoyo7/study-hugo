...
기준 문서([스택 토폴로지]({{< relref "01-stack-topology.md" >}})·[가용성]({{< relref "../hyperdx-operating/03-availability.md" >}})·[Keeper]({{< relref "05-keeper.md" >}})·[복제·failover]({{< relref "06-replication-failover.md" >}}))가 표준 ClickStack을 다룬다면, 이 문서는 **우리가 실제로 배포한 형상**을 한 장에 모은다. 표준과 다른 점은 RUM 데이터(브라우저 SDK·Mobile RUM)를 받기 위해 별도로 만든 **자체 RUM 컨버터**다 — Datadog Agent가 RUM을 보내는 방식을 참조해 구현했고, OTel Collector를 거치지 않고 ClickHouse에 직접 적재한다. 나머지는 HyperDX Only + Altinity CHI/CHK 기본 형상 그대로다(HyperDX 웹 데이터 경로만 일부 손봤다).

## 수집·저장 토폴로지

{{< flow caption="두 인제스트 경로가 ClickHouse에서 합류한다. RUM(브라우저 SDK·Mobile RUM)은 자체 컨버터(Datadog Agent의 RUM 전송 방식 참조)를 통해 ClickHouse에 직접 적재하고, 표준 텔레메트리는 OTel Collector가 적재한다 — 컨버터·Collector는 서로 직접 호출하지 않는다. Keeper 3노드는 데이터가 지나가는 길이 아니라 복제 정족수를 잡는 조정 계층이다(점선). HyperDX api는 ClickHouse(쿼리)·MongoDB(메타)를 읽는다. ClickHouse·Keeper는 Altinity operator가 StatefulSet으로 관리." >}}
{
  "groups": [
  ...
  "nodes": [
    ...
    {"id": "otel", "col": 1, "row": 3, "label": "OTel Collector",     "sub": "OTLP 4317/4318 · 큐",     "kind": "proc"},
    ...
    {"id": "app",  "col": 0, "row": 5, "label": "HyperDX app",        "sub": "UI · 무상태",            "kind": "sink"},
    {"id": "api",  "col": 1, "row": 5, "label": "HyperDX api",        "sub": "쿼리·알럿 · 무상태",      "kind": "query"},
    {"id": "mongo","col": 2, "row": 5, "label": "MongoDB",            "sub": "ReplicaSet ×3",          "kind": "store"}
  ],
  "edges": [
