...
RUM 인제스트 경로에 **MongoDB는 없습니다** — 브라우저 SDK가 OTel Collector로 직접 보내고 MongoDB는 UI에서 대시보드·알럿·소스를 만들 때만 쓰입니다. 그래서 MongoDB 다운은 "관측 정지"가 아니라 "**설정·알럿·UI 정지**"이고, 이것이 MongoDB를 아주 작게 돌려도 되는 구조적 근거입니다 `✓`. 포트·컴포넌트별 역할·세션 리플레이 적재 테이블은 {{< relref "01-stack-topology.md" >}}가 정본입니다.
