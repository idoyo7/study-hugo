...
**두 스토리지 전략은 충돌이 아니다.** [로컬 NVMe 문서]({{< relref "../clickhouse/02-storage-local-nvme.md" >}})는 20TB+·성능 극대화 전제에서 출발하고, 이 챕터는 0.7TB/월·운영 단순성·내구성 우선 전제에서 EBS를 1차로 둔다. EBS-first의 값어치는 성능이 아니라 **재수화가 필요 없다**는 운영 프로파일이다 — 이벤트별 재수화 필요 여부와 재수화 위험 창은 [hot 스토리지·EBS]({{< relref "02-hot-storage-ebs.md" >}})가 정본이고, 창의 정의·MTTR 산식은 로컬 NVMe 문서가 소유한다. 노드 유실 시의 물리 역학은 {{< relref "04-operator-topology-downtime.md" >}}.
{{< /callout >}}

**operator 분기(중요)** — 표준 Helm 2-차트가 딸려 오는 ClickHouse operator는 Altinity가 아니라 **ClickHouse Inc.의 공식 operator**이고, 우리는 `clickhouse.enabled: false`(HyperDX Only)로 ClickHouse를 차트 밖으로 빼 **Altinity operator의 CHI/CHK**로 분리 운영한다 `✓`. 이 챕터 전체가 이 전제 위에 있다 — CRD 이름·채택 근거·이 분기를 흐렸을 때의 오독은 {{< relref "01-stack-topology.md" >}}와 [operator 선택]({{< relref "../clickhouse/03-operator.md" >}})이 정본이다.

{{< callout type="info" >}}
**배포 모드 이름 — 섞으면 결론이 뒤집힌다** `✓`
...
RUM 인제스트 경로에 **MongoDB는 없다** — 브라우저 SDK가 OTel Collector로 직접 보내고 MongoDB는 UI에서 대시보드·알럿·소스를 만들 때만 쓰인다. 그래서 MongoDB 다운은 "관측 정지"가 아니라 "**설정·알럿·UI 정지**"이고, 이것이 MongoDB를 아주 작게 돌려도 되는 구조적 근거다 `✓`. 포트·컴포넌트별 역할·세션 리플레이 적재 테이블은 {{< relref "01-stack-topology.md" >}}가 정본이다.
...
**HyperDX-only + Altinity CHI/CHK + MongoDB(MCK 또는 Atlas)** 로 조립하고, hot은 **단일 gp3**, cold는 **S3 + TTL MOVE**, 조정은 **Keeper 3노드**, 토폴로지는 **1 shard × RF2(2 AZ)** 로 시작한다. io2·로컬 NVMe·RF3·샤딩은 전부 **트리거 기반 승급**으로 미뤄두는 것이 0.7TB/월 규모의 정답이다 — 이 규모에서 조기 수평 확장·고성능 스토리지는 비용과 운영 부채만 남긴다.

두 가지는 배포 전에 못박아야 하고, 둘 다 지금은 `≈`·`?`다. **"월 0.7TB"의 해석(raw ingest냐 on-disk냐)** 과 **세션 리플레이 압축비·구성비·ClickStack 기본 TTL**이다. 해석 분기가 배포 규모·비용에 어떻게 번지는지, 그리고 무엇을 어떤 쿼리로 실측해 `✓`으로 올리는지는 [용량 산정]({{< relref "07-capacity-planning.md" >}})이 정본이다 — 이 실측 한 번을 위해 staging을 두는 것이 캐파 관점의 이유다. 시점 기준 2026-08.
