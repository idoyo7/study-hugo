...
- **사이징 델타**: 03에서 S3로 내리던 logs/traces/metrics가 전부 gp3에 상주 → gp3 상주가 [07]({{< relref "09-capacity-planning.md" >}}) hot 대비 **1.6x(3개월)~3.7x(12개월)**. 짧은 보존이면 S3 티어링과 비용 근접, 길어지면 발산(gp3 $0.08 vs S3 $0.023, ~3.5x/GB).
...
S3가 없으니 TTL은 `TO VOLUME 'cold'` MOVE 절이 사라지고 **DELETE만** 남는다. 03이 구분하던 "TTL 창(hot 며칠) vs DELETE 지평"의 두 층이 하나로 접혀 **보존일 = DELETE일** 하나로 단순화된다. 지평별 값(90/180/30)은 [03 §4]({{< relref "03-s3-cold-tiering.md" >}})·[07 §6]({{< relref "09-capacity-planning.md" >}})이 정본이고, 여기선 **MOVE가 빠진 형태**만 보인다.
...
**사이징 정본은 [07]({{< relref "09-capacity-planning.md" >}})다.** 여기선 "S3로 내리던 분이 전부 gp3에 상주할 때의 델타"만 산출한다. 07은 on-disk 해석 B(단일사본, 블렌디드 압축 ~6x)를 1차 모델로 삼는다.
...
- **머지 헤드룸이 더 중요해진다**: 블록 온리는 데이터가 계속 쌓여 볼륨이 차기 쉽다. 머지는 여유 공간을 예약(§5)하므로 **30~40% 여유**를 항상 남긴다([07 §8.1]({{< relref "09-capacity-planning.md" >}})의 헤드룸/경보 기준 계승). S3 탈출구가 없어 **여유 소진 시 대응이 온라인 확장뿐**(§4). `✓/≈`
...
- 블록 온리에서 데이터·파티션이 많아지면 배치/`async_insert` 튜닝으로 part 생성 빈도를 낮춘다(정본 경보표는 [07 §8.2]({{< relref "09-capacity-planning.md" >}}): 파티션당 active parts >300 조치).
...
- **비용 트레이드는 명확하다**: 리플레이는 어차피 S3 안 가므로 델타 대상은 logs/traces/metrics뿐. 이들이 gp3에 상주하면 gp3 상주가 [07]({{< relref "09-capacity-planning.md" >}}) hot 대비 **1.6x(3개월)~3.7x(12개월)**, 스토리지 비용 델타 +$47~$290/mo. 3개월이면 S3 티어링과 근접, 1년이면 명백히 비싸다.
...
- **업그레이드 관점** — 블록 온리는 S3 티어 정합 걱정이 없어 업그레이드가 단순하다(EBS 스냅샷 롤백·`allowVolumeExpansion`↔업그레이드 상호작용). 상세는 [버전·업그레이드 호환성]({{< relref "08-version-upgrade-compat.md" >}})으로 이어진다. 시점 기준 2026-07.
