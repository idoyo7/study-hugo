...
- `insert_quorum` — ack 전에 파트가 N벌 존재함을 보장 → 단일 노드 소실에도 손실↓. 그래도 앞단 버퍼는 아닙니다. quorum 미달이면 쓰기가 차단됩니다(내구성↔가용성 트레이드오프) `✓`.
- `insert_quorum_parallel` — read-after-write를 엄격히 원하면 0으로 직렬화해야 합니다 `✓`.
- `select_sequential_consistency` — 읽기 일관성↑, 읽기 지연·가용성은 일부 희생 `✓`.
...
CH 내부 노브로는 "재시도 멱등"과 "확정 강도"까지만 삽니다. CH가 다운타임이거나 과부하일 때 in-flight 이벤트를 붙잡아 둘 곳은 CH 앞단의 실제 큐입니다.
