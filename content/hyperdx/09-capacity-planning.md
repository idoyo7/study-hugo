...
| ClickHouse Keeper | **3× t4g.medium** | 2 vCPU / 4 GB / gp3 20GB(영속) | 정족수 3, 4GB면 충분 `✓` → {{< relref "06-keeper.md" >}} |
...
- **Keeper**: 정족수 3(1 장애 허용), 4GB RAM·gp3 20GB면 충분. Keeper 부하는 데이터량이 아니라 **INSERT 빈도·파트 생성 수**에 비례하므로 지평이 늘어도 커지지 않는다({{< relref "06-keeper.md" >}}) `✓`.
...
gp3 vs io2 Block Express 실전 스펙(gp3 80,000 IOPS/2,000 MiB/s/64TiB `✓`, io2 BE 256,000 IOPS/4,000 MiB/s/99.999% `✓`)·요금 3분해는 {{< relref "02-hot-storage-ebs.md" >}}가 정본이다. 캐파 관점의 판단만 요약한다.
