...
Keeper znode **전체 인벤토리**와 "Keeper는 durable queue가 아니다"(CH가 죽어도 in-flight INSERT는 큐잉되지 않는다)는 정정은 [Keeper]({{< relref "05-keeper.md" >}})가 정본이다. 여기서는 그 경로들이 **복제를 어떻게 구동하는가**(role)만 짚는다(경로는 05-keeper 표와 정합) `✓`.
