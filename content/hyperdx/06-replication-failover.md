...
{{< flow caption="복제 실행 파이프라인 — /log → replication_queue → 타입별 실행(pull 모델)" >}}
{
  "nodes": [
    { "id": "log", "col": 0, "row": 1, "label": "Keeper /log", "sub": "공용 복제 로그", "kind": "store" },
    { "id": "queue", "col": 1, "row": 1, "label": "/replicas/{r}/queue", "sub": "(= system.replication_queue)", "kind": "store" },
    { "id": "exec", "col": 2, "row": 1, "label": "타입별 실행", "kind": "proc" },
    { "id": "fetch", "col": 3, "row": 0, "label": "sibling replica에서", "sub": "part 바이트 직접 fetch", "kind": "proc" },
    { "id": "merge", "col": 3, "row": 1, "label": "로컬에서 동일 입력 머지", "kind": "proc" },
    { "id": "mutate", "col": 3, "row": 2, "label": "mutation 적용", "kind": "proc" },
    { "id": "partition", "col": 3, "row": 3, "label": "파티션 조작", "kind": "proc" }
  ],
  "edges": [
    { "from": "log", "to": "queue", "label": "폴링·새 엔트리 발견", "rate": 700 },
    { "from": "queue", "to": "exec", "label": "log_pointer 전진", "rate": 700 },
    { "from": "exec", "to": "fetch", "label": "GET_PART", "rate": 600 },
    { "from": "exec", "to": "merge", "label": "MERGE_PARTS", "rate": 600 },
    { "from": "exec", "to": "mutate", "label": "MUTATE_PART", "rate": 600 },
    { "from": "exec", "to": "partition", "label": "DROP_RANGE / REPLACE_RANGE", "rate": 600 }
  ]
}
{{< /flow >}}
...
{{< flow caption="네트워크 분할 시 Keeper 정족수에 따른 split-brain 방지" >}}
{
  "nodes": [
    { "id": "partition_event", "col": 0, "row": 1, "label": "네트워크 분할", "kind": "src" },
    { "id": "keeper_maj", "col": 1, "row": 0, "label": "Keeper 과반 O", "sub": "다수파 (Keeper 2/3 노드)", "kind": "store" },
    { "id": "keeper_min", "col": 1, "row": 2, "label": "Keeper 과반 X", "sub": "소수파 (Keeper 1/3 노드)", "kind": "store" },
    { "id": "ch_a", "col": 2, "row": 0, "label": "CH replica-A", "sub": "read+write OK", "kind": "proc" },
    { "id": "ch_b", "col": 2, "row": 2, "label": "CH replica-B", "sub": "read OK / write 거부 TABLE_IS_READ_ONLY", "kind": "proc" }
  ],
  "edges": [
    { "from": "partition_event", "to": "keeper_maj", "dashed": true },
    { "from": "partition_event", "to": "keeper_min", "dashed": true },
    { "from": "keeper_maj", "to": "ch_a", "label": "쓰기 허용", "rate": 700 },
    { "from": "keeper_min", "to": "ch_b", "label": "쓰기 거부", "rate": 700 }
  ]
}
{{< /flow >}}
