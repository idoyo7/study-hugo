...
이 페이지는 질문 하나에 답한다 — **"Iceberg 가 대체 뭔가. 그리고 그게 'self-host 에서 S3 를 메인 스토리지로' 문제의 답이 되는가."** [스토리지 · 로컬 NVMe]({{< relref "02-storage-local-nvme.md" >}})가 self-host 의 스토리지 매체를 고르고 "S3 는 primary 가 아니라 cold tier"라고 결론냈고, [Managed vs Self-hosted]({{< relref "01-managed-vs-selfhosted.md" >}})가 그 이유를 아키텍처(SharedMergeTree 는 Cloud 전용)로 설명했다. 그 두 페이지를 읽고 나면 자연스럽게 남는 미련이 하나 있다 — "업계가 다 한다는 그 레이크하우스로 가면 S3 를 메인으로 쓸 수 있는 거 아닌가."

답은 "아니다"인데, 그 이유가 "Iceberg 가 미성숙해서"가 아니라 **애초에 다른 문제를 푸는 기술이기 때문**이라는 점이 이 장의 핵심이다. 그래서 결론을 먼저 던지지 않고 층을 쌓아 올린다. Iceberg 를 한 번도 안 다뤄봤다는 독자를 전제로 §1~§2 에서 개념을 세우고, §3 에서 ClickHouse 가 실제로 할 수 있는 일을 버전·설정키 단위로 확정하고, §4~§6 에서 성능·워크로드 적합성을 따진 뒤, §7 에서 "S3 메인"의 세 갈래를 분리한다.

{{% details title="근거 등급 태그 · 출처 규칙" closed="true" %}}
근거 등급 태그는 입력 조사의 판정을 이어받는다(`✓`·`Ⓥ`·`Ⓑ`·`≈`·`?`, 이 페이지의 신규 종합 판단은 `Σ`). ClickHouse Inc./Altinity 블로그의 자사 유리한 수치는 `Ⓥ`로 격하하고 `✓`로 승격하지 않는다. URL 출처는 이 페이지가 아니라 [출처]({{< relref "10-sources.md" >}})가 담당한다.

**1차 출처 범위** — 버전 게이트는 로컬 CHANGELOG 원문으로 축어 검증했다. 2023·2024·2025 연도별 changelog 와 26.1~26.7(2026-01-29~2026-07-22)을 모두 사용했고, 25.7~25.9 항목은 2025 changelog 의 해당 릴리스 섹션에 그대로 있다 — 25.7 #82692 / 25.8 #83983·#85549·#85843·#85395·#85848 / 25.9 #86059·#86783 `✓`. 이 장에서 `≈`·`?` 로 남은 것은 CHANGELOG 가 다루지 않는 항목(설정키 리네임의 새 이름, 카탈로그 커넥터의 self-host 성숙도, Antalya 준비도)에 한한다.

§7-② 의 축어 인용은 공식 "Separation of storage and compute" 가이드, 아래 25.7 인용은 PR #82692 본문에서 가져왔다 — URL 은 [출처]({{< relref "10-sources.md" >}}).
{{% /details %}}

## S3 에 없는 것 — Iceberg 가 존재하는 이유

S3 는 데이터베이스가 아니다. 그리고 파일시스템도 아니다. S3 에 없는 것을 세 개만 꼽으면 이렇다 `Σ`.
...
Hive 시절엔 이걸 **경로 규약**으로 때웠다. `s3://bucket/tbl/dt=2026-08-12/*.parquet` 같은 디렉토리 관례를 쿼리 엔진이 알아서 해석하게 하는 방식이다. 결과는 세 가지 고질병이었다 `Σ` — 파티션을 찾으려면 LIST 를 반복해야 해서 느리고, 쓰는 중에 읽으면 절반만 올라간 파일 집합을 테이블로 착각하고, 스키마를 바꾸면 과거 파일과 현재 쿼리의 계약이 깨진다.

**Iceberg 는 그 경로 규약을 명시적 메타데이터로 승격시킨 것이다.** "어디에 뭐가 있는지"를 디렉토리 구조에 암묵적으로 인코딩하는 대신, 유효한 파일 목록·스키마·파티션 규칙·파일별 통계를 메타데이터 파일에 적어두고, "지금 유효한 메타데이터는 이것"이라는 포인터를 한 곳에 둔다. 이게 전부다. 나머지 성질(스냅샷 격리·time travel·스키마 진화)은 이 설계에서 자동으로 따라 나오는 부산물이다 `Σ`.

ClickHouse 저자들의 표현을 그대로 옮기면, 테이블 포맷은 "loose collections of files"를 "coherent, mutable tables"로 바꾼다 `✓`.

## 4층 케이크 — 파일 포맷 / 테이블 포맷 / 카탈로그 / 엔진

레이크하우스 논의가 어려운 이유의 절반은 이 네 층을 섞어 부르는 데서 온다. "Parquet 로 갈까 Iceberg 로 갈까"는 애초에 성립하지 않는 질문이다 — Iceberg 는 Parquet 위에 얹히는 층이다 `✓`.
...
① 을 조금 더 풀면 ② 가 왜 필요한지가 선명해진다. Parquet 파일 하나는 컬럼별 **column chunk** 로 나뉘고, chunk 는 다시 수 MB 규모의 **page** 로 쪼개져 page 단위로 압축된다. 그리고 chunk 들을 데이터셋의 가로 슬라이스인 **row group** 으로 묶는다 — 대부분의 엔진이 이 row group 을 병렬 처리 단위로 쓴다 `✓`. 파일 맨 뒤 **footer** 에 스키마·인코딩·컬럼 min/max 가 들어가고, page 에도 min/max 가 붙고, row group 단위로 Bloom filter 를 지원한다 `✓`. 여기까지가 "파일 한 개"의 이야기이고, **파일 여러 개를 하나의 테이블로 취급하는 규칙은 Parquet 스펙에 없다** — 그 공백이 ② 의 존재 이유다 `Σ`.

층 구분에서 가장 실용적인 결론은 **③ 이 원자성의 소재지**라는 것이다. 쓰는 쪽은 새 데이터 파일과 새 메타데이터 파일을 다 써놓고 마지막에 카탈로그 포인터만 바꾼다. 읽는 쪽은 쿼리 시작 시점의 포인터를 잡고 그 스냅샷만 본다. 그래서 전환 순간에도 반쪽 데이터가 보이지 않는다 — 저자들의 표현으로 "commits are handled atomically by the catalog" `✓`.

그리고 ④ 의 "여러 엔진 공유"가 Iceberg 의 **정치적** 존재 이유다. ClickHouse 저자들조차 이 점은 명확히 인정한다 — 테이블 포맷은 "eliminate vendor lock-in"하고 "decouple storage from compute, creating a neutral storage layer that any query engine can attach to" 한다 `✓`. Iceberg 를 도입하는 회사가 실제로 사는 것은 성능이 아니라 **엔진 교체 자유**다 `Σ`.
...
스키마 진화가 **관측성에 특히 값비싼 기능**이라는 점은 짚어둬야 공정하다. 텔레메트리는 속성이 계속 새로 생기고, 그걸 커스텀 메타데이터 레이어로 직접 관리하는 것이 원래 로그 파이프라인의 고질적 부채다. 테이블 포맷은 그 부채를 표준화해서 없앤다 `✓`. Iceberg 를 깎을 이유가 없는 영역이다.
{{< /callout >}}

### 층이 청구하는 것 — 정렬·compaction·row group 크기는 누가 하나

②③ 이 데이터베이스 같은 의미론을 주지만, 데이터베이스가 자동으로 해주던 일 몇 가지를 **사용자에게 되돌려준다**. 이게 레이크하우스 도입의 실제 비용이고, 이 장의 결론이 걸리는 지점이다.
...
25.7 도입 당시 PR 본문은 범위를 스스로 못박았다 — "The current version supports only `insert` operations for local tables ... Integration with catalogs and support for `create` will be in next MRs" `✓`. 즉 **쓰기는 처음부터 단계적으로 열렸고, 그 계단이 25.7 → 25.8 → 25.9 다**.

성숙도를 읽는 다른 신호도 있다. 26.7 CHANGELOG 에는 "Fix a crash when reading Iceberg tables with equality delete files"(#109551)가 버그픽스로 올라와 있다 `✓`. equality delete 읽기가 25.8 에 들어온 뒤 1년가량 지난 시점에도 크래시 수정이 나온다는 뜻이다 — 기능 존재와 프로덕션 신뢰도는 별개라는 판단의 근거가 된다 `Σ`.
...
`DataLakeCatalog` 데이터베이스 엔진이 Glue·Unity Catalog·Hive Metastore·Iceberg REST·OneLake 를 붙인다. 활성화에는 카탈로그별 설정이 필요하다 — `allow_experimental_database_iceberg`, `allow_experimental_database_unity_catalog`, `allow_experimental_database_glue_catalog`, `allow_experimental_database_hms_catalog`, `allow_experimental_database_paimon_rest_catalog` `≈`. 이 중 Unity·Glue·REST·Hive Metastore 네 개는 PR #85848(25.8 반영)로 experimental → beta 승격됐다 — 2025 CHANGELOG 25.8 섹션 축어로 "Unity, Glue, Rest, and Hive Metastore data lake catalogs are promoted from experimental to beta" `✓`. 승격과 함께 `allow_experimental_database_*` 에 `allow_database_*` 별칭이 붙었으나, 문서 축어로 확인되는 것은 `allow_experimental_database_iceberg` ↔ `allow_database_iceberg` 한 쌍이고 나머지는 같은 규칙의 유추다 `≈`. 이름에는 아직 experimental 이 남아 있다. 공식 설정 레퍼런스의 Beta 표 등재 여부는 원문을 확보하지 못했다 `?`.

{{< callout type="important" >}}
**여기서 정직해야 할 지점이 있다.** 엔진 코드 자체는 OSS 이지만, 카탈로그 통합을 소개하는 ClickHouse 공식 자료는 Cloud 맥락에 치우쳐 있다 — 대표 블로그가 "엔진 자체는 오픈소스이나 이 글은 Cloud 에 초점을 둔다"고 스스로 명시한다 `≈`. 그래서 **self-host 에서 Glue/Unity 연동이 실제로 얼마나 매끄러운지에 대한 1차 근거가 우리에게 없다** — 도입을 검토한다면 이 부분은 반드시 스테이징 실측으로 메워야 하는 공백이다 `Σ`.
{{< /callout >}}

Unity Catalog 데뷔는 24.12 무렵, Glue + Delta-on-Unity 는 25.3 언급으로 보이지만 정확한 마이너 버전은 확인하지 못했다 `?`.

### 인접 경로 — Delta/Hudi, s3/s3Cluster, S3Queue

Iceberg 만 보면 시야가 좁아진다. S3 를 데이터 소스로 쓰는 경로가 여럿이고, 층이 서로 겹친다.
...
**`S3Queue` 는 성격이 다르다.** 위 경로들이 S3 를 "쿼리 대상"으로 쓰는 반면, S3Queue 는 S3 를 **인제스트 소스**로 쓴다 — 버킷에 새로 떨어진 파일을 감지해 MergeTree 로 밀어 넣는 경로다. 23.8 실험 도입, 23.11 릴리스 블로그에서 "significantly improved since its experimental release and is now production ready"로 공식 발표됐고, Keeper 의존(`keeper_path`)이 필수다 `✓`.

{{< callout type="important" >}}
**S3Queue 의 exactly-once 를 Cloud 의 S3 ClickPipes 와 혼동하면 안 된다.** 공식 문서는 S3Queue 가 exactly-once 를 보장하지 **않는다**고 명시하고 중복 시나리오를 열거한다 — (1) 파싱 실패 후 재시도, (2) 멀티서버 환경에서 Keeper 세션 만료(25.8 부터 영구 처리 노드 사용으로 완화), (3) 서버 비정상 종료 `✓`. 또 `after_processing='delete'` 이면서 `fsync_after_insert=1` 이 아니면 전력 손실 시 행 유실이 가능하고, Ordered 모드는 파일명이 알파뉴메릭 오름차순이어야 하며 멀티서버에서 `loading_retries` 를 지원하지 않는다(문서가 향후 수정 필요로 인정) `✓`.

반면 ClickHouse Cloud 의 S3 ClickPipes 는 "S3 ClickPipe guarantees exactly-once semantics, so no duplicates make it into your target table"라고 광고한다 `Ⓥ`. **이건 Cloud 관리형 레이어의 보장이고 OSS S3Queue 의 보장이 아니다** — 두 문장을 같은 근거로 쓰면 설계가 틀어진다 `Σ`.
{{< /callout >}}

## 성능 — MergeTree 와 Parquet-on-S3 의 실제 격차

숫자는 두 진영에서 나오고, 두 진영 모두 자기에게 유리한 조건을 깔았다. 그래서 등급을 나눠 읽어야 한다.
...
**ClickBench 수치에는 저자들이 직접 단 면책이 붙어 있다** — 공정한 비교가 아니라는 것이다. Parquet 은 범용 포맷이고 MergeTree 는 전용 튜닝된 엔진이라는 취지다 `✓`. 이 면책을 떼고 "2배 느리다"만 인용하면 부정확하다. 리더 v3 개선(Arrow 중간 레이어 제거, row group 내 컬럼 병렬 처리, PREWHERE 지원)은 격차가 **좁혀지는 방향**이라는 신호로 읽는 게 맞다 `Σ`.

**Altinity 수치는 `Ⓥ`로 격하해야 한다.** 이유는 셋이다 `Σ` — (1) MergeTree 쪽을 튜닝하지 않은 기본 설정으로 두고 비교했다(ZSTD 압축·pread 등 수동 설정이 필요한데 그걸 하지 않았다), (2) Iceberg 쪽에는 4노드 swarm 을 동원했다, (3) 애초에 바닐라 OSS 가 아니라 **Antalya 라는 별도 배포판** 자료다. 비교 대상 버전도 특정이 어렵다 `?`. "Iceberg 가 MergeTree 를 이겼다"는 헤드라인을 우리 결정의 근거로 쓸 수 없다는 뜻이지, 측정이 거짓이라는 뜻은 아니다.

## 왜 Iceberg 가 MergeTree 를 대체하지 못하는가 — 같은 층이 아니다

성능 차이의 원인을 "Iceberg 가 덜 최적화돼서"로 요약하면 틀린다. 둘은 **다른 층에서 다른 일**을 한다.
...
한 문장으로: **MergeTree 는 "빠른 엔진"이고 Iceberg 는 "중립 테이블"이다.** 전자를 후자로 바꾸는 것은 업그레이드가 아니라 **교환**이며, 교환의 대가는 포인트 조회 지연과 compaction 운영이고 얻는 것은 엔진 자유와 저장 비용이다 `Σ`.

## 관측성에는 맞는가 — 저자들이 열거한 다섯 가지 한계

ClickHouse 공식 블로그 "Are open-table-formats + lakehouses the future of observability?"(Melvyn Peignon, Dale McDiarmid, 2025-10-16)가 이 질문을 정면으로 다룬다(저자·게재일 원문 확인 `✓`). 결론부터 정확히 옮기면, **저자들은 장기적으로 낙관하지만 현재의 관측성 메인 스토리지로는 다섯 가지 한계를 든다** `✓`. TL;DR 은 "becoming viable"이고 본문 후반은 "open table formats will ultimately become a central component of open, cost-efficient observability architectures"라고 쓴다 — 즉 "안 된다"가 아니라 "아직 아니다"다 `✓`.
...
**편향을 먼저 밝힌다.** 이 글은 MergeTree 를 만든 회사가 썼고, 같은 글에서 MergeTree 가 "combines many of the strengths of lakehouse and open table formats while also addressing and simplifying their challenges"라고 자사 엔진을 옹호한다 `✓`. 열거된 한계가 자사에 유리한 방향으로 선택됐을 가능성을 배제할 수 없다.

**그럼에도 다섯 개 모두 메커니즘상 타당하다** `Σ`. 1·2·5 는 Parquet 의 물리 레이아웃(page 단위 압축, footer 선행 읽기, definition/repetition level 순차 디코딩)에서 직접 따라 나오는 결과이고, 3·4 는 §2 에서 본 "카탈로그 포인터 원자 교체"라는 커밋 모델의 필연적 부산물이다. 관측성 워크로드가 요구하는 것(`trace_id` 한 건 조회, 초당 수만 건 인제스트, 계속 새로 생기는 속성)이 정확히 이 다섯 지점을 때린다.

그리고 저자들이 제시하는 **현실 대안이 이중 쓰기**라는 점이 결정적이다. "In real-world observability deployments, some users have adopted a dual-write architecture. Observability data is written both to ClickHouse's MergeTree tables for hot, real-time analysis and to open table formats for long-term cold retention." `✓` 이 패턴을 Netflix 같은 조직이 쓰고 있지만, 저자들 스스로 비효율을 인정한다 — "remains popular but introduces inefficiency - data must be written twice and managed separately" `✓`.

즉 **관측성 진영에서 검증된 Iceberg 활용형은 "메인 스토리지 교체"가 아니라 "핫은 MergeTree, 콜드는 오픈 테이블 포맷"의 병행**이고, 그 병행조차 데이터를 두 번 쓰는 대가를 낸다 `Σ`.

### 격차를 좁히는 것들 — 재검토의 기술적 조건

같은 글이 다섯 한계를 메우려는 움직임도 정리해 둔다. 이것들이 어디까지 왔는지가 곧 §8 의 재검토 시점을 정하므로, 항목별로 무엇을 해결하는지 붙여 읽는다.
...
저자들 스스로 신규 포맷들에 대해 "none have yet been tested at the full scale or complexity of production observability pipelines"라고 못박는다 `✓`. **다섯 한계가 사라지는 게 아니라 옮겨가는 중이고, 지금 결정을 내리는 사람에게는 아직 존재하는 한계다** `Σ`.

## 그래서 S3 메인의 답인가 — 세 갈래를 분리한다

"S3 를 메인으로"라는 한 문장 안에 서로 다른 세 가지가 뭉쳐 있다. 이걸 분리하는 것이 이 장이 독자에게 남기려는 결론이다 `Σ`.
...
`ENGINE =` 뒤에 오는 것은 `MergeTree` 뿐이고, S3 는 `storage_policy` 로 붙는다(디스크 정의는 `storage_configuration` 의 `<type>s3</type>` 디스크 + `<type>cache</type>` 캐시 디스크 조합) `✓`. 문서 자신이 "Note that we didn't have to specify the engine as `S3BackedMergeTree`. ClickHouse automatically converts the engine type internally if it detects the table is using S3 for storage"라고 설명한다 — 문서 산문 속 설명어일 뿐 엔진 클래스가 아니다 `✓`. 따라서 자료에서 이 이름을 보면 엔진명이 아니라 설명어로 읽어야 하고, `system.tables` 의 engine 컬럼에 그 이름이 나온다는 서술은 근거가 없다 `Σ`.
...
"Iceberg 를 메인으로"가 실제로 통한다는 주장의 출처는 바닐라 OSS 가 아니라 대부분 **Altinity Antalya** 다. Antalya 는 ClickHouse 에 stateless compute swarm, 분산 캐싱, tiered storage-on-Iceberg 를 얹은 **별도 브랜치/배포판**이고, Altinity.Cloud(관리형/BYOC)와 self-managed 설치 양쪽에 쓰인다 `≈`. "10배 저렴한 Iceberg 스토리지 위에서 무한 확장 쿼리"는 그 배포판의 아키텍처 산물이고 OSS 표준 기능이 아니다 `Ⓥ`.

실무적 함의는 단순하다 `Σ` — **바닐라 OSS 로 가면 Antalya 의 swarm·캐싱 최적화 없이 §3 의 기본 Iceberg 읽기/쓰기 기능만 갖는다.** 따라서 Antalya 벤치마크를 근거로 바닐라 OSS 의 성능을 기대하면 안 된다. Antalya 자체의 프로덕션 준비도 등급(벤더가 프로덕션 레디로 표기하는지 여부)은 이 조사에서 확인하지 못했다 `?` — 별도 배포판이라는 사실만 확정된 것이고, 채택 검토를 한다면 이 등급부터 벤더에게 확인해야 한다.
...
우리 스택은 EKS self-host + ReplicatedMergeTree 이고 콜드는 S3 tier 다. hot 매체는 전제에 따라 갈린다 — 이 챕터의 전제(인력·20TB+·스토리지 성능 세 조건)에서는 로컬 NVMe([스토리지 · 로컬 NVMe]({{< relref "02-storage-local-nvme.md" >}})), 실제 현 시점 RUM 규모(월 유입 약 0.7TB)에서는 EBS-first 가 결론이다([HyperDX 내재화]({{< relref "../hyperdx/_index.md" >}})). 이 장의 판단은 두 경우 모두 같다 — **Iceberg 를 지금 도입하지 않는다** `Σ`. 근거는 성숙도가 아니라 **워크로드 불일치와 규모**다.
...
그때의 형태도 미리 정해둔다. **"관측성 메인을 Iceberg 로 이전"이 아니라 "MergeTree 에서 TTL 로 만료되는 데이터를 Iceberg 로 내보내는 아카이브 경로"** 다 `Σ`. 이 형태는 §6 의 이중 쓰기와 달리 같은 데이터를 두 번 쓰지 않고, 관측성 조회 경로(HyperDX → MergeTree)를 건드리지 않으며, 실패해도 되돌릴 수 있다. 검토를 시작할 때의 첫 두 작업은 (a) self-host 에서 `DataLakeCatalog` + Glue 연동 스테이징 실측, (b) 그 시점의 Iceberg 쓰기 설정 등급(Beta/Experimental) 재확인이다 — 이 장의 `?` 항목들이 그때까지 `✓`로 승격돼 있을지가 판단을 가른다. 시점 기준 2026-08.
