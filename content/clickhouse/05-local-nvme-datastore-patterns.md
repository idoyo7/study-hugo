...
근거 등급 태그는 입력 조사(11-1~11-4 및 종합)의 판정을 승계한다(`[확인됨]`·`[벤더]`·`[추정]`·`[미확인]`, 본 페이지의 신규 종합 판단은 `[종합]`). URL 출처는 이 페이지가 아니라 [출처]({{< relref "06-sources.md" >}})가 담당한다.
...
각 사례가 "무엇을 증명하는가"만 압축한다 — 상세 수치·출처는 [출처]({{< relref "06-sources.md" >}}).
