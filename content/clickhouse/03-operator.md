...
채택이 결정된 경우, operator는 **Altinity로 통일**한다 — replica≥2가 되는 순간 손익분기점을 넘고, 7년+ 트랙레코드가 알파 공식 operator·폐기 경로 Bitnami·수동 STS를 모두 앞선다. ClickStack은 `clickhouse.enabled: false`로 내장 CH를 끄고 Altinity가 관리하는 CH(또는 HyperDX only)를 참조하게 해, 관측성용과 범용 분석용 CH를 하나의 성숙한 operator로 수렴시킨다. 공식 operator는 스테이징에서 베타/GA 승격을 추적하다 재평가한다. 로컬 NVMe·Keeper·백업 등 운영 how의 나머지는 [스토리지 · 로컬 NVMe]({{< relref "02-storage-local-nvme.md" >}})와 [프로덕션 운영 사례]({{< relref "04-production-usecases.md" >}})에서 이어진다. 시점 기준 2026-07.
