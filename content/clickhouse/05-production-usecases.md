...
{{% details title="그 밖의 확인 사례 — Zomato · Shopee · Ahrefs · OpenAI" closed="true" %}}
**Zomato**(EC2 10×m6g.16xlarge, gp3 hot→cold TTL, ES 대비 연 $1M+ 절감 `[벤더]`), **Shopee**(분산 트레이싱 ~3M rows/s, 30B+ trace rows `[확인됨(요약)]`). **Ahrefs**(초대형 베어메탈)와 **OpenAI**(내부 사용설)는 이번 조사에서 노드 수·용량을 명시한 1차 출처를 확보하지 못해 **`[미확인]`**으로 둔다.
{{% /details %}}
