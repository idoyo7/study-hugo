...
- 그 대가가 self-host의 운영 형태를 규정합니다 — 스케일아웃 = **리샤딩**이고, RF2를 쓰면 S3 tier에서도 사본이 두 배가 되며, 내구성은 디스크가 아니라 **replica로 직접 확보**해야 합니다 `✓`. 로컬 디스크 복제(RMT) + 선택적 S3 cold tier 조합이 정석이다([RMT 내구성 설계]({{< relref "02-storage-local-nvme.md" >}}) 참고).
